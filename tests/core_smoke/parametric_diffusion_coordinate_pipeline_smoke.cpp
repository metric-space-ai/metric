...
// while the composed parametric diffusion coordinate workflow lives under mtrc::modify. These fully
// qualified paths must resolve to classes -- there is no mtrc::diffusion_coordinate or
// mtrc::coordinate_solver product namespace.
...
static_assert(std::is_class<mtrc::modify::map::ParametricDiffusionCoordinateMap<double>>::value,
			  "parametric diffusion coordinate mapping must live under mtrc::modify::map");
static_assert(std::is_class<mtrc::modify::compose::ParametricDiffusionCoordinatePipeline<double>>::value,
			  "parametric diffusion coordinate pipeline must live under mtrc::modify::compose");
...
auto make_coordinate_solver_network(double learning_rate = 0.001) -> mtrc::solve::parametric::dnn::Network<double>
{
...
{
	mtrc::solve::parametric::dnn::AutoencoderModel<double> model(make_coordinate_solver_network());
...
int main()
{
	...
	const auto targets = mtrc::modify::map::diffusion_coordinate_targets<decltype(space), double>(space, geometry);
	...
	try {
		(void)mtrc::modify::map::diffusion_coordinate_targets<decltype(space), double>(space, limited_geometry);
	...
	auto pipeline = mtrc::modify::compose::parametric_diffusion_coordinates(
		pipeline_builder, mtrc::solve::parametric::dnn::AutoencoderModel<double>(make_coordinate_solver_network()), geometry, calibration);
	...
	auto cached_calibration = calibration;
	cached_calibration.steps = 2;
	const auto cached_distance_builder = mtrc::modify::compose::parametric_diffusion_coordinate_pipeline_builder(0.05, 1.0)
											 .use_distance_table_pairwise_distances()
											 .use_exponential_affinity_kernel()
											 .use_lazy_row_normalized_diffusion_operator();
	auto cached_distance_pipeline = mtrc::modify::compose::parametric_diffusion_coordinates(
		cached_distance_builder, mtrc::solve::parametric::dnn::AutoencoderModel<double>(make_coordinate_solver_network()), geometry,
		cached_calibration);
	...
	auto cached_distance_model = mtrc::modify::map::derive_from(cached_distance_pipeline, space);
	const auto cached_distance_reduced = mtrc::modify::map::transform(cached_distance_model, space);
	assert(cached_distance_reduced.mapping == "parametric_diffusion_coordinates");
	assert(cached_distance_reduced.strategy == "native_metric_diffusion_coordinate_solver");
	...
	assert(reduced.mapping == "parametric_diffusion_coordinates");
	assert(reduced.strategy == "native_metric_diffusion_coordinate_solver");
	...
	const auto oos_stability =
		mtrc::modify::map::out_of_sample_neighbor_stability(mapping_artifact, space, new_space, 1);
	assert(oos_stability.diagnostic == "out_of_sample_neighbor_stability");
	assert(oos_stability.mapping == "parametric_diffusion_coordinates");
	assert(oos_stability.strategy == "native_metric_diffusion_coordinate_solver");
	...
	assert(mtrc::solve::parametric::dnn::AutoencoderModel<double>(make_coordinate_solver_network()).backend_name() ==
		   "native_dnn");

	// lc1: the parametric diffusion coordinate mapping result carries an exact one-to-one lineage aligned
	...
	// tracks the transformed space while the model keeps the source-derived count.
	{
		auto shifted_space = mtrc::make_space(records, mtrc::Euclidean<double>{});
		assert(shifted_space.erase(shifted_space.id(0)));
		const auto shifted_reduced = mtrc::modify::map::transform(mapping_artifact, shifted_space);
		...
		assert(mapping_artifact.source_record_count() == space.size());
	}

	// lc4: neighbor-preservation rejects tampered lineage on a real parametric diffusion coordinate
	...
	// coordinates (no loose inequality).
	{
		...
		mtrc::modify::map::DiffusionCoordinateSpec<double> chain_spec;
		chain_spec.dimensions = 1;
		chain_spec.diffusion_steps = 1;
		chain_spec.kernel_scale = 2.0;
		const auto chain_targets =
			mtrc::modify::map::diffusion_coordinate_targets<decltype(chain_space), double>(chain_space, chain_spec);
		...
		auto chain_mapping = mtrc::make_mapping_result(
			...
			std::move(chain_lineage.representative_records), chain_space.size(), false, "parametric_diffusion_coordinates",
			"diffusion_potential_anchor_coordinates", "metric_space",
			mtrc::core::metric_traits<mtrc::Euclidean<double>>::law, false,
			"in-sample coordinate fixture for neighbor-preservation diagnostics");
	...
	const auto restored = mapping_artifact.inverse_transform(reduced);
