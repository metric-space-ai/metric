...
//
// Hero gate: the Metric-Space Mapping Pipeline (PHATE-AE framed as a finite
...
int main()
{
	...
	assert(rejects([&] {
		auto bad = geometry;
		bad.dimensions = 0;
		(void)mtrc::modify::map::phate_geometry_targets<decltype(space), double>(space, bad);
	}));
	assert(rejects([&] {
		auto bad = geometry;
	...
	assert(rejects([&] {
		(void)mtrc::modify::map::phate_geometry_targets<decltype(space), double>(space, geometry,
	...
	// the solver moved the bottleneck geometry loss downward.
	...
	assert(reloaded.latent_dimension() == model.latent_dimension());
	assert(reloaded.mapping_name() == model.mapping_name());
	assert(reloaded.strategy_name() == model.strategy_name());
	...
	// inverse parity.
	const auto restored_a = model.inverse_transform(latent);
	...
	// negative load case: a foreign backend is rejected.
	{
		...
		assert(rejects(
			[&] { (void)mtrc::modify::map::load_native_phate_autoencoder_model<record_type, double>(foreign); }));
	}
	...
	assert(phate_preservation.recall >= 0.5 && phate_preservation.recall <= 1.0); // non-degenerate
	...
	// lineage): PHATE-AE is invertible and out-of-sample capable; classical MDS
	...
	const auto oos = mtrc::modify::map::out_of_sample_neighbor_stability(model, space, query_space, neighbor_count);
