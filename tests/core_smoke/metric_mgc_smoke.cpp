...
int main()
{
	...
	mtrc::MGC<FirstRecord, mtrc::Euclidean<double>, SecondRecord, mtrc::Manhattan<float>> mgc;
	...
	mtrc::MGC<SecondRecord, mtrc::Manhattan<float>, FirstRecord, mtrc::Euclidean<double>> reversed_mgc;
	const double reversed_result = reversed_mgc(second, first);
	assert(std::abs(reversed_result - expected) < 1e-12);

	const std::vector<double> expected_xcorr = {0.3406052387919164, 0.36670317239506234, 0.35527060120466675,
												0.3899053662191226, 0.2884565911772877,	 0.37837904236096026,
												0.4589100366738734, 0.3811881240213428,	 0.44657481646290537};
	...
	for (std::size_t index = 0; index < xcorr.size(); ++index) {
		if (std::abs(xcorr[index] - expected_xcorr[index]) >= xcorr_tolerance) {
			std::cerr << std::setprecision(17) << "xcorr[" << index << "] = " << xcorr[index] << ", expected "
					  << expected_xcorr[index] << "\n";
		}
