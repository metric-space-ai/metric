...
int main()
{
	...
	mtrc::modify::map::PhateGeometrySpec<double> spec;
	spec.dimensions = 2;
	spec.diffusion_steps = 3;
	spec.kernel_scale = 1.0;
	spec.max_dense_records = records.size();

	const auto targets = mtrc::modify::map::phate_geometry_targets<decltype(space), double>(space, spec);
	const auto repeated = mtrc::modify::map::phate_geometry_targets<decltype(space), double>(space, spec);
	assert_same_targets(targets, repeated);
	const auto cached_targets = mtrc::modify::map::phate_geometry_targets<decltype(space), double>(
		space, spec, "distance_table_pairwise_distances");
	const auto exponential_targets = mtrc::modify::map::phate_geometry_targets<decltype(space), double>(
		space, spec, "exact_space_distances", "exponential_affinity_kernel");
	const auto lazy_diffusion_targets = mtrc::modify::map::phate_geometry_targets<decltype(space), double>(
	...
	const auto repeated_anchor_targets =
		mtrc::modify::map::phate_geometry_targets<decltype(space), double>(space, repeated_anchor_spec);
	...
	assert(rejects([&] { (void)mtrc::modify::map::phate_geometry_targets<decltype(space), double>(space, invalid); }));
	...
	assert(rejects([&] {
		(void)mtrc::modify::map::phate_geometry_targets<decltype(space), double>(space, spec, "unknown_provider");
	}));
	assert(rejects([&] {
		(void)mtrc::modify::map::phate_geometry_targets<decltype(space), double>(
	...
	assert(rejects(
		[&] { (void)mtrc::modify::map::phate_geometry_targets<decltype(empty_space), double>(empty_space, spec); }));
	...
	// separation distance and of the diffusion-step count.
	{
		...
		mtrc::modify::map::PhateGeometrySpec<double> pair_spec;
		pair_spec.dimensions = 2;
		pair_spec.diffusion_steps = 1;
		pair_spec.kernel_scale = 1.0;
		const auto pair_targets =
			mtrc::modify::map::phate_geometry_targets<decltype(pair_space), double>(pair_space, pair_spec);
		...
		const auto far_targets =
			mtrc::modify::map::phate_geometry_targets<decltype(far_space), double>(far_space, pair_spec);
	...
	// coordinates after centering and scaling.
	{
		...
		mtrc::modify::map::PhateGeometrySpec<double> identical_spec;
		identical_spec.dimensions = 2;
		identical_spec.diffusion_steps = 2;
		const auto identical_targets =
			mtrc::modify::map::phate_geometry_targets<decltype(identical_space), double>(identical_space, identical_spec);
	...
	// the lone record collapses to zero coordinates.
	{
		...
		mtrc::modify::map::PhateGeometrySpec<double> single_spec;
		single_spec.dimensions = 3;
		single_spec.diffusion_steps = 2;
		single_spec.kernel_scale = 1.0;
		const auto single_targets =
			mtrc::modify::map::phate_geometry_targets<decltype(single_space), double>(single_space, single_spec);
	...
	{
		mtrc::modify::map::PhateGeometrySpec<double> tiny_scale_spec;
		...
		const auto tiny_scale_targets =
			mtrc::modify::map::phate_geometry_targets<decltype(space), double>(space, tiny_scale_spec);
