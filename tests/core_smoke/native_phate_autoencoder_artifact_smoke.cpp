...
int main()
{
	...
	auto pipeline_builder = mtrc::modify::compose::native_phate_autoencoder_pipeline_builder(0.05, 1.0)
	...
	assert(artifact.manifest.at("mapping").at("name") == "native_phate_autoencoder");
	assert(artifact.manifest.at("mapping").at("strategy") == "native_dnn_phate_ae");
	assert(artifact.manifest.at("pipeline").at("name") == "native_phate_autoencoder_pipeline");
	...
	assert(has_pipeline_component(artifact.manifest.at("pipeline").at("components"), "diffusion_operator",
								  "lazy_row_normalized_diffusion_operator"));
	assert(artifact.manifest.at("phate_geometry").at("spec").at("dimensions") == geometry.dimensions);
	assert(artifact.manifest.at("phate_geometry").at("spec").at("diffusion_steps") == geometry.diffusion_steps);
	assert(artifact.manifest.at("phate_geometry").at("spec").at("max_dense_records") == geometry.max_dense_records);
	assert(artifact.manifest.at("phate_geometry").at("targets").at("target_count") == records.size());
	assert(artifact.manifest.at("phate_geometry").at("targets").at("pairwise_distances") ==
		   "distance_table_pairwise_distances");
	assert(artifact.manifest.at("phate_geometry").at("targets").at("affinity_kernel") == "exponential_affinity_kernel");
	assert(artifact.manifest.at("phate_geometry").at("targets").at("diffusion_operator") ==
		   "lazy_row_normalized_diffusion_operator");
	assert(artifact.manifest.at("phate_geometry").at("targets").at("dense_distance_evaluations") ==
	...
	assert(artifact.diagnostics.at("epoch_count") == training.epochs);

	// tc1: training metadata is recorded on the fitted model and serialized into
	// the artifact's manifest and diagnostics blocks.
	assert(model.native_model().training_report().epochs.size() == training.epochs);
	assert(model.native_model().training_report().epochs.front().terms.size() == 2);
	assert(artifact.diagnostics.at("epochs").size() == training.epochs);
	assert(artifact.diagnostics.at("stopped_early") == false);
	assert(artifact.manifest.at("training_spec").at("seed") == training.seed);
	assert(artifact.manifest.at("training_spec").at("epochs") == training.epochs);

	auto loaded = mtrc::modify::map::load_native_phate_autoencoder_model<record_type, double>(artifact);

	// tc1 (load contract): training metadata is provenance-only. Loading rebuilds
	// the network from the artifact, so the loaded model has no training report.
	assert(loaded.native_model().training_report().epochs.empty());
	assert(loaded.mapping_name() == model.mapping_name());
	assert(loaded.strategy_name() == model.strategy_name());
	assert(loaded.source_record_count() == model.source_record_count());
	...
	assert_same_records(model.inverse_transform(latent), loaded.inverse_transform(loaded_latent));
	...
	{
		auto pipeline_a = mtrc::modify::compose::native_phate_autoencoder(
			pipeline_builder, mtrc::solve::parametric::dnn::AutoencoderModel<double>(make_autoencoder_network()), geometry,
			training);
		auto pipeline_b = mtrc::modify::compose::native_phate_autoencoder(
			pipeline_builder, mtrc::solve::parametric::dnn::AutoencoderModel<double>(make_autoencoder_network()), geometry,
			training);
		auto model_a = mtrc::modify::map::fit(pipeline_a, space);
		auto model_b = mtrc::modify::map::fit(pipeline_b, space);
		const auto artifact_a = mtrc::modify::map::make_native_phate_autoencoder_artifact(model_a, objective, training,
																						  geometry, targets, space.version());
		const auto artifact_b = mtrc::modify::map::make_native_phate_autoencoder_artifact(model_b, objective, training,
																						  geometry, targets, space.version());
			assert(artifact_a.network_binary == artifact_b.network_binary);
		assert(artifact_a.diagnostics.at("epochs").back().at("total_loss") ==
			   artifact_b.diagnostics.at("epochs").back().at("total_loss"));
	...
	// the vector record codec.
	{
		...
		try {
			(void)mtrc::modify::map::load_native_phate_autoencoder_model<record_type, double>(foreign_backend);
		...
		foreign_codec.manifest["codec"]["type"] = "FeatureRecordCodec";
		bool rejected_codec = false;
		try {
			(void)mtrc::modify::map::load_native_phate_autoencoder_model<record_type, double>(foreign_codec);
		} catch (const std::invalid_argument &) {
			rejected_codec = true;
		}
