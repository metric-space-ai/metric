...
int main()
{
	...
	constexpr std::size_t feature_count = 6;
	...
	assert(codec.to_json().at("type") == "FeatureRecordCodec");
	assert(codec.to_json().at("name") == "process_curve_feature_codec");
	...
	assert(model.training_report().epochs.size() == training.epochs);
	...
	const auto stability = mtrc::modify::map::out_of_sample_neighbor_stability(model, space, query_space, 1);
