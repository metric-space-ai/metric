...
|Class                                                                                                                                                   |Language|Constructor                                                           |()-Operator                  |Default Parameters                                    |
|--------------------------------------------------------------------------------------------------------------------------------------------------------|--------|----------------------------------------------------------------------|-----------------------------|------------------------------------------------------|
|VOI (Variation of information)                                                                                                                          |Python  |f = distance.VOI(k=3, logbase=2)                                      |result = f(dataA, dataB)     |defaults: k=3, logbase=2                              |
|                                                                                                                                                        |C++     |auto f = metric::VOI_kl(3, 2);                                        |auto result = f(dataA, dataB)|                                                      |
|VOI normalized                                                                                                                                          |Python  |f =VOI(k=3, logbase=2)                                                |result = f(dataA, dataB)     |defaults: k=3, logbase=3                              |
...
|                                                                                                                                                        |C++     |auto f = metric::Euclidean_thresholded<double>(1, 3)                  |auto result = f(dataA, dataB)|                                                      |
...
|Earth Mover's Distance Metric (EMD)                                                                                                                                                   |Python  |f = distance.EMD(cost_mat, extra_mass_penalty) f = distance.EMD(rows, cols, extra_mass_penalty) |result = f(dataA, dataB)     |defaults: cost_mat={}, extra_mass_penalty=-1          |
...
|                                                                                                                                                        |C++     |auto f = metric::kohonen_distance<double>(train_data, w, h)           |auto result = f(sample1, sample2)|                                                      |
                                                                                                                              |C++     |auto f = metric::kohonen_distance<double>(train_data, w, h)           |auto result = f(sample1, sample2)|                                      |
...
|Description                                                                                                                                             |Language|Constructor                                                           |()-Operator                  |Parameters                            |
|--------------------------------------------------------------------------------------------------------------------------------------------------------|--------|----------------------------------------------------------------------|-----------------------------|--------------------------------------|
...
|Kohonen Distance Clustering is one of the Neural Network unsupervised learning algorithms. This algorithm is used in solving problems in various areas, especially in clustering complex data sets.|Python  |f = mapping.KOC_factory(w, h)                                         |koc = f(samples, num_clusters) |-                                     |-                        |defaults: nodes_width=5, nodes_height=4, anomaly_sigma=1.0, start_learn_rate=0.8, finish_learn_rate=0, iterations=20, distribution_min=-1, distribution_max=1,  min_cluster_size=1|
|                                                                                                                                                        |C++     |auto f = mapping::KOC_factory(w, h)                                   |auto koc = f(samples, num_clusters)|-                                     |-                        |                                                                                                                                                                                  |
...
|                                                                                                                                                        |C++     |auto f = metric::Autoencoder<uint8_t, double>()                       |                             |                                      |                         |                                                                                                                                                                                  |
...
|ESN                                                                                                                                                     |C++     |auto f = metric::ESN<std::vector<double>, Euclidean<double>>()        |-                            |-                                     |                         |                                                                                                                                                                                  |
...
|affprop                                                                                                                                                 |C++     |auto f = mapping.affprop<std::vector<double>>, std::_1, 1.0, 100      |auto result = f(data)        |                                      |                         |                                                                                                                                                                                  |
...
|kmeans                                                                                                                                                  |C++     |auto f = metric::kmeans                                               |auto result = f(data, k)     |                                      |                         |                                                                                                                                                                                  |
...
|                                                                                                                                                        |Python  |f = correlation.MGC()                                                 |auto result = f(dataA, dataB)|auto result= f.estimate(dataA, dataB) |                                                                                                                                                             |
...
|                                                                                                                                                        |Python  |f = distance.Entropy(metric=Manhatten(), k=3, p=30)                   |result = f(dataA)            |result = f.estimate(data)             |                                                                                                                                                             |


**Calls METRIC | UTILS**


|Class                                                                                                                                                   |Language|Constructor                                                           |()-Operator                  |Parameters                                            |
|--------------------------------------------------------------------------------------------------------------------------------------------------------|--------|----------------------------------------------------------------------|-----------------------------|------------------------------------------------------|
