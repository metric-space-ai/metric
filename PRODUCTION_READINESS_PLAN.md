...
- **WS-1 Build/Dist:** [x] ODR-Fix · [x] workflow-Link-Ehrlichkeit · [x] LAPACK-Target · [x] pybind11-Submodul entfernt · [x] Versions-Matrix (CI baut cp310-cp314, Classifiers korrekt) · [x] Package-Manager (FetchContent-Rezept + CI-Wheel-Matrix).
...
- **WS-4 Doku:** [x] Python-Banner + falsche Claims · [x] README-LAPACK · [~] kanonische C++-Oberfläche (TUTORIAL nutzt eine konsistente) · [x] TUTORIAL.md gefüllt · [x] tutorial.cpp auf aktuelle API · [x] generierte API-Referenz (docs/Doxyfile).
...
- **WS-6 Inspektion:** [x] `operator<<`/`summary` alle Results · [x] Metadata-`operator<<` · [x] Space-`operator<<` (MetricSpace) + StructureDescription · [x] Viz/Export (`render::write_csv`/`write_histogram`) · [~] Progress-Callback (gehört in DistanceTable-Build = storage/**, anderer Track) · [~] Python describe (konvergiert mit Codex).
...
- **WS-8 API-Konsistenz:** [x] Verben nach `mtrc::` (entropy/intrinsic/regular_sample) · [x] Options re-exportieren (dbscan/k_medoids/mgc) · [~] kanonische Oberfläche (Doku) · [~] Beispiele auf workflow.hpp/materialize (TUTORIAL+tutorial.cpp zeigen den Pfad; Legacy-19 zurückgestellt, Codex-churned) · [x] quickstart.hpp · [x] compose/custom implementiert.
- **WS-9 Metrik-Auswahl:** [x] Custom-Metrik-Law (`make_true_metric`) + Doc · [~] from_records-Warnung (storage-Track) · [x] Admission/`[[deprecated]]` (Quarantäne-Marker + make_true_metric) · [x] discover_metrics (Codex) · [~] Advisory-Helfer (dokumentiert).
- **WS-10 Hygiene:** [x] SOM/KOC/DSPCC `[[deprecated]]`-Konstruktoren · [~] KOC aus engine.hpp (Scope-Doku) · [x] Quarantäne `[[deprecated]]` (RandomEMD/CramervonMises/KolmogorovSmirnov) · [x] Python Nicht-Metriken markiert (`[[deprecated]]`) + RandomEMD-Guard (SIGSEGV/Hang behoben) · [x] mapping.hpp-Banner · [x] Scope-Doku.
