...
class access {
  public:
	template <typename Archive, typename T> static void member_load(Archive &ar, T &t) { t.load(ar, 0); }

	template <typename Archive, typename T> static void member_save(Archive &ar, const T &t) { t.save(ar, 0); }
};
template <class Archive, class T> struct member_saver {
	static void invoke(Archive &ar, const T &t) { access::member_save(ar, t); }
};
...
// detail
template <class Archive, class T> inline void split_member(Archive &ar, T &t)
{
	using typex = typename std::conditional<Archive::is_saving, serialize::member_saver<Archive, T>,
											serialize::member_loader<Archive, T>>::type;
	typex::invoke(ar, t);
}
...
#define SERIALIZE_SPLIT_MEMBERS()                                                                                      \
	friend class access;                                                                                               \
	template <class Archive> void serialize(Archive &ar, const unsigned int) { serialize::split_member(ar, *this); }

template <typename Archive, typename Container> void save_collection(Archive &ar, const Container &c)
{
...
template <typename Archive, typename T> inline void save(Archive &ar, const T &t)
{
...
template <class Container> class reserve_imp {
  public:
	void operator()(Container &s, unsigned int count) const { s.reserve(count); }
};

template <class Container> class no_reserve_imp {
  public:
	void operator()(Container & /* s */, unsigned int /* count */) const {}
};
// sequential container input
template <class Archive, class Container> struct archive_input_seq {
...
// map input
template <class Archive, class Container> struct archive_input_map {
	inline void operator()(Archive &ar, Container &s)
	{
...
// set input
template <class Archive, class Container> struct archive_input_set {
	inline void operator()(Archive &ar, Container &s)
	{
		typedef typename Container::value_type type;
		type t;
...
template <typename Archive, typename T> inline void load(Archive &ar, T &t) { t.serialize(ar, 0); }

template <typename Archive, typename T, typename Allocator> inline void load(Archive &ar, std::vector<T, Allocator> &v)
{
	serialize::load_collection<Archive, std::vector<T, Allocator>,
							   archive_input_seq<Archive, std::vector<T, Allocator>>,
							   reserve_imp<std::vector<T, Allocator>>>(ar, v);
}

template <typename Archive, typename T, typename Allocator> inline void load(Archive &ar, std::list<T, Allocator> &v)
{
	load_collection<Archive, std::list<T, Allocator>, archive_input_seq<Archive, std::list<T, Allocator>>,
					no_reserve_imp<std::list<T, Allocator>>>(ar, v);
}

template <typename Archive, typename T, typename Allocator> inline void load(Archive &ar, std::deque<T, Allocator> &v)
{
	load_collection<Archive, std::deque<T, Allocator>, archive_input_seq<Archive, std::deque<T, Allocator>>,
					no_reserve_imp<std::deque<T, Allocator>>>(ar, v);
}

template <typename Archive, typename Key, typename Type, typename Compare, typename Allocator>
inline void load(Archive &ar, std::map<Key, Type, Compare, Allocator> &v)
{
	load_collection<Archive, std::map<Key, Type, Compare, Allocator>,
					archive_input_map<Archive, std::map<Key, Type, Compare, Allocator>>,
					no_reserve_imp<std::map<Key, Type, Compare, Allocator>>>(ar, v);
}

template <typename Archive, typename Key, typename Compare, typename Allocator>
inline void load(Archive &ar, std::set<Key, Compare, Allocator> &v)
{
	load_collection<Archive, std::set<Key, Compare, Allocator>,
					archive_input_set<Archive, std::set<Key, Compare, Allocator>>,
					no_reserve_imp<std::set<Key, Compare, Allocator>>>(ar, v);
}

template <typename Archive, typename First, typename Second> inline void load(Archive &ar, std::pair<First, Second> &v)
{
	ar & v.first & v.second;
}
//! outer wrapper for saving char strings
template <typename Archive> inline void save(Archive &ar, const char *s)
{
...
template <typename Archive> inline void save(Archive &ar, const std::string &s)
{
	std::size_t l = s.size();
	ar.save(l);
...
template <typename Archive> inline void load(Archive &ar, char *s)
{
	std::size_t l = 0;
	ar.load(l);
	ar.binary_load(s, l);
...
template <typename Archive> inline void load(Archive &ar, std::string &s)
{
	std::size_t l = 0;
	ar.load(l);
	s.resize(l);
...
template <typename T, typename Archive> struct compound_saver {
	static void invoke(Archive &ar, const T &t) { serialize::save(ar, t); }
};

//! partial specialization of struct saver for fundamental types
template <typename T, typename Archive> struct binary_saver {
	static void invoke(Archive &ar, const T &t) { ar.binary_save(&t, sizeof(T)); }
};
 ...
 *
 */
template <typename T, typename Archive> struct compound_loader {
	static void invoke(Archive &ar, T &t) { serialize::load(ar, t); }
};
//! partial specialization of struct loader for fundumental types
template <typename T, typename Archive> struct binary_loader {
	static void invoke(Archive &ar, T &t) { ar.binary_load(&t, sizeof(T)); }
};
 ...
 *
 */
template <typename OStream> class oarchive {
	OStream &os;
  ...
  public:
	...
	template <typename T> oarchive &operator&(const T &t)
	{
		save(t);
		return *this;
	}
	...
	{
		using typex =
			typename std::conditional<std::is_fundamental<T>::value, serialize::binary_saver<T, oarchive<OStream>>,
									  serialize::compound_saver<T, oarchive<OStream>>>::type;
		typex::invoke(*this, t);
	}

	void binary_save(const void *address, std::size_t count)
	{
		count = (count + sizeof(typename OStream::char_type) - 1) / sizeof(typename OStream::char_type);
 ...
 *
 */
template <typename IStream> class iarchive {
  private:
	IStream &is;
  ...
  public:
	...
	template <typename T> iarchive &operator>>(T &t) { return *this & t; }
	template <typename T> iarchive &operator&(T &t)
	{
		load(t);
		return *this;
	}
	...
	{
		using typex =
			typename std::conditional<std::is_fundamental<T>::value, serialize::binary_loader<T, iarchive<IStream>>,
									  serialize::compound_loader<T, iarchive<IStream>>>::type;
		typex::invoke(*this, t);
	}
