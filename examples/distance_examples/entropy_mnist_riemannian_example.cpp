...
int main()
{
	...
	for (size_t i = 0; i < 10; ++i) {
		for (size_t j = i; j < 10; ++j) {
			std::cout << "Riemannian distance for digits " << i << ", " << j << ": "
					  << rd(imgs_by_digit_encoded[i], imgs_by_digit_encoded[j]) << std::endl;
		}
		std::cout << "Riemannian distance for digits " << i
				  << ", all: " << rd(imgs_by_digit_encoded[i], imgs_mixed_encoded) << std::endl;
	}
