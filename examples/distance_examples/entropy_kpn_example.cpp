...
#include <tuple>
#include <vector>
// #include "metric/correlation/epmgp.cpp"
...
int main()
{
	...
	mtrc::numeric::DynamicVector<double> x = {1, 2, 3};
	std::cout << "\nmvnpdf at point: " << mtrc::mvnpdf(x) << "\n\n";
...
//                     ", std::exp(a*a): " << std::exp(a*a) <<
	...
	//    mtrc::numeric::DynamicVector<double> m = {0, 0};
	//    mtrc::numeric::DynamicMatrix<double> K = {{1, 0}, {0, 1}};
	//    mtrc::numeric::DynamicVector<double> lowerB = {-0.5, -0.5};
	//    mtrc::numeric::DynamicVector<double> upperB = {0.5, 0.5};

	//    mtrc::numeric::DynamicVector<double> m = {5, -5};
	//    mtrc::numeric::DynamicMatrix<double> K = {{1, 0.3}, {0.3, 1}};
	//    mtrc::numeric::DynamicVector<double> lowerB = {0.5, 1.5};
	//    mtrc::numeric::DynamicVector<double> upperB = {1.5, 2.5};

	mtrc::numeric::DynamicVector<double> m = {-250, -250};
	mtrc::numeric::DynamicMatrix<double> K = {{1, 0}, {0, 1}};
	mtrc::numeric::DynamicVector<double> lowerB = {200, 200};
	mtrc::numeric::DynamicVector<double> upperB = {30000, 30000};

	//    mtrc::numeric::DynamicVector<double> m = {5, -5, -3};
	//    mtrc::numeric::DynamicMatrix<double> K = {{1, 0.3, 0.7}, {0.3, 1, 0.5}, {0.7, 0.5, 1}};
	//    mtrc::numeric::DynamicVector<double> lowerB = {0.5, 1.5, 0};
	//    mtrc::numeric::DynamicVector<double> upperB = {1.5, 2.5, 3};
	...
	{
		// auto e_f = mtrc::Entropy<void, mtrc::Chebyshev<double>>();
	...
	auto ekpn_cheb3 = mtrc::Entropy<void, mtrc::Chebyshev<double>>(mtrc::Chebyshev<double>(), 3, 10);
	auto ekpn_cheb7 = mtrc::Entropy<void, mtrc::Chebyshev<double>>(mtrc::Chebyshev<double>(), 7, 10);
	auto ekpn_eucl = mtrc::Entropy<void, mtrc::Euclidean<double>>(mtrc::Euclidean<double>(), 3, 10);

	std::cout << "using Chebyshev: " << ekpn_cheb3(urv) << ", " << ekpn_cheb7(urv) << std::endl;

	// auto e = mtrc::Entropy_kpN<std::vector<double>, mtrc::Euclidean<double>>(urv, mtrc::Euclidean<double>(), 3,
	// 10);
