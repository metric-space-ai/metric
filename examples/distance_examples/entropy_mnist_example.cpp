...
// NOTE: this example calls Datasets::getMnist(), which in the dependency-free C++ core no
// longer reads the legacy "data.cereal" MNIST archive (it throws at runtime). It is kept for
// reference and built only when -DMETRIC_EXAMPLES_WITH_MNIST=ON. The cereal archive format is
// NOT a METRIC dependency.
...
int main()
{
	...
	for (auto el : features) {
		...
		auto y = cnt % shape[2];
		if (cnt >= shape[1] * margin && cnt < shape[2] * (shape[1] - margin) && y >= margin &&
			y < (shape[2] - margin))			 // crop
	...
	auto pcfa = mtrc::PCFA<std::vector<double>, void>(imgs_mixed, 20);
	...
	// auto estimator = mtrc::Entropy<void, mtrc::Euclidean<double>>();
	auto estimator = mtrc::EntropySimple<void, mtrc::Euclidean<double>>(mtrc::Euclidean<double>(), 7);
	//    auto estimator = mtrc::EntropySimple<void, mtrc::Chebyshev<double>>(mtrc::Chebyshev<double>(), 7);
	//    auto estimator = mtrc::Entropy<std::vector<double>>();
	//    auto estimator = mtrc::EntropySimple<std::vector<double>>();
	...
	// auto vm = mtrc::VMixing<void, mtrc::Euclidean<double>>(mtrc::Euclidean<double>(), 7, 70);
	auto vms = mtrc::VMixing_simple<void, mtrc::Euclidean<double>>(mtrc::Euclidean<double>(), 7);

	// std::cout << "digit1 digit2 entropy1 entropy2 | entropy_mixed entropy_mixed_half entropy_mixed_u
	// entropy_mixed_half_u entropy_joint | diff_mixed diff_mixed_half diff_mixed_u diff_mixed_half_u VOI\n"; std::cout
	// << "digit1 digit2 entropy1 entropy2 | entropy_mixed entropy_joint | diff_mixed VOI\n"; std::cout << "digit1
	// digit2 entropy1 entropy2 | entropy_mixed entropy_mixed_half entropy_joint | diff_mixed diff_mixed_half VOI\n";
	...
	for (size_t i = 0; i < 10; ++i) { // TODO set i = 0
		for (size_t j = i; j < 10; ++j) {
			...
			//            concat_ds_half.insert(concat_ds_half.end(), imgs_by_digit_encoded[i].begin(),
			//            imgs_by_digit_encoded[i].begin() + imgs_by_digit_encoded[i].size()/2);
			//            concat_ds_half.insert(concat_ds_half.end(), imgs_by_digit_encoded[j].begin(),
			//            imgs_by_digit_encoded[j].begin() + imgs_by_digit_encoded[j].size()/2); auto H_concat_half =
			//            estimator(concat_ds_half);
			std::cout <<
				//                         "VMixed for digits " << i << ", " << j << ": " <<
				i << " " << j << " " << H_i << " " << H_j << " | " << H_concat << " " <<
				//                         H_concat_half << " " <<
				//                         H_joint <<
				" | " << H_concat * 2 - H_i - H_j << " " <<
				...
				//                         H_joint*2 - H_i - H_j << " " <<
				" | " <<
				// mtrc::VOI_simple(imgs_by_digit_encoded[i], imgs_by_digit_encoded[j], 7) <<
				// mtrc::VOI_simple<std::vector<std::vector<double>>,
				// mtrc::Euclidean<double>>(imgs_by_digit_encoded[i], imgs_by_digit_encoded[j], 7) <<
		...
		//        concat_ds_half.insert(concat_ds_half.end(), imgs_by_digit_encoded[i].begin(),
		//        imgs_by_digit_encoded[i].begin() + imgs_by_digit_encoded[i].size()/2);
		//        concat_ds_half.insert(concat_ds_half.end(), imgs_mixed_encoded.begin(), imgs_mixed_encoded.begin() +
		//        imgs_mixed_encoded.size()/2); auto H_concat_half = estimator(concat_ds_half);
		std::cout << i << " * " << H_i << " " << H_j << " | " << H_concat << " " <<
			//                     H_concat_half << " " <<
			//                     H_joint <<
			" | " << H_concat * 2 - H_i - H_j << " " <<
			//                     H_concat_half*2 - H_i - H_j << " " <<
			...
			// mtrc::VOI_simple(imgs_by_digit_encoded[i], imgs_mixed_encoded, 7) <<
			// mtrc::VOI_simple<std::vector<std::vector<double>>, mtrc::Euclidean<double>>(imgs_by_digit_encoded[i],
			// imgs_mixed_encoded, 7) <<
	...
	//    imgs_mixed_encoded.begin() + imgs_mixed_encoded.size()/2); auto H_concat_half = estimator(concat_ds_half);
	std::cout << "* * " << H_i << " " << H_j << " | " << H_concat << " " <<
		//                 H_concat_half << " " <<
		//                 H_joint <<
		" | " << H_concat * 2 - H_i - H_j << " " <<
		//                 H_concat_half*2 - H_i - H_j << " " <<
		...
		// mtrc::VOI_simple(imgs_by_digit_encoded[i], imgs_mixed_encoded, 7) <<
		// mtrc::VOI_simple<std::vector<std::vector<double>>, mtrc::Euclidean<double>>(imgs_mixed_encoded,
		// imgs_mixed_encoded, 7) <<
