...
template <typename T> std::vector<T> extract_energies(std::vector<T> x)
{
...
template <typename T> std::vector<std::vector<T>> sequential_DWT(std::vector<T> x)
{
	auto [x0, tmp1] = wavelet::dwt(x, 5);
	auto [x1, tmp2] = wavelet::dwt(x0, 5);
	auto [x11, tmp3] = wavelet::dwt(x1, 5);
...
template <typename T> std::vector<T> get_energies(std::vector<std::vector<T>> subbands)
{
	std::vector<T> energies(subbands.size());
	for (int i = 0; i < subbands.size(); ++i) {
		T sum = 0;
...
Container<Container<Container<ValueType, A1>, A2>, std::allocator<Container<Container<ValueType, A1>, A2>>>
divide_timeseries(Container<Container<ValueType, A1>, A2> ts, size_t block_length)
{
	auto output =
		Container<Container<Container<ValueType, A1>, A2>, std::allocator<Container<Container<ValueType, A1>, A2>>>();
...
template <template <class, class> class Container, class ValueType, class A1, class A2>
Container<Container<ValueType, A1>, A2>
transpose_timeseries(Container<Container<ValueType, A1>, A2> ts) // TODO remove and update csv reader this way
{
...
template <template <class, class> class Container, class ValueType, class A1, class A2>
Container<Container<Container<ValueType, A1>, A2>, std::allocator<Container<Container<ValueType, A1>, A2>>>
apply_seq_DWT(Container<Container<ValueType, A1>, A2> ts)
{
	auto output =
		Container<Container<Container<ValueType, A1>, A2>, std::allocator<Container<Container<ValueType, A1>, A2>>>();
...
Container<Container<ValueType, A1>, A2>
apply_energy_extractor(Container<Container<Container<ValueType, A1>, A2>, A3> ts_blocks)
{
	auto output = Container<Container<ValueType, A1>, A2>();
...
template <template <class, class> class Container, class ValueType, class A1, class A2, class A3>
Container<Container<Container<ValueType, A1>, A2>, A3>
apply_energy_extractor_separate_subbands(Container<Container<Container<ValueType, A1>, A2>, A3> ts_blocks)
{
...
int main()
{
	...
	// single
	//    auto training_dataset = read_csv_matrix<float>("Pt01_Ch01_Grooves_1_to_7.csv");
	//    // std::cout << training_dataset << "\n";

	//    auto test_data = read_csv_matrix<float>("Pt01_Ch01_Groove09.csv");

	//    mat2bmp::matrix_to_bmp(training_dataset, "training_dataset.bmp");
	//    mat2bmp::matrix_to_bmp(test_data, "test_data.bmp");
