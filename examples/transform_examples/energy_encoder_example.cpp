...
int main()
{
	...
	auto e = mtrc::EnergyEncoder(5, 1);
	// auto e = mtrc::energy_encoder();
	auto result = e(data);
	...
	for (size_t i = 1; i < 18; ++i) {
		// std::cout << mtrc::subband_size(lx, i, lf) << "\n";
		std::stack<size_t> subband_length;
		auto split = mtrc::sequential_DWT<std::vector, std::vector<double>, std::allocator<std::vector<double>>>(
			x, subband_length, 5, subbands_num);
		...
		auto lr_estim = mtrc::subband_size(lx, i, lf);
		auto lx_estim = mtrc::original_size(lr, i, lf);
		std::cout << lr_estim << " " << lr << " | " << mtrc::original_size(lr_estim, i, lf) << " " << lx_estim
				  << " | " << mtrc::subband_size(lx_estim, i, lf) << "\n";
	...
	auto e2 = mtrc::EnergyEncoder(5, 4);
