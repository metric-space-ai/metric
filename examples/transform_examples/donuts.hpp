...
mtrc::numeric::DynamicMatrix<double> create_filter(double sigma) {
	std::size_t size = ceil(sigma)*3 + 1;
	mtrc::numeric::DynamicMatrix<double> out (size, size, 0);
...
mtrc::numeric::DynamicMatrix<double> gblur3(mtrc::numeric::DynamicMatrix<double> in) {
	auto im = matrix_to_cv(in);
	cv::Mat imb;
	cv::GaussianBlur(im, imb, cv::Size(3, 3), 0);
	return cv_to_matrix(imb);
}


mtrc::numeric::DynamicMatrix<double> gaussianBlur(mtrc::numeric::DynamicMatrix<double> in, double sigma) {
	auto im = matrix_to_cv(in);
	cv::Mat krn = matrix_to_cv(create_filter(sigma));
	cv::Mat imb;
...
template <typename T> std::vector<std::vector<T>> binarize(std::vector<std::vector<T>> in, T level)
{
...
template <typename T>
std::vector<std::vector<T>> otsu_binarize(std::vector<std::vector<T>> in, float expo_corr = 0, T max = 1,
										  int levels = 100)
{
...
mtrc::numeric::SymmetricMatrix<mtrc::numeric::CompressedMatrix<int>>
imgToGraph4(std::vector<std::vector<int>> in) {
	...
	mtrc::numeric::SymmetricMatrix<mtrc::numeric::CompressedMatrix<int>> out;
...
std::vector<std::vector<double>> radial_diff(std::vector<std::vector<double>> in, double step = 1.5)
{
	...
	for (int y = 0; y < (int)in.size(); ++y) {
		...
		for (int x = 0; x < (int)in[0].size(); ++x) {
			...
			double fi =
				atan2(x - x0, y - y0); // TODO remove trigonometry functions, components can be computed without angle!
...
template <typename T>
mtrc::numeric::DynamicMatrix<T> radial_diff(const mtrc::numeric::DynamicMatrix<T> &in, double step = 1.5)
{
	...
	mtrc::numeric::DynamicMatrix<T> out(in.rows(), in.columns());
...
template <typename T> mtrc::numeric::DynamicMatrix<T> weightingMask(size_t h, size_t w, T radius, T sigma)
{ // radius is where we expect the donut outline
	T center_x = T(w) / 2.0;
	T center_y = T(h) / 2.0;
	auto mask = mtrc::numeric::DynamicMatrix<T>(h, w);
...
void eat_donut(int number) {  // old test batch processing function

	boost::gil::rgb8_image_t img;
	//boost::gil::read_image("assets/" + std::to_string(number) + ".jpg", img, boost::gil::jpeg_tag());
	//boost::gil::read_image("assets/new/crop_2020-07-27_16_23_01_776_donut" + std::to_string(number) + ".png", img,
boost::gil::jpeg_tag());
	//boost::gil::png_read_and_convert_image("assets/new/crop_2020-07-27_16_23_01_776_donut" + std::to_string(number) +
".png", img); boost::gil::read_image("assets/new/crop_2020-07-27_16_23_01_776_donut" + std::to_string(number) + ".png",
img, boost::gil::png_tag()); auto gray = boost::gil::color_converted_view<boost::gil::gray8_pixel_t>(const_view(img));
...
//    double max_val = 0;
//    mtrc::numeric::DynamicMatrix<double> inner_numeric (inner.size(), inner[0].size(), 0);
//    for (size_t y=0; y<inner.size(); ++y) {
//        for (size_t x=0; x<inner[0].size(); ++x) {
//            inner_numeric(y, x) = inner[y][x]*255;
//            if (inner_numeric(y, x) > max_val) {
//                max_val = inner_numeric(y, x);
//            }
//        }
//    }
//    mtrc::numeric::DynamicMatrix<double> outer_numeric (outer.size(), outer[0].size(), 0);
//    for (size_t y=0; y<outer.size(); ++y) {
//        for (size_t x=0; x<outer[0].size(); ++x) {
//            outer_numeric(y, x) = outer[y][x]*255;
//            if (outer_numeric(y, x) > max_val) {
//                max_val = outer_numeric(y, x);
//            }
//        }
//    }
	//auto inner_result = mtrc::fit_hysteresis(inner_numeric);
	//auto outer_result = mtrc::fit_hysteresis(outer_numeric);
