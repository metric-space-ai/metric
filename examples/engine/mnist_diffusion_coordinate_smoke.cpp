...
struct Options {
	...
	std::filesystem::path export_dir{"docs/examples/assets/mnist-diffusion-coordinate-smoke"};
	std::size_t per_class{6};
	std::size_t calibration_steps{4};
...
int main(int argc, char **argv)
{
	...
	if (!has_mnist_calibration_files(options.mnist_dir)) {
		std::cout << "mnist diffusion coordinate smoke skipped: MNIST train IDX files not found in "
	...
	std::cout << "mnist diffusion coordinate smoke dataset = MNIST train IDX\n";
	std::cout << "mnist diffusion coordinate smoke records = " << records.size() << "\n";
	std::cout << "mnist diffusion coordinate smoke per class = " << options.per_class << "\n";
	std::cout << "mnist diffusion coordinate smoke metric = Euclidean(normalized_pixels)\n";
	std::cout << "mnist diffusion coordinate smoke dense evaluations = " << targets.dense_distance_evaluations << "\n";
	std::cout << "mnist diffusion coordinate smoke calibration steps = " << options.calibration_steps << "\n";
	std::cout << "mnist diffusion coordinate smoke initial coordinate target mse = " << initial_coordinate_error << "\n";
	std::cout << "mnist diffusion coordinate smoke final coordinate target mse = " << final_coordinate_error << "\n";
	if (options.export_files) {
		std::cout << "mnist diffusion coordinate smoke export dir = " << options.export_dir.string() << "\n";
	}
}
