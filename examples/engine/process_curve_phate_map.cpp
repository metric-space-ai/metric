...
int main()
{
	...
	constexpr std::size_t feature_count = 7;
	auto codec = mtrc::solve::parametric::dnn::make_feature_record_codec<ProcessCurve, double>(
		feature_count, CurveFeatureEncoder{feature_count}, "process_curve_feature_codec");
	...
	assert(model.codec().to_json().at("name") == "process_curve_feature_codec");
	assert(!model.codec().inverse_supported());
	...
	try {
		(void)model.inverse_transform(latent);
	...
	std::cout << "process PHATE records = " << source_records.size() << "\n";
	std::cout << "process PHATE queries = " << held_out_queries.size() << "\n";
	std::cout << "process PHATE source metric = aligned_curve_distance\n";
	std::cout << "process PHATE feature codec = " << model.codec().to_json().at("name").template get<std::string>()
			  << "\n";
	std::cout << "process PHATE pipeline components = " << model.pipeline_plan().component_count() << "\n";
	std::cout << "process PHATE pipeline codec = " << hooks.codec << "\n";
	std::cout << "process PHATE distance provider = " << hooks.pairwise_distances << "\n";
	std::cout << "process PHATE affinity kernel = " << hooks.affinity_kernel << "\n";
	std::cout << "process PHATE diffusion operator = " << hooks.diffusion_operator << "\n";
	std::cout << "process PHATE geometry target evaluations = " << targets.dense_distance_evaluations << "\n";
	std::cout << "process PHATE metric nearest family = " << first_metric_family << "\n";
	std::cout << "process PHATE vector baseline family = " << first_vector_family << "\n";
	std::cout << "process PHATE latent nearest family = " << first_latent_family << "\n";
	std::cout << "process PHATE metric correct = " << metric_correct << "/" << held_out_queries.size() << "\n";
	std::cout << "process PHATE vector misses = " << vector_misses << "/" << held_out_queries.size() << "\n";
	std::cout << "process PHATE latent correct = " << latent_correct << "/" << held_out_queries.size() << "\n";
	for (const auto &diagnostic : query_diagnostics) {
		std::cout << "process PHATE query = " << diagnostic << "\n";
	}
	std::cout << "process PHATE bottleneck loss = " << initial_bottleneck << " -> " << final_bottleneck << "\n";
	std::cout << "process PHATE out-of-sample support = parametric_space_transform\n";
	std::cout << "process PHATE out-of-sample anchor recall = " << oos_stability.anchor_recall << "\n";
	std::cout << "process PHATE mapped-anchor source rank = " << oos_stability.average_mapped_best_source_rank << "\n";
	std::cout << "process PHATE max mapped-anchor source rank = " << oos_stability.maximum_mapped_best_source_rank
			  << "\n";
	std::cout << "process PHATE original-metric distance penalty = " << oos_stability.average_best_distance_penalty
			  << "\n";
	std::cout << "process PHATE max original-metric distance penalty = " << oos_stability.maximum_best_distance_penalty
			  << "\n";
	std::cout << "process PHATE inverse support = none_feature_codec\n";
	std::cout << "process PHATE deterministic seed = " << training.seed << "\n";
