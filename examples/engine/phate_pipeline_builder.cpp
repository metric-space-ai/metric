...
int main()
{
	...
	assert(plan.name() == "native_phate_autoencoder_pipeline");
	...
	assert(plan.has_component_parameter("loss", "bottleneck_coordinate_mse_loss", "weight", "1"));

	auto metadata_builder = mtrc::modify::compose::native_phate_autoencoder_pipeline_builder(0.05, 1.0);
	...
	const auto executable_default = mtrc::modify::compose::resolve_native_phate_autoencoder_executable_hooks<double>(
		mtrc::modify::compose::native_phate_autoencoder_pipeline_plan(0.05, 1.0));
	assert(executable_default.executable());
	const auto executable_replaced = mtrc::modify::compose::resolve_native_phate_autoencoder_executable_hooks<double>(plan);
	...
	const auto executable_metadata =
		mtrc::modify::compose::resolve_native_phate_autoencoder_executable_hooks<double>(metadata_plan);
	assert(!executable_metadata.executable());

	std::cout << "phate pipeline builder plan = " << plan.name() << "\n";
	std::cout << "phate pipeline builder components = " << plan.component_count() << "\n";
	std::cout << "phate pipeline builder replacement points = " << replacement_points.size() << "\n";
	std::cout << "phate pipeline default executable = yes\n";
	std::cout << "phate pipeline distance-table executable = yes\n";
	std::cout << "phate pipeline metadata replacement executable = no\n";
	std::cout << "phate pipeline builder codec = vector_record_codec\n";
	std::cout << "phate pipeline builder distance provider = distance_table_pairwise_distances\n";
	std::cout << "phate pipeline builder affinity kernel = exponential_affinity_kernel\n";
	std::cout << "phate pipeline builder diffusion operator = lazy_row_normalized_diffusion_operator\n";
