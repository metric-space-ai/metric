...
int main()
{
	...
	mtrc::numeric::DynamicMatrix<value_type> ds =
		read_csv_matrix<value_type>("training_ds_2_fragm.csv"); //, ",", 10000);
	// mtrc::numeric::DynamicMatrix<value_type> ds = read_csv_matrix<value_type>("slice.csv"); //, ",", 10000);
	// mtrc::numeric::DynamicMatrix<value_type> ds = read_csv_matrix<value_type>("slice_small.csv"); //, ",", 10000);
	...
	// matrix_to_csv(ds, "input.csv"); // resave with fixed name for ploting script

	auto d = DetSwitchDetector<value_type>();

	auto switches = d.encode(ds);

	matrix_to_csv(switches, "switches.csv");
