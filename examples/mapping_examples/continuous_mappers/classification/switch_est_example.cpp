...
template <class ContainerType> void v_to_csv(const ContainerType data, const std::string filename) // single column
{
	std::ofstream outputFile;
	outputFile.open(filename);
...
int main()
{
	...
	// Numeric example: dataset passed as Numeric matrix, data points in COLUMNS
	...
	// read dataset and save model
	{
		...
		mtrc::numeric::DynamicMatrix<value_type> ds_in =
			read_csv_matrix<value_type>("training_ds_1_fragm.csv"); //, ",", 10000);
		mtrc::numeric::DynamicMatrix<value_type> training_ds(ds_in.rows(), 3, 0);
		mtrc::numeric::submatrix(training_ds, 0, 0, ds_in.rows(), 3) =
			mtrc::numeric::submatrix(ds_in, 0, 1, ds_in.rows(), 3);

		mtrc::numeric::DynamicMatrix<value_type> labels(ds_in.rows(), 1);
		mtrc::numeric::column(labels, 0) = mtrc::numeric::column(ds_in, 4);
		...
		model.save("tmp.numeric");
	}

	// load and apply the model
	{
		auto model = SwitchPredictor<value_type>("tmp.numeric");

		auto start_time = std::chrono::steady_clock::now();

		mtrc::numeric::DynamicMatrix<value_type> ds_pred =
			read_csv_matrix<value_type>("training_ds_2_fragm.csv"); //, ",", 10000);
		mtrc::numeric::DynamicMatrix<value_type> ds(ds_pred.rows(), 3, 0);
		mtrc::numeric::submatrix(ds, 0, 0, ds_pred.rows(), 3) =
			mtrc::numeric::submatrix(ds_pred, 0, 1, ds_pred.rows(), 3);

		auto est = model.encode(ds);

		matrix_to_csv(est, "estimation1.csv");
	...
	// load and apply the model
	{
		...
		auto model = SwitchPredictor<value_type>("tmp2.numeric");
