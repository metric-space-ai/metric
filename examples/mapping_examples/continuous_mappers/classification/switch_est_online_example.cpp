...
int main()
{
	...
	auto model = SwitchPredictor<value_type>("model.numeric");
...

	{
		...
		while (passed + sizes[s_idx] < ds.size()) { // we still can fetch the slice of given size
			...
			auto pairs =
				model.encode(slice_indices, slice); // adds to buffer and gets output once the buffer is fulfilled
	...
	std::cout << std::endl
			  << std::endl
			  << "online estimation with pair output completed in "
			  << double(std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count()) / 1000000
			  << " s" << std::endl;
	...
	model = SwitchPredictor<value_type>("model.numeric"); // reset the model in order to empty the buffer
...

	{
		...
		while (passed + sizes[s_idx] < ds.size()) { // we still can fetch the slice of given size
			...
			auto raw_res =
				model.encode_raw(slice_indices, slice); // adds to buffer and gets output once the buffer is fulfilled
			// returns "raw" estimations used for making the full output .csv that contains all samples and is easy to
			// plot
	...
	std::cout << std::endl
			  << std::endl
			  << "online estimation with pair and raw output completed in "
			  << double(std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count()) / 1000000
			  << " s" << std::endl;
