...
int main()
{
...

	{
		...
		print_stats(normalized_err_stats<mtrc::Euclidean<double>>(d, d_upd));
	...
	mat2bmp::matrix_to_bmp_norm(vdata, "input.bmp", magnitude);

	//    std::stack<size_t> length_stack;
	//    auto decomposed = mtrc::sequential_DWT(vdata[0], length_stack, 5, 8);
	//    auto restored = mtrc::sequential_iDWT(decomposed, length_stack, 5);
	...
	for (float mix = 0; mix <= 1; mix += 0.25) {
		...
		auto vDSPCC = mtrc::DSPCC<std::vector<double>, void>(vdata, 10, 16, mix, 10);
		// dataset,
		...
		if (visualize) {
			mat2bmp::matrix_to_bmp_norm(v_decoded, "decoded.bmp", magnitude);
		...
		auto err_tf = normalized_err_stats<mtrc::Euclidean<double>>(vdata, v_decoded);
		print_stats(err_tf);
		...
		if (visualize) {
			...
			mat2bmp::matrix_to_bmp_norm(v_pre_decoded, "pre_decoded.bmp", magnitude);
		}

		std::cout << std::endl << "test of pre-compression done, pre-decoded data saved" << std::endl;
		auto err_pre = normalized_err_stats<mtrc::Euclidean<double>>(vdata, v_pre_decoded);
		print_stats(err_pre);
		...
		if (visualize) {
			mat2bmp::matrix_to_bmp_norm(v_encoded2, "encoded2.bmp", magnitude);
			write_csv(transpose_timeseries(v_encoded2), "encoded2.csv", ";");
			mat2bmp::matrix_to_bmp_norm(v_decoded2, "decoded2.bmp", magnitude);
			write_csv(transpose_timeseries(v_decoded2), "decoded2.csv", ";");
		}

		std::cout << std::endl << "completely encoded data saved" << std::endl;
		auto err_full = normalized_err_stats<mtrc::Euclidean<double>>(vdata, v_decoded2);
