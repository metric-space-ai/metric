 ...
 */
template <typename Container> void binarize_each(Container &values, double threshold = 30.0)
{
	for (auto &vec : values) {
		for (auto &v : vec) {
 ...
 */
template <typename Container> double mean(const Container &container)
{
	double mean = 0.0;
 ...
 */
template <typename Container> double stddev(const Container &container, double mean)
{
	double std = 0.0;
	for (auto &value : container) {
 ...
 */
template <typename Container> void normalize_each(Container &values)
{
	for (auto &vec : values) {
		// zero-mean
 ...
 */
template <typename Dataset> void binarize_dataset(Dataset &dataset)
{
 ...
 */
template <typename Dataset> void normalize_dataset(Dataset &dataset)
{
