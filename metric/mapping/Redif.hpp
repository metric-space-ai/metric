 ...
 *
 */
template <typename Tv = double, class Metric = mtrc::Euclidean<Tv>> class Redif {
  public:
	/**
	 * @brief
  ...
  private:
	 ...
	 * @param end
	 */
	template <bool flag = mtrc::numeric::rowVector>
	void Quicksort(mtrc::numeric::DynamicVector<size_t> &idx, mtrc::numeric::DynamicVector<Tv, flag> &data,
				   size_t start, size_t end);

	template <bool flag = mtrc::numeric::rowVector>
	size_t partition(mtrc::numeric::DynamicVector<size_t> &idx, mtrc::numeric::DynamicVector<Tv, flag> &data,
					 size_t start, size_t end);
	 ...
	 * @return denoised (encoded) records.
	 */
	mtrc::numeric::DynamicMatrix<Tv> encode(const mtrc::numeric::DynamicMatrix<Tv> &x);
	 ...
	 * @return Decoded records.
	 */
	mtrc::numeric::DynamicMatrix<Tv> decode(const mtrc::numeric::DynamicMatrix<Tv> &xEncoded);
	 ...
	 */
	mtrc::numeric::DynamicMatrix<Tv> getLocalDistMatrix(const mtrc::numeric::DynamicMatrix<Tv> &dataSample);
	 ...
	 */
	mtrc::numeric::DynamicMatrix<Tv> calcWeightedGraphLaplacian(const mtrc::numeric::DynamicMatrix<Tv> &localDist);
	 ...
	 */
	mtrc::numeric::DynamicMatrix<Tv> trainModel(size_t nIter);
	 ...
	 */
	mtrc::numeric::DynamicMatrix<Tv> xTrain;

	/**
	 * @brief
	 * Encoded train dataset.
	 */
	mtrc::numeric::DynamicMatrix<Tv> xTrainEncoded;

	/**
	 * @brief
	 * Diffusion Model. Laplacian Matrices of each iteration.
	 */
	std::vector<mtrc::numeric::DynamicMatrix<Tv>> LArray;
};
