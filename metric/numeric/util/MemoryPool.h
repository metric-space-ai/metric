...
#ifndef METRIC_NUMERIC_UTIL_MEMORYPOOL_H
#define METRIC_NUMERIC_UTIL_MEMORYPOOL_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
*/
template <typename Type, size_t Blocksize> class MemoryPool : private NonCopyable {
  private:
	//**union FreeObject****************************************************************************
...
*/
template <typename Type, size_t Blocksize> inline void MemoryPool<Type, Blocksize>::Block::init()
{
...
// \return void
*/
template <typename Type, size_t Blocksize> inline void MemoryPool<Type, Blocksize>::Block::free()
{
	delete[] rawMemory_;
}
//*************************************************************************************************
 ...
 */
template <typename Type, size_t Blocksize> inline MemoryPool<Type, Blocksize>::MemoryPool()
{
...
*/
template <typename Type, size_t Blocksize> inline void *MemoryPool<Type, Blocksize>::malloc()
{
	if (!freeList_) {
		blocks_.push_back(Block());
		Block &block = blocks_.back();
...
// \return void
*/
template <typename Type, size_t Blocksize> inline void MemoryPool<Type, Blocksize>::free(void *rawMemory)
{
	FreeObject *ptr = reinterpret_cast<FreeObject *>(rawMemory);
	METRIC_NUMERIC_INTERNAL_ASSERT(checkMemory(ptr), "Memory pool check failed");
	ptr->next_ = freeList_;
	freeList_ = ptr;
}
//*************************************************************************************************
...
inline bool MemoryPool<Type, Blocksize>::checkMemory(FreeObject *toRelease) const
{
	for (typename Blocks::const_iterator it = blocks_.begin(); it != blocks_.end(); ++it) {
		...
		if (toRelease >= it->rawMemory_ && toRelease < it->rawMemory_ + Blocksize) {
			...
			if ((ptr1 - ptr2) % sizeof(FreeObject) != 0)
				return false;
