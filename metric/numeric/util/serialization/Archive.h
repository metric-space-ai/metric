...
#ifndef METRIC_NUMERIC_UTIL_SERIALIZATION_ARCHIVE_H
#define METRIC_NUMERIC_UTIL_SERIALIZATION_ARCHIVE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class Archive {
  public:
	...
	/*!\name Constructors */
	//@{
	template <typename... Args> explicit inline Archive(Args &&...args);
	...
	//@{
	template <typename T> EnableIf_t<IsNumeric_v<T>, Archive &> operator<<(const T &value);

	template <typename T> DisableIf_t<IsNumeric_v<T>, Archive &> operator<<(const T &value);

	template <typename T> EnableIf_t<IsNumeric_v<T>, Archive &> operator>>(T &value);

	template <typename T> DisableIf_t<IsNumeric_v<T>, Archive &> operator>>(T &value);

	template <typename Type> inline EnableIf_t<IsNumeric_v<Type>, Archive &> write(const Type *array, size_t count);

	template <typename Type> inline EnableIf_t<IsNumeric_v<Type>, Archive &> read(Type *array, size_t count);
	//@}
	//**********************************************************************************************
...
inline EnableIf_t<IsNumeric_v<Type>, Archive<Stream> &> Archive<Stream>::write(const Type *array, size_t count)
{
	using CharType = typename Stream::char_type;
...
template <typename Stream> // Type of the bound stream
template <typename Type>   // Type of the array elements
inline EnableIf_t<IsNumeric_v<Type>, Archive<Stream> &> Archive<Stream>::read(Type *array, size_t count)
{
	using CharType = typename Stream::char_type;
