...
#ifndef METRIC_NUMERIC_UTIL_ALGORITHMS_MAX_H
#define METRIC_NUMERIC_UTIL_ALGORITHMS_MAX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename T1, typename T2, typename R1 = RemoveCVRef_t<T1>, typename R2 = RemoveCVRef_t<T2>,
		  EnableIf_t<HasLessThan_v<R1, R2> && !(IsSigned_v<R1> && IsUnsigned_v<R2>) &&
...
*/
template <typename T1, typename T2, typename T3> constexpr decltype(auto) max(T1 &&a, T2 &&b, T3 &&c)
{
	using mtrc::numeric::max;

	return max(max(std::forward<T1>(a), std::forward<T2>(b)), std::forward<T3>(c));
}
//*************************************************************************************************
...
constexpr decltype(auto) max(T1 &&a, T2 &&b, T3 &&c, Ts &&...args)
{
	...
	return max(max(max(std::forward<T1>(a), std::forward<T2>(b)), std::forward<T3>(c)), std::forward<Ts>(args)...);
}
//*************************************************************************************************
