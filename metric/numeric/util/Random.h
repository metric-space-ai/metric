...
#ifndef METRIC_NUMERIC_UTIL_RANDOM_H
#define METRIC_NUMERIC_UTIL_RANDOM_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <ctime>
#include <limits>
#include <metric/numeric/system/Random.h>
#include <metric/numeric/util/Assert.h>
#include <metric/numeric/util/Complex.h>
#include <metric/numeric/util/EnableIf.h>
#include <metric/numeric/util/NonCreatable.h>
...
// the Metric numeric library. In order to create series of random numbers, the following functions are
// are provided:
//
// - mtrc::numeric::rand<T>();
// - mtrc::numeric::getSeed();
// - mtrc::numeric::setSeed( uint32_t seed );
//
...
template <typename T, typename... Args> T rand(Args &&...args);

template <typename T> void randomize(T &&value);

template <typename T, typename... Args> void randomize(T &&value, Args &&...args);

uint32_t defaultSeed();

template <typename RNG = DefaultRNG> uint32_t getSeed();

template <typename RNG = DefaultRNG> void setSeed(uint32_t seed);
//@}
//*************************************************************************************************
...
class Rand<T, EnableIf_t<IsIntegral_v<T>>> {
  public:
	...
	*/
	...
	{
		METRIC_NUMERIC_INTERNAL_ASSERT(min <= max, "Invalid min/max value pair");
	...
	*/
	inline void randomize(T &value) const { value = generate(); }
	//**********************************************************************************************
	...
	// Therefore the returned value is undefined if \a min is larger than \a max.
	*/
	inline void randomize(T &value, T min, T max) const { value = generate(min, max); }
	//**********************************************************************************************
};
/*! \endcond */
//*************************************************************************************************
...
class Rand<T, EnableIf_t<IsFloatingPoint_v<T>>> {
  public:
	...
	*/
	inline T generate(T min, T max) const
	{
		METRIC_NUMERIC_INTERNAL_ASSERT(min <= max, "Invalid min/max values");
...
class Rand<complex<T>> {
  public:
	...
	*/
	inline const complex<T> generate(const T &realmin, const T &realmax, const T &imagmin, const T &imagmax) const
	{
		Rand<T> tmp;
	...
	*/
	inline void randomize(complex<T> &value) const { value = generate(); }
	//*************************************************************************************************
	...
	// undefined if \a min is larger than \a max.
	*/
	inline void randomize(complex<T> &value, const T &min, const T &max) const { value = generate(min, max); }
	//*************************************************************************************************
	...
	// than \a imagmax.
	*/
	inline void randomize(complex<T> &value, const T &realmin, const T &realmax, const T &imagmin,
						  const T &imagmax) const
	{
