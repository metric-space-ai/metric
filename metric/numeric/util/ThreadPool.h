...
#ifndef METRIC_NUMERIC_UTIL_THREADPOOL_H
#define METRIC_NUMERIC_UTIL_THREADPOOL_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/util/Thread.h>
#include <metric/numeric/util/Types.h>
#include <metric/numeric/util/threadpool/Task.h>
#include <metric/numeric/util/threadpool/TaskQueue.h>
#include <vector>
...
// \ingroup util
//
// The Metric numeric library offers the capability to either create individual threads for specific
...
// thread functionality as provided by standard C++ synchronization primitives. Via the four
// template parameters it is possible to configure a ThreadPool instance as either a C++11 thread
// pool backed by standard C++ synchronization primitives:
...
//
// The following example demonstrates how to configure the ThreadPool class template as a
// C++11 standard thread pool:

   \code
   using StdThreadPool = mtrc::numeric::ThreadPool< std::thread
											  , std::mutex
...
// [2]. Scheduled tasks should capture exceptions explicitly and rethrow them on the waiting thread.
   ...
   void work()
   {
	  ...
	  StdThreadPool threadpool( 2 );
	  threadpool.schedule( [&] { task(error); } );
	  threadpool.wait();
...
class ThreadPool : private NonCopyable {
  ...
  public:
	...
	/*!\name Task scheduling */
	//@{
	template <typename Callable, typename... Args> void schedule(Callable func, Args &&...args);
	//@}
	//**********************************************************************************************
...
#if !(defined _MSC_VER)
	if (n == 0UL) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid number of threads");
	}
#endif
...
bool ThreadPool<TT, MT, LT, CT>::executeTask()
{
	...
	// Acquiring a scheduled task
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(!taskqueue_.isEmpty(), "Empty task queue detected");
		task = taskqueue_.pop();
	}
