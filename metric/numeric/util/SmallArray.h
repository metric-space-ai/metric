...
#ifndef METRIC_NUMERIC_UTIL_SMALLARRAY_H
#define METRIC_NUMERIC_UTIL_SMALLARRAY_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/util/algorithms/UninitializedDefaultConstruct.h>
...
#include <metric/numeric/util/smallarray/SmallArrayData.h>
...
class SmallArray : private SmallArrayData<T, N>, private A {
  public:
	//**Type definitions****************************************************************************
	...
	template <typename InputIt> inline SmallArray(InputIt first, InputIt last, const A &alloc = A());

	template <typename U> inline SmallArray(initializer_list<U> list, const A &alloc = A());
	...
	//@{
	template <typename U> inline SmallArray &operator=(initializer_list<U> list);
  ...
  private:
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_CONST(T);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_VOLATILE(T);
	/*! \endcond */
	//**********************************************************************************************
};
//*************************************************************************************************
...
inline SmallArray<T, N, A>::SmallArray(const A &alloc) : SmallArray(0UL, alloc, Uninitialized())
{
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline SmallArray<T, N, A>::SmallArray(size_t n, const A &alloc) : SmallArray(n, alloc, Uninitialized())
{
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline SmallArray<T, N, A>::SmallArray(size_t n, const T &init, const A &alloc) : SmallArray(n, alloc, Uninitialized())
{
...
// within a constructor call:
   ...
   \code
   mtrc::numeric::SmallArray<double,8UL> v1{ 4.2, 6.3, -1.2 };
   \endcode
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline SmallArray<T, N, A>::SmallArray(const SmallArray &sa) : SmallArray(sa.size(), A(), Uninitialized())
{
	  ...
	  final_(sa.final_) // Pointer to the very end of the currently used storage
{
	if (!sa.isDynamic()) {
		...
		end_ = begin_ + sa.size();
		final_ = begin_ + N;
		mtrc::numeric::uninitialized_move(sa.begin_, sa.end_, begin_);
		mtrc::numeric::destroy(sa.begin_, sa.end_);
	}
...
{
	mtrc::numeric::destroy(begin_, end_);
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::Reference SmallArray<T, N, A>::operator[](size_t index) noexcept
{
	METRIC_NUMERIC_USER_ASSERT(index < size(), "Invalid small array access index");
	return begin_[index];
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::ConstReference SmallArray<T, N, A>::operator[](size_t index) const noexcept
{
	METRIC_NUMERIC_USER_ASSERT(index < size(), "Invalid small array access index");
	return begin_[index];
}
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::Reference SmallArray<T, N, A>::at(size_t index)
{
	if (index >= size()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid small array access index");
	}
	return begin_[index];
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::ConstReference SmallArray<T, N, A>::at(size_t index) const
{
	if (index >= size()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid small array access index");
	}
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::Pointer SmallArray<T, N, A>::data() noexcept
{
	return begin_;
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::ConstPointer SmallArray<T, N, A>::data() const noexcept
{
	return begin_;
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::Iterator SmallArray<T, N, A>::begin() noexcept
{
	return begin_;
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::ConstIterator SmallArray<T, N, A>::begin() const noexcept
{
	return begin_;
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::ConstIterator SmallArray<T, N, A>::cbegin() const noexcept
{
	return begin_;
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::Iterator SmallArray<T, N, A>::end() noexcept
{
	return end_;
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::ConstIterator SmallArray<T, N, A>::end() const noexcept
{
	return end_;
}
//*************************************************************************************************
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
inline typename SmallArray<T, N, A>::ConstIterator SmallArray<T, N, A>::cend() const noexcept
{
	return end_;
}
//*************************************************************************************************
...
// array by means of an initializer list:
   ...
   \code
   mtrc::numeric::SmallArray<double,8UL> v;
   v = { 4.2, 6.3, -1.2 };
   \endcode
...
inline SmallArray<T, N, A> &SmallArray<T, N, A>::operator=(SmallArray &&rhs)
{
	...
	mtrc::numeric::destroy(rhs.begin_, rhs.end_);
...
void SmallArray<T, N, A>::resize(size_t n)
{
	if (n > size()) {
		reserve(n);
		mtrc::numeric::uninitialized_default_construct(begin_ + size(), begin_ + n);
		end_ = begin_ + n;
	} else if (n < size()) {
		mtrc::numeric::destroy(begin_ + n, end_);
		end_ = begin_ + n;
	}
}
//*************************************************************************************************
...
void SmallArray<T, N, A>::reserve(size_t n)
{
	...
	if (n > oldCapacity) {
		...
		if (IsNothrowMoveConstructible_v<T>) {
			mtrc::numeric::uninitialized_move(begin_, end_, tmp);
		} else {
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
typename SmallArray<T, N, A>::Iterator SmallArray<T, N, A>::insert(Iterator pos, const T &value)
{
	const size_t oldCapacity(capacity());
	const size_t oldSize(size());
	...
	if (oldSize == oldCapacity) {
		...
		mtrc::numeric::uninitialized_move(begin_, pos, tmp);
		::new (newpos) T(value);
		mtrc::numeric::uninitialized_move(pos, end_, tmp + index + 1UL);
		mtrc::numeric::destroy(begin_, end_);
		...
		try {
			...
			mtrc::numeric::destroy_at(pos);
			::new (pos) T(value);
			++end_;
		} catch (...) {
			mtrc::numeric::destroy_at(end_);
			throw;
		}
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
typename SmallArray<T, N, A>::Iterator SmallArray<T, N, A>::insert(Iterator pos, T &&value)
{
	const size_t oldCapacity(capacity());
	const size_t oldSize(size());
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
typename SmallArray<T, N, A>::Iterator SmallArray<T, N, A>::erase(Iterator pos)
{
...
template <typename T // Data type of the elements
		  ...
		  ,
		  typename A> // Type of the allocator
typename SmallArray<T, N, A>::Iterator SmallArray<T, N, A>::erase(Iterator first, Iterator last)
{
	METRIC_NUMERIC_USER_ASSERT(first <= last, "Invalid range detected");

	const size_t n(last - first);

	std::move(last, end_, first);
	end_ -= n;
	mtrc::numeric::destroy(end_, end_ + n);
...
void SmallArray<T, N, A>::swap(SmallArray &sa) noexcept(IsNothrowMoveConstructible_v<T>)
{
	...
	if (isDynamic() && sa.isDynamic()) {
		...
		mtrc::numeric::uninitialized_move(sa.begin_, sa.end_, this->array());
		mtrc::numeric::destroy(sa.begin_, sa.end_);
		...
		mtrc::numeric::uninitialized_move(begin_, end_, sa.array());
		mtrc::numeric::destroy(begin_, end_);
		...
		mtrc::numeric::uninitialized_move(pos, end_, sa.end_);
		mtrc::numeric::destroy(pos, end_);
		end_ -= n;
		sa.end_ += n;
	} else {
		...
		mtrc::numeric::uninitialized_move(pos, sa.end_, end_);
		mtrc::numeric::destroy(pos, sa.end_);
		end_ += n;
		sa.end_ -= n;
	}
}
...
template <typename T, size_t N, typename A> inline typename SmallArray<T, N, A>::Iterator end(SmallArray<T, N, A> &sa);
...
template <typename T, size_t N, typename A> inline void clear(SmallArray<T, N, A> &sa);

template <typename T, size_t N, typename A>
inline void swap(SmallArray<T, N, A> &a, SmallArray<T, N, A> &b) noexcept(IsNothrowMoveConstructible_v<T>);
//@}
//*************************************************************************************************
...
*/
template <typename T, size_t N, typename A> inline typename SmallArray<T, N, A>::Iterator begin(SmallArray<T, N, A> &sa)
{
	return sa.begin();
}
//*************************************************************************************************
