...
#ifndef METRIC_NUMERIC_UTIL_SINGLETON_SINGLETON_H
#define METRIC_NUMERIC_UTIL_SINGLETON_SINGLETON_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/util/NullType.h>
...
template <typename T // The type to be checked for cyclic lifetime dependencies
		  ,
		  typename TL> // Type list of checked lifetime dependencies
struct HasCyclicDependency<T, TL, true> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
*/
#define METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(T)                                                            \
	static_assert((!mtrc::numeric::HasCyclicDependency<T, mtrc::numeric::NullType>::value),                        \
...
*/
#define METRIC_NUMERIC_BEFRIEND_SINGLETON                                                                              \
	template <typename, typename, typename, typename, typename, typename, typename, typename, typename>                \
	friend class mtrc::numeric::Singleton;                                                                           \
	template <typename, typename, bool> friend struct mtrc::numeric::HasCyclicDependency;                            \
	template <typename> friend class mtrc::numeric::Dependency;
//*************************************************************************************************
...
//
// In the Metric numeric library the singleton pattern is realized by the Singleton class template.
...
// argument, the Metric numeric Singleton implementation cannot be used!
//
//
...
//     called Logger.
//  -# The class needs to befriend the Singleton via the mtrc::numeric::METRIC_NUMERIC_BEFRIEND_SINGLETON macro.
...
//     the constructor must be accessible. This is achieved by the mtrc::numeric::METRIC_NUMERIC_BEFRIEND_SINGLETON
//     macro. The following example demonstrates this by means of the MySingleton class:
   ...
   class MySingleton : private Singleton<MySingleton,Logger>
   {
	private:
	  ...
	  ...
	  METRIC_NUMERIC_BEFRIEND_SINGLETON;
	  ...
   };
   \endcode
...
// Metric numeric library. Classes, which are supposed to be implemented in terms of the singleton
...
// of a default constructor. In case no default constructor is available, the Metric numeric singleton
...
class Singleton : private NonCopyable {
  ...
  protected:
	...
	*/
	...
	{
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(T, SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D1, typename D1::SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D2, typename D2::SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D3, typename D3::SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D4, typename D4::SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D5, typename D5::SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D6, typename D6::SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D7, typename D7::SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D8, typename D8::SingletonType);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D1);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D2);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D3);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D4);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D5);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D6);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D7);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D8);
	}
	//**********************************************************************************************
...
class Singleton<T, D1, NullType, NullType, NullType, NullType, NullType, NullType, NullType> : private NonCopyable {
  ...
  protected:
	...
	// In case a cyclic lifetime dependency is detected, a compilation error is created.
	*/
	Singleton() : dependency1_(D1::instance()) // Handle to the lifetime dependency
	{
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(T, SingletonType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(D1, typename D1::SingletonType);
		METRIC_NUMERIC_DETECT_CYCLIC_LIFETIME_DEPENDENCY(D1);
...
class Singleton<T, NullType, NullType, NullType, NullType, NullType, NullType, NullType, NullType>
  ...
  protected:
	...
	// In case a cyclic lifetime dependency is detected, a compilation error is created.
	*/
	Singleton() { METRIC_NUMERIC_CONSTRAINT_MUST_BE_DERIVED_FROM(T, SingletonType); }
	//**********************************************************************************************
