...
#ifndef METRIC_NUMERIC_MATH_LAPACK_GEEV_H
#define METRIC_NUMERIC_MATH_LAPACK_GEEV_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/lapack/clapack/geev.h>
...
#include <metric/numeric/util/constraints/Complex.h>
...
//@{
template <typename MT, bool SO, typename VT, bool TF> void geev(DenseMatrix<MT, SO> &A, DenseVector<VT, TF> &w);
...
inline auto geev_backend(DenseMatrix<MT, SO> &A, DenseVector<VT, TF> &w) -> DisableIf_t<IsComplex_v<ElementType_t<MT>>>
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*A), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*w).size() == (*A).rows(), "Invalid vector dimension detected");

	using CT = ElementType_t<VT>;
	using BT = ElementType_t<MT>;

	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COMPLEX_TYPE(CT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_BUILTIN_TYPE(BT);
	...
	geev('N', 'N', n, (*A).data(), lda, wr.get(), wi.get(), nullptr, 1, nullptr, 1, work.get(), lwork, &info);
...
template <typename MT // Type of the matrix A
		  ...
		  ,
		  bool TF> // Transpose flag of the vector w
inline auto geev_backend(DenseMatrix<MT, SO> &A, DenseVector<VT, TF> &w) -> EnableIf_t<IsComplex_v<ElementType_t<MT>>>
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*A), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*w).size() == (*A).rows(), "Invalid vector dimension detected");
	...
	geev('N', 'N', n, (*A).data(), lda, (*w).data(), nullptr, 1, nullptr, 1, work.get(), lwork, rwork.get(), &info);
...
inline void geev(DenseMatrix<MT, SO> &A, DenseVector<VT, TF> &w)
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COMPLEX_TYPE(ElementType_t<VT>);
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*A), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*VL), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*VL).rows() == (*A).rows(), "Invalid matrix dimension detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*w).size() == (*A).rows(), "Invalid vector dimension detected");
	...
	geev((SO1 ? 'V' : 'N'), (SO1 ? 'N' : 'V'), n, (*A).data(), lda, wr.get(), wi.get(), (SO1 ? vl.get() : nullptr),
		 (SO1 ? n : 1), (SO1 ? nullptr : vl.get()), (SO1 ? 1 : n), work.get(), lwork, &info);
	...
	-> EnableIf_t<IsComplex_v<ElementType_t<MT1>>>
{
	...
	geev((SO1 ? 'V' : 'N'), (SO1 ? 'N' : 'V'), n, (*A).data(), lda, (*w).data(), (SO1 ? (*VL).data() : nullptr),
		 (SO1 ? ldvl : 1), (SO1 ? nullptr : (*VL).data()), (SO1 ? 1 : ldvl), work.get(), lwork, rwork.get(), &info);
...
inline void geev(DenseMatrix<MT1, SO1> &A, DenseMatrix<MT2, SO2> &VL, DenseVector<VT, TF> &w)
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COMPLEX_TYPE(ElementType_t<MT2>);
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*A), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*VR), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*VR).rows() == (*A).rows(), "Invalid matrix dimension detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*w).size() == (*A).rows(), "Invalid vector dimension detected");
	...
	geev((SO1 ? 'N' : 'V'), (SO1 ? 'V' : 'N'), n, (*A).data(), lda, wr.get(), wi.get(), (SO1 ? nullptr : vr.get()),
		 (SO1 ? 1 : n), (SO1 ? vr.get() : nullptr), (SO1 ? n : 1), work.get(), lwork, &info);
	...
	-> EnableIf_t<IsComplex_v<ElementType_t<MT1>>>
{
	...
	geev((SO1 ? 'N' : 'V'), (SO1 ? 'V' : 'N'), n, (*A).data(), lda, (*w).data(), (SO1 ? nullptr : (*VR).data()),
		 (SO1 ? 1 : ldvr), (SO1 ? (*VR).data() : nullptr), (SO1 ? ldvr : 1), work.get(), lwork, rwork.get(), &info);
...
inline auto geev_backend(DenseMatrix<MT1, SO1> &A, DenseMatrix<MT2, SO2> &VL, DenseVector<VT, TF> &w,
						 DenseMatrix<MT3, SO3> &VR) -> DisableIf_t<IsComplex_v<ElementType_t<MT1>>>
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*A), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*VL), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*VR), "Invalid non-square matrix detected");
	...
	geev('V', 'V', n, (*A).data(), lda, wr.get(), wi.get(), (SO1 ? vl.get() : vr.get()), n, (SO1 ? vr.get() : vl.get()),
		 n, work.get(), lwork, &info);
...
template <typename MT1 // Type of the matrix A
		  ...
		  bool SO3> // Storage order of the matrix VR
inline auto geev_backend(DenseMatrix<MT1, SO1> &A, DenseMatrix<MT2, SO2> &VL, DenseVector<VT, TF> &w,
						 DenseMatrix<MT3, SO3> &VR) -> EnableIf_t<IsComplex_v<ElementType_t<MT1>>>
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*A), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*VL), "Invalid non-square matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(*VR), "Invalid non-square matrix detected");
	...
	geev('V', 'V', n, (*A).data(), lda, (*w).data(), (SO1 ? (*VL).data() : (*VR).data()), (SO1 ? ldvl : ldvr),
		 (SO1 ? (*VR).data() : (*VL).data()), (SO1 ? ldvr : ldvl), work.get(), lwork, rwork.get(), &info);
...
template <typename MT1 // Type of the matrix A
		  ...
		  ,
		  bool SO3> // Storage order of the matrix VR
inline void geev(DenseMatrix<MT1, SO1> &A, DenseMatrix<MT2, SO2> &VL, DenseVector<VT, TF> &w, DenseMatrix<MT3, SO3> &VR)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_ADAPTOR_TYPE(MT1);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_COMPUTATION_TYPE(MT1);
	METRIC_NUMERIC_CONSTRAINT_MUST_HAVE_MUTABLE_DATA_ACCESS(MT1);
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COMPLEX_TYPE(ElementType_t<MT3>);
