...
#ifndef METRIC_NUMERIC_MATH_TYPETRAITS_HASSIMDEQUAL_H
#define METRIC_NUMERIC_MATH_TYPETRAITS_HASSIMDEQUAL_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
struct HasSIMDEqualHelper : public FalseType {};
/*! \endcond */
//*************************************************************************************************
...
struct HasSIMDEqualHelper<T, T, EnableIf_t<IsNumeric_v<T> && IsIntegral_v<T>>>
	: public BoolConstant<(bool(METRIC_NUMERIC_SSE2_MODE) && sizeof(T) <= 4UL) ||
						  (bool(METRIC_NUMERIC_SSE4_MODE) && sizeof(T) == 8UL) || (bool(METRIC_NUMERIC_AVX2_MODE)) ||
...
// \a FalseType. The following example assumes that AVX is available:
   ...
   \code
   mtrc::numeric::HasSIMDEqual< int, int >::value     // Evaluates to 1
   mtrc::numeric::HasSIMDEqual< float, float >::Type  // Results in TrueType
   mtrc::numeric::HasSIMDEqual< double, double >      // Is derived from TrueType
   mtrc::numeric::HasSIMDEqual< bool, bool >::value   // Evaluates to 0
   mtrc::numeric::HasSIMDEqual< float, int >::Type    // Results in FalseType
   mtrc::numeric::HasSIMDEqual< float, double >       // Is derived from FalseType
   \endcode
*/
template <typename T1 // Type of the left-hand side operand
		  ,
		  ...
		  ,
		  typename = void> // Restricting condition
struct HasSIMDEqual : public BoolConstant<HasSIMDEqualHelper<RemoveCVRef_t<T1>, RemoveCVRef_t<T2>>::value> {};
//*************************************************************************************************
...
// the following two statements are identical:
   ...
   \code
   constexpr bool value1 = mtrc::numeric::HasSIMDEqual<T1,T2>::value;
   constexpr bool value2 = mtrc::numeric::HasSIMDEqual_v<T1,T2>;
   \endcode
*/
template <typename T1 // Type of the left-hand side operand
		  ,
