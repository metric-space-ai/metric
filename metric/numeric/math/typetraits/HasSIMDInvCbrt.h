...
#ifndef METRIC_NUMERIC_MATH_TYPETRAITS_HASSIMDINVCBRT_H
#define METRIC_NUMERIC_MATH_TYPETRAITS_HASSIMDINVCBRT_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
using HasSIMDInvCbrtHelper =
	BoolConstant<(IsFloat_v<T> || IsDouble_v<T>) && bool(METRIC_NUMERIC_SVML_MODE) &&
				 (bool(METRIC_NUMERIC_SSE_MODE) || bool(METRIC_NUMERIC_AVX_MODE) || bool(METRIC_NUMERIC_AVX512F_MODE))>;
/*! \endcond */
//*************************************************************************************************
...
// following example assumes that the Intel SVML is available:
   ...
   \code
   mtrc::numeric::HasSIMDInvCbrt< float >::value         // Evaluates to 1
   mtrc::numeric::HasSIMDInvCbrt< double >::Type         // Results in TrueType
   mtrc::numeric::HasSIMDInvCbrt< const double >         // Is derived from TrueType
   mtrc::numeric::HasSIMDInvCbrt< unsigned int >::value  // Evaluates to 0
   mtrc::numeric::HasSIMDInvCbrt< long double >::Type    // Results in FalseType
   mtrc::numeric::HasSIMDInvCbrt< complex<double> >      // Is derived from FalseType
   \endcode
*/
template <typename T> // Type of the operand
struct HasSIMDInvCbrt : public BoolConstant<HasSIMDInvCbrtHelper<RemoveCVRef_t<T>>::value> {};
//*************************************************************************************************
...
// two statements are identical:
   ...
   \code
   constexpr bool value1 = mtrc::numeric::HasSIMDInvCbrt<T>::value;
   constexpr bool value2 = mtrc::numeric::HasSIMDInvCbrt_v<T>;
   \endcode
*/
template <typename T> // Type of the operand
