...
#ifndef METRIC_NUMERIC_MATH_TYPETRAITS_HASSIMDSHIFTRI_H
#define METRIC_NUMERIC_MATH_TYPETRAITS_HASSIMDSHIFTRI_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
using HasSIMDShiftRIHelper =
	BoolConstant<(IsNumeric_v<T> && IsIntegral_v<T>) &&
				 ((bool(METRIC_NUMERIC_SSE2_MODE) &&
				   ((IsSigned_v<T> && sizeof(T) >= 2UL && sizeof(T) <= 4UL) || (!IsSigned_v<T> && sizeof(T) >= 2UL))) ||
...
// that AVX2 is available:
   ...
   \code
   mtrc::numeric::HasSIMDShiftRI< int >::value   // Evaluates to 1
   mtrc::numeric::HasSIMDShiftRI< short >::Type  // Results in TrueType
   mtrc::numeric::HasSIMDShiftRI< unsigned >     // Is derived from TrueType
   mtrc::numeric::HasSIMDShiftRI< bool >::value  // Evaluates to 0
   mtrc::numeric::HasSIMDShiftRI< float >::Type  // Results in FalseType
   mtrc::numeric::HasSIMDShiftRI< double >       // Is derived from FalseType
   \endcode
*/
template <typename T> // Type of the operand
struct HasSIMDShiftRI : public BoolConstant<HasSIMDShiftRIHelper<RemoveCVRef_t<T>>::value> {};
//*************************************************************************************************
...
// \a T2 the following two statements are identical:
   ...
   \code
   constexpr bool value1 = mtrc::numeric::HasSIMDShiftRI<T1,T2>::value;
   constexpr bool value2 = mtrc::numeric::HasSIMDShiftRI_v<T1,T2>;
   \endcode
*/
template <typename T> // Type of the operand
