...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATTSMATSCHUREXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATTSMATSCHUREXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DMatTSMatSchurExpr : public SchurExpr<SparseMatrix<DMatTSMatSchurExpr<MT1, MT2>, true>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	class ConstIterator {
	  public:
		...
		*/
		inline ReturnType value() const { return left_(right_->index(), col_) * right_->value(); }
		//*******************************************************************************************
	...
	*/
	inline ConstIterator begin(size_t j) const { return ConstIterator(lhs_, rhs_.begin(j), j); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator end(size_t j) const { return ConstIterator(lhs_, rhs_.end(j), j); }
	//**********************************************************************************************
	...
	*/
	inline size_t nonZeros(size_t j) const { return rhs_.nonZeros(j); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline auto assign(DenseMatrix<MT, SO2> &lhs, const DMatTSMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
	...
	*/
	template <typename MT> // Type of the target sparse matrix
	friend inline auto assign(SparseMatrix<MT, true> &lhs, const DMatTSMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
...
inline const DMatTSMatSchurExpr<MT1, MT2> dmattsmatschur(const DenseMatrix<MT1, false> &lhs,
...
inline decltype(auto) dmattsmatschur(const DenseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
// sparse matrix:
   ...
   mtrc::numeric::CompressedMatrix<double,columnMajor> B, C;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ,
		  typename MT2> // Type of the right-hand side sparse matrix
inline decltype(auto) operator%(const DenseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  DisableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>) ||
					  ...
					  (IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) || (IsUpper_v<MT1> && IsStrictlyLower_v<MT2>) ||
					  IsZero_v<MT2>> * = nullptr>
inline const DMatTSMatSchurExpr<MT1, MT2> tdmattsmatschur(const DenseMatrix<MT1, true> &lhs,
														  const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  EnableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>)> * = nullptr>
inline decltype(auto) tdmattsmatschur(const DenseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ,
		  typename MT2> // Type of the right-hand side sparse matrix
inline decltype(auto) operator%(const DenseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<DMatTSMatSchurExpr<MT1, MT2>, 0UL> : public Max_t<Size<MT1, 0UL>, Size<MT2, 0UL>> {};

template <typename MT1, typename MT2>
struct Size<DMatTSMatSchurExpr<MT1, MT2>, 1UL> : public Max_t<Size<MT1, 1UL>, Size<MT2, 1UL>> {};
/*! \endcond */
//*************************************************************************************************
