...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATSMATKRONEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATSMATKRONEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DMatSMatKronExpr : public MatMatKronExpr<SparseMatrix<DMatSMatKronExpr<MT1, MT2, SO>, SO>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	*/
	inline size_t nonZeros() const { return lhs_.rows() * lhs_.columns() * rhs_.nonZeros(); }
	//**********************************************************************************************
  ...
  private:
	...
	friend inline void assign(DenseMatrix<MT, SO2> &lhs, const DMatSMatKronExpr &rhs)
	{
		...
		if (SO) {
			for (size_t j = 0UL; j < A.columns(); ++j) {
				...
				const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j : j + 1UL) : (A.rows()));
			...
			for (size_t i = 0UL; i < A.rows(); ++i) {
				...
				const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i : i + 1UL) : (A.columns()));
	...
	*/
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(MT2, SO);
...
inline const DMatSMatKronExpr<MT1, MT2, SO2> dmatsmatkron(const DenseMatrix<MT1, SO1> &lhs,
...
inline decltype(auto) dmatsmatkron(const DenseMatrix<MT1, SO1> &lhs, const SparseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;

	using ReturnType = const KronTrait_t<ResultType_t<MT1>, ResultType_t<MT2>>;

	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(ReturnType, SO2);
...
// This kron() function computes the Kronecker product of the given dense matrix and sparse matrix:
   ...
   mtrc::numeric::CompressedMatrix<double> B, C;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side sparse matrix
inline decltype(auto) kron(const DenseMatrix<MT1, SO1> &lhs, const SparseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
