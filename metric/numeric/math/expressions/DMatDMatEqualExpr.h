...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATDMATEQUALEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATDMATEQUALEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
	...
	-> EnableIf_t<DMatDMatEqualExprHelper<MT1, MT2>::value, bool>
{
	...
	for (size_t i = 0UL; i < M; ++i) {
		...
		for (; (j + SIMDSIZE * 3UL) < jpos; j += SIMDSIZE * 4UL) {
			if (!equal<RF>(A.load(i, j), B.load(i, j)))
				return false;
			if (!equal<RF>(A.load(i, j + SIMDSIZE), B.load(i, j + SIMDSIZE)))
				return false;
			if (!equal<RF>(A.load(i, j + SIMDSIZE * 2UL), B.load(i, j + SIMDSIZE * 2UL)))
				return false;
			if (!equal<RF>(A.load(i, j + SIMDSIZE * 3UL), B.load(i, j + SIMDSIZE * 3UL)))
				return false;
		}
	...
	-> EnableIf_t<DMatDMatEqualExprHelper<MT1, MT2>::value, bool>
{
	...
	for (size_t j = 0UL; j < N; ++j) {
		...
		for (; (i + SIMDSIZE * 3UL) < ipos; i += SIMDSIZE * 4UL) {
			if (!equal<RF>(A.load(i, j), B.load(i, j)))
				return false;
			if (!equal<RF>(A.load(i + SIMDSIZE, j), B.load(i + SIMDSIZE, j)))
				return false;
			if (!equal<RF>(A.load(i + SIMDSIZE * 2UL, j), B.load(i + SIMDSIZE * 2UL, j)))
				return false;
			if (!equal<RF>(A.load(i + SIMDSIZE * 3UL, j), B.load(i + SIMDSIZE * 3UL, j)))
				return false;
		}
