...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SMATTSMATKRONEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SMATTSMATKRONEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SMatTSMatKronExpr : public MatMatKronExpr<SparseMatrix<SMatTSMatKronExpr<MT1, MT2>, true>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	friend inline void assign(SparseMatrix<MT, false> &lhs, const SMatTSMatKronExpr &rhs)
	{
		...
		for (size_t i = 0UL; i < A.rows(); ++i)
			for (auto aelem = A.begin(i); aelem != A.end(i); ++aelem)
				for (size_t l = 0UL; l < N; ++l)
					for (auto belem = B.begin(l); belem != B.end(l); ++belem)
						(*lhs).append(i * M + belem->index(), aelem->index() * N + l, aelem->value() * belem->value(),
									  true);
	}
	/*! \endcond */
	//**********************************************************************************************
...
inline const SMatTSMatKronExpr<MT1, MT2> smattsmatkron(const SparseMatrix<MT1, false> &lhs,
...
inline decltype(auto) smattsmatkron(const SparseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side sparse matrix
inline decltype(auto) kron(const SparseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
