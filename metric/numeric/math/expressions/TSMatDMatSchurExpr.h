...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATDMATSCHUREXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATDMATSCHUREXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class TSMatDMatSchurExpr : public SchurExpr<SparseMatrix<TSMatDMatSchurExpr<MT1, MT2>, true>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	class ConstIterator {
	  public:
		...
		*/
		inline ReturnType value() const { return left_->value() * right_(left_->index(), col_); }
		//*******************************************************************************************
	...
	*/
	inline ConstIterator begin(size_t j) const { return ConstIterator(lhs_.begin(j), rhs_, j); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator end(size_t j) const { return ConstIterator(lhs_.end(j), rhs_, j); }
	//**********************************************************************************************
	...
	*/
	inline size_t nonZeros(size_t j) const { return lhs_.nonZeros(j); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline auto assign(DenseMatrix<MT, SO2> &lhs, const TSMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
	...
	*/
	template <typename MT> // Type of the target sparse matrix
	friend inline auto assign(SparseMatrix<MT, true> &lhs, const TSMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
...
inline const TSMatDMatSchurExpr<MT1, MT2> tsmatdmatschur(const SparseMatrix<MT1, true> &lhs,
...
inline decltype(auto) tsmatdmatschur(const SparseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
// dense matrix:
   ...
   mtrc::numeric::CompressedMatrix<double,columnMajor> A, C;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  DisableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>) ||
					  ...
					  (IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) || (IsUpper_v<MT1> && IsStrictlyLower_v<MT2>) ||
					  IsZero_v<MT1>> * = nullptr>
inline const TSMatDMatSchurExpr<MT1, MT2> tsmattdmatschur(const SparseMatrix<MT1, true> &lhs,
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  ,
		  EnableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>)> * = nullptr>
inline decltype(auto) tsmattdmatschur(const SparseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side dense matrix
inline decltype(auto) operator%(const SparseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<TSMatDMatSchurExpr<MT1, MT2>, 0UL> : public Max_t<Size<MT1, 0UL>, Size<MT2, 0UL>> {};

template <typename MT1, typename MT2>
struct Size<TSMatDMatSchurExpr<MT1, MT2>, 1UL> : public Max_t<Size<MT1, 1UL>, Size<MT2, 1UL>> {};
/*! \endcond */
//*************************************************************************************************
