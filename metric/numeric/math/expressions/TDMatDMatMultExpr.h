...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATDMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATDMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/blas/gemm.h>
#include <metric/numeric/math/blas/trmm.h>
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool UF> // Upper flag
class TDMatDMatMultExpr : public MatMatMultExpr<DenseMatrix<TDMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, true>>,
						  private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool UseBlasKernel_v =
		(METRIC_NUMERIC_BLAS_MODE && METRIC_NUMERIC_USE_BLAS_MATRIX_MATRIX_MULTIPLICATION && !SYM && !HERM && !LOW &&
		 !UPP && IsContiguous_v<T1> && HasMutableDataAccess_v<T1> && IsContiguous_v<T2> && HasConstDataAccess_v<T2> &&
		 IsContiguous_v<T3> && HasConstDataAccess_v<T3> && !IsDiagonal_v<T2> && !IsDiagonal_v<T3> && T1::simdEnabled &&
		 T2::simdEnabled && T3::simdEnabled && IsBLASCompatible_v<ElementType_t<T1>> &&
		 IsBLASCompatible_v<ElementType_t<T2>> && IsBLASCompatible_v<ElementType_t<T3>> &&
		 IsSame_v<ElementType_t<T1>, ElementType_t<T2>> && IsSame_v<ElementType_t<T1>, ElementType_t<T3>>);
	/*! \endcond */
	//**********************************************************************************************
	...
	static constexpr bool UseVectorizedDefaultKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T2> && !IsDiagonal_v<T3> && T1::simdEnabled && T2::simdEnabled &&
		 T3::simdEnabled && IsSIMDCombinable_v<ElementType_t<T1>, ElementType_t<T2>, ElementType_t<T3>> &&
		 HasSIMDAdd_v<ElementType_t<T2>, ElementType_t<T3>> && HasSIMDMult_v<ElementType_t<T2>, ElementType_t<T3>>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	static constexpr bool simdEnabled = (!(IsDiagonal_v<MT1> && IsDiagonal_v<MT2>) && MT1::simdEnabled &&
										 MT2::simdEnabled && HasSIMDAdd_v<ET1, ET2> && HasSIMDMult_v<ET1, ET2>);
	...
	*/
	inline bool isAligned() const noexcept { return lhs_.isAligned() && rhs_.isAligned(); }
	//**********************************************************************************************
	...
	*/
	...
	{
		return (!METRIC_NUMERIC_BLAS_MODE || !METRIC_NUMERIC_USE_BLAS_MATRIX_MATRIX_MULTIPLICATION ||
				!METRIC_NUMERIC_BLAS_IS_PARALLEL || (rows() * columns() < TDMATDMATMULT_THRESHOLD)) &&
			   (rows() * columns() >= SMP_TDMATDMATMULT_THRESHOLD) && !IsDiagonal_v<MT1> && !IsDiagonal_v<MT2>;
	}
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	static inline void selectAssignKernel(MT3 &C, const MT4 &A, const MT5 &B)
	{
		if ((IsDiagonal_v<MT4> && IsDiagonal_v<MT5>) ||
			(!METRIC_NUMERIC_DEBUG_MODE && IsRowMajorMatrix_v<MT3> && B.columns() <= SIMDSIZE * 10UL) ||
			(!METRIC_NUMERIC_DEBUG_MODE && IsColumnMajorMatrix_v<MT3> && A.rows() <= SIMDSIZE * 10UL) ||
	...
	*/
		...
		-> EnableIf_t<IsRowMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(!(SYM || HERM || LOW || UPP) || (M == N), "Broken invariant detected");

		for (size_t i = 0UL; i < M; ++i) {
			const size_t kbegin((IsUpper_v<MT4>) ? (IsStrictlyUpper_v<MT4> ? i + 1UL : i) : (0UL));
			const size_t kend((IsLower_v<MT4>) ? (IsStrictlyLower_v<MT4> ? i : i + 1UL) : (K));
			METRIC_NUMERIC_INTERNAL_ASSERT(kbegin <= kend, "Invalid loop indices detected");
...

			{
				const size_t jbegin((IsUpper_v<MT5>)
										? ((IsStrictlyUpper_v<MT5>) ? (UPP ? max(i, kbegin + 1UL) : kbegin + 1UL)
																	: (UPP ? max(i, kbegin) : kbegin))
										: (UPP ? i : 0UL));
				const size_t jend((IsLower_v<MT5>)
									  ? ((IsStrictlyLower_v<MT5>) ? (LOW ? min(i + 1UL, kbegin) : kbegin)
			...
			for (size_t k = kbegin + 1UL; k < kend; ++k) {
				const size_t jbegin((IsUpper_v<MT5>)
										? ((IsStrictlyUpper_v<MT5>) ? (SYM || HERM || UPP ? max(i, k + 1UL) : k + 1UL)
																	: (SYM || HERM || UPP ? max(i, k) : k))
										: (SYM || HERM || UPP ? i : 0UL));
				const size_t jend((IsLower_v<MT5>) ? ((IsStrictlyLower_v<MT5>) ? (LOW ? min(i + 1UL, k - 1UL) : k - 1UL)
																			   : (LOW ? min(i + 1UL, k) : k))
												   : (LOW ? i + 1UL : N));
	...
	*/
		...
		-> EnableIf_t<IsColumnMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		for (size_t j = 0UL; j < N; ++j) {
			const size_t kbegin((IsLower_v<MT5>) ? (IsStrictlyLower_v<MT5> ? j + 1UL : j) : (0UL));
			const size_t kend((IsUpper_v<MT5>) ? (IsStrictlyUpper_v<MT5> ? j : j + 1UL) : (K));
			METRIC_NUMERIC_INTERNAL_ASSERT(kbegin <= kend, "Invalid loop indices detected");
...

			{
				const size_t ibegin((IsLower_v<MT4>)
										? ((IsStrictlyLower_v<MT4>) ? (LOW ? max(j, kbegin + 1UL) : kbegin + 1UL)
																	: (LOW ? max(j, kbegin) : kbegin))
										: (LOW ? j : 0UL));
				const size_t iend((IsUpper_v<MT4>)
									  ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? min(j + 1UL, kbegin) : kbegin)
			...
			for (size_t k = kbegin + 1UL; k < kend; ++k) {
				const size_t ibegin((IsLower_v<MT4>)
										? ((IsStrictlyLower_v<MT4>) ? (SYM || HERM || LOW ? max(j, k + 1UL) : k + 1UL)
																	: (SYM || HERM || LOW ? max(j, k) : k))
										: (SYM || HERM || LOW ? j : 0UL));
				const size_t iend((IsUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? min(j + 1UL, k - 1UL) : k - 1UL)
																			   : (UPP ? min(j + 1UL, k) : k))
												   : (UPP ? j + 1UL : M));
		...
		for (size_t ii = 0UL; ii < M; ii += block) {
			...
			for (size_t jj = 0UL; jj < N; jj += block) {
				...
				for (size_t i = ii; i < iend; ++i) {
					const size_t jbegin((IsUpper_v<MT4>) ? (max((IsStrictlyUpper_v<MT4> ? i + 1UL : i), jj)) : (jj));
					const size_t jpos((IsLower_v<MT4>) ? (min((IsStrictlyLower_v<MT4> ? i : i + 1UL), jend)) : (jend));
	...
	*/
		...
		-> EnableIf_t<IsColumnMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && IsDiagonal_v<MT5>>
	{
		...
		for (size_t j = 0UL; j < N; ++j) {
			const size_t ibegin((IsLower_v<MT4>) ? (IsStrictlyLower_v<MT4> ? j + 1UL : j) : (0UL));
			const size_t iend((IsUpper_v<MT4>) ? (IsStrictlyUpper_v<MT4> ? j : j + 1UL) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin <= iend, "Invalid loop indices detected");
	...
	*/
		...
		-> EnableIf_t<IsRowMajorMatrix_v<MT3> && IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		for (size_t i = 0UL; i < M; ++i) {
			const size_t jbegin((IsUpper_v<MT5>) ? (IsStrictlyUpper_v<MT5> ? i + 1UL : i) : (0UL));
			const size_t jend((IsLower_v<MT5>) ? (IsStrictlyLower_v<MT5> ? i : i + 1UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
		...
		for (size_t jj = 0UL; jj < N; jj += block) {
			...
			for (size_t ii = 0UL; ii < M; ii += block) {
				...
				for (size_t j = jj; j < jend; ++j) {
					const size_t ibegin((IsLower_v<MT5>) ? (max((IsStrictlyLower_v<MT5> ? j + 1UL : j), ii)) : (ii));
					const size_t ipos((IsUpper_v<MT5>) ? (min((IsStrictlyUpper_v<MT5> ? j : j + 1UL), iend)) : (iend));
		...
		-> EnableIf_t<IsRowMajorMatrix_v<MT3> && UseVectorizedDefaultKernel_v<MT3, MT4, MT5>>
	{
			...
			for (; !SYM && !HERM && !LOW && !UPP && (j + SIMDSIZE * 7UL) < jpos; j += SIMDSIZE * 8UL) {
				for (size_t i = 0UL; i < M; ++i) {
					const size_t kbegin((IsUpper_v<MT4>)
											? ((IsLower_v<MT5>) ? (max((IsStrictlyUpper_v<MT4> ? i + 1UL : i), j))
		...
		for (; !SYM && !HERM && !LOW && !UPP && (j + SIMDSIZE * 4UL) < jpos; j += SIMDSIZE * 5UL) {
			...
			for (; (i + 2UL) <= M; i += 2UL) {
				...
				const size_t kend(
					(IsLower_v<MT4>)
						? ((IsUpper_v<MT5>) ? (min((IsStrictlyLower_v<MT4> ? i + 1UL : i + 2UL), j + SIMDSIZE * 5UL, K))
		...
		for (; !(LOW && UPP) && (j + SIMDSIZE * 3UL) < jpos; j += SIMDSIZE * 4UL) {
			...
			for (; (i + 2UL) <= iend; i += 2UL) {
				...
				const size_t kend(
					(IsLower_v<MT4>)
						? ((IsUpper_v<MT5>) ? (min((IsStrictlyLower_v<MT4> ? i + 1UL : i + 2UL), j + SIMDSIZE * 4UL, K))
											: (IsStrictlyLower_v<MT4> ? i + 1UL : i + 2UL))
		...
		for (; (j + SIMDSIZE * 2UL) < jpos; j += SIMDSIZE * 3UL) {
			...
			for (; (i + 2UL) <= iend; i += 2UL) {
				...
				const size_t kend(
					(IsLower_v<MT4>)
						? ((IsUpper_v<MT5>) ? (min((IsStrictlyLower_v<MT4> ? i + 1UL : i + 2UL), j + SIMDSIZE * 3UL, K))
											: (IsStrictlyLower_v<MT4> ? i + 1UL : i + 2UL))
		...
		for (; (j + SIMDSIZE) < jpos; j += SIMDSIZE * 2UL) {
			...
			for (; (i + 4UL) <= iend; i += 4UL) {
				...
				const size_t kend(
					(IsLower_v<MT4>)
						? ((IsUpper_v<MT5>) ? (min((IsStrictlyLower_v<MT4> ? i + 3UL : i + 4UL), j + SIMDSIZE * 2UL, K))
			...
			for (; (i + 3UL) <= iend; i += 3UL) {
				...
				const size_t kend(
					(IsLower_v<MT4>)
						? ((IsUpper_v<MT5>) ? (min((IsStrictlyLower_v<MT4> ? i + 2UL : i + 3UL), j + SIMDSIZE * 2UL, K))
			...
			for (; (i + 2UL) <= iend; i += 2UL) {
				...
				const size_t kend(
					(IsLower_v<MT4>)
						? ((IsUpper_v<MT5>) ? (min((IsStrictlyLower_v<MT4> ? i + 1UL : i + 2UL), j + SIMDSIZE * 2UL, K))
											: (IsStrictlyLower_v<MT4> ? i + 1UL : i + 2UL))
						: (IsUpper_v<MT5> ? min(j + SIMDSIZE * 2UL, K) : K));
			...
			for (; (i + 4UL) <= iend; i += 4UL) {
				const size_t kbegin((IsUpper_v<MT4>)
										? ((IsLower_v<MT5>) ? (max((IsStrictlyUpper_v<MT4> ? i + 1UL : i), j))
															: (IsStrictlyUpper_v<MT4> ? i + 1UL : i))
										: (IsLower_v<MT5> ? j : 0UL));
				const size_t kend((IsLower_v<MT4>) ? (IsStrictlyLower_v<MT4> ? i + 3UL : i + 4UL) : (K));
			...
			for (; (i + 3UL) <= iend; i += 3UL) {
				const size_t kbegin((IsUpper_v<MT4>)
										? ((IsLower_v<MT5>) ? (max((IsStrictlyUpper_v<MT4> ? i + 1UL : i), j))
															: (IsStrictlyUpper_v<MT4> ? i + 1UL : i))
										: (IsLower_v<MT5> ? j : 0UL));
				const size_t kend((IsLower_v<MT4>) ? (IsStrictlyLower_v<MT4> ? i + 2UL : i + 3UL) : (K));
			...
			for (; (i + 2UL) <= iend; i += 2UL) {
				const size_t kbegin((IsUpper_v<MT4>)
										? ((IsLower_v<MT5>) ? (max((IsStrictlyUpper_v<MT4> ? i + 1UL : i), j))
															: (IsStrictlyUpper_v<MT4> ? i + 1UL : i))
										: (IsLower_v<MT5> ? j : 0UL));
				const size_t kend((IsLower_v<MT4>) ? (IsStrictlyLower_v<MT4> ? i + 1UL : i + 2UL) : (K));
		...
		-> EnableIf_t<IsColumnMajorMatrix_v<MT3> && UseVectorizedDefaultKernel_v<MT3, MT4, MT5>>
	{
			...
			for (; !SYM && !HERM && !LOW && !UPP && (i + SIMDSIZE * 7UL) < ipos; i += SIMDSIZE * 8UL) {
				for (size_t j = 0UL; j < N; ++j) {
					const size_t kbegin((IsLower_v<MT5>)
											? ((IsUpper_v<MT4>) ? (max(i, (IsStrictlyLower_v<MT5> ? j + 1UL : j)))
		...
		for (; !SYM && !HERM && !LOW && !UPP && (i + SIMDSIZE * 4UL) < ipos; i += SIMDSIZE * 5UL) {
			...
			for (; (j + 2UL) <= N; j += 2UL) {
				...
				const size_t kend(
					(IsUpper_v<MT5>)
						? ((IsLower_v<MT4>) ? (min(i + SIMDSIZE * 5UL, K, (IsStrictlyUpper_v<MT5> ? j + 1UL : j + 2UL)))
		...
		for (; !(LOW && UPP) && (i + SIMDSIZE * 3UL) < ipos; i += SIMDSIZE * 4UL) {
			...
			for (; (j + 2UL) <= jend; j += 2UL) {
				...
				const size_t kend(
					(IsUpper_v<MT5>)
						? ((IsLower_v<MT4>) ? (min(i + SIMDSIZE * 4UL, K, (IsStrictlyUpper_v<MT5> ? j + 1UL : j + 2UL)))
											: (IsStrictlyUpper_v<MT5> ? j + 1UL : j + 2UL))
		...
		for (; (i + SIMDSIZE * 2UL) < ipos; i += SIMDSIZE * 3UL) {
			...
			for (; (j + 2UL) <= jend; j += 2UL) {
				...
				const size_t kend(
					(IsUpper_v<MT5>)
						? ((IsLower_v<MT4>) ? (min(i + SIMDSIZE * 3UL, K, (IsStrictlyUpper_v<MT5> ? j + 1UL : j + 2UL)))
											: (IsStrictlyUpper_v<MT5> ? j + 1UL : j + 2UL))
		...
		for (; (i + SIMDSIZE) < ipos; i += SIMDSIZE * 2UL) {
			...
			for (; (j + 4UL) <= jend; j += 4UL) {
				...
				const size_t kend(
					(IsUpper_v<MT5>)
						? ((IsLower_v<MT4>) ? (min(i + SIMDSIZE * 2UL, K, (IsStrictlyUpper_v<MT5> ? j + 3UL : j + 4UL)))
			...
			for (; (j + 3UL) <= jend; j += 3UL) {
				...
				const size_t kend(
					(IsUpper_v<MT5>)
						? ((IsLower_v<MT4>) ? (min(i + SIMDSIZE * 2UL, K, (IsStrictlyUpper_v<MT5> ? j + 2UL : j + 3UL)))
			...
			for (; (j + 2UL) <= jend; j += 2UL) {
				...
				const size_t kend(
					(IsUpper_v<MT5>)
						? ((IsLower_v<MT4>) ? (min(i + SIMDSIZE * 2UL, K, (IsStrictlyUpper_v<MT5> ? j + 1UL : j + 2UL)))
											: (IsStrictlyUpper_v<MT5> ? j + 1UL : j + 2UL))
						: (IsLower_v<MT4> ? min(i + SIMDSIZE * 2UL, K) : K));
			...
			for (; (j + 4UL) <= jend; j += 4UL) {
				const size_t kbegin((IsLower_v<MT5>)
										? ((IsUpper_v<MT4>) ? (max(i, (IsStrictlyLower_v<MT5> ? j + 1UL : j)))
															: (IsStrictlyLower_v<MT5> ? j + 1UL : j))
										: (IsUpper_v<MT4> ? i : 0UL));
				const size_t kend((IsUpper_v<MT5>) ? (IsStrictlyUpper_v<MT5> ? j + 3UL : j + 4UL) : (K));
			...
			for (; (j + 3UL) <= jend; j += 3UL) {
				const size_t kbegin((IsLower_v<MT5>)
										? ((IsUpper_v<MT4>) ? (max(i, (IsStrictlyLower_v<MT5> ? j + 1UL : j)))
															: (IsStrictlyLower_v<MT5> ? j + 1UL : j))
										: (IsUpper_v<MT4> ? i : 0UL));
				const size_t kend((IsUpper_v<MT5>) ? (IsStrictlyUpper_v<MT5> ? j + 2UL : j + 3UL) : (K));
			...
			for (; (j + 2UL) <= jend; j += 2UL) {
				const size_t kbegin((IsLower_v<MT5>)
										? ((IsUpper_v<MT4>) ? (max(i, (IsStrictlyLower_v<MT5> ? j + 1UL : j)))
															: (IsStrictlyLower_v<MT5> ? j + 1UL : j))
										: (IsUpper_v<MT4> ? i : 0UL));
				const size_t kend((IsUpper_v<MT5>) ? (IsStrictlyUpper_v<MT5> ? j + 1UL : j + 2UL) : (K));
...
#if METRIC_NUMERIC_BLAS_MODE && METRIC_NUMERIC_USE_BLAS_MATRIX_MATRIX_MULTIPLICATION
	/*! \cond METRIC_NUMERIC_INTERNAL */
	...
	*/
	...
	friend inline void assign(SparseMatrix<MT, SO> &lhs, const TDMatDMatMultExpr &rhs)
	{
		...
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(ResultType);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_MAJOR_MATRIX_TYPE(OppositeType);
	...
	*/
		...
		-> EnableIf_t<IsRowMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(!(LOW || UPP) || (M == N), "Broken invariant detected");
		...
		for (size_t i = 0UL; i < M; ++i) {
			...
			for (size_t k = kbegin; k < kend; ++k) {
				const size_t jbegin((IsUpper_v<MT5>) ? ((IsStrictlyUpper_v<MT5>) ? (UPP ? max(i, k + 1UL) : k + 1UL)
																				 : (UPP ? max(i, k) : k))
													 : (UPP ? i : 0UL));
				const size_t jend((IsLower_v<MT5>) ? ((IsStrictlyLower_v<MT5>) ? (LOW ? min(i + 1UL, k) : k)
																			   : (LOW ? min(i, k) + 1UL : k + 1UL))
												   : (LOW ? i + 1UL : N));

				if ((LOW || UPP) && (jbegin >= jend))
					continue;
				METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");

				const size_t jnum(jend - jbegin);
				const size_t jpos(jbegin + prevMultiple(jnum, 2UL));
				METRIC_NUMERIC_INTERNAL_ASSERT(jpos <= jbegin + jnum, "Invalid end calculation");
	...
	*/
		...
		-> EnableIf_t<IsColumnMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		for (size_t j = 0UL; j < N; ++j) {
			...
			for (size_t k = kbegin; k < kend; ++k) {
				const size_t ibegin((IsLower_v<MT4>) ? ((IsStrictlyLower_v<MT4>) ? (LOW ? max(j, k + 1UL) : k + 1UL)
																				 : (LOW ? max(j, k) : k))
													 : (LOW ? j : 0UL));
				const size_t iend((IsUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? min(j + 1UL, k) : k)
																			   : (UPP ? min(j, k) + 1UL : k + 1UL))
												   : (UPP ? j + 1UL : M));

				if ((LOW || UPP) && ibegin >= iend)
					continue;
				METRIC_NUMERIC_INTERNAL_ASSERT(ibegin <= iend, "Invalid loop indices detected");

				const size_t inum(iend - ibegin);
				const size_t ipos(ibegin + prevMultiple(inum, 2UL));
				METRIC_NUMERIC_INTERNAL_ASSERT(ipos <= ibegin + inum, "Invalid end calculation");
	...
	*/
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MT2);
...
class DMatScalarMultExpr<TDMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, ST, true>
	: public MatScalarMultExpr<
		  DenseMatrix<DMatScalarMultExpr<TDMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, ST, true>, true>>,
	  private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	template <typename T1, typename T2, typename T3, typename T4>
	static constexpr bool UseVectorizedDefaultKernel_v =
		(useOptimizedKernels && !(IsDiagonal_v<T2> && IsDiagonal_v<T3>) &&
		 !(IsDiagonal_v<T2> && IsColumnMajorMatrix_v<T1>) && !(IsDiagonal_v<T3> && IsRowMajorMatrix_v<T1>) &&
		 T1::simdEnabled && T2::simdEnabled && T3::simdEnabled &&
		 IsSIMDCombinable_v<ElementType_t<T1>, ElementType_t<T2>, ElementType_t<T3>, T4> &&
		 HasSIMDAdd_v<ElementType_t<T2>, ElementType_t<T3>> && HasSIMDMult_v<ElementType_t<T2>, ElementType_t<T3>>);
	//**********************************************************************************************
  ...
  public:
	...
	//! Result type for expression template evaluations.
	using ResultType =
		typename If_t<HERM, DeclHermTrait<MultTrait_t<RES, ST>>,
					  If_t<SYM, DeclSymTrait<MultTrait_t<RES, ST>>,
						   If_t<LOW, If_t<UPP, DeclDiagTrait<MultTrait_t<RES, ST>>, DeclLowTrait<MultTrait_t<RES, ST>>>,
								If_t<UPP, DeclUppTrait<MultTrait_t<RES, ST>>, MultTrait<RES, ST>>>>>::Type;
	...
	static constexpr bool simdEnabled =
		(!(IsDiagonal_v<MT1> && IsDiagonal_v<MT2>) && MT1::simdEnabled && MT2::simdEnabled &&
		 IsSIMDCombinable_v<ET1, ET2, ST> && HasSIMDAdd_v<ET1, ET2> && HasSIMDMult_v<ET1, ET2>);
	...
	*/
	inline ReturnType operator()(size_t i, size_t j) const
	{
		METRIC_NUMERIC_INTERNAL_ASSERT(i < matrix_.rows(), "Invalid row access index");
		METRIC_NUMERIC_INTERNAL_ASSERT(j < matrix_.columns(), "Invalid column access index");
	...
	*/
	...
	// \return The number of rows of the matrix.
	*/
	inline size_t rows() const { return matrix_.rows(); }
	//**********************************************************************************************
	...
	// \return The number of columns of the matrix.
	*/
	inline size_t columns() const { return matrix_.columns(); }
	//**********************************************************************************************
	...
	*/
	inline LeftOperand leftOperand() const { return matrix_; }
	//**********************************************************************************************
	...
	*/
	inline RightOperand rightOperand() const { return scalar_; }
	//**********************************************************************************************
	...
	// \return \a true in case the expression can alias, \a false otherwise.
	*/
	template <typename T> inline bool canAlias(const T *alias) const { return matrix_.canAlias(alias); }
	//**********************************************************************************************
	...
	// \return \a true in case an alias effect is detected, \a false otherwise.
	*/
	template <typename T> inline bool isAliased(const T *alias) const { return matrix_.isAliased(alias); }
	//**********************************************************************************************
	...
	// \return \a true in case the operands are aligned, \a false if not.
	*/
	inline bool isAligned() const { return matrix_.isAligned(); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline void assign(DenseMatrix<MT, SO> &lhs, const DMatScalarMultExpr &rhs)
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(A.rows() == left.rows(), "Invalid number of rows");
		METRIC_NUMERIC_INTERNAL_ASSERT(A.columns() == left.columns(), "Invalid number of columns");
		METRIC_NUMERIC_INTERNAL_ASSERT(B.rows() == right.rows(), "Invalid number of rows");
		METRIC_NUMERIC_INTERNAL_ASSERT(B.columns() == right.columns(), "Invalid number of columns");
		METRIC_NUMERIC_INTERNAL_ASSERT(A.rows() == (*lhs).rows(), "Invalid number of rows");
		METRIC_NUMERIC_INTERNAL_ASSERT(B.columns() == (*lhs).columns(), "Invalid number of columns");
	...
	*/
	...
	//**Compile time checks*************************************************************************
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MMM);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(MMM);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MT1);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(MT1);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MT2);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_MAJOR_MATRIX_TYPE(MT2);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SCALAR_TYPE(ST);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TYPE(ST, RightOperand);
	//**********************************************************************************************
};
/*! \endcond */
//*************************************************************************************************
...
// dense matrix:
   ...
   mtrc::numeric::DynamicMatrix<double,columnMajor> A, C;
   mtrc::numeric::DynamicMatrix<double,rowMajor> B;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ,
		  typename MT2> // Type of the right-hand side dense matrix
inline decltype(auto) operator*(const DenseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<TDMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 0UL> : public Size<MT1, 0UL> {};

template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct Size<TDMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 1UL> : public Size<MT2, 1UL> {};
/*! \endcond */
//*************************************************************************************************
