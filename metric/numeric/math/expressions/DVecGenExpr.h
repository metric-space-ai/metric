...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DVECGENEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DVECGENEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DVecGenExpr : public VecGenExpr<DenseVector<DVecGenExpr<VT, OP, TF>, TF>>, private Computation {
  public:
	//**Type definitions****************************************************************************
	...
	class ConstIterator {
	  public:
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE ConstIterator(size_t index, OP op)
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE ReturnType operator*() const { return op_(index_); }
		//*******************************************************************************************
  ...
  public:
	...
	*/
	...
	// \return The resulting value.
	*/
	inline ReturnType operator[](size_t index) const { return op_(index); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator begin() const { return ConstIterator(0UL, op_); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator end() const { return ConstIterator(size_, op_); }
	//**********************************************************************************************
...
// The following example demonstrates the use of the \a generate() function:
   ...
   using C = mtrc::numeric::DynamicVector<double,columnVector>;
   const auto c = generate<C>( 4UL, []( size_t index ){ return mtrc::numeric::exp10( 1.0 + 1.0*index ); } );

   // Generates the vector of integer vectors ( ( 1, 2 ), ( 2, 3 ), ( 3, 4 ), ( 4, 5 ) )
   using VT = StaticVector<int,2UL>;
   using D = mtrc::numeric::DynamicVector<VT,columnVector>;
...
// via the \c METRIC_NUMERIC_DEFAULT_TRANSPOSE_FLAG switch. Alternatively it is possible to specify the
...
// The following example demonstrates the use of the \a generate() function:
   ...
   // Generates the uniform integer vector ( 2, 2, 2, 2, 2 )
   mtrc::numeric::DynamicVector<int,columnVector> a;
   a = generate( 5UL, []( size_t index ){ return 2; } );

   // Generates the linearly spaced float vector ( 2.1, 3.2, 4.3, 5.4 )
   mtrc::numeric::DynamicVector<float,columnVector> b;
   b = generate( 4UL, []( size_t index ){ return 2.1F + 1.1F*index; } );

   // Generates the logarithmically spaced double vector ( 1.0, 10.0, 100.0, 1000.0 )
   mtrc::numeric::DynamicVector<double,columnVector> c;
   c = generate<columnVector>( 4UL, []( size_t index ){ return mtrc::numeric::exp10( 1.0 + 1.0*index ); } );

   // Generates the vector of integer vectors ( ( 1, 2 ), ( 2, 3 ), ( 3, 4 ), ( 4, 5 ) )
   using VT = StaticVector<int,2UL>;
   mtrc::numeric::DynamicVector<VT,rowVector> d;
...
// be changed via the \c METRIC_NUMERIC_DEFAULT_TRANSPOSE_FLAG switch. Alternatively it is possible to
// specify the transpose flag explicitly.\n
// The following example demonstrates the use of the \a linspace() function:

   \code
   using mtrc::numeric::linspace;
   using mtrc::numeric::columnVector;
   using mtrc::numeric::rowVector;
   ...
   mtrc::numeric::DynamicVector<int,columnVector> b;
   b = linspace<columnVector>( 5UL, 6, 2 );

   // Generates the linearly spaced float vector ( 2.1, 3.2, 4.3, 5.4 )
   mtrc::numeric::DynamicVector<float,rowVector> c;
...
// can be changed via the \c METRIC_NUMERIC_DEFAULT_TRANSPOSE_FLAG switch. Alternatively it is possible to
// specify the transpose flag explicitly.\n
// The following example demonstrates the use of the \a logspace() function:

   \code
   using mtrc::numeric::logspace;
   using mtrc::numeric::columnVector;
   using mtrc::numeric::rowVector;
...
inline decltype(auto) subvector(const DVecGenExpr<VT, OP, TF> &expr, RSAs... args)
{
	...
	if (isChecked) {
		...
		METRIC_NUMERIC_USER_ASSERT(I + N <= expr.size(), "Invalid subvector specification");
	}

	return generate(N, [op = expr.operation()](size_t i) { return op(i + I); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) subvector(const DVecGenExpr<VT, OP, TF> &expr, size_t index, size_t size, RSAs... args)
{
	...
	if (isChecked) {
		...
		METRIC_NUMERIC_USER_ASSERT(index + size <= expr.size(), "Invalid subvector specification");
	}

	return generate(size, [op = expr.operation(), index](size_t i) { return op(i + index); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) elements(const DVecGenExpr<VT, OP, TF> &expr, T *indices, size_t n, REAs... args)
{
	...
	return generate(n, [op = expr.operation(), newIndices](size_t i) { return op(newIndices[i]); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) elements(const DVecGenExpr<VT, OP, TF> &expr, P p, size_t n, REAs... args)
{
	...
	return generate(n, [op = expr.operation(), p](size_t i) { return op(p(i)); });
}
/*! \endcond */
//*************************************************************************************************
