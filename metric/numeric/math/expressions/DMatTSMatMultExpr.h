...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATTSMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATTSMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool UF> // Upper flag
class DMatTSMatMultExpr : public MatMatMultExpr<DenseMatrix<DMatTSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, false>>,
						  private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool UseOptimizedKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T2> && !IsResizable_v<ElementType_t<T1>> && !IsResizable_v<ET2>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  private:
	...
	*/
		...
		-> DisableIf_t<CanExploitSymmetry_v<MT, MT1, MT2>>
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(A.columns() == B.rows(), "Invalid matrix sizes");
...

		{
			...
			for (; (i + 4UL) <= M; i += 4UL) {
				for (size_t j = (SYM || HERM || UPP ? i : 0UL); j < (LOW ? i + 4UL : N); ++j) {
					auto element((IsUpper_v<MT4>) ? (IsStrictlyUpper_v<MT4> ? B.upperBound(i, j) : B.lowerBound(i, j))
												  : (B.begin(j)));
			...
			for (; i < M; ++i) {
				for (size_t j = (SYM || HERM || UPP ? i : 0UL); j < (LOW ? i + 1UL : N); ++j) {
					auto element((IsUpper_v<MT4>) ? (IsStrictlyUpper_v<MT4> ? B.upperBound(i, j) : B.lowerBound(i, j))
												  : (B.begin(j)));
					const auto end((IsLower_v<MT4>) ? (IsStrictlyLower_v<MT4> ? B.lowerBound(i, j) : B.upperBound(i, j))
													: (B.end(j)));
...
inline const DMatTSMatMultExpr<MT1, MT2, false, false, false, false> dmattsmatmult(const DenseMatrix<MT1, false> &lhs,
...
inline const MT1 &dmattsmatmult(const DenseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) dmattsmatmult(const DenseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<DMatTSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 0UL> : public Size<MT1, 0UL> {};

template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct Size<DMatTSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 1UL> : public Size<MT2, 1UL> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct IsAligned<DMatTSMatMultExpr<MT1, MT2, SF, HF, LF, UF>> : public IsAligned<MT1> {};
/*! \endcond */
//*************************************************************************************************
