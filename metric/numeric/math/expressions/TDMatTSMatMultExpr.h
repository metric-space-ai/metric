...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATTSMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATTSMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool UF> // Upper flag
class TDMatTSMatMultExpr : public MatMatMultExpr<DenseMatrix<TDMatTSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, true>>,
						   private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool UseVectorizedKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T2> && T1::simdEnabled && T2::simdEnabled && IsColumnMajorMatrix_v<T1> &&
	...
	static constexpr bool UseOptimizedKernel_v =
		(useOptimizedKernels && !UseVectorizedKernel_v<T1, T2, T3> && !IsDiagonal_v<T2> &&
		 !IsResizable_v<ElementType_t<T1>> && !IsResizable_v<ET2>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	static constexpr bool simdEnabled =
		(!IsDiagonal_v<MT1> && MT1::simdEnabled && HasSIMDAdd_v<ET1, ET2> && HasSIMDMult_v<ET1, ET2>);
				...
				for (; element != end; ++element) {
					...
					if (IsDiagonal_v<MT4>) {
						...
						const size_t ibegin((IsLower_v<MT4>)
												? ((LOW) ? (max(j, ii, (IsStrictlyLower_v<MT4> ? j1 + 1UL : j1)))
		...
		for (size_t ii = 0UL; ii < A.rows(); ii += block) {
			...
			for (size_t j = 0UL; j < B.columns(); ++j) {
				...
				for (size_t k = 0UL; k < kpos; k += 4UL) {
					...
					METRIC_NUMERIC_INTERNAL_ASSERT(j1 < j2 && j2 < j3 && j3 < j4,
					...
					for (size_t i = ibegin; i < ipos; i += 4UL) {
						...
						C(i + 1UL, j) +=
							A(i + 1UL, j1) * v1 + A(i + 1UL, j2) * v2 + A(i + 1UL, j3) * v3 + A(i + 1UL, j4) * v4;
						C(i + 2UL, j) +=
							A(i + 2UL, j1) * v1 + A(i + 2UL, j2) * v2 + A(i + 2UL, j3) * v3 + A(i + 2UL, j4) * v4;
						C(i + 3UL, j) +=
							A(i + 3UL, j1) * v1 + A(i + 3UL, j2) * v2 + A(i + 3UL, j3) * v3 + A(i + 3UL, j4) * v4;
					}
		...
		-> EnableIf_t<UseVectorizedKernel_v<MT3, MT4, MT5>>
	{
		...
		for (size_t j = 0UL; j < B.columns(); ++j) {
			...
			for (size_t k = 0UL; k < kpos; k += 4UL) {
				...
				METRIC_NUMERIC_INTERNAL_ASSERT(j1 < j2 && j2 < j3 && j3 < j4, "Invalid sparse matrix index detected");
				...
				const size_t iend((IsUpper_v<MT4>)
									  ? ((IsStrictlyUpper_v<MT4>) ? (SYM || HERM || UPP ? max(j + 1UL, j4) : j4)
				...
				for (; i < ipos; i += SIMDSIZE) {
					C.store(i, j,
							C.load(i, j) + A.load(i, j1) * xmm1 + A.load(i, j2) * xmm2 + A.load(i, j3) * xmm3 +
								A.load(i, j4) * xmm4);
				}
			...
			for (; element != end; ++element) {
				...
				const size_t ibegin((IsLower_v<MT4>)
										...
										: (LOW ? prevMultiple(j, SIMDSIZE) : 0UL));
				const size_t iend((IsUpper_v<MT4>)
									  ? ((IsStrictlyUpper_v<MT4>) ? (SYM || HERM || UPP ? max(j + 1UL, j1) : j1)
				...
				for (; element != end; ++element) {
					...
					if (IsDiagonal_v<MT4>) {
						...
						const size_t ibegin((IsLower_v<MT4>)
												...
												: (LOW ? max(j, ii) : ii));
						const size_t iend((IsUpper_v<MT4>)
											  ? ((UPP) ? (min(j + 1UL, itmp, (IsStrictlyUpper_v<MT4> ? j1 : j1 + 1UL)))
													   : (min(itmp, (IsStrictlyUpper_v<MT4> ? j1 : j1 + 1UL))))
			...
			for (size_t j = 0UL; j < B.columns(); ++j) {
				...
				for (size_t k = 0UL; k < kpos; k += 4UL) {
					...
					const size_t ibegin((IsLower_v<MT4>)
											...
											: (LOW ? max(j, ii) : ii));
					const size_t iend((IsUpper_v<MT4>)
										  ? ((UPP) ? (min(j + 1UL, itmp, (IsStrictlyUpper_v<MT4> ? j4 : j4 + 1UL)))
												   : (min(itmp, (IsStrictlyUpper_v<MT4> ? j4 : j4 + 1UL))))
										  : (UPP ? min(j + 1UL, itmp) : itmp));
			...
			for (size_t k = 0UL; k < kpos; k += 4UL) {
				...
				const size_t ibegin((IsLower_v<MT4>)
										? ((IsStrictlyLower_v<MT4>)
											   ...
											   : (prevMultiple((LOW ? max(j, j1) : j1), SIMDSIZE)))
										: (LOW ? prevMultiple(j, SIMDSIZE) : 0UL));
				const size_t iend((IsUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? max(j + 1UL, j4) : j4)
			...
			for (; element != end; ++element) {
				...
				const size_t ibegin((IsLower_v<MT4>)
										? ((IsStrictlyLower_v<MT4>)
											   ...
											   : (prevMultiple((LOW ? max(j, j1) : j1), SIMDSIZE)))
										: (LOW ? prevMultiple(j, SIMDSIZE) : 0UL));
				const size_t iend((IsUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? max(j + 1UL, j1) : j1)
...
inline const TDMatTSMatMultExpr<MT1, MT2, false, false, false, false> tdmattsmatmult(const DenseMatrix<MT1, true> &lhs,
...
inline const MT1 &tdmattsmatmult(const DenseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) tdmattsmatmult(const DenseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
// sparse matrix:
   ...
   mtrc::numeric::CompressedMatrix<double,columnMajor> B;
...
struct Size<TDMatTSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 0UL> : public Size<MT1, 0UL> {};

template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct Size<TDMatTSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 1UL> : public Size<MT2, 1UL> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct IsAligned<TDMatTSMatMultExpr<MT1, MT2, SF, HF, LF, UF>> : public IsAligned<MT1> {};
/*! \endcond */
//*************************************************************************************************
