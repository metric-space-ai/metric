...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATSVECMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATSVECMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class TDMatSVecMultExpr : public MatVecMultExpr<DenseVector<TDMatSVecMultExpr<MT, VT>, false>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool UseVectorizedKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T2> && T1::simdEnabled && T2::simdEnabled &&
	...
	/*! In case a vectorized computation of the matrix/vector multiplication is not possible, but
		...
		be 0. */
	template <typename T1, typename T2, typename T3>
	static constexpr bool UseOptimizedKernel_v = (!UseVectorizedKernel_v<T1, T2, T3> && !IsDiagonal_v<T2> &&
  ...
  public:
	...
	static constexpr bool simdEnabled =
		(!IsDiagonal_v<MT> && MT::simdEnabled && HasSIMDAdd_v<MET, VET> && HasSIMDMult_v<MET, VET>);
	...
	*/
	inline bool canSMPAssign() const noexcept { return (size() > SMP_TDMATSVECMULT_THRESHOLD); }
	//**********************************************************************************************
  ...
  private:
		...
		-> EnableIf_t<UseDefaultKernel_v<VT1, MT1, VT2>>
	{
		...
		for (; element != end; ++element) {
			...
			if (IsDiagonal_v<MT1>) {
				...
				const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? index + 1UL : index) : (0UL));
				const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? index : index + 1UL) : (M));
	...
	*/
		...
		-> EnableIf_t<UseOptimizedKernel_v<VT1, MT1, VT2>>
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(jpos <= x.nonZeros(), "Invalid end calculation");
		...
		if (jpos > 3UL) {
			...
			METRIC_NUMERIC_INTERNAL_ASSERT(j1 < j2 && j2 < j3 && j3 < j4, "Invalid sparse vector index detected");
		...
		for (size_t j = (jpos > 3UL) ? (4UL) : (1UL); (j + 4UL) <= jpos; j += 4UL) {
			...
			const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? j1 + 1UL : j1) : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j4 : j4 + 1UL) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin <= iend, "Invalid loop indices detected");
		...
		for (; element != end; ++element) {
			...
			const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? j1 + 1UL : j1) : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j1 : j1 + 1UL) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin <= iend, "Invalid loop indices detected");
	...
	*/
		...
		-> EnableIf_t<UseVectorizedKernel_v<VT1, MT1, VT2>>
	{
		...
		for (size_t j = (jpos > 3UL) ? (4UL) : (1UL); (j + 4UL) <= jpos; j += 4UL) {
			...
			const size_t ibegin((IsLower_v<MT1>) ? (prevMultiple((IsStrictlyLower_v<MT1> ? j1 + 1UL : j1), SIMDSIZE))
												 : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j4 : j4 + 1UL) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin <= iend, "Invalid loop indices detected");
			...
			for (; i < ipos; i += SIMDSIZE) {
				y.store(i, y.load(i) + A.load(i, j1) * xmm1 + A.load(i, j2) * xmm2 + A.load(i, j3) * xmm3 +
							   A.load(i, j4) * xmm4);
			}
...
inline const TDMatSVecMultExpr<MT, VT> tdmatsvecmult(const DenseMatrix<MT, true> &mat,
...
inline decltype(auto) tdmatsvecmult(const DenseMatrix<MT, true> &mat, const SparseVector<VT, false> &vec)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT // Type of the left-hand side dense matrix
		  ,
		  typename VT> // Type of the right-hand side sparse vector
inline decltype(auto) operator*(const DenseMatrix<MT, true> &mat, const SparseVector<VT, false> &vec)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, typename VT> struct IsAligned<TDMatSVecMultExpr<MT, VT>> : public IsAligned<MT> {};
/*! \endcond */
//*************************************************************************************************
