...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATDMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATDMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool UF> // Upper flag
class TSMatDMatMultExpr : public MatMatMultExpr<DenseMatrix<TSMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, false>>,
						  private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	template <typename T1, typename T2, typename T3> static constexpr bool CanExploitSymmetry_v = IsSymmetric_v<T2>;
	/*! \endcond */
	//**********************************************************************************************
	...
	static constexpr bool UseOptimizedKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T3> && !IsResizable_v<ElementType_t<T1>> && !IsResizable_v<ET1>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	//! Base type of this TSMatDMatMultExpr instance.
	...
	*/
	inline TSMatDMatMultExpr(const MT1 &lhs, const MT2 &rhs) noexcept
	...
	*/
	inline bool isAligned() const noexcept { return rhs_.isAligned(); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	static inline void selectAssignKernel(MT3 &C, const MT4 &A, const MT5 &B)
	{
		...
		if ((IsRowMajorMatrix_v<MT3> && size < TSMATDMATMULT_THRESHOLD) || (IsColumnMajorMatrix_v<MT3> && size < 625UL))
				...
				for (size_t i = 0UL; i < A.columns(); ++i) {
					...
					for (; element != end; ++element) {
						...
						const size_t jend((IsLower_v<MT5>)
											  ? ((LOW) ? (min(i1 + 1UL, IsStrictlyLower_v<MT5> ? i : i + 1UL, jpos))
		...
		-> EnableIf_t<UseOptimizedKernel_v<MT3, MT4, MT5>>
	{
		...
		for (size_t jj = 0UL; jj < B.columns(); jj += block) {
			...
			for (size_t i = 0UL; i < A.columns(); ++i) {
				...
				METRIC_NUMERIC_INTERNAL_ASSERT(kpos <= nonzeros, "Invalid end calculation");
				...
				for (size_t k = 0UL; k < kpos; k += 4UL) {
					...
					METRIC_NUMERIC_INTERNAL_ASSERT(i1 < i2 && i2 < i3 && i3 < i4,
												   "Invalid sparse matrix index detected");

					const size_t jbegin((IsUpper_v<MT5>) ? ((SYM || HERM || UPP)
																? (max(i1, IsStrictlyUpper_v<MT5> ? i + 1UL : i, jj))
																: (max(IsStrictlyUpper_v<MT5> ? i + 1UL : i, jj)))
														 : (SYM || HERM || UPP ? max(i1, jj) : jj));
					const size_t jend((IsLower_v<MT5>)
										  ? ((LOW) ? (min(i4 + 1UL, IsStrictlyLower_v<MT5> ? i : i + 1UL, jpos))
												   : (min(IsStrictlyLower_v<MT5> ? i : i + 1UL, jpos)))
			...
			for (size_t jj = 0UL; jj < B.columns(); jj += block) {
				...
				for (size_t i = 0UL; i < A.columns(); ++i) {
					...
					for (; element != end; ++element) {
						...
						const size_t jbegin((IsUpper_v<MT5>)
												? ((UPP) ? (max(i1, IsStrictlyUpper_v<MT5> ? i + 1UL : i, jj))
														 : (max(IsStrictlyUpper_v<MT5> ? i + 1UL : i, jj)))
												: (UPP ? max(i1, jj) : jj));
						const size_t jend((IsLower_v<MT5>)
											  ? ((LOW) ? (min(i1 + 1UL, IsStrictlyLower_v<MT5> ? i : i + 1UL, jpos))
		...
		for (size_t jj = 0UL; jj < B.columns(); jj += block) {
			...
			for (size_t i = 0UL; i < A.columns(); ++i) {
				...
				for (size_t k = 0UL; k < kpos; k += 4UL) {
					...
					const size_t jbegin((IsUpper_v<MT5>) ? ((UPP) ? (max(i1, IsStrictlyUpper_v<MT5> ? i + 1UL : i, jj))
																  : (max(IsStrictlyUpper_v<MT5> ? i + 1UL : i, jj)))
														 : (UPP ? max(i1, jj) : jj));
					const size_t jend((IsLower_v<MT5>)
										  ? ((LOW) ? (min(i4 + 1UL, IsStrictlyLower_v<MT5> ? i : i + 1UL, jpos))
...
template <
	...
	typename MT2 // Type of the right-hand side sparse matrix
	,
	DisableIf_t<(IsIdentity_v<MT1> && IsSame_v<ElementType_t<MT1>, ElementType_t<MT2>>) || IsZero_v<MT1>> * = nullptr>
inline const TSMatDMatMultExpr<MT1, MT2, false, false, false, false> tsmatdmatmult(const SparseMatrix<MT1, true> &lhs,
																				   const DenseMatrix<MT2, false> &rhs)
{
	...
	return TSMatDMatMultExpr<MT1, MT2, false, false, false, false>(*lhs, *rhs);
}
/*! \endcond */
//*************************************************************************************************
...
//        and a row-major dense matrix (\f$ A=B*C \f$).
// \ingroup dense_matrix
//
...
// a column-major identity matrix and a row-major dense matrix. It returns a reference to the
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  ,
		  EnableIf_t<IsIdentity_v<MT1> && IsSame_v<ElementType_t<MT1>, ElementType_t<MT2>>> * = nullptr>
inline const MT2 &tsmatdmatmult(const SparseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
//
// This function implements a performance optimized treatment of the multiplication between a
// column-major zero matrix and a row-major dense matrix. It returns a zero matrix.
*/
template <typename MT1 // Type of the left-hand side dense matrix
		  ,
		  typename MT2 // Type of the right-hand side sparse matrix
...
inline decltype(auto) tsmatdmatmult(const SparseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
// dense matrix:
   ...
   mtrc::numeric::DynamicMatrix<double,columnMajor> C;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side dense matrix
inline decltype(auto) operator*(const SparseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool UF> // Upper flag
inline decltype(auto) declunilow(const TSMatDMatMultExpr<MT1, MT2, SF, HF, false, UF> &dm)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool UF> // Upper flag
inline decltype(auto) declstrlow(const TSMatDMatMultExpr<MT1, MT2, SF, HF, false, UF> &dm)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool LF> // Lower flag
inline decltype(auto) decluniupp(const TSMatDMatMultExpr<MT1, MT2, SF, HF, LF, false> &dm)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool LF> // Lower flag
inline decltype(auto) declstrupp(const TSMatDMatMultExpr<MT1, MT2, SF, HF, LF, false> &dm)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<TSMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 0UL> : public Size<MT1, 0UL> {};

template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct Size<TSMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 1UL> : public Size<MT2, 1UL> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct IsAligned<TSMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>> : public IsAligned<MT2> {};
/*! \endcond */
//*************************************************************************************************
