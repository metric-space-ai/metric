...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATDMATSCHUREXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATDMATSCHUREXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DMatDMatSchurExpr : public SchurExpr<DenseMatrix<DMatDMatSchurExpr<MT1, MT2, SO>, SO>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool useAssign = (RequiresEvaluation_v<MT1> || RequiresEvaluation_v<MT2> || !returnExpr);
  ...
  public:
	...
	class ConstIterator {
	  public:
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE ConstIterator(LeftIteratorType left, RightIteratorType right)
		...
		*/
		inline ReturnType operator*() const { return (*left_) * (*right_); }
		//*******************************************************************************************
		...
		*/
		inline auto load() const noexcept { return left_.load() * right_.load(); }
		//*******************************************************************************************
	...
	static constexpr bool simdEnabled = (MT1::simdEnabled && MT2::simdEnabled && HasSIMDMult_v<ET1, ET2>);
	...
	*/
	inline ConstIterator begin(size_t i) const { return ConstIterator(lhs_.begin(i), rhs_.begin(i)); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator end(size_t i) const { return ConstIterator(lhs_.end(i), rhs_.end(i)); }
	//**********************************************************************************************
	...
	*/
	...
	{
		return lhs_.canSMPAssign() || rhs_.canSMPAssign() || (rows() * columns() >= SMP_DMATDMATSCHUR_THRESHOLD);
	}
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline auto assign(SparseMatrix<MT, SO2> &lhs, const DMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO2> // Storage order of the target dense matrix
	friend inline auto addAssign(DenseMatrix<MT, SO2> &lhs, const DMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO2> // Storage order of the target dense matrix
	friend inline auto subAssign(DenseMatrix<MT, SO2> &lhs, const DMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_FORM_VALID_SCHUREXPR(MT1, MT2);
	/*! \endcond */
	//**********************************************************************************************
};
//*************************************************************************************************
...
template <
	...
	bool SO // Storage order
	,
	DisableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>) ||
				(IsStrictlyLower_v<MT1> && IsUpper_v<MT2>) || (IsStrictlyUpper_v<MT1> && IsLower_v<MT2>) ||
				(IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) || (IsUpper_v<MT1> && IsStrictlyLower_v<MT2>)> * = nullptr>
inline const DMatDMatSchurExpr<MT1, MT2, SO> dmatdmatschur(const DenseMatrix<MT1, SO> &lhs,
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  bool SO // Storage order
		  ,
		  EnableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>)> * = nullptr>
inline decltype(auto) dmatdmatschur(const DenseMatrix<MT1, SO> &lhs, const DenseMatrix<MT2, SO> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(ReturnType, SO);
...
template <
	...
	bool SO // Storage order
	,
	EnableIf_t<(IsStrictlyLower_v<MT1> && IsUpper_v<MT2>) || (IsStrictlyUpper_v<MT1> && IsLower_v<MT2>) ||
			   (IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) || (IsUpper_v<MT1> && IsStrictlyLower_v<MT2>)> * = nullptr>
inline decltype(auto) dmatdmatschur(const DenseMatrix<MT1, SO> &lhs, const DenseMatrix<MT2, SO> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO> // Storage order
inline decltype(auto) operator%(const DenseMatrix<MT1, SO> &lhs, const DenseMatrix<MT2, SO> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct IsAligned<DMatDMatSchurExpr<MT1, MT2, SO>> : public BoolConstant<IsAligned_v<MT1> && IsAligned_v<MT2>> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT1, typename MT2, bool SO>
struct IsPadded<DMatDMatSchurExpr<MT1, MT2, SO>> : public BoolConstant<IsPadded_v<MT1> && IsPadded_v<MT2>> {};
/*! \endcond */
//*************************************************************************************************
