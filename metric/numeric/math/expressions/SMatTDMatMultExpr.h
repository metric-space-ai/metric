...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SMATTDMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SMATTDMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  ,
		  bool UF> // Upper flag
class SMatTDMatMultExpr : public MatMatMultExpr<DenseMatrix<SMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, true>>,
						  private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	template <typename T1, typename T2, typename T3> static constexpr bool CanExploitSymmetry_v = IsSymmetric_v<T3>;
	/*! \endcond */
	//**********************************************************************************************
...

		{
			...
			for (; (j + 4UL) <= N; j += 4UL) {
				for (size_t i = (SYM || HERM || LOW ? j : 0UL); i < (UPP ? j + 4UL : M); ++i) {
					const auto end((IsUpper_v<MT5>)
									   ...
									   : (A.end(i)));
					auto element((IsLower_v<MT5>) ? (IsStrictlyLower_v<MT5> ? A.upperBound(i, j) : A.lowerBound(i, j))
												  : (A.begin(i)));
			...
			for (; j < N; ++j) {
				for (size_t i = (SYM || HERM || LOW ? j : 0UL); i < (UPP ? j + 1UL : M); ++i) {
					const auto end((IsUpper_v<MT5>) ? (IsStrictlyUpper_v<MT5> ? A.lowerBound(i, j) : A.upperBound(i, j))
													: (A.end(i)));
					auto element((IsLower_v<MT5>) ? (IsStrictlyLower_v<MT5> ? A.upperBound(i, j) : A.lowerBound(i, j))
												  : (A.begin(i)));
...
inline const SMatTDMatMultExpr<MT1, MT2, false, false, false, false> smattdmatmult(const SparseMatrix<MT1, false> &lhs,
...
inline const MT2 &smattdmatmult(const SparseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) smattdmatmult(const SparseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<SMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 0UL> : public Size<MT1, 0UL> {};

template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct Size<SMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 1UL> : public Size<MT2, 1UL> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct IsAligned<SMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>> : public IsAligned<MT2> {};
/*! \endcond */
//*************************************************************************************************
