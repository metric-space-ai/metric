...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SMATDMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SMATDMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  ,
		  bool UF> // Upper flag
class SMatDMatMultExpr : public MatMatMultExpr<DenseMatrix<SMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, false>>,
						 private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool UseVectorizedKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T3> && T1::simdEnabled && T3::simdEnabled && IsRowMajorMatrix_v<T1> &&
	...
	static constexpr bool UseOptimizedKernel_v =
		(useOptimizedKernels && !UseVectorizedKernel_v<T1, T2, T3> && !IsDiagonal_v<T3> &&
		 !IsResizable_v<ElementType_t<T1>> && !IsResizable_v<ET1>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	static constexpr bool simdEnabled =
		(!IsDiagonal_v<MT2> && MT2::simdEnabled && HasSIMDAdd_v<ET1, ET2> && HasSIMDMult_v<ET1, ET2>);
				...
				for (; element != end; ++element) {
					...
					if (IsDiagonal_v<MT5>) {
						...
						const size_t jbegin((IsUpper_v<MT5>)
												? ((UPP) ? (max(i, jj, (IsStrictlyUpper_v<MT5> ? i1 + 1UL : i1)))
			...
			for (size_t i = 0UL; i < A.rows(); ++i) {
				...
				for (size_t k = 0UL; k < kpos; k += 4UL) {
					...
					for (size_t j = jbegin; j < jpos; j += 4UL) {
						...
						C(i, j + 1UL) +=
							v1 * B(i1, j + 1UL) + v2 * B(i2, j + 1UL) + v3 * B(i3, j + 1UL) + v4 * B(i4, j + 1UL);
						C(i, j + 2UL) +=
							v1 * B(i1, j + 2UL) + v2 * B(i2, j + 2UL) + v3 * B(i3, j + 2UL) + v4 * B(i4, j + 2UL);
						C(i, j + 3UL) +=
							v1 * B(i1, j + 3UL) + v2 * B(i2, j + 3UL) + v3 * B(i3, j + 3UL) + v4 * B(i4, j + 3UL);
					}
		...
		-> EnableIf_t<UseVectorizedKernel_v<MT3, MT4, MT5>>
	{
		...
		for (size_t i = 0UL; i < A.rows(); ++i) {
			...
			for (size_t k = 0UL; k < kpos; k += 4UL) {
				...
				METRIC_NUMERIC_INTERNAL_ASSERT(i1 < i2 && i2 < i3 && i3 < i4, "Invalid sparse matrix index detected");
				...
				const size_t jend((IsLower_v<MT5>)
									  ? ((IsStrictlyLower_v<MT5>) ? (SYM || HERM || LOW ? min(i + 1UL, i4) : i4)
				...
				for (; j < jpos; j += SIMDSIZE) {
					C.store(i, j,
							C.load(i, j) + xmm1 * B.load(i1, j) + xmm2 * B.load(i2, j) + xmm3 * B.load(i3, j) +
								xmm4 * B.load(i4, j));
				}
			...
			for (; element != end; ++element) {
				...
				const size_t jbegin((IsUpper_v<MT5>)
										...
										: (UPP ? prevMultiple(i, SIMDSIZE) : 0UL));
				const size_t jend((IsLower_v<MT5>)
									  ? ((IsStrictlyLower_v<MT5>) ? (SYM || HERM || LOW ? min(i + 1UL, i1) : i1)
				...
				for (; element != end; ++element) {
					...
					if (IsDiagonal_v<MT5>) {
						...
						const size_t jbegin((IsUpper_v<MT5>)
												...
												: (jj));
						const size_t jend((IsLower_v<MT5>)
											  ? ((LOW) ? (min(i + 1UL, jtmp, (IsStrictlyLower_v<MT5> ? i1 : i1 + 1UL)))
													   : (min(jtmp, (IsStrictlyLower_v<MT5> ? i1 : i1 + 1UL))))
			...
			for (size_t i = 0UL; i < A.rows(); ++i) {
				...
				for (size_t k = 0UL; k < kpos; k += 4UL) {
					...
					const size_t jbegin((IsUpper_v<MT5>)
											...
											: (UPP ? max(i, jj) : jj));
					const size_t jend((IsLower_v<MT5>)
										  ? ((LOW) ? (min(i + 1UL, jtmp, (IsStrictlyLower_v<MT5> ? i4 : i4 + 1UL)))
												   : (min(jtmp, (IsStrictlyLower_v<MT5> ? i4 : i4 + 1UL))))
										  : (LOW ? min(i + 1UL, jtmp) : jtmp));
			...
			for (size_t k = 0UL; k < kpos; k += 4UL) {
				...
				const size_t jbegin((IsUpper_v<MT5>)
										? ((IsStrictlyUpper_v<MT5>)
											   ...
											   : (prevMultiple((UPP ? max(i, i1) : i1), SIMDSIZE)))
										: (UPP ? prevMultiple(i, SIMDSIZE) : 0UL));
				const size_t jend((IsLower_v<MT5>) ? ((IsStrictlyLower_v<MT5>) ? (LOW ? min(i + 1UL, i4) : i4)
			...
			for (; element != end; ++element) {
				...
				const size_t jbegin((IsUpper_v<MT5>)
										? ((IsStrictlyUpper_v<MT5>)
											   ...
											   : (prevMultiple((UPP ? max(i, i1) : i1), SIMDSIZE)))
										: (UPP ? prevMultiple(i, SIMDSIZE) : 0UL));
				const size_t jend((IsLower_v<MT5>) ? ((IsStrictlyLower_v<MT5>) ? (LOW ? min(i + 1UL, i1) : i1)
...
inline const SMatDMatMultExpr<MT1, MT2, false, false, false, false> smatdmatmult(const SparseMatrix<MT1, false> &lhs,
...
inline const MT2 &smatdmatmult(const SparseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) smatdmatmult(const SparseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<SMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 0UL> : public Size<MT1, 0UL> {};

template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct Size<SMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 1UL> : public Size<MT2, 1UL> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct IsAligned<SMatDMatMultExpr<MT1, MT2, SF, HF, LF, UF>> : public IsAligned<MT2> {};
/*! \endcond */
//*************************************************************************************************
