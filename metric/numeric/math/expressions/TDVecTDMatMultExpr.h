...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TDVECTDMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TDVECTDMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class TDVecTDMatMultExpr : public TVecMatMultExpr<DenseVector<TDVecTDMatMultExpr<VT, MT>, true>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  private:
	...
	*/
		...
		-> EnableIf_t<UseVectorizedDefaultKernel_v<VT1, VT2, MT1>>
	{
		...
		for (; (j + 8UL) <= N; j += 8UL) {
			const size_t ibegin((IsLower_v<MT1>) ? (prevMultiple((IsStrictlyLower_v<MT1> ? j + 1UL : j), SIMDSIZE))
												 : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j + 7UL : j + 8UL) : (M));
		...
		for (; (j + 4UL) <= N; j += 4UL) {
			const size_t ibegin((IsLower_v<MT1>) ? (prevMultiple((IsStrictlyLower_v<MT1> ? j + 1UL : j), SIMDSIZE))
												 : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j + 3UL : j + 4UL) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin < iend, "Invalid loop indices detected");
		...
		for (; (j + 3UL) <= N; j += 3UL) {
			const size_t ibegin((IsLower_v<MT1>) ? (prevMultiple((IsStrictlyLower_v<MT1> ? j + 1UL : j), SIMDSIZE))
												 : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j + 2UL : j + 3UL) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin < iend, "Invalid loop indices detected");
		...
		for (; (j + 2UL) <= N; j += 2UL) {
			const size_t ibegin((IsLower_v<MT1>) ? (prevMultiple((IsStrictlyLower_v<MT1> ? j + 1UL : j), SIMDSIZE))
												 : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j + 1UL : j + 2UL) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin < iend, "Invalid loop indices detected");
		...
		-> EnableIf_t<UseVectorizedDefaultKernel_v<VT1, VT2, MT1>>
	{
		...
		for (; (j + 8UL) <= N; j += 8UL) {
			...
			for (; (i + SIMDSIZE * 3UL) < ipos; i += SIMDSIZE * 4UL) {
				...
				y[j + 1UL] += sum(x1 * A.load(i, j + 1UL) + x2 * A.load(i1, j + 1UL) + x3 * A.load(i2, j + 1UL) +
								  x4 * A.load(i3, j + 1UL));
				y[j + 2UL] += sum(x1 * A.load(i, j + 2UL) + x2 * A.load(i1, j + 2UL) + x3 * A.load(i2, j + 2UL) +
								  x4 * A.load(i3, j + 2UL));
				y[j + 3UL] += sum(x1 * A.load(i, j + 3UL) + x2 * A.load(i1, j + 3UL) + x3 * A.load(i2, j + 3UL) +
								  x4 * A.load(i3, j + 3UL));
				y[j + 4UL] += sum(x1 * A.load(i, j + 4UL) + x2 * A.load(i1, j + 4UL) + x3 * A.load(i2, j + 4UL) +
								  x4 * A.load(i3, j + 4UL));
				y[j + 5UL] += sum(x1 * A.load(i, j + 5UL) + x2 * A.load(i1, j + 5UL) + x3 * A.load(i2, j + 5UL) +
								  x4 * A.load(i3, j + 5UL));
				y[j + 6UL] += sum(x1 * A.load(i, j + 6UL) + x2 * A.load(i1, j + 6UL) + x3 * A.load(i2, j + 6UL) +
								  x4 * A.load(i3, j + 6UL));
				y[j + 7UL] += sum(x1 * A.load(i, j + 7UL) + x2 * A.load(i1, j + 7UL) + x3 * A.load(i2, j + 7UL) +
								  x4 * A.load(i3, j + 7UL));
			}
...
class DVecScalarMultExpr<TDVecTDMatMultExpr<VT, MT>, ST, true>
  ...
  private:
	...
	static constexpr bool UseSMPAssign_v = (T1::smpAssignable && (evaluateVector || evaluateMatrix));
	//**********************************************************************************************
...
template <typename VT // Type of the left-hand side dense vector
		  ,
		  typename MT> // Type of the right-hand side dense matrix
inline decltype(auto) operator*(const DenseVector<VT, true> &vec, const DenseMatrix<MT, true> &mat)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct IsAligned<TDVecTDMatMultExpr<VT, MT>> : public BoolConstant<IsAligned_v<VT> && IsAligned_v<MT>> {};
/*! \endcond */
//*************************************************************************************************
