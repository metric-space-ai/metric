...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SMATDMATSCHUREXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SMATDMATSCHUREXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/util/mpl/Max.h>
...
class SMatDMatSchurExpr : public SchurExpr<SparseMatrix<SMatDMatSchurExpr<MT1, MT2>, false>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	class ConstIterator {
	  public:
		...
		*/
		inline ReturnType value() const { return left_->value() * right_(row_, left_->index()); }
		//*******************************************************************************************
		...
		*/
		inline size_t index() const { return left_->index(); }
		//*******************************************************************************************
	...
	*/
	inline ConstIterator begin(size_t i) const { return ConstIterator(lhs_.begin(i), rhs_, i); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator end(size_t i) const { return ConstIterator(lhs_.end(i), rhs_, i); }
	//**********************************************************************************************
	...
	*/
	inline size_t nonZeros() const { return lhs_.nonZeros(); }
	//**********************************************************************************************
	...
	*/
	inline size_t nonZeros(size_t i) const { return lhs_.nonZeros(i); }
	//**********************************************************************************************
	...
	*/
	...
	{
		METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(MT1);
  ...
  private:
	...
	*/
	...
	friend inline auto assign(DenseMatrix<MT, SO2> &lhs, const SMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
	...
	*/
	...
	friend inline auto assign(SparseMatrix<MT, false> &lhs, const SMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename MT> // Type of the target sparse matrix
	friend inline auto assign(SparseMatrix<MT, true> &lhs, const SMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO2> // Storage order of the target dense matrix
	friend inline auto addAssign(DenseMatrix<MT, SO2> &lhs, const SMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO2> // Storage order of the target dense matrix
	friend inline auto subAssign(DenseMatrix<MT, SO2> &lhs, const SMatDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  DisableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>) ||
					  ...
					  (IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) || (IsUpper_v<MT1> && IsStrictlyLower_v<MT2>) ||
					  IsZero_v<MT1>> * = nullptr>
inline const SMatDMatSchurExpr<MT1, MT2> smatdmatschur(const SparseMatrix<MT1, false> &lhs,
...
inline decltype(auto) smatdmatschur(const SparseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
// matrix:
   ...
   mtrc::numeric::CompressedMatrix<double,rowMajor> A, C;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side dense matrix
inline decltype(auto) operator%(const SparseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  DisableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>) ||
					  ...
					  (IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) || (IsUpper_v<MT1> && IsStrictlyLower_v<MT2>) ||
					  IsZero_v<MT1>> * = nullptr>
inline const SMatDMatSchurExpr<MT1, MT2> smattdmatschur(const SparseMatrix<MT1, false> &lhs,
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  ,
		  EnableIf_t<(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) || (IsUniUpper_v<MT1> && IsUniLower_v<MT2>)> * = nullptr>
inline decltype(auto) smattdmatschur(const SparseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side dense matrix
inline decltype(auto) operator%(const SparseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<SMatDMatSchurExpr<MT1, MT2>, 0UL> : public Max_t<Size<MT1, 0UL>, Size<MT2, 0UL>> {};

template <typename MT1, typename MT2>
struct Size<SMatDMatSchurExpr<MT1, MT2>, 1UL> : public Max_t<Size<MT1, 1UL>, Size<MT2, 1UL>> {};
/*! \endcond */
//*************************************************************************************************
