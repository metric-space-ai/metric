...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SMATSMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SMATSMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SMatSMatMultExpr : public MatMatMultExpr<SparseMatrix<SMatSMatMultExpr<MT1, MT2>, false>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool CanExploitSymmetry_v = (IsColumnMajorMatrix_v<T1> && IsSymmetric_v<T2> && IsSymmetric_v<T3>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	*/
	constexpr size_t nonZeros() const noexcept { return 0UL; }
	//**********************************************************************************************
	...
	*/
	inline bool canSMPAssign() const noexcept { return (rows() * columns() >= SMP_SMATSMATMULT_THRESHOLD); }
	//**********************************************************************************************
  ...
  private:
		...
		for (size_t i = 0UL; i < C.rows(); ++i) {
			...
			for (auto lelem = A.begin(i); lelem != lend; ++lelem) {
				...
				for (auto relem = B.begin(lelem->index()); relem != rend; ++relem) {
					if (IsResizable_v<ElementType_t<MT3>> && isDefault(C(i, relem->index()))) {
	...
	*/
	...
	friend inline void assign(SparseMatrix<MT, false> &lhs, const SMatSMatMultExpr &rhs)
	{
		...
		for (size_t i = 0UL; i < (*lhs).rows(); ++i) {
			...
			for (auto lelem = A.begin(i); lelem != lend; ++lelem) {
				...
				for (auto relem = B.begin(lelem->index()); relem != rend; ++relem) {
					if (!valid[relem->index()]) {
						...
						++nonzeros;
						if (relem->index() < minIndex)
							minIndex = relem->index();
						if (relem->index() > maxIndex)
							maxIndex = relem->index();
					} else {
						values[relem->index()] += lelem->value() * relem->value();
					}
				}
			...
			METRIC_NUMERIC_INTERNAL_ASSERT(nonzeros <= (*lhs).columns(), "Invalid number of non-zero elements");

			if (nonzeros > 0UL) {
				METRIC_NUMERIC_INTERNAL_ASSERT(minIndex <= maxIndex, "Invalid index detected");
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO> // Storage order of the target dense matrix
	friend inline void smpSchurAssign(DenseMatrix<MT, SO> &lhs, const SMatSMatMultExpr &rhs)
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  typename MT2 // Type of the right-hand side sparse matrix
		  ,
		  DisableIf_t<((IsIdentity_v<MT1> || IsIdentity_v<MT2>) && IsSame_v<ElementType_t<MT1>, ElementType_t<MT2>>) ||
					  (IsIdentity_v<MT1> && IsIdentity_v<MT2>) || (IsZero_v<MT1> || IsZero_v<MT2>)> * = nullptr>
inline const SMatSMatMultExpr<MT1, MT2> smatsmatmult(const SparseMatrix<MT1, false> &lhs,
...
template <
	...
	typename MT2 // Type of the right-hand side sparse matrix
	,
	EnableIf_t<!IsIdentity_v<MT1> && IsIdentity_v<MT2> && IsSame_v<ElementType_t<MT1>, ElementType_t<MT2>>> * = nullptr>
inline const MT1 &smatsmatmult(const SparseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <
	...
	,
	EnableIf_t<IsIdentity_v<MT1> && !IsIdentity_v<MT2> && IsSame_v<ElementType_t<MT1>, ElementType_t<MT2>>> * = nullptr>
inline const MT2 &smatsmatmult(const SparseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) smatsmatmult(const SparseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
// This operator represents the multiplication of two row-major sparse matrices:
   ...
   mtrc::numeric::CompressedMatrix<double,rowMajor> A, B, C;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side sparse matrix
inline decltype(auto) operator*(const SparseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
