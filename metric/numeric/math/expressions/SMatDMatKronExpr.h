...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SMATDMATKRONEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SMATDMATKRONEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SMatDMatKronExpr : public MatMatKronExpr<SparseMatrix<SMatDMatKronExpr<MT1, MT2, SO>, SO>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	*/
	inline size_t nonZeros() const { return lhs_.nonZeros() * rhs_.rows() * rhs_.columns(); }
	//**********************************************************************************************
  ...
  private:
			...
			for (size_t j = 0UL; j < A.columns(); ++j) {
				...
				for (auto aelem = A.begin(j); aelem != aend; ++aelem) {
					for (size_t l = 0UL; l < N; ++l) {
						const size_t kbegin((IsLower_v<MT2>) ? ((IsStrictlyLower_v<MT2> ? l + 1UL : l)) : (0UL));
						const size_t kend((IsUpper_v<MT2>) ? (IsStrictlyUpper_v<MT2> ? l : l + 1UL) : (M));
			...
			for (size_t i = 0UL; i < A.rows(); ++i) {
				...
				for (auto aelem = A.begin(i); aelem != aend; ++aelem) {
					for (size_t k = 0UL; k < M; ++k) {
						const size_t lbegin((IsUpper_v<MT2>) ? ((IsStrictlyUpper_v<MT2> ? k + 1UL : k)) : (0UL));
						const size_t lend((IsLower_v<MT2>) ? (IsStrictlyLower_v<MT2> ? k : k + 1UL) : (N));
						METRIC_NUMERIC_INTERNAL_ASSERT(lbegin <= lend, "Invalid loop indices detected");
	...
	*/
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(MT1, SO);
...
inline const SMatDMatKronExpr<MT1, MT2, SO1> smatdmatkron(const SparseMatrix<MT1, SO1> &lhs,
...
inline decltype(auto) smatdmatkron(const SparseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;

	using ReturnType = const KronTrait_t<ResultType_t<MT1>, ResultType_t<MT2>>;

	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(ReturnType, SO1);
...
// This kron() function computes the Kronecker product of the given sparse matrix and dense matrix:
   ...
   \code
   mtrc::numeric::CompressedMatrix<double> A, C;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side dense matrix
inline decltype(auto) kron(const SparseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
