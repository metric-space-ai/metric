...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SVECSVECMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SVECSVECMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SVecSVecMultExpr : public VecVecMultExpr<SparseVector<SVecSVecMultExpr<VT1, VT2, TF>, TF>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline void multAssign(SparseVector<VT, TF> &lhs, const SVecSVecMultExpr &rhs)
	{
		...
		for (; i1 != end1; ++i1) {
			while (i2 != end2 && i2->index() < i1->index())
				++i2;
			if (i2 == end2)
				break;
			while (i3 != end3 && i3->index() < i1->index())
				++i3;
			if (i3 == end3)
				break;
...
inline const SVecSVecMultExpr<VT1, VT2, TF> svecsvecmult(const SparseVector<VT1, TF> &lhs,
...
inline decltype(auto) svecsvecmult(const SparseVector<VT1, TF> &lhs, const SparseVector<VT2, TF> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
