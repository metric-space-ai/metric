...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SMATSMATKRONEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SMATSMATKRONEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SMatSMatKronExpr : public MatMatKronExpr<SparseMatrix<SMatSMatKronExpr<MT1, MT2>, false>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	*/
	inline size_t nonZeros(size_t i) const { return lhs_.nonZeros(i / rhs_.rows()) * rhs_.nonZeros(i % rhs_.rows()); }
	//**********************************************************************************************
	...
	friend inline void assign(SparseMatrix<MT, false> &lhs, const SMatSMatKronExpr &rhs)
	{
		...
		for (size_t i = 0UL; i < A.rows(); ++i) {
			for (size_t k = 0UL; k < M; ++k) {
				for (auto aelem = A.begin(i); aelem != A.end(i); ++aelem)
					for (auto belem = B.begin(k); belem != B.end(k); ++belem)
						(*lhs).append(i * M + k, aelem->index() * N + belem->index(), aelem->value() * belem->value(),
									  true);
				(*lhs).finalize(i * M + k);
			}
		}
...
inline const SMatSMatKronExpr<MT1, MT2> smatsmatkron(const SparseMatrix<MT1, false> &lhs,
...
inline decltype(auto) smatsmatkron(const SparseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side sparse matrix
inline decltype(auto) kron(const SparseMatrix<MT1, false> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
