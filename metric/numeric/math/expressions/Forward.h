...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_FORWARD_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_FORWARD_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename VT, bool TF> decltype(auto) trans(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) trans(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) trans(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) trans(const SparseMatrix<MT, SO> &);

template <bool TTF, typename VT, bool TF> decltype(auto) transTo(const DenseVector<VT, TF> &);

template <bool TTF, typename VT, bool TF> decltype(auto) transTo(const SparseVector<VT, TF> &);

template <bool B, typename MT, bool SO> decltype(auto) transIf(const DenseMatrix<MT, SO> &);

template <bool B, typename MT, bool SO> decltype(auto) transIf(const SparseMatrix<MT, SO> &);

template <typename VT, bool TF> decltype(auto) eval(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) eval(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) eval(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) eval(const SparseMatrix<MT, SO> &);

template <typename VT, bool TF> decltype(auto) serial(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) serial(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) serial(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) serial(const SparseMatrix<MT, SO> &);

template <typename VT, bool TF> decltype(auto) noalias(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) noalias(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) noalias(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) noalias(const SparseMatrix<MT, SO> &);

template <typename VT, bool TF> decltype(auto) nosimd(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) nosimd(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) nosimd(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) nosimd(const SparseMatrix<MT, SO> &);

template <typename VT, bool TF> decltype(auto) fix(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) fix(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) fix(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) fix(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) inv(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) matexp(const DenseMatrix<MT, SO> &);
...
template <typename MT, bool SO> decltype(auto) eigen(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) svd(const DenseMatrix<MT, SO> &);

template <typename VT, bool TF, typename OP> decltype(auto) map(const DenseVector<VT, TF> &, OP);

template <typename VT, bool TF, typename OP> decltype(auto) map(const SparseVector<VT, TF> &, OP);

template <typename MT, bool SO, typename OP> decltype(auto) map(const DenseMatrix<MT, SO> &, OP);

template <typename MT, bool SO, typename OP> decltype(auto) map(const SparseMatrix<MT, SO> &, OP);
...
template <typename VT, bool TF, typename OP> decltype(auto) reduce(const DenseVector<VT, TF> &, OP);

template <typename VT, bool TF, typename OP> decltype(auto) reduce(const SparseVector<VT, TF> &, OP);

template <typename MT, bool SO, typename OP> decltype(auto) reduce(const DenseMatrix<MT, SO> &, OP);

template <ReductionFlag RF, typename MT, bool SO, typename OP> decltype(auto) reduce(const DenseMatrix<MT, SO> &, OP);

template <typename MT, bool SO, typename OP> decltype(auto) reduce(const SparseMatrix<MT, SO> &, OP);

template <ReductionFlag, typename MT, bool SO, typename OP> decltype(auto) reduce(const SparseMatrix<MT, SO> &, OP);

template <typename VT, bool TF> decltype(auto) expand(const DenseVector<VT, TF> &, size_t);

template <size_t E, typename VT, bool TF> decltype(auto) expand(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) expand(const SparseVector<VT, TF> &, size_t);

template <size_t E, typename VT, bool TF> decltype(auto) expand(const SparseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) repeat(const DenseVector<VT, TF> &, size_t);

template <size_t R0, typename VT, bool TF> decltype(auto) repeat(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) repeat(const SparseVector<VT, TF> &, size_t);

template <size_t R0, typename VT, bool TF> decltype(auto) repeat(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) repeat(const DenseMatrix<MT, SO> &, size_t, size_t);

template <size_t R0, size_t R1, typename MT, bool SO> decltype(auto) repeat(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) repeat(const SparseMatrix<MT, SO> &, size_t, size_t);

template <size_t R0, size_t R1, typename MT, bool SO> decltype(auto) repeat(const SparseMatrix<MT, SO> &);

template <typename VT, bool TF> decltype(auto) mean(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) mean(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) mean(const DenseMatrix<MT, SO> &);

template <ReductionFlag, typename MT, bool SO> decltype(auto) mean(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) mean(const SparseMatrix<MT, SO> &);

template <ReductionFlag, typename MT, bool SO> decltype(auto) mean(const SparseMatrix<MT, SO> &);

template <typename VT, bool TF> decltype(auto) var(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) var(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) var(const DenseMatrix<MT, SO> &);

template <ReductionFlag, typename MT, bool SO> decltype(auto) var(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) var(const SparseMatrix<MT, SO> &);

template <ReductionFlag, typename MT, bool SO> decltype(auto) var(const SparseMatrix<MT, SO> &);

template <typename VT, bool TF> decltype(auto) stddev(const DenseVector<VT, TF> &);

template <typename VT, bool TF> decltype(auto) stddev(const SparseVector<VT, TF> &);

template <typename MT, bool SO> decltype(auto) stddev(const DenseMatrix<MT, SO> &);

template <ReductionFlag, typename MT, bool SO> decltype(auto) stddev(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) stddev(const SparseMatrix<MT, SO> &);

template <ReductionFlag, typename MT, bool SO> decltype(auto) stddev(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declsym(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declsym(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declherm(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declherm(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) decllow(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) decllow(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declunilow(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declunilow(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declstrlow(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declstrlow(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declupp(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declupp(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) decluniupp(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) decluniupp(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declstrupp(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) declstrupp(const SparseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) decldiag(const DenseMatrix<MT, SO> &);

template <typename MT, bool SO> decltype(auto) decldiag(const SparseMatrix<MT, SO> &);
