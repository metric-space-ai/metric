...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATGENEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATGENEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DMatGenExpr : public MatGenExpr<DenseMatrix<DMatGenExpr<MT, OP, SO>, SO>>, private Computation {
  public:
	//**Type definitions****************************************************************************
	...
	class ConstIterator {
	  public:
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE ConstIterator(size_t m, size_t n, OP op)
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE ReturnType operator*() const { return op_(m_, n_); }
		//*******************************************************************************************
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE bool operator==(const ConstIterator &rhs) const noexcept
		{
			if (SO)
				return m_ == rhs.m_;
			else
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE DifferenceType operator-(const ConstIterator &rhs) const noexcept
		{
			if (SO)
				return m_ - rhs.m_;
			else
		...
		// \return The incremented iterator.
		*/
		friend inline METRIC_NUMERIC_DEVICE_CALLABLE const ConstIterator operator+(const ConstIterator &it, size_t inc)
		{
			if (SO)
		...
		// \return The incremented iterator.
		*/
		friend inline METRIC_NUMERIC_DEVICE_CALLABLE const ConstIterator operator+(size_t inc, const ConstIterator &it)
		{
			if (SO)
				return ConstIterator(it.m_ + inc, it.n_, it.op_);
			else
  ...
  public:
	...
	*/
	inline ConstIterator begin(size_t i) const { return ConstIterator((SO ? 0UL : i), (SO ? i : 0UL), op_); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator end(size_t i) const { return ConstIterator((SO ? m_ : i), (SO ? i : n_), op_); }
	//**********************************************************************************************
	...
	*/
	inline size_t rows() const noexcept { return m_; }
	//**********************************************************************************************
	...
	*/
	inline size_t columns() const noexcept { return n_; }
	//**********************************************************************************************
	...
	*/
	inline bool isAligned() const noexcept { return true; }
	//**********************************************************************************************
	...
	*/
	inline bool canSMPAssign() const noexcept { return true; }
	//**********************************************************************************************
...
// The following example demonstrates the use of the \a generate() function:
   ...
   using B = mtrc::numeric::DynamicMatrix<float,rowMajor>;
   ...
   using C = mtrc::numeric::DynamicMatrix<double,rowMajor>;
   const auto c = generate<C>( 2UL, 2UL, []( size_t i, size_t j ) { return mtrc::numeric::exp10( 1.0 + 1.0*(i*2UL+j)
); } );

   // Generates the vector of integer vectors ( ( 1, 2 ), ( 2, 3 ), ( 3, 4 ), ( 4, 5 ) )
   using VT = StaticVector<int,2UL>;
   using D = mtrc::numeric::DynamicMatrix<VT,columnMajor>;
...
inline decltype(auto) generate(size_t m, size_t n, OP op)
{
	...
	if (Size_v<MT, 0UL> != DefaultSize_v && size_t(Size_v<MT, 0UL>) != m && Size_v<MT, 1UL> != DefaultSize_v &&
		size_t(Size_v<MT, 1UL>) != n) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid size specification");
	}
...
// can be changed via the \c METRIC_NUMERIC_DEFAULT_STORAGE_ORDER switch. Alternatively it is possible
...
// The following example demonstrates the use of the \a generate() function:
   ...
   // Generates the linearly spaced float matrix ( ( 2.1, 3.2, 4.3 ), ( 5.4, 6.5, 7.6 ) )
   mtrc::numeric::DynamicMatrix<float,rowMajor> B;
   B = generate( 2UL, 3UL, []( size_t i, size_t j ){ return 2.1F + 1.1F*(i*3UL+j); } );

   // Generates the logarithmically spaced double vector ( ( 1.0, 10.0 ), ( 100.0, 1000.0 ) )
   mtrc::numeric::DynamicMatrix<double,rowMajor> C;
   C = generate<rowMajor>( 2UL, 2UL, []( size_t i, size_t j ) { return mtrc::numeric::exp10( 1.0 + 1.0*(i*2UL+j) ); }
);

   // Generates the vector of integer vectors ( ( 1, 2 ), ( 2, 3 ), ( 3, 4 ), ( 4, 5 ) )
   using VT = StaticVector<int,2UL>;
   mtrc::numeric::DynamicMatrix<VT,columnMajor> D;
...
inline decltype(auto) submatrix(const DMatGenExpr<MT, OP, SO> &expr, RSAs... args)
{
	...
	if (isChecked) {
		...
		METRIC_NUMERIC_USER_ASSERT(I + M <= expr.rows(), "Invalid submatrix specification");
		METRIC_NUMERIC_USER_ASSERT(J + N <= expr.columns(), "Invalid submatrix specification");
	}

	return generate(M, N, [op = expr.operation()](size_t i, size_t j) { return op(i + I, j + J); });
}
/*! \endcond */
//*************************************************************************************************
...
template <AlignmentFlag AF // Alignment flag
		  ...
		  ,
		  typename... RSAs> // Optional arguments
inline decltype(auto) submatrix(const DMatGenExpr<MT, OP, SO> &expr, size_t row, size_t column, size_t m, size_t n,
								RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
	...
	if (isChecked) {
		...
		METRIC_NUMERIC_USER_ASSERT(row + m <= expr.rows(), "Invalid submatrix specification");
		METRIC_NUMERIC_USER_ASSERT(column + n <= expr.columns(), "Invalid submatrix specification");
	}

	return generate(m, n, [op = expr.operation(), row, column](size_t i, size_t j) { return op(i + row, j + column); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) row(const DMatGenExpr<MT, OP, SO> &expr, RRAs... args)
{
	...
	if (isChecked) {
		...
		METRIC_NUMERIC_USER_ASSERT(I < expr.rows(), "Invalid row access index");
	}

	return generate<rowVector>(expr.columns(), [op = expr.operation()](size_t i) { return op(I, i); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) row(const DMatGenExpr<MT, OP, SO> &expr, size_t index, RRAs... args)
{
	...
	if (isChecked) {
		...
		METRIC_NUMERIC_USER_ASSERT(index < expr.rows(), "Invalid row access index");
	}

	return generate<rowVector>(expr.columns(), [op = expr.operation(), index](size_t i) { return op(index, i); });
}
/*! \endcond */
//*************************************************************************************************
	...
	if (isChecked) {
		...
		for (size_t i = 0UL; i < sizeof...(Is) + 1UL; ++i) {
			if (expr.rows() <= indices[i]) {
				METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid elements specification");
			}
		}
	}
...
inline decltype(auto) rows(const DMatGenExpr<MT, OP, SO> expr, T *indices, size_t n, RRAs... args)
{
	...
	return generate(n, expr.columns(),
					[op = expr.operation(), newIndices](size_t i, size_t j) { return op(newIndices[i], j); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) rows(const DMatGenExpr<MT, OP, SO> &expr, P p, size_t n, RRAs... args)
{
	...
	return generate(n, expr.columns(), [op = expr.operation(), p](size_t i, size_t j) { return op(p(i), j); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) column(const DMatGenExpr<MT, OP, SO> &expr, CRAs... args)
{
	...
	if (isChecked) {
		...
		METRIC_NUMERIC_USER_ASSERT(I < expr.columns(), "Invalid column access index");
	}

	return generate<columnVector>(expr.rows(), [op = expr.operation()](size_t i) { return op(i, I); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) column(const DMatGenExpr<MT, OP, SO> &expr, size_t index, CRAs... args)
{
	...
	if (isChecked) {
		...
		METRIC_NUMERIC_USER_ASSERT(index < expr.columns(), "Invalid column access index");
	}

	return generate<columnVector>(expr.rows(), [op = expr.operation(), index](size_t i) { return op(i, index); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) columns(const DMatGenExpr<MT, OP, SO> expr, T *indices, size_t n, RCAs... args)
{
	...
	return generate(expr.rows(), n,
					[op = expr.operation(), newIndices](size_t i, size_t j) { return op(i, newIndices[j]); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) columns(const DMatGenExpr<MT, OP, SO> &expr, P p, size_t n, RCAs... args)
{
	...
	return generate(expr.rows(), n, [op = expr.operation(), p](size_t i, size_t j) { return op(i, p(j)); });
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) band(const DMatGenExpr<MT, OP, SO> &expr, RBAs... args)
{
	...
	if (isChecked) {
		if ((bd.band() > 0L && column >= expr.columns()) || (bd.band() < 0L && row >= expr.rows())) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid band access index");
		}
	} else {
		METRIC_NUMERIC_USER_ASSERT(bd.band() <= 0L || column < expr.columns(), "Invalid band access index");
		METRIC_NUMERIC_USER_ASSERT(bd.band() >= 0L || row < expr.rows(), "Invalid band access index");
	}

	return generate(size, [op = expr.operation(), row, column](size_t i) { return op(i + row, i + column); });
}
/*! \endcond */
//*************************************************************************************************
