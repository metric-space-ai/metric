...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATTDMATSCHUREXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATTDMATSCHUREXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DMatTDMatSchurExpr : public SchurExpr<DenseMatrix<DMatTDMatSchurExpr<MT1, MT2>, false>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	*/
	...
	{
		return lhs_.canSMPAssign() || rhs_.canSMPAssign() || (rows() * columns() >= SMP_DMATTDMATSCHUR_THRESHOLD);
	}
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline auto assign(DenseMatrix<MT, SO> &lhs, const DMatTDMatSchurExpr &rhs) -> DisableIf_t<UseAssign_v<MT>>
	{
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO> // Storage order of the target dense matrix
	friend inline auto addAssign(DenseMatrix<MT, SO> &lhs, const DMatTDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO> // Storage order of the target dense matrix
	friend inline auto subAssign(DenseMatrix<MT, SO> &lhs, const DMatTDMatSchurExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	...
	METRIC_NUMERIC_CONSTRAINT_MATRICES_MUST_HAVE_DIFFERENT_STORAGE_ORDER(MT1, MT2);
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  typename MT2 // Type of the right-hand side dense matrix
		  ,
		  EnableIf_t<!IsSymmetric_v<MT1> && !IsSymmetric_v<MT2> && !(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) &&
					 !(IsUniUpper_v<MT1> && IsUniLower_v<MT2>) && !(IsStrictlyLower_v<MT1> && IsUpper_v<MT2>) &&
					 !(IsStrictlyUpper_v<MT1> && IsLower_v<MT2>) && !(IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) &&
					 !(IsUpper_v<MT1> && IsStrictlyLower_v<MT2>)> * = nullptr>
inline const DMatTDMatSchurExpr<MT1, MT2> dmattdmatschur(const DenseMatrix<MT1, false> &lhs,
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  EnableIf_t<IsSymmetric_v<MT1> && !IsSymmetric_v<MT2> && !(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) &&
					 ...
					 !(IsStrictlyUpper_v<MT1> && IsLower_v<MT2>) && !(IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) &&
					 !(IsUpper_v<MT1> && IsStrictlyLower_v<MT2>)> * = nullptr>
inline decltype(auto) dmattdmatschur(const DenseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  typename MT2 // Type of the right-hand side dense matrix
		  ,
		  EnableIf_t<IsSymmetric_v<MT2> && !(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) &&
					 !(IsUniUpper_v<MT1> && IsUniLower_v<MT2>) && !(IsStrictlyLower_v<MT1> && IsUpper_v<MT2>) &&
					 !(IsStrictlyUpper_v<MT1> && IsLower_v<MT2>) && !(IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) &&
					 !(IsUpper_v<MT1> && IsStrictlyLower_v<MT2>)> * = nullptr>
inline decltype(auto) dmattdmatschur(const DenseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, true> &rhs)
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  EnableIf_t<!IsSymmetric_v<MT1> && !IsSymmetric_v<MT2> && !(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) &&
					 ...
					 !(IsStrictlyUpper_v<MT1> && IsLower_v<MT2>) && !(IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) &&
					 !(IsUpper_v<MT1> && IsStrictlyLower_v<MT2>)> * = nullptr>
inline const DMatTDMatSchurExpr<MT1, MT2> tdmatdmatschur(const DenseMatrix<MT1, true> &lhs,
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  EnableIf_t<!IsSymmetric_v<MT1> && IsSymmetric_v<MT2> && !(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) &&
					 ...
					 !(IsStrictlyUpper_v<MT1> && IsLower_v<MT2>) && !(IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) &&
					 !(IsUpper_v<MT1> && IsStrictlyLower_v<MT2>)> * = nullptr>
inline decltype(auto) tdmatdmatschur(const DenseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  typename MT2 // Type of the right-hand side dense matrix
		  ,
		  EnableIf_t<IsSymmetric_v<MT1> && !(IsUniLower_v<MT1> && IsUniUpper_v<MT2>) &&
					 !(IsUniUpper_v<MT1> && IsUniLower_v<MT2>) && !(IsStrictlyLower_v<MT1> && IsUpper_v<MT2>) &&
					 !(IsStrictlyUpper_v<MT1> && IsLower_v<MT2>) && !(IsLower_v<MT1> && IsStrictlyUpper_v<MT2>) &&
					 !(IsUpper_v<MT1> && IsStrictlyLower_v<MT2>)> * = nullptr>
inline decltype(auto) tdmatdmatschur(const DenseMatrix<MT1, true> &lhs, const DenseMatrix<MT2, false> &rhs)
...
// This operator represents the Schur product of a column-major and a row-major dense matrix:
   ...
   mtrc::numeric::DynamicMatrix<double,rowMajor> B, C;
   // ... Resizing and initialization
   C = A % B;
   \endcode
...
struct IsAligned<DMatTDMatSchurExpr<MT1, MT2>> : public BoolConstant<IsAligned_v<MT1> && IsAligned_v<MT2>> {};
/*! \endcond */
//*************************************************************************************************
