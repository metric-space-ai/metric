...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATTSMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATTSMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class TSMatTSMatMultExpr : public MatMatMultExpr<SparseMatrix<TSMatTSMatMultExpr<MT1, MT2>, true>>,
						   private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	*/
	inline bool canSMPAssign() const noexcept { return (rows() * columns() >= SMP_TSMATTSMATMULT_THRESHOLD); }
	//**********************************************************************************************
		...
		for (size_t j = 0UL; j < C.columns(); ++j) {
			...
			for (auto relem = B.begin(j); relem != rend; ++relem) {
				...
				for (auto lelem = A.begin(relem->index()); lelem != lend; ++lelem) {
					if (IsResizable_v<ElementType_t<MT3>> && isDefault(C(lelem->index(), j))) {
	...
	*/
	...
	friend inline void assign(SparseMatrix<MT, true> &lhs, const TSMatTSMatMultExpr &rhs)
	{
		...
		for (size_t j = 0UL; j < (*lhs).columns(); ++j) {
			...
			for (auto relem = B.begin(j); relem != rend; ++relem) {
				...
				for (auto lelem = A.begin(relem->index()); lelem != lend; ++lelem) {
					if (!valid[lelem->index()]) {
						...
						++nonzeros;
						if (lelem->index() < minIndex)
							minIndex = lelem->index();
						if (lelem->index() > maxIndex)
							maxIndex = lelem->index();
					} else {
						values[lelem->index()] += lelem->value() * relem->value();
					}
				}
			...
			METRIC_NUMERIC_INTERNAL_ASSERT(nonzeros <= (*lhs).rows(), "Invalid number of non-zero elements");
...
inline const TSMatTSMatMultExpr<MT1, MT2> tsmattsmatmult(const SparseMatrix<MT1, true> &lhs,
...
inline const MT1 &tsmattsmatmult(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <
	...
	,
	EnableIf_t<IsIdentity_v<MT1> && !IsIdentity_v<MT2> && IsSame_v<ElementType_t<MT1>, ElementType_t<MT2>>> * = nullptr>
inline const MT2 &tsmattsmatmult(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) tsmattsmatmult(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side sparse matrix
inline decltype(auto) operator*(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
