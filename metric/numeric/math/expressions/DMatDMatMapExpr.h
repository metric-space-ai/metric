...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATDMATMAPEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATDMATMAPEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DMatDMatMapExpr : public MatMatMapExpr<DenseMatrix<DMatDMatMapExpr<MT1, MT2, OP, SO>, SO>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	template <typename MT> static constexpr bool UseAssign_v = useAssign;
	/*! \endcond */
	//**********************************************************************************************
	...
	static constexpr bool UseSMPAssign_v = ((!MT1::smpAssignable || !MT2::smpAssignable) && useAssign);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	class ConstIterator {
	  public:
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE bool operator==(const ConstIterator &rhs) const
		{
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE DifferenceType operator-(const ConstIterator &rhs) const
		{
			return left_ - rhs.left_;
		}
		//*******************************************************************************************
		...
		*/
		...
		// \return The decremented iterator.
		*/
		friend inline METRIC_NUMERIC_DEVICE_CALLABLE const ConstIterator operator-(const ConstIterator &it, size_t dec)
		{
	...
	static constexpr bool simdEnabled = (MT1::simdEnabled && MT2::simdEnabled &&
	...
	*/
	inline ReturnType operator()(size_t i, size_t j) const
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(j < lhs_.columns(), "Invalid column access index");
	...
	*/
	METRIC_NUMERIC_ALWAYS_INLINE auto load(size_t i, size_t j) const noexcept
	{
		METRIC_NUMERIC_INTERNAL_ASSERT(i < lhs_.rows(), "Invalid row access index");
		METRIC_NUMERIC_INTERNAL_ASSERT(j < lhs_.columns(), "Invalid column access index");
		METRIC_NUMERIC_INTERNAL_ASSERT(!SO || (i % SIMDSIZE == 0UL), "Invalid row access index");
		METRIC_NUMERIC_INTERNAL_ASSERT(SO || (j % SIMDSIZE == 0UL), "Invalid column access index");
	...
	*/
	inline ConstIterator begin(size_t i) const { return ConstIterator(lhs_.begin(i), rhs_.begin(i), op_); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator end(size_t i) const { return ConstIterator(lhs_.end(i), rhs_.end(i), op_); }
	//**********************************************************************************************
	...
	*/
	...
	{
		return (IsExpression_v<MT1> && lhs_.canAlias(alias)) || (IsExpression_v<MT2> && rhs_.canAlias(alias));
	}
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline auto assign(DenseMatrix<MT, SO2> &lhs, const DMatDMatMapExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(A.columns() == (*lhs).columns(), "Invalid number of columns");
		METRIC_NUMERIC_INTERNAL_ASSERT(B.rows() == (*lhs).rows(), "Invalid number of rows");
	...
	*/
	...
	friend inline auto assign(SparseMatrix<MT, SO2> &lhs, const DMatDMatMapExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
		...
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(OppositeType, !SO);
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO2> // Storage order of the target dense matrix
	friend inline auto addAssign(DenseMatrix<MT, SO2> &lhs, const DMatDMatMapExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO2> // Storage order of the target dense matrix
	friend inline auto subAssign(DenseMatrix<MT, SO2> &lhs, const DMatDMatMapExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename MT // Type of the target dense matrix
			  ,
			  bool SO2> // Storage order of the target dense matrix
	friend inline auto schurAssign(DenseMatrix<MT, SO2> &lhs, const DMatDMatMapExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	...
	METRIC_NUMERIC_CONSTRAINT_MATRICES_MUST_HAVE_SAME_STORAGE_ORDER(MT1, MT2);
	/*! \endcond */
	//**********************************************************************************************
};
//*************************************************************************************************
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  typename OP> // Type of the custom operation
inline decltype(auto) map(const DenseMatrix<MT1, SO> &lhs, const DenseMatrix<MT2, SO> &rhs, OP op)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the first dense matrix
		  ...
		  ,
		  typename OP> // Type of the custom operation
inline decltype(auto) map(const DenseMatrix<MT1, SO> &dm1, const DenseMatrix<MT2, SO> &dm2,
...
inline decltype(auto) min(const DenseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side dense matrix
inline decltype(auto) max(const DenseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side dense matrix
inline decltype(auto) hypot(const DenseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side dense matrix
inline decltype(auto) pow(const DenseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side dense matrix
inline decltype(auto) atan2(const DenseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
// The following example demonstrates the use of the \a selec() function:
   ...
   \code
   mtrc::numeric::DynamicMatrix<bool> cond{ { true, false }, { true false } };
   mtrc::numeric::DynamicMatrix<int> A{ { 1, -1 }, { 1, -1 } };
   mtrc::numeric::DynamicMatrix<int> B{ { -2, 2 }, { -2, 2 } };
   mtrc::numeric::DynamicMatrix<int> C;
   // ... Resizing and initialization
...
template <typename MT1 // Type of the conditional dense matrix
		  ...
		  ,
		  bool SO> // Storage order
inline decltype(auto) select(const DenseMatrix<MT1, SO> &cond, const DenseMatrix<MT2, SO> &lhs,
...
// This operator represents the elementwise left-shift of a given dense matrix:
   ...
   \code
   mtrc::numeric::DynamicMatrix<unsigned int> A, B, C;
   // ... Resizing and initialization
   C = A << B;
   \endcode
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side dense matrix
inline decltype(auto) operator<<(const DenseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side dense matrix
inline decltype(auto) operator<<(const DMatDMatMapExpr<MT1, MT2, ShiftLV, SO1> &lhs, const DenseMatrix<MT3, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side dense matrix
inline decltype(auto) operator>>(const DMatDMatMapExpr<MT1, MT2, ShiftRV, SO1> &lhs, const DenseMatrix<MT3, SO2> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
// This operator represents the logical AND of the given two dense matrices:
   ...
   \code
   mtrc::numeric::DynamicMatrix<bool> A, B, C;
   // ... Resizing and initialization
   C = A && B;
   \endcode
...
struct IsAligned<DMatDMatMapExpr<MT1, MT2, OP, SO>> : public BoolConstant<IsAligned_v<MT1> && IsAligned_v<MT2>> {};
/*! \endcond */
//*************************************************************************************************
