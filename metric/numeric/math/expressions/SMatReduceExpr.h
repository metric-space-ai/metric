...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_SMATREDUCEEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_SMATREDUCEEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SMatReduceExpr<MT, OP, columnwise>
	: public MatReduceExpr<DenseVector<SMatReduceExpr<MT, OP, columnwise>, true>, columnwise>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	template <typename VT> static constexpr bool UseSMPAssign_v = (!MT::smpAssignable && RequiresEvaluation_v<MT>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	*/
	...
	{
		METRIC_NUMERIC_INTERNAL_ASSERT(index < sm_.columns(), "Invalid vector access index");
	...
	*/
	inline size_t size() const noexcept { return sm_.columns(); }
	//**********************************************************************************************
	...
	*/
	template <typename T> inline bool canAlias(const T *alias) const noexcept { return (sm_.isAliased(alias)); }
	//**********************************************************************************************
	...
	*/
	template <typename T> inline bool isAliased(const T *alias) const noexcept { return (sm_.isAliased(alias)); }
	//**********************************************************************************************
	...
	*/
	inline bool canSMPAssign() const noexcept { return sm_.canSMPAssign() || (size() > SMP_SMATREDUCE_THRESHOLD); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline void assign(DenseVector<VT1, true> &lhs, const SMatReduceExpr &rhs)
	{
		...
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_SPARSE_MATRIX_TYPE(OT);
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(OT);
		METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(OT);
	...
	*/
	...
	friend inline auto smpAssign(Vector<VT1, true> &lhs, const SMatReduceExpr &rhs) -> EnableIf_t<UseSMPAssign_v<VT1>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
...
class SMatReduceExpr<MT, OP, rowwise>
	: public MatReduceExpr<DenseVector<SMatReduceExpr<MT, OP, rowwise>, false>, rowwise>, private Computation {
  private:
	//**Type definitions****************************************************************************
	using RT = ResultType_t<MT>; //!< Result type of the sparse matrix expression.
	//**********************************************************************************************
	...
	/*! This variable template is a helper for the selection of the parallel evaluation strategy.
		...
		evaluation, the variable is set to 1 and the expression specific evaluation strategy is
		selected. Otherwise the variable is set to 0 and the default strategy is chosen. */
	template <typename VT> static constexpr bool UseSMPAssign_v = (!MT::smpAssignable && useAssign);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	class ConstIterator {
	  public:
		...
		*/
		inline const ConstIterator operator++(int) { return ConstIterator(index_++); }
		//*******************************************************************************************
		...
		*/
		...
		// \return The resulting value.
		*/
		inline ReturnType operator*() const { return reduce(row(sm_, index_, unchecked), op_); }
		//*******************************************************************************************
		...
		*/
		inline bool operator==(const ConstIterator &rhs) const { return index_ == rhs.index_; }
		//*******************************************************************************************
		...
		*/
		inline DifferenceType operator-(const ConstIterator &rhs) const { return index_ - rhs.index_; }
		//*******************************************************************************************
	...
	*/
	inline ReturnType operator[](size_t index) const
	{
		METRIC_NUMERIC_INTERNAL_ASSERT(index < sm_.rows(), "Invalid vector access index");
	...
	*/
	inline ConstIterator begin() const { return ConstIterator(sm_, 0UL, op_); }
	//**********************************************************************************************
	...
	*/
	inline ConstIterator end() const { return ConstIterator(sm_, size(), op_); }
	//**********************************************************************************************
	...
	// \return The size of the vector.
	*/
	inline size_t size() const noexcept { return sm_.rows(); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	template <typename VT1> // Type of the target vector
	friend inline auto assign(Vector<VT1, false> &lhs, const SMatReduceExpr &rhs) -> EnableIf_t<UseAssign_v<VT1>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename VT1> // Type of the target vector
	friend inline auto addAssign(Vector<VT1, false> &lhs, const SMatReduceExpr &rhs) -> EnableIf_t<UseAssign_v<VT1>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename VT1> // Type of the target vector
	friend inline auto subAssign(Vector<VT1, false> &lhs, const SMatReduceExpr &rhs) -> EnableIf_t<UseAssign_v<VT1>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename VT1> // Type of the target vector
	friend inline auto multAssign(Vector<VT1, false> &lhs, const SMatReduceExpr &rhs) -> EnableIf_t<UseAssign_v<VT1>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename VT1> // Type of the target vector
	friend inline auto divAssign(Vector<VT1, false> &lhs, const SMatReduceExpr &rhs) -> EnableIf_t<UseAssign_v<VT1>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
	...
	*/
	template <typename VT1> // Type of the target vector
	friend inline auto smpAssign(Vector<VT1, false> &lhs, const SMatReduceExpr &rhs) -> EnableIf_t<UseSMPAssign_v<VT1>>
	{
		METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) reduce(const SparseMatrix<MT, SO> &sm, OP op)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(tmp.rows() == M, "Invalid number of rows");
	METRIC_NUMERIC_INTERNAL_ASSERT(tmp.columns() == N, "Invalid number of columns");
...
// given reduction operation \a op. In case the reduction flag \a RF is set to \a mtrc::numeric::columnwise,
// the elements of the matrix are reduced column-wise and the result is a row vector. In case
// \a RF is set to \a mtrc::numeric::rowwise, the elements of the matrix are reduced row-wise and the
// result is a column vector:

   \code
   using mtrc::numeric::columnwise;

   mtrc::numeric::CompressedMatrix<double> A;
   mtrc::numeric::DynamicVector<double,rowVector> colsum1, colsum2;
   // ... Resizing and initialization

   colsum1 = reduce<columnwise>( A, mtrc::numeric::Add() );
   colsum2 = reduce<columnwise>( A, []( double a, double b ){ return a + b; } );
   \endcode

   \code
   using mtrc::numeric::rowwise;

   mtrc::numeric::CompressedMatrix<double> A;
   mtrc::numeric::DynamicVector<double,columnVector> rowsum1, rowsum2;
   // ... Resizing and initialization

   rowsum1 = reduce<rowwise>( A, mtrc::numeric::Add() );
...
// addition:
   ...
   \code
   mtrc::numeric::CompressedMatrix<int> A{ { 1, 2 }, { 3, 4 } };
...
// \a sm by means of addition. In case the reduction flag \a RF is set to \a mtrc::numeric::columnwise,
// the elements of the matrix are reduced column-wise and the result is a row vector. In case
// \a RF is set to \a mtrc::numeric::rowwise, the elements of the matrix are reduced row-wise and the
// result is a column vector:
   ...
   mtrc::numeric::CompressedMatrix<int> A{ { 1, 0, 2 }, { 1, 3, 4 } };
   mtrc::numeric::DynamicVector<int,rowVector> colsum;
   ...
   mtrc::numeric::CompressedMatrix<int> A{ { 1, 0, 2 }, { 1, 3, 4 } };
   mtrc::numeric::DynamicVector<int,columnVector> rowsum;
...
// \a mtrc::numeric::columnwise, the elements of the matrix are reduced column-wise and the result
// is a row vector. In case \a RF is set to \a mtrc::numeric::rowwise, the elements of the matrix
...
// However, the minimum of this matrix is 1:
   ...
   \code
   mtrc::numeric::CompressedMatrix<int> A{ { 1, 0 }, { 3, 0 } };
...
// matrix \a sm. In case the reduction flag \a RF is set to \a mtrc::numeric::columnwise, a row
// vector containing the smallest element of each column is returned. In case \a RF is set to
// \a mtrc::numeric::rowwise, a column vector containing the smallest element of each row is returned.
//
// \note In case the sparse matrix is not completely filled, the implicit zero elements are NOT
// taken into account:
   ...
   mtrc::numeric::CompressedMatrix<int> A{ { 1, 0, 2 }, { 1, 3, 4 } };
   mtrc::numeric::DynamicVector<int,rowVector> colmin;
   ...
   mtrc::numeric::CompressedMatrix<int> A{ { 1, 0, 2 }, { 1, 3, 4 } };
   mtrc::numeric::DynamicVector<int,columnVector> rowmin;
...
// In case the reduction flag \a RF is set to \a mtrc::numeric::columnwise, a row vector containing the
// largest element of each column is returned. In case \a RF is set to \a mtrc::numeric::rowwise, a column
...
// taken into account:
   ...
   mtrc::numeric::CompressedMatrix<int> A{ { -1, 0, -2 }, { -1, -3, -4 } };
   mtrc::numeric::DynamicVector<int,rowVector> colmax;
   ...
   mtrc::numeric::CompressedMatrix<int> A{ { -1, 0, -2 }, { -1, -3, -4 } };
   mtrc::numeric::DynamicVector<int,columnVector> rowmax;
