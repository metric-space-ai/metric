...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATSMATKRONEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATSMATKRONEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class TSMatSMatKronExpr : public MatMatKronExpr<SparseMatrix<TSMatSMatKronExpr<MT1, MT2>, false>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	friend inline void assign(SparseMatrix<MT, false> &lhs, const TSMatSMatKronExpr &rhs)
	{
		...
		for (size_t j = 0UL; j < A.columns(); ++j)
			for (auto aelem = A.begin(j); aelem != A.end(j); ++aelem)
				for (size_t k = 0UL; k < M; ++k)
					for (auto belem = B.begin(k); belem != B.end(k); ++belem)
						(*lhs).append(aelem->index() * M + k, j * N + belem->index(), aelem->value() * belem->value(),
									  true);
	}
	/*! \endcond */
	//**********************************************************************************************
...
inline const TSMatSMatKronExpr<MT1, MT2> tsmatsmatkron(const SparseMatrix<MT1, true> &lhs,
...
inline decltype(auto) tsmatsmatkron(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT1 // Type of the left-hand side sparse matrix
		  ,
		  typename MT2> // Type of the right-hand side sparse matrix
inline decltype(auto) kron(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
