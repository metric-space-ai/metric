...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TDVECDMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TDVECDMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class TDVecDMatMultExpr : public TVecMatMultExpr<DenseVector<TDVecDMatMultExpr<VT, MT>, true>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool UseBlasKernel_v =
		(METRIC_NUMERIC_BLAS_MODE && METRIC_NUMERIC_USE_BLAS_MATRIX_VECTOR_MULTIPLICATION && IsContiguous_v<T1> &&
		 ...
		 HasConstDataAccess_v<T3> && !IsDiagonal_v<T3> && T1::simdEnabled && T2::simdEnabled && T3::simdEnabled &&
  ...
  public:
	...
	static constexpr bool simdEnabled =
		(!IsDiagonal_v<MT> && VT::simdEnabled && MT::simdEnabled && HasSIMDAdd_v<VET, MET> && HasSIMDMult_v<VET, MET>);
	...
	*/
	inline bool isAligned() const noexcept { return vec_.isAligned() && mat_.isAligned(); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline void assign(DenseVector<VT1, true> &lhs, const TDVecDMatMultExpr &rhs)
	{
		...
		if (rhs.mat_.rows() == 0UL || (IsStrictlyTriangular_v<MT> && rhs.mat_.rows() == 1UL)) {
			reset(*lhs);
			return;
	...
	static inline void selectDefaultAssignKernel(VT1 &y, const VT2 &x, const MT1 &A)
	{
		...
		for (size_t i = (IsLower_v<MT1> && !IsStrictlyLower_v<MT1> ? 0UL : 1UL); i < M; ++i) {
			if (IsDiagonal_v<MT1>) {
				...
				const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i - 1UL : i) : (N));
	...
	*/
		...
		-> EnableIf_t<UseVectorizedDefaultKernel_v<VT1, VT2, MT1>>
	{
		...
		for (; (j + SIMDSIZE * 7UL) < jpos; j += SIMDSIZE * 8UL) {
			...
			const size_t iend((IsUpper_v<MT1>) ? (min(j + SIMDSIZE * 8UL, M) - (IsStrictlyUpper_v<MT1> ? 1UL : 0UL))
											   : (M));
		...
		for (; (j + SIMDSIZE * 3UL) < jpos; j += SIMDSIZE * 4UL) {
			const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? j + 1UL : j) : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (min(j + SIMDSIZE * 4UL, M) - (IsStrictlyUpper_v<MT1> ? 1UL : 0UL))
											   : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin < iend, "Invalid loop indices detected");
		...
		for (; (j + SIMDSIZE * 2UL) < jpos; j += SIMDSIZE * 3UL) {
			const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? j + 1UL : j) : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (min(j + SIMDSIZE * 3UL, M) - (IsStrictlyUpper_v<MT1> ? 1UL : 0UL))
											   : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin < iend, "Invalid loop indices detected");
		...
		for (; (j + SIMDSIZE) < jpos; j += SIMDSIZE * 2UL) {
			const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? j + 1UL : j) : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (min(j + SIMDSIZE * 2UL, M) - (IsStrictlyUpper_v<MT1> ? 1UL : 0UL))
											   : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin < iend, "Invalid loop indices detected");
		...
		for (; j < jpos; j += SIMDSIZE) {
			const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? j + 1UL : j) : (0UL));
			const size_t iend((IsUpper_v<MT1>) ? (min(j + SIMDSIZE, M) - (IsStrictlyUpper_v<MT1> ? 1UL : 0UL)) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin < iend, "Invalid loop indices detected");
	...
	*/
		...
		-> EnableIf_t<UseVectorizedDefaultKernel_v<VT1, VT2, MT1>>
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT((jblock % SIMDSIZE) == 0UL, "Invalid block size detected");
		...
		for (size_t jj = 0U; jj < N; jj += jblock) {
			for (size_t ii = 0UL; ii < M; ii += iblock) {
				...
				const size_t jend((IsLower_v<MT1>) ? (min(jtmp, (IsStrictlyLower_v<MT1> ? iend - 1UL : iend)))
												   : (jtmp));
				...
				size_t j((IsUpper_v<MT1>) ? (max(jj, prevMultiple((IsStrictlyUpper_v<MT1> ? ii + 1UL : ii), SIMDSIZE)))
										  : (jj));
	...
	static inline void selectDefaultAddAssignKernel(VT1 &y, const VT2 &x, const MT1 &A)
	{
		...
		for (size_t i = 0UL; i < M; ++i) {
			if (IsDiagonal_v<MT1>) {
				...
				const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i + 1UL : i) : (0UL));
				const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i : i + 1UL) : (N));
				METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
...
class DVecScalarMultExpr<TDVecDMatMultExpr<VT, MT>, ST, true>
  ...
  public:
	...
	//! Compilation switch for the expression template evaluation strategy.
	static constexpr bool simdEnabled =
		(!IsDiagonal_v<MT> && VT::simdEnabled && MT::simdEnabled && IsSIMDCombinable_v<VET, MET, ST> &&
		 HasSIMDAdd_v<VET, MET> && HasSIMDMult_v<VET, MET>);
  ...
  private:
	...
	*/
	...
	friend inline void assign(DenseVector<VT1, true> &lhs, const DVecScalarMultExpr &rhs)
	{
		...
		if (right.rows() == 0UL || (IsStrictlyTriangular_v<MT> && right.rows() == 1UL)) {
			reset(*lhs);
			return;
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(x.size() == left.size(), "Invalid vector size");
		METRIC_NUMERIC_INTERNAL_ASSERT(A.rows() == right.rows(), "Invalid number of rows");
		METRIC_NUMERIC_INTERNAL_ASSERT(A.columns() == right.columns(), "Invalid number of columns");
		METRIC_NUMERIC_INTERNAL_ASSERT(A.columns() == (*lhs).size(), "Invalid vector size");
	...
	*/
	...
	friend inline void addAssign(DenseVector<VT1, true> &lhs, const DVecScalarMultExpr &rhs)
	{
		...
		if (right.rows() == 0UL || right.columns() == 0UL || (IsStrictlyTriangular_v<MT> && right.rows() == 1UL)) {
			return;
		}
	...
	*/
	...
	//**Compile time checks*************************************************************************
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_VECTOR_TYPE(VMM);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_VECTOR_TYPE(VMM);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_VECTOR_TYPE(VT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_VECTOR_TYPE(VT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_MAJOR_MATRIX_TYPE(MT);
...
template <typename VT // Type of the left-hand side dense vector
		  ,
		  typename MT> // Type of the right-hand side dense matrix
inline decltype(auto) operator*(const DenseVector<VT, true> &vec, const DenseMatrix<MT, false> &mat)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct IsAligned<TDVecDMatMultExpr<VT, MT>> : public BoolConstant<IsAligned_v<VT> && IsAligned_v<MT>> {};
/*! \endcond */
//*************************************************************************************************
