...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DVECEXPANDEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DVECEXPANDEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/expressions/ExpandExprData.h>
...
class DVecExpandExpr : public VecExpandExpr<DenseMatrix<DVecExpandExpr<VT, TF, CEAs...>, !TF>, CEAs...>,
  ...
  private:
	...
	template <typename MT> static constexpr bool UseSMPAssign_v = (!VT::smpAssignable && useAssign);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	*/
	...
	{
		if (TF) {
			METRIC_NUMERIC_INTERNAL_ASSERT(i < expansion(), "Invalid row access index");
			METRIC_NUMERIC_INTERNAL_ASSERT(j < dv_.size(), "Invalid column access index");
			return dv_[j];
		} else {
			METRIC_NUMERIC_INTERNAL_ASSERT(i < dv_.size(), "Invalid row access index");
			METRIC_NUMERIC_INTERNAL_ASSERT(j < expansion(), "Invalid column access index");
			return dv_[i];
		}
	}
	//**********************************************************************************************
	...
	*/
	METRIC_NUMERIC_ALWAYS_INLINE auto load(size_t i, size_t j) const noexcept
	{
		if (TF) {
			METRIC_NUMERIC_INTERNAL_ASSERT(i < expansion(), "Invalid row access index");
			METRIC_NUMERIC_INTERNAL_ASSERT(j < dv_.size(), "Invalid column access index");
			METRIC_NUMERIC_INTERNAL_ASSERT(j % SIMDSIZE == 0UL, "Invalid row access index");
			return dv_.load(j);
		} else {
			METRIC_NUMERIC_INTERNAL_ASSERT(i < dv_.size(), "Invalid row access index");
			METRIC_NUMERIC_INTERNAL_ASSERT(j < expansion(), "Invalid column access index");
	...
	*/
	inline size_t rows() const noexcept { return (TF ? expansion() : dv_.size()); }
	//**********************************************************************************************
	...
	*/
	inline size_t columns() const noexcept { return (TF ? dv_.size() : expansion()); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline auto assign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto addAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto subAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto schurAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto multAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto smpAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseSMPAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto smpAddAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseSMPAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto smpSubAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseSMPAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto smpSchurAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseSMPAssign_v<MT>>
	{
		using mtrc::numeric::expand;
	...
	*/
	template <typename MT // Type of the target matrix
			  ,
			  bool SO> // Storage order of the target matrix
	friend inline auto smpMultAssign(Matrix<MT, SO> &lhs, const DVecExpandExpr &rhs) -> EnableIf_t<UseSMPAssign_v<MT>>
	{
		using mtrc::numeric::expand;
...
// This function returns an expression representing the expansion of the given dense vector:
   ...
   mtrc::numeric::DynamicVector<int,columnVector> a{ 1, 5, -2, 4 };
   mtrc::numeric::DynamicVector<int,rowVector> b{ 3, -1, 7, 0 };
...
struct IsAligned<DVecExpandExpr<VT, TF, CEAs...>> : public IsAligned<VT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename VT, bool TF, size_t... CEAs>
struct IsPadded<DVecExpandExpr<VT, TF, CEAs...>> : public IsPadded<VT> {};
/*! \endcond */
//*************************************************************************************************
