...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATSMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATSMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool UF> // Upper flag
class TDMatSMatMultExpr : public MatMatMultExpr<DenseMatrix<TDMatSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, true>>,
						  private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	*/
	template <typename T> inline bool isAliased(const T *alias) const
	{
		return (lhs_.isAliased(alias) || rhs_.isAliased(alias));
	}
	//**********************************************************************************************
		...
		for (size_t j = 0UL; j < B.rows(); ++j) {
			...
			if (IsDiagonal_v<MT4>) {
				...
				for (; element != end; ++element) {
					...
					const size_t ibegin((IsLower_v<MT4>) ? ((IsStrictlyLower_v<MT4>)
																? (SYM || HERM || LOW ? max(j1, j + 1UL) : j + 1UL)
																: (SYM || HERM || LOW ? max(j1, j) : j))
														 : (SYM || HERM || LOW ? j1 : 0UL));
					const size_t iend((IsUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? min(j1 + 1UL, j) : j)
		...
		for (size_t j = 0UL; j < B.rows(); ++j) {
			...
			for (size_t k = 0UL; k < kpos; k += 4UL) {
				...
				const size_t ibegin((IsLower_v<MT4>)
										? ((IsStrictlyLower_v<MT4>) ? (SYM || HERM || LOW ? max(j1, j + 1UL) : j + 1UL)
																	: (SYM || HERM || LOW ? max(j1, j) : j))
										: (SYM || HERM || LOW ? j1 : 0UL));
				const size_t iend((IsUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? min(j4 + 1UL, j) : j)
		...
		for (size_t j = 0UL; j < B.rows(); ++j) {
			...
			if (IsDiagonal_v<MT4>) {
				...
				for (; element != end; ++element) {
					...
					const size_t ibegin(
						(IsLower_v<MT4>)
							? ((IsStrictlyLower_v<MT4>) ? (LOW ? max(j1, j + 1UL) : j + 1UL) : (LOW ? max(j1, j) : j))
							: (LOW ? j1 : 0UL));
					const size_t iend((IsUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? min(j1 + 1UL, j) : j)
																				   : (UPP ? min(j1, j) + 1UL : j + 1UL))
													   : (UPP ? j1 + 1UL : A.rows()));
		...
		-> EnableIf_t<UseOptimizedKernel_v<MT3, MT4, MT5>>
	{
		for (size_t j = 0UL; j < B.rows(); ++j) {
			...
			for (size_t k = 0UL; k < kpos; k += 4UL) {
				...
				const size_t ibegin((IsLower_v<MT4>) ? ((IsStrictlyLower_v<MT4>) ? (LOW ? max(j1, j + 1UL) : j + 1UL)
																				 : (LOW ? max(j1, j) : j))
													 : (LOW ? j1 : 0UL));
				const size_t iend((IsUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT4>) ? (UPP ? min(j4 + 1UL, j) : j)
																			   : (UPP ? min(j4, j) + 1UL : j + 1UL))
...
inline const TDMatSMatMultExpr<MT1, MT2, false, false, false, false> tdmatsmatmult(const DenseMatrix<MT1, true> &lhs,
...
inline const MT1 &tdmatsmatmult(const DenseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) tdmatsmatmult(const DenseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<TDMatSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 0UL> : public Size<MT1, 0UL> {};

template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct Size<TDMatSMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 1UL> : public Size<MT2, 1UL> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct IsAligned<TDMatSMatMultExpr<MT1, MT2, SF, HF, LF, UF>> : public IsAligned<MT1> {};
/*! \endcond */
//*************************************************************************************************
