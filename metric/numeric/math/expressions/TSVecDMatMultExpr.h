...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TSVECDMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TSVECDMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class TSVecDMatMultExpr : public TVecMatMultExpr<DenseVector<TSVecDMatMultExpr<VT, MT>, true>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool UseVectorizedKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T3> && T1::simdEnabled && T3::simdEnabled &&
	...
	static constexpr bool UseOptimizedKernel_v =
		(useOptimizedKernels && !UseVectorizedKernel_v<T1, T2, T3> && !IsDiagonal_v<T3> &&
		 !IsResizable_v<ElementType_t<T1>> && !IsResizable_v<VET>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	static constexpr bool simdEnabled =
		(!IsDiagonal_v<MT> && MT::simdEnabled && HasSIMDAdd_v<VET, MET> && HasSIMDMult_v<VET, MET>);
	...
	*/
	inline bool canSMPAssign() const noexcept { return (size() > SMP_TSVECDMATMULT_THRESHOLD); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	{
		METRIC_NUMERIC_INTERNAL_ASSERT(x.nonZeros() != 0UL, "Invalid number of non-zero elements");
		...
		for (; element != end; ++element) {
			...
			if (IsDiagonal_v<MT1>) {
				...
				const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? index + 1UL : index) : (0UL));
				const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? index : index + 1UL) : (N));
	...
	*/
		...
		-> EnableIf_t<UseOptimizedKernel_v<VT1, VT2, MT1>>
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(ipos <= x.nonZeros(), "Invalid end calculation");
		...
		if (ipos > 3UL) {
			...
			METRIC_NUMERIC_INTERNAL_ASSERT(i1 < i2 && i2 < i3 && i3 < i4, "Invalid sparse vector index detected");
		...
		for (size_t i = (ipos > 3UL) ? (4UL) : (1UL); (i + 4UL) <= ipos; i += 4UL) {
			...
			const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i1 + 1UL : i1) : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i4 : i4 + 1UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
		...
		for (; element != end; ++element) {
			...
			const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i1 + 1UL : i1) : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i1 : i1 + 1UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
	...
	*/
		...
		-> EnableIf_t<UseVectorizedKernel_v<VT1, VT2, MT1>>
	{
		...
		for (size_t i = (ipos > 3UL) ? (4UL) : (1UL); (i + 4UL) <= ipos; i += 4UL) {
			...
			const size_t jbegin((IsUpper_v<MT1>) ? (prevMultiple((IsStrictlyUpper_v<MT1> ? i1 + 1UL : i1), SIMDSIZE))
												 : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i4 : i4 + 1UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
			...
			for (; j < jpos; j += SIMDSIZE) {
				y.store(j, y.load(j) + xmm1 * A.load(i1, j) + xmm2 * A.load(i2, j) + xmm3 * A.load(i3, j) +
							   xmm4 * A.load(i4, j));
			}
	...
	*/
		...
		-> EnableIf_t<UseOptimizedKernel_v<VT1, VT2, MT1>>
	{
		...
		for (size_t i = 0UL; (i + 4UL) <= ipos; i += 4UL) {
			...
			const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i + 1UL : i1) : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i4 : i4 + 1UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
...
inline const TSVecDMatMultExpr<VT, MT> tsvecdmatmult(const SparseVector<VT, true> &vec,
...
inline decltype(auto) tsvecdmatmult(const SparseVector<VT, true> &vec, const DenseMatrix<MT, false> &mat)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename VT // Type of the left-hand side sparse vector
		  ,
		  typename MT> // Type of the right-hand side dense matrix
inline decltype(auto) operator*(const SparseVector<VT, true> &vec, const DenseMatrix<MT, false> &mat)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename VT, typename MT> struct IsAligned<TSVecDMatMultExpr<VT, MT>> : public IsAligned<MT> {};
/*! \endcond */
//*************************************************************************************************
