...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATSMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TSMATSMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class TSMatSMatMultExpr : public MatMatMultExpr<SparseMatrix<TSMatSMatMultExpr<MT1, MT2>, !IsIdentity_v<MT1>>>,
						  private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool CanExploitSymmetry_v =
		((IsRowMajorMatrix_v<T1> && IsSymmetric_v<T2>) || (IsColumnMajorMatrix_v<T1> && IsSymmetric_v<T3>));
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	*/
	inline bool canSMPAssign() const noexcept { return (rows() * columns() >= SMP_TSMATSMATMULT_THRESHOLD); }
	//**********************************************************************************************
  ...
  private:
		...
		for (size_t j = 0UL; j < A.columns(); ++j) {
			...
			for (auto lelem = A.begin(j); lelem != lend; ++lelem) {
				...
				for (auto relem = B.begin(j); relem != rend; ++relem) {
					if (IsResizable_v<ElementType_t<MT3>> && isDefault(C(lelem->index(), relem->index()))) {
	...
	*/
		...
		-> DisableIf_t<CanExploitSymmetry_v<MT, MT1, MT2>>
	{
		...
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_MAJOR_MATRIX_TYPE(OppositeType_t<MT1>);
	...
	*/
		...
		-> DisableIf_t<CanExploitSymmetry_v<MT, MT1, MT2>>
	{
		...
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(OppositeType_t<MT2>);
...
inline const TSMatSMatMultExpr<MT1, MT2> tsmatsmatmult(const SparseMatrix<MT1, true> &lhs,
...
inline const MT1 &tsmatsmatmult(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <
	...
	,
	EnableIf_t<IsIdentity_v<MT1> && !IsIdentity_v<MT2> && IsSame_v<ElementType_t<MT1>, ElementType_t<MT2>>> * = nullptr>
inline const MT2 &tsmatsmatmult(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) tsmatsmatmult(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) tsmatsmatmult(const SparseMatrix<MT1, true> &lhs, const SparseMatrix<MT2, false> &rhs)
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(ReturnType, IsZero_v<MT1>);
