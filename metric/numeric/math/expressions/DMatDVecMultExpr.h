...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATDVECMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATDVECMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DMatDVecMultExpr : public MatVecMultExpr<DenseVector<DMatDVecMultExpr<MT, VT>, false>>, private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  private:
	...
	*/
		...
		-> EnableIf_t<UseVectorizedDefaultKernel_v<VT1, MT1, VT2>>
	{
		...
		for (; (i + 8UL) <= M; i += 8UL) {
			const size_t jbegin((IsUpper_v<MT1>) ? (prevMultiple((IsStrictlyUpper_v<MT1> ? i + 1UL : i), SIMDSIZE))
												 : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i + 7UL : i + 8UL) : (N));
		...
		for (; (i + 4UL) <= M; i += 4UL) {
			const size_t jbegin((IsUpper_v<MT1>) ? (prevMultiple((IsStrictlyUpper_v<MT1> ? i + 1UL : i), SIMDSIZE))
												 : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i + 3UL : i + 4UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin < jend, "Invalid loop indices detected");
		...
		for (; (i + 3UL) <= M; i += 3UL) {
			const size_t jbegin((IsUpper_v<MT1>) ? (prevMultiple((IsStrictlyUpper_v<MT1> ? i + 1UL : i), SIMDSIZE))
												 : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i + 2UL : i + 3UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin < jend, "Invalid loop indices detected");
		...
		for (; (i + 2UL) <= M; i += 2UL) {
			const size_t jbegin((IsUpper_v<MT1>) ? (prevMultiple((IsStrictlyUpper_v<MT1> ? i + 1UL : i), SIMDSIZE))
												 : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? i + 1UL : i + 2UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin < jend, "Invalid loop indices detected");
		...
		-> EnableIf_t<UseVectorizedDefaultKernel_v<VT1, MT1, VT2>>
	{
		...
		for (; (i + 8UL) <= M; i += 8UL) {
			...
			for (; (j + SIMDSIZE * 3UL) < jpos; j += SIMDSIZE * 4UL) {
				...
				y[i + 1UL] += sum(A.load(i + 1UL, j) * x1 + A.load(i + 1UL, j1) * x2 + A.load(i + 1UL, j2) * x3 +
								  A.load(i + 1UL, j3) * x4);
				y[i + 2UL] += sum(A.load(i + 2UL, j) * x1 + A.load(i + 2UL, j1) * x2 + A.load(i + 2UL, j2) * x3 +
								  A.load(i + 2UL, j3) * x4);
				y[i + 3UL] += sum(A.load(i + 3UL, j) * x1 + A.load(i + 3UL, j1) * x2 + A.load(i + 3UL, j2) * x3 +
								  A.load(i + 3UL, j3) * x4);
				y[i + 4UL] += sum(A.load(i + 4UL, j) * x1 + A.load(i + 4UL, j1) * x2 + A.load(i + 4UL, j2) * x3 +
								  A.load(i + 4UL, j3) * x4);
				y[i + 5UL] += sum(A.load(i + 5UL, j) * x1 + A.load(i + 5UL, j1) * x2 + A.load(i + 5UL, j2) * x3 +
								  A.load(i + 5UL, j3) * x4);
				y[i + 6UL] += sum(A.load(i + 6UL, j) * x1 + A.load(i + 6UL, j1) * x2 + A.load(i + 6UL, j2) * x3 +
								  A.load(i + 6UL, j3) * x4);
				y[i + 7UL] += sum(A.load(i + 7UL, j) * x1 + A.load(i + 7UL, j1) * x2 + A.load(i + 7UL, j2) * x3 +
								  A.load(i + 7UL, j3) * x4);
			}
...
template <typename MT // Type of the left-hand side dense matrix
		  ,
		  typename VT> // Type of the right-hand side dense vector
inline decltype(auto) operator*(const DenseMatrix<MT, false> &mat, const DenseVector<VT, false> &vec)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct IsAligned<DMatDVecMultExpr<MT, VT>> : public BoolConstant<IsAligned_v<MT> && IsAligned_v<VT>> {};
/*! \endcond */
//*************************************************************************************************
