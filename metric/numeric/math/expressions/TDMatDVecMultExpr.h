...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATDVECMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_TDMATDVECMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/blas/gemv.h>
#include <metric/numeric/math/blas/trmv.h>
...
class TDMatDVecMultExpr : public MatVecMultExpr<DenseVector<TDMatDVecMultExpr<MT, VT>, false>>, private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool evaluateMatrix =
		((IsComputation_v<MT> && IsSame_v<MET, VET> && IsBLASCompatible_v<MET>) || RequiresEvaluation_v<MT>);
	//**********************************************************************************************
	...
	static constexpr bool UseBlasKernel_v =
		(METRIC_NUMERIC_BLAS_MODE && METRIC_NUMERIC_USE_BLAS_MATRIX_VECTOR_MULTIPLICATION && IsContiguous_v<T1> &&
		 HasMutableDataAccess_v<T1> && IsContiguous_v<T2> && HasConstDataAccess_v<T2> && IsContiguous_v<T3> &&
		 HasConstDataAccess_v<T3> && !IsDiagonal_v<T2> && T1::simdEnabled && T2::simdEnabled && T3::simdEnabled &&
		 IsBLASCompatible_v<ElementType_t<T1>> && IsBLASCompatible_v<ElementType_t<T2>> &&
		 IsBLASCompatible_v<ElementType_t<T3>> && IsSame_v<ElementType_t<T1>, ElementType_t<T2>> &&
	...
	static constexpr bool UseVectorizedDefaultKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T2> && T1::simdEnabled && T2::simdEnabled && T3::simdEnabled &&
		 IsSIMDCombinable_v<ElementType_t<T1>, ElementType_t<T2>, ElementType_t<T3>> &&
  ...
  public:
	...
	static constexpr bool simdEnabled =
		(!IsDiagonal_v<MT> && MT::simdEnabled && VT::simdEnabled && HasSIMDAdd_v<MET, VET> && HasSIMDMult_v<MET, VET>);
	...
	*/
	inline bool isAligned() const noexcept { return mat_.isAligned() && vec_.isAligned(); }
	//**********************************************************************************************
	...
	*/
	...
	{
		return (!METRIC_NUMERIC_BLAS_MODE || !METRIC_NUMERIC_USE_BLAS_MATRIX_VECTOR_MULTIPLICATION ||
				!METRIC_NUMERIC_BLAS_IS_PARALLEL || (IsComputation_v<MT> && !evaluateMatrix) ||
  ...
  private:
	...
	*/
	...
	friend inline void assign(DenseVector<VT1, false> &lhs, const TDMatDVecMultExpr &rhs)
	{
		...
		if (rhs.mat_.rows() == 0UL) {
			return;
		} else if (rhs.mat_.columns() == 0UL || (IsStrictlyTriangular_v<MT> && rhs.mat_.columns() == 1UL)) {
			reset(*lhs);
			return;
		}
	...
	*/
	...
	{
		if ((IsDiagonal_v<MT1>) || (IsComputation_v<MT> && !evaluateMatrix) ||
	...
	static inline void selectDefaultAssignKernel(VT1 &y, const MT1 &A, const VT2 &x)
	{
		...
		for (size_t j = (IsUpper_v<MT1> && !IsStrictlyUpper_v<MT1> ? 0UL : 1UL); j < N; ++j) {
			if (IsDiagonal_v<MT1>) {
				...
				const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? j + 1UL : j) : (0UL));
				const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j - 1UL : j) : (M));
	...
	*/
		...
		-> EnableIf_t<UseVectorizedDefaultKernel_v<VT1, MT1, VT2>>
	{
		...
		for (; (i + SIMDSIZE * 7UL) < ipos; i += SIMDSIZE * 8UL) {
			const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i + 1UL : i) : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (min(i + SIMDSIZE * 8UL, N) - (IsStrictlyLower_v<MT1> ? 1UL : 0UL))
											   : (N));
		...
		for (; (i + SIMDSIZE * 3UL) < ipos; i += SIMDSIZE * 4UL) {
			const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i + 1UL : i) : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (min(i + SIMDSIZE * 4UL, N) - (IsStrictlyLower_v<MT1> ? 1UL : 0UL))
											   : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin < jend, "Invalid loop indices detected");
		...
		for (; (i + SIMDSIZE * 2UL) < ipos; i += SIMDSIZE * 3UL) {
			const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i + 1UL : i) : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (min(i + SIMDSIZE * 3UL, N) - (IsStrictlyLower_v<MT1> ? 1UL : 0UL))
											   : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin < jend, "Invalid loop indices detected");
		...
		for (; (i + SIMDSIZE) < ipos; i += SIMDSIZE * 2UL) {
			const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i + 1UL : i) : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (min(i + SIMDSIZE * 2UL, N) - (IsStrictlyLower_v<MT1> ? 1UL : 0UL))
											   : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin < jend, "Invalid loop indices detected");
		...
		for (; i < ipos; i += SIMDSIZE) {
			const size_t jbegin((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? i + 1UL : i) : (0UL));
			const size_t jend((IsLower_v<MT1>) ? (min(i + SIMDSIZE, N) - (IsStrictlyLower_v<MT1> ? 1UL : 0UL)) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin < jend, "Invalid loop indices detected");
	...
	*/
		...
		-> EnableIf_t<UseVectorizedDefaultKernel_v<VT1, MT1, VT2>>
	{
		...
		METRIC_NUMERIC_INTERNAL_ASSERT((iblock % SIMDSIZE) == 0UL, "Invalid block size detected");
		...
		for (size_t ii = 0U; ii < M; ii += iblock) {
			for (size_t jj = 0UL; jj < N; jj += jblock) {
				...
				const size_t iend((IsUpper_v<MT1>) ? (min(itmp, (IsStrictlyUpper_v<MT1> ? jend - 1UL : jend)))
												   : (itmp));
				...
				size_t i((IsLower_v<MT1>) ? (max(ii, prevMultiple((IsStrictlyLower_v<MT1> ? jj + 1UL : jj), SIMDSIZE)))
										  : (ii));
...
#if METRIC_NUMERIC_BLAS_MODE && METRIC_NUMERIC_USE_BLAS_MATRIX_VECTOR_MULTIPLICATION
	/*! \cond METRIC_NUMERIC_INTERNAL */
	...
	static inline void selectDefaultAddAssignKernel(VT1 &y, const MT1 &A, const VT2 &x)
	{
		...
		for (size_t j = 0UL; j < N; ++j) {
			if (IsDiagonal_v<MT1>) {
				...
				const size_t ibegin((IsLower_v<MT1>) ? (IsStrictlyLower_v<MT1> ? j + 1UL : j) : (0UL));
				const size_t iend((IsUpper_v<MT1>) ? (IsStrictlyUpper_v<MT1> ? j : j + 1UL) : (M));
				METRIC_NUMERIC_INTERNAL_ASSERT(ibegin <= iend, "Invalid loop indices detected");
...
class DVecScalarMultExpr<TDMatDVecMultExpr<MT, VT>, ST, false>
  ...
  private:
	//**Type definitions****************************************************************************
	using MVM =
		TDMatDVecMultExpr<MT, VT>;	 //!< Type of the transpose dense matrix-dense vector multiplication expression.
	...
	static constexpr bool UseBlasKernel_v =
		(METRIC_NUMERIC_BLAS_MODE && METRIC_NUMERIC_USE_BLAS_MATRIX_VECTOR_MULTIPLICATION && IsContiguous_v<T1> &&
		 ...
		 IsBLASCompatible_v<ElementType_t<T1>> && IsBLASCompatible_v<ElementType_t<T2>> &&
		 IsBLASCompatible_v<ElementType_t<T3>> && IsSame_v<ElementType_t<T1>, ElementType_t<T2>> &&
		 IsSame_v<ElementType_t<T1>, ElementType_t<T3>> && !(IsBuiltin_v<ElementType_t<T1>> && IsComplex_v<T4>));
	//**********************************************************************************************
  ...
  public:
	...
	//! Compilation switch for the expression template evaluation strategy.
	static constexpr bool simdEnabled =
		(!IsDiagonal_v<MT> && MT::simdEnabled && VT::simdEnabled && IsSIMDCombinable_v<MET, VET, ST> &&
		 HasSIMDAdd_v<MET, VET> && HasSIMDMult_v<MET, VET>);
	...
	*/
	inline ReturnType operator[](size_t index) const
	{
		METRIC_NUMERIC_INTERNAL_ASSERT(index < vector_.size(), "Invalid vector access index");
	...
	*/
	inline LeftOperand leftOperand() const { return vector_; }
	//**********************************************************************************************
	...
	*/
	...
	// \return \a true in case the expression can alias, \a false otherwise.
	*/
	template <typename T> inline bool canAlias(const T *alias) const { return vector_.canAlias(alias); }
	//**********************************************************************************************
	...
	// \return \a true in case an alias effect is detected, \a false otherwise.
	*/
	template <typename T> inline bool isAliased(const T *alias) const { return vector_.isAliased(alias); }
	//**********************************************************************************************
	...
	// \return \a true in case the operands are aligned, \a false if not.
	*/
	inline bool isAligned() const { return vector_.isAligned(); }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	friend inline void assign(DenseVector<VT1, false> &lhs, const DVecScalarMultExpr &rhs)
	{
		...
		if (left.rows() == 0UL) {
			return;
		} else if (left.columns() == 0UL || (IsStrictlyTriangular_v<MT> && left.columns() == 1UL)) {
			reset(*lhs);
			return;
		}
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(x.size() == right.size(), "Invalid vector size");
		METRIC_NUMERIC_INTERNAL_ASSERT(A.rows() == (*lhs).size(), "Invalid vector size");
	...
	*/
	...
	friend inline void addAssign(DenseVector<VT1, false> &lhs, const DVecScalarMultExpr &rhs)
	{
		...
		if (left.rows() == 0UL || left.columns() == 0UL || (IsStrictlyTriangular_v<MT> && left.rows() == 1UL)) {
			return;
		}
	...
	*/
	...
	//**Compile time checks*************************************************************************
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_VECTOR_TYPE(MVM);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_VECTOR_TYPE(MVM);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_VECTOR_TYPE(VT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_VECTOR_TYPE(VT);
...
template <typename MT // Type of the left-hand side dense matrix
		  ,
		  typename VT> // Type of the right-hand side dense vector
inline decltype(auto) operator*(const DenseMatrix<MT, true> &mat, const DenseVector<VT, false> &vec)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct IsAligned<TDMatDVecMultExpr<MT, VT>> : public BoolConstant<IsAligned_v<MT> && IsAligned_v<VT>> {};
/*! \endcond */
//*************************************************************************************************
