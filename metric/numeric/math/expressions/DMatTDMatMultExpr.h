...
#ifndef METRIC_NUMERIC_MATH_EXPRESSIONS_DMATTDMATMULTEXPR_H
#define METRIC_NUMERIC_MATH_EXPRESSIONS_DMATTDMATMULTEXPR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool UF> // Upper flag
class DMatTDMatMultExpr : public MatMatMultExpr<DenseMatrix<DMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, false>>,
						  private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	*/
	inline bool canSMPAssign() const noexcept
	{
		return (!METRIC_NUMERIC_BLAS_MODE || !METRIC_NUMERIC_USE_BLAS_MATRIX_MATRIX_MULTIPLICATION ||
				!METRIC_NUMERIC_BLAS_IS_PARALLEL || (rows() * columns() < DMATTDMATMULT_THRESHOLD)) &&
			   (rows() * columns() >= SMP_DMATTDMATMULT_THRESHOLD) && !IsDiagonal_v<MT1> && !IsDiagonal_v<MT2>;
	}
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	{
		if ((IsDiagonal_v<MT4> || IsDiagonal_v<MT5>) || (C.rows() * C.columns() < DMATTDMATMULT_THRESHOLD))
	...
	*/
		...
		-> EnableIf_t<IsRowMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		const size_t ibegin((IsStrictlyLower_v<MT4>) ? ((IsStrictlyLower_v<MT5> && M > 1UL) ? 2UL : 1UL) : (0UL));
		const size_t iend((IsStrictlyUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT5> && M > 1UL) ? M - 2UL : M - 1UL) : (M));
		...
		for (size_t i = ibegin; i < iend; ++i) {
			const size_t jbegin((IsUpper_v<MT4> && IsUpper_v<MT5>)
									? ((IsStrictlyUpper_v<MT4>) ? (IsStrictlyUpper_v<MT5> ? i + 2UL : i + 1UL)
																: (IsStrictlyUpper_v<MT5> ? i + 1UL : i))
									: ((IsStrictlyUpper_v<MT5>) ? (SYM || HERM || UPP ? max(i, 1UL) : 1UL)
																: (SYM || HERM || UPP ? i : 0UL)));
			const size_t jend(
				(IsLower_v<MT4> && IsLower_v<MT5>)
					? ((IsStrictlyLower_v<MT4>) ? (IsStrictlyLower_v<MT5> ? i - 1UL : i)
												: (IsStrictlyLower_v<MT5> ? i : i + 1UL))
					: ((IsStrictlyLower_v<MT5>) ? (LOW ? min(i + 1UL, N - 1UL) : N - 1UL) : (LOW ? i + 1UL : N)));
			...
			for (size_t j = jbegin; j < jend; ++j) {
				const size_t kbegin((IsUpper_v<MT4>)
										? ((IsLower_v<MT5>) ? (max((IsStrictlyUpper_v<MT4> ? i + 1UL : i),
																   (IsStrictlyLower_v<MT5> ? j + 1UL : j)))
															: (IsStrictlyUpper_v<MT4> ? i + 1UL : i))
										: ((IsLower_v<MT5>) ? (IsStrictlyLower_v<MT5> ? j + 1UL : j) : (0UL)));
				const size_t kend((IsLower_v<MT4>) ? ((IsUpper_v<MT5>) ? (min((IsStrictlyLower_v<MT4> ? i : i + 1UL),
																			  (IsStrictlyUpper_v<MT5> ? j : j + 1UL)))
																	   : (IsStrictlyLower_v<MT4> ? i : i + 1UL))
												   : ((IsUpper_v<MT5>) ? (IsStrictlyUpper_v<MT5> ? j : j + 1UL) : (K)));
	...
	*/
		...
		-> EnableIf_t<IsColumnMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		const size_t jbegin((IsStrictlyUpper_v<MT5>) ? ((IsStrictlyUpper_v<MT4> && N > 1UL) ? 2UL : 1UL) : (0UL));
		const size_t jend((IsStrictlyLower_v<MT5>) ? ((IsStrictlyLower_v<MT4> && N > 1UL) ? N - 2UL : N - 1UL) : (N));
		METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
		...
		for (size_t j = jbegin; j < jend; ++j) {
			const size_t ibegin((IsLower_v<MT4> && IsLower_v<MT5>)
									? ((IsStrictlyLower_v<MT4>) ? (IsStrictlyLower_v<MT5> ? j + 2UL : j + 1UL)
																: (IsStrictlyLower_v<MT5> ? j + 1UL : j))
									: ((IsStrictlyLower_v<MT4>) ? (SYM || HERM || LOW ? max(j, 1UL) : 1UL)
																: (SYM || HERM || LOW ? j : 0UL)));
			const size_t iend(
				(IsUpper_v<MT4> && IsUpper_v<MT5>)
					? ((IsStrictlyUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT5>) ? (j - 1UL) : (j))
												: ((IsStrictlyUpper_v<MT5>) ? (j) : (j + 1UL)))
					: ((IsStrictlyUpper_v<MT4>) ? (UPP ? min(j + 1UL, M - 1UL) : M - 1UL) : (UPP ? j + 1UL : M)));
	...
	*/
		...
		-> EnableIf_t<IsRowMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && IsDiagonal_v<MT5>>
	{
		...
		for (size_t i = 0UL; i < M; ++i) {
			const size_t jbegin((IsUpper_v<MT4>) ? (IsStrictlyUpper_v<MT4> ? i + 1UL : i) : (0UL));
			const size_t jend((IsLower_v<MT4>) ? (IsStrictlyLower_v<MT4> ? i : i + 1UL) : (N));
			METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
		...
		for (size_t jj = 0UL; jj < N; jj += block) {
			...
			for (size_t ii = 0UL; ii < M; ii += block) {
				...
				for (size_t j = jj; j < jend; ++j) {
					const size_t ibegin((IsLower_v<MT4>) ? (max((IsStrictlyLower_v<MT4> ? j + 1UL : j), ii)) : (ii));
					const size_t ipos((IsUpper_v<MT4>) ? (min((IsStrictlyUpper_v<MT4> ? j : j + 1UL), iend)) : (iend));
		...
		for (size_t ii = 0UL; ii < M; ii += block) {
			...
			for (size_t jj = 0UL; jj < N; jj += block) {
				...
				for (size_t i = ii; i < iend; ++i) {
					const size_t jbegin((IsUpper_v<MT5>) ? (max((IsStrictlyUpper_v<MT5> ? i + 1UL : i), jj)) : (jj));
					const size_t jpos((IsLower_v<MT5>) ? (min((IsStrictlyLower_v<MT5> ? i : i + 1UL), jend)) : (jend));
	...
	*/
		...
		-> EnableIf_t<IsColumnMajorMatrix_v<MT3> && IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		for (size_t j = 0UL; j < N; ++j) {
			const size_t ibegin((IsLower_v<MT5>) ? (IsStrictlyLower_v<MT5> ? j + 1UL : j) : (0UL));
			const size_t iend((IsUpper_v<MT5>) ? (IsStrictlyUpper_v<MT5> ? j : j + 1UL) : (M));
			METRIC_NUMERIC_INTERNAL_ASSERT(ibegin <= iend, "Invalid loop indices detected");
		...
		-> EnableIf_t<IsRowMajorMatrix_v<MT3> && UseVectorizedDefaultKernel_v<MT3, MT4, MT5>>
	{
		...
		for (; !(LOW && UPP) && (i + 3UL) <= M; i += 3UL) {
			...
			for (; (j + 3UL) <= jend; j += 3UL) {
				const size_t kbegin((IsUpper_v<MT4>) ? (prevMultiple((IsLower_v<MT5> ? max(i, j) : i), SIMDSIZE))
													 : (IsLower_v<MT5> ? prevMultiple(j, SIMDSIZE) : 0UL));
				const size_t kend((IsLower_v<MT4>) ? (IsUpper_v<MT5> ? min(i + 3UL, j + 3UL) : (i + 3UL))
												   : (IsUpper_v<MT5> ? (j + 3UL) : K));

				const size_t kpos(remainder ? prevMultiple(kend, SIMDSIZE) : kend);
				METRIC_NUMERIC_INTERNAL_ASSERT(kpos <= kend, "Invalid end calculation");
			...
			for (; (j + 2UL) <= jend; j += 2UL) {
				const size_t kbegin((IsUpper_v<MT4>) ? (prevMultiple((IsLower_v<MT5> ? max(i, j) : i), SIMDSIZE))
													 : (IsLower_v<MT5> ? prevMultiple(j, SIMDSIZE) : 0UL));
				const size_t kend((IsLower_v<MT4>) ? (IsUpper_v<MT5> ? min(i + 3UL, j + 2UL) : (i + 3UL))
		...
		for (; !(LOW && UPP) && (i + 2UL) <= M; i += 2UL) {
			...
			for (; (j + 4UL) <= jend; j += 4UL) {
				const size_t kbegin((IsUpper_v<MT4>) ? (prevMultiple((IsLower_v<MT5> ? max(i, j) : i), SIMDSIZE))
													 : (IsLower_v<MT5> ? prevMultiple(j, SIMDSIZE) : 0UL));
				const size_t kend((IsLower_v<MT4>) ? (IsUpper_v<MT5> ? min(i + 2UL, j + 4UL) : (i + 2UL))
			...
			for (; (j + 2UL) <= jend; j += 2UL) {
				const size_t kbegin((IsUpper_v<MT4>) ? (prevMultiple((IsLower_v<MT5> ? max(i, j) : i), SIMDSIZE))
													 : (IsLower_v<MT5> ? prevMultiple(j, SIMDSIZE) : 0UL));
				const size_t kend((IsLower_v<MT4>) ? (IsUpper_v<MT5> ? min(i + 2UL, j + 2UL) : (i + 2UL))
												   : (IsUpper_v<MT5> ? (j + 2UL) : K));
		...
		for (; i < M; ++i) {
			...
			if (j < jend) {
				...
				METRIC_NUMERIC_INTERNAL_ASSERT(kpos <= K, "Invalid end calculation");
		...
		-> EnableIf_t<IsColumnMajorMatrix_v<MT3> && UseVectorizedDefaultKernel_v<MT3, MT4, MT5>>
	{
		...
		for (; !(LOW && UPP) && (i + 4UL) <= M; i += 4UL) {
			...
			for (; (j + 2UL) <= jend; j += 2UL) {
				const size_t kbegin((IsUpper_v<MT4>) ? (prevMultiple((IsLower_v<MT5> ? max(i, j) : i), SIMDSIZE))
													 : (IsLower_v<MT5> ? prevMultiple(j, SIMDSIZE) : 0UL));
				const size_t kend((IsLower_v<MT4>) ? (IsUpper_v<MT5> ? min(i + 4UL, j + 2UL) : (i + 4UL))
												   : (IsUpper_v<MT5> ? (j + 2UL) : K));
		...
		-> EnableIf_t<IsRowMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		for (size_t i = ibegin; i < iend; ++i) {
			const size_t jbegin((IsUpper_v<MT4> && IsUpper_v<MT5>)
									? ((IsStrictlyUpper_v<MT4>) ? (IsStrictlyUpper_v<MT5> ? i + 2UL : i + 1UL)
																: (IsStrictlyUpper_v<MT5> ? i + 1UL : i))
									: ((IsStrictlyUpper_v<MT5>) ? (UPP ? max(i, 1UL) : 1UL) : (UPP ? i : 0UL)));
			const size_t jend(
				(IsLower_v<MT4> && IsLower_v<MT5>)
					? ((IsStrictlyLower_v<MT4>) ? (IsStrictlyLower_v<MT5> ? i - 1UL : i)
												: (IsStrictlyLower_v<MT5> ? i : i + 1UL))
			...
			for (size_t j = jbegin; j < jend; ++j) {
				...
				METRIC_NUMERIC_INTERNAL_ASSERT(kpos <= kbegin + knum, "Invalid end calculation");
		...
		-> EnableIf_t<IsColumnMajorMatrix_v<MT3> && !IsDiagonal_v<MT4> && !IsDiagonal_v<MT5>>
	{
		...
		for (size_t j = jbegin; j < jend; ++j) {
			const size_t ibegin((IsLower_v<MT4> && IsLower_v<MT5>)
									? ((IsStrictlyLower_v<MT4>) ? (IsStrictlyLower_v<MT5> ? j + 2UL : j + 1UL)
																: (IsStrictlyLower_v<MT5> ? j + 1UL : j))
									: ((IsStrictlyLower_v<MT4>) ? (LOW ? max(j, 1UL) : 1UL) : (LOW ? j : 0UL)));
			const size_t iend(
				(IsUpper_v<MT4> && IsUpper_v<MT5>)
					? ((IsStrictlyUpper_v<MT4>) ? ((IsStrictlyUpper_v<MT5>) ? (j - 1UL) : (j))
												: ((IsStrictlyUpper_v<MT5>) ? (j) : (j + 1UL)))
...
class DMatScalarMultExpr<DMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, ST, false>
	: public MatScalarMultExpr<
		  DenseMatrix<DMatScalarMultExpr<DMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, ST, false>, false>>,
	  private Computation {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool UseVectorizedDefaultKernel_v =
		(useOptimizedKernels && !IsDiagonal_v<T2> && !IsDiagonal_v<T3> && T1::simdEnabled && T2::simdEnabled &&
		 T3::simdEnabled && IsSIMDCombinable_v<ElementType_t<T1>, ElementType_t<T2>, ElementType_t<T3>, T4> &&
		 HasSIMDAdd_v<ElementType_t<T2>, ElementType_t<T3>> && HasSIMDMult_v<ElementType_t<T2>, ElementType_t<T3>>);
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_MAJOR_MATRIX_TYPE(MMM);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MT1);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_MAJOR_MATRIX_TYPE(MT1);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MT2);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(MT2);
...
// dense matrix:
   ...
   mtrc::numeric::DynamicMatrix<double,columnMajor> B;
...
template <typename MT1 // Type of the left-hand side dense matrix
		  ,
		  typename MT2> // Type of the right-hand side dense matrix
inline decltype(auto) operator*(const DenseMatrix<MT1, false> &lhs, const DenseMatrix<MT2, true> &rhs)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
struct Size<DMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 0UL> : public Size<MT1, 0UL> {};

template <typename MT1, typename MT2, bool SF, bool HF, bool LF, bool UF>
struct Size<DMatTDMatMultExpr<MT1, MT2, SF, HF, LF, UF>, 1UL> : public Size<MT2, 1UL> {};
/*! \endcond */
//*************************************************************************************************
