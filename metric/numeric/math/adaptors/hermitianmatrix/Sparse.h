...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_HERMITIANMATRIX_SPARSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_HERMITIANMATRIX_SPARSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/adaptors/hermitianmatrix/HermitianElement.h>
#include <metric/numeric/math/adaptors/hermitianmatrix/HermitianProxy.h>
...
class HermitianMatrix<MT, SO, false> : public SparseMatrix<HermitianMatrix<MT, SO, false>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = HermitianMatrix<OT, !SO, false>;  //!< Result type with opposite storage order for expression
	...
	class Iterator {
	  public:
		...
		*/
		inline ReferenceType operator*() const { return ReferenceType(pos_, matrix_, index_); }
		//*******************************************************************************************
		...
		*/
		inline PointerType operator->() const { return PointerType(pos_, matrix_, index_); }
		//*******************************************************************************************
		...
		*/
		inline operator ConstIterator() const { return pos_; }
		//*******************************************************************************************
		...
		*/
		inline DifferenceType operator-(const Iterator &rhs) const { return pos_ - rhs.pos_; }
		//*******************************************************************************************
		...
		*/
		inline IteratorType base() const { return pos_; }
		//*******************************************************************************************
	...
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> DisableIf_t<IsComputation_v<MT2>, HermitianMatrix &>;

	template <typename MT2, bool SO2>
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> EnableIf_t<IsComputation_v<MT2>, HermitianMatrix &>;
	...
	template <typename ST> inline auto operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, HermitianMatrix &>;
  ...
  private:
	...
	//@{
	template <typename MT2, bool SO2, typename T> inline decltype(auto) construct(const Matrix<MT2, SO2> &m, T);

	template <typename MT2> inline decltype(auto) construct(const Matrix<MT2, !SO> &m, TrueType);
	//@}
	//**********************************************************************************************
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(OT, !SO);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_WITH_STORAGE_ORDER(TT, !SO);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SCALAR_TYPE(ElementType);
	METRIC_NUMERIC_STATIC_ASSERT((Size_v<MT, 0UL> == Size_v<MT, 1UL>));
	//**********************************************************************************************
};
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO>													 // Storage order of the adapted sparse matrix
inline HermitianMatrix<MT, SO, false>::HermitianMatrix() : matrix_() // The adapted sparse matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square Hermitian matrix detected");
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline HermitianMatrix<MT, SO, false>::HermitianMatrix(size_t n) : matrix_(n, n) // The adapted sparse matrix
{
...
// matrix by means of an initializer list:
   ...
   using MT = mtrc::numeric::HermitianMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> >;
	...
	: matrix_(list) // The adapted sparse matrix
{
	if (!isHermitian(matrix_)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of Hermitian matrix");
	}

	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(), "Broken invariant detected");
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO>	   // Storage order of the adapted sparse matrix
template <typename MT2 // Type of the other matrix
		  ,
...
//
// Note that this function only performs an index check in case METRIC_NUMERIC_USER_ASSERT() is active. In
// contrast, the at() function is guaranteed to perform a check of the given access indices.
*/
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Reference HermitianMatrix<MT, SO, false>::operator()(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename HermitianMatrix<MT, SO, false>::ConstReference HermitianMatrix<MT, SO, false>::operator()(size_t i,
																									   size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Reference HermitianMatrix<MT, SO, false>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename HermitianMatrix<MT, SO, false>::ConstReference HermitianMatrix<MT, SO, false>::at(size_t i, size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::begin(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::ConstIterator HermitianMatrix<MT, SO, false>::begin(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::ConstIterator HermitianMatrix<MT, SO, false>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::end(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::ConstIterator HermitianMatrix<MT, SO, false>::end(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::ConstIterator HermitianMatrix<MT, SO, false>::cend(size_t i) const
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::HermitianMatrix< mtrc::numeric::CompressedMatrix<cplx,rowMajor> > A;
...
HermitianMatrix<MT, SO, false>::operator=(initializer_list<initializer_list<ElementType>> list)
{
	...
	if (!isHermitian(tmp)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to Hermitian matrix");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline HermitianMatrix<MT, SO, false> &HermitianMatrix<MT, SO, false>::operator=(const HermitianMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline HermitianMatrix<MT, SO, false> &HermitianMatrix<MT, SO, false>::operator=(HermitianMatrix &&rhs) noexcept
{
...
inline auto HermitianMatrix<MT, SO, false>::operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, HermitianMatrix &>
{
	matrix_ *= rhs;
	return *this;
}
...
inline auto HermitianMatrix<MT, SO, false>::operator/=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, HermitianMatrix &>
{
	METRIC_NUMERIC_USER_ASSERT(!isZero(rhs), "Division by zero detected");
...
// The following example demonstrates this by means of a \f$ 5 \times 5 \f$ Hermitian matrix:
   ...
   \code
   mtrc::numeric::HermitianMatrix< mtrc::numeric::CompressedMatrix<int> > A;
...
inline void HermitianMatrix<MT, SO, false>::reset(size_t i)
{
	...
	for (auto it = matrix_.begin(i); it != matrix_.end(i); ++it) {
		...
		if (SO) {
			...
			METRIC_NUMERIC_INTERNAL_ASSERT(pos != matrix_.end(j), "Missing element detected");
			erase(matrix_, j, pos);
		} else {
...
inline void HermitianMatrix<MT, SO, false>::clear()
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(matrix_.rows() == 0UL, "Invalid number of rows");
	METRIC_NUMERIC_INTERNAL_ASSERT(matrix_.columns() == 0UL, "Invalid number of columns");
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::set(size_t i, size_t j,
																							 const ElementType &value)
{
	const bool isDiagonal(i == j);

	if (IsComplex_v<ElementType> && isDiagonal && !isReal(value)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to diagonal matrix element");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::insert(size_t i, size_t j,
																							 const ElementType &value)
{
	const bool isDiagonal(i == j);
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::erase(size_t i, Iterator pos)
{
	using mtrc::numeric::erase;
	...
	if (i == j) {
		METRIC_NUMERIC_INTERNAL_ASSERT(matrix_.find(i, i) != matrix_.end(i), "Missing element detected");
		return Iterator(erase(matrix_, i, base), matrix_, i);
	}

	if (SO) {
		METRIC_NUMERIC_INTERNAL_ASSERT(matrix_.find(i, j) != matrix_.end(j), "Missing element detected");
		erase(matrix_, j, matrix_.find(i, j));
		return Iterator(erase(matrix_, i, base), matrix_, i);
	} else {
		METRIC_NUMERIC_INTERNAL_ASSERT(matrix_.find(j, i) != matrix_.end(j), "Missing element detected");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::erase(size_t i, Iterator first,
																							   Iterator last)
{
	using mtrc::numeric::erase;
...
// all elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::HermitianMatrix< CompressedMatrix<double,mtrc::numeric::rowMajor> > A;
...
inline void HermitianMatrix<MT, SO, false>::erase(Pred predicate)
{
	...
	erase(matrix_,
		  [predicate = predicate](const ElementType &value) { return predicate(value) || predicate(conj(value)); });
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO>		 // Storage order of the adapted sparse matrix
template <typename Pred> // Type of the unary predicate
inline void HermitianMatrix<MT, SO, false>::erase(size_t i, Iterator first, Iterator last, Pred predicate)
{
	using mtrc::numeric::erase;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::find(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::ConstIterator HermitianMatrix<MT, SO, false>::find(size_t i,
																								   size_t j) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::lowerBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::ConstIterator HermitianMatrix<MT, SO, false>::lowerBound(size_t i,
																										 size_t j) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::Iterator HermitianMatrix<MT, SO, false>::upperBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename HermitianMatrix<MT, SO, false>::ConstIterator HermitianMatrix<MT, SO, false>::upperBound(size_t i,
																										 size_t j) const
{
...
inline HermitianMatrix<MT, SO, false> &HermitianMatrix<MT, SO, false>::scale(const Other &scalar)
{
