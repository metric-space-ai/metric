...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_HERMITIANMATRIX_BASETEMPLATE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_HERMITIANMATRIX_BASETEMPLATE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// The following examples give an impression of several possible Hermitian matrices:
   ...
   mtrc::numeric::HermitianMatrix< mtrc::numeric::StaticMatrix<int,3UL,3UL,rowMajor> > A;

   // Definition of a resizable column-major dense Hermitian matrix based on HybridMatrix
   mtrc::numeric::HermitianMatrix< mtrc::numeric::HybridMatrix<float,4UL,4UL,columnMajor> B;

   // Definition of a resizable row-major dense Hermitian matrix based on DynamicMatrix
   mtrc::numeric::HermitianMatrix< mtrc::numeric::DynamicMatrix<std::complex<double>,rowMajor> > C;

   // Definition of a fixed-size row-major dense diagonal matrix based on CustomMatrix
   mtrc::numeric::HermitianMatrix< mtrc::numeric::CustomMatrix<double,unaligned,unpadded,rowMajor> > D;

   // Definition of a compressed row-major single precision complex Hermitian matrix
   mtrc::numeric::HermitianMatrix< mtrc::numeric::CompressedMatrix<std::complex<float>,rowMajor> > E;
   \endcode
...
// mtrc::numeric::rowMajor), the Hermitian matrix will also be a row-major matrix. Otherwise, if the
// adapted matrix is column-major (i.e. is specified as mtrc::numeric::columnMajor), the Hermitian matrix
// will also be a column-major matrix.
//
//
// \n \section hermitianmatrix_vs_symmetricmatrix Hermitian Matrices vs. Symmetric Matrices
//
// The mtrc::numeric::HermitianMatrix adaptor and the mtrc::numeric::SymmetricMatrix adaptor share several traits.
...
//
// From an implementation point of view, \b Metric numeric restricts Hermitian matrices to numeric data
...
// thrown:
   ...
   CustomHermitian B( new double[9UL], 3UL, mtrc::numeric::ArrayDelete() );  // Throws an exception
   \endcode
...
// the matrix:
   ...
   \code
   using mtrc::numeric::DynamicMatrix;
   using mtrc::numeric::HermtianMatrix;
...
// \n \subsection hermitianmatrix_initialization The Elements of a Dense Hermitian Matrix are Always Default
Initialized!
//
...
// \b Metric numeric library tries to exploit the properties of Hermitian (symmetric) matrices whenever
...
//
// When multiplying two matrices, at least one of which is symmetric, \b Metric numeric can exploit the fact
...
// for maximum performance. However, \b Metric numeric evaluates the multiplication as
...
// \b Metric numeric exploits the fact that \c A is symmetric, selects the best suited storage order and
...
// elements. However, in case of symmetric matrices, \b Metric numeric instead uses the according column of
// the matrix, which provides the same performance as if the matrix would be row-major. Note that
// this also works for column views on row-major matrices, where \b Metric numeric can use the according
