...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_STRICTLYUPPERMATRIX_DENSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_STRICTLYUPPERMATRIX_DENSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/adaptors/strictlyuppermatrix/StrictlyUpperProxy.h>
...
class StrictlyUpperMatrix<MT, SO, true> : public DenseMatrix<StrictlyUpperMatrix<MT, SO, true>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = StrictlyUpperMatrix<OT, !SO, true>; //!< Result type with opposite storage order for expression
	...
	template <typename Other> inline StrictlyUpperMatrix(size_t n, const Other *array);

	template <typename Other, size_t N> inline StrictlyUpperMatrix(const Other (&array)[N][N]);
	...
	template <typename Other, size_t N> inline StrictlyUpperMatrix &operator=(const Other (&array)[N][N]);
	...
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> DisableIf_t<IsComputation_v<MT2>, StrictlyUpperMatrix &>;

	template <typename MT2, bool SO2>
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> EnableIf_t<IsComputation_v<MT2>, StrictlyUpperMatrix &>;
	...
	template <typename MT2, bool SO2> inline auto operator%=(const Matrix<MT2, SO2> &rhs) -> StrictlyUpperMatrix &;

	template <typename ST> inline auto operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, StrictlyUpperMatrix &>;
  ...
  private:
	...
	//**Friend declarations*************************************************************************
	template <typename MT2, bool SO2, bool DF2> friend MT2 &derestrict(StrictlyUpperMatrix<MT2, SO2, DF2> &m);
	//**********************************************************************************************
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO>															// Storage order of the adapted dense matrix
inline StrictlyUpperMatrix<MT, SO, true>::StrictlyUpperMatrix() : matrix_() // The adapted dense matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square strictly upper matrix detected");
...
// upper matrix by means of an initializer list:
   ...
   mtrc::numeric::StrictlyUpperMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A{ { 0, 2, 3 },
																		  { 0, 0 },
																		  { 0, 0, 0 } };
   \endcode
	...
	: matrix_(list) // The adapted dense matrix
{
	if (!isStrictlyUpper(matrix_)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of strictly upper matrix");
	}
...
// matrix with a dynamic array:
   ...
   mtrc::numeric::StrictlyUpperMatrix< mtrc::numeric::DynamicMatrix<int,rowMajor> > v( 4UL, 5UL, array );
   delete[] array;
   \endcode
...
// matrix with a static array:
   ...
   const int init[3][3] = { { 0, 1, 2 },
							{ 0, 0 },
							{ 0, 0, 0 } };
   mtrc::numeric::StrictlyUpperMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A( init );
   \endcode
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename StrictlyUpperMatrix<MT, SO, true>::Reference StrictlyUpperMatrix<MT, SO, true>::operator()(size_t i,
																										   size_t j)
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename StrictlyUpperMatrix<MT, SO, true>::Reference StrictlyUpperMatrix<MT, SO, true>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename StrictlyUpperMatrix<MT, SO, true>::ConstReference StrictlyUpperMatrix<MT, SO, true>::at(size_t i,
																										size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename StrictlyUpperMatrix<MT, SO, true>::ConstPointer StrictlyUpperMatrix<MT, SO, true>::data() const noexcept
{
	return matrix_.data();
}
/*! \endcond */
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename StrictlyUpperMatrix<MT, SO, true>::Iterator StrictlyUpperMatrix<MT, SO, true>::begin(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
inline
	typename StrictlyUpperMatrix<MT, SO, true>::ConstIterator StrictlyUpperMatrix<MT, SO, true>::begin(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
inline
	typename StrictlyUpperMatrix<MT, SO, true>::ConstIterator StrictlyUpperMatrix<MT, SO, true>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename StrictlyUpperMatrix<MT, SO, true>::Iterator StrictlyUpperMatrix<MT, SO, true>::end(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename StrictlyUpperMatrix<MT, SO, true>::ConstIterator StrictlyUpperMatrix<MT, SO, true>::end(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename StrictlyUpperMatrix<MT, SO, true>::ConstIterator StrictlyUpperMatrix<MT, SO, true>::cend(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline StrictlyUpperMatrix<MT, SO, true> &StrictlyUpperMatrix<MT, SO, true>::operator=(const ElementType &rhs)
{
	if (SO) {
...
// upper matrix by means of an initializer list:
   ...
   mtrc::numeric::StrictlyUpperMatrix< mtrc::numeric::StaticMatrix<int,3UL,3UL,rowMajor> > A;
   A = { { 0, 2, 3 },
		 { 0, 0 },
		 { 0, 0, 0 } };
   \endcode
...
StrictlyUpperMatrix<MT, SO, true>::operator=(initializer_list<initializer_list<ElementType>> list)
{
	...
	if (!isStrictlyUpper(tmp)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to strictly upper matrix");
	}
...
inline StrictlyUpperMatrix<MT, SO, true> &StrictlyUpperMatrix<MT, SO, true>::operator=(const Other (&array)[N][N])
{
	MT tmp(array);
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline StrictlyUpperMatrix<MT, SO, true> &StrictlyUpperMatrix<MT, SO, true>::operator=(const StrictlyUpperMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
{
	if (IsUniTriangular_v<MT2> || (!IsStrictlyUpper_v<MT2> && !isStrictlyUpper(*rhs))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to strictly upper matrix");
	}
...
template <typename MT2 // Type of the right-hand side matrix
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline auto StrictlyUpperMatrix<MT, SO, true>::operator%=(const Matrix<MT2, SO2> &rhs) -> StrictlyUpperMatrix &
{
	if (!IsSquare_v<MT2> && !isSquare(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to strictly upper matrix");
	}
...
inline auto StrictlyUpperMatrix<MT, SO, true>::operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, StrictlyUpperMatrix &>
{
	matrix_ *= rhs;
	return *this;
}
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::StrictlyUpperMatrix< mtrc::numeric::DynamicMatrix<int> > A;
...
inline StrictlyUpperMatrix<MT, SO, true> &StrictlyUpperMatrix<MT, SO, true>::scale(const Other &scalar)
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
METRIC_NUMERIC_ALWAYS_INLINE typename StrictlyUpperMatrix<MT, SO, true>::SIMDType
...
inline const MT StrictlyUpperMatrix<MT, SO, true>::construct(const Matrix<MT2, SO2> &m, T)
{
	...
	if (IsUniTriangular_v<MT2> || (!IsStrictlyUpper_v<MT2> && !isStrictlyUpper(tmp))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of strictly upper matrix");
	}
