...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_SPARSENONSCALAR_H
#define METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_SPARSENONSCALAR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/adaptors/symmetricmatrix/NonScalarProxy.h>
#include <metric/numeric/math/adaptors/symmetricmatrix/SharedValue.h>
...
class SymmetricMatrix<MT, SO, false, false> : public SparseMatrix<SymmetricMatrix<MT, SO, false, false>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = SymmetricMatrix<OT, !SO, false, false>; //!< Result type with opposite storage order for
																 //!< expression template evaluations.
	using TransposeType =
		SymmetricMatrix<TT, !SO, false, false>;	  //!< Transpose type for expression template evaluations.
	...
	class SharedElement : private SparseElement {
	  public:
		...
		*/
		inline SharedElement(IteratorType pos) : pos_(pos) // Iterator to the current sparse symmetric matrix element
		{
		}
		//*******************************************************************************************
		...
		*/
		inline Reference value() { return *pos_->value(); }
		//*******************************************************************************************
		...
		// \return The current value of the symmetric element.
		*/
		inline ConstReference value() const { return *pos_->value(); }
		//*******************************************************************************************
		...
		*/
		inline IndexType index() const { return pos_->index(); }
		//*******************************************************************************************
	...
	class SharedIterator {
	  public:
		 ...
		 */
		inline SharedIterator() : pos_() // Iterator to the current sparse symmetric matrix element
		{
		}
		//*******************************************************************************************
		...
		*/
		inline SharedIterator(IteratorType pos) : pos_(pos) // Iterator to the current sparse symmetric matrix element
		{
		}
		//*******************************************************************************************
		...
		*/
		inline ReferenceType operator*() const { return ReferenceType(pos_); }
		//*******************************************************************************************
		...
		*/
		inline PointerType operator->() const { return PointerType(pos_); }
		//*******************************************************************************************
		...
		*/
		inline bool operator==(const SharedIterator &rhs) const { return pos_ == rhs.pos_; }
		//*******************************************************************************************
		...
		*/
		inline bool operator!=(const SharedIterator &rhs) const { return !(*this == rhs); }
		//*******************************************************************************************
		...
		*/
		inline DifferenceType operator-(const SharedIterator &rhs) const { return pos_ - rhs.pos_; }
		//*******************************************************************************************
	...
	using Iterator = SharedIterator<SharedElement<Iterator_t<MatrixType>>, Iterator_t<MatrixType>>;

	//! Iterator over constant elements.
	using ConstIterator = SharedIterator<const SharedElement<ConstIterator_t<MatrixType>>, ConstIterator_t<MatrixType>>;
	//**********************************************************************************************
	...
	inline auto operator=(const Matrix<MT2, SO> &rhs) -> DisableIf_t<IsComputation_v<MT2>, SymmetricMatrix &>;

	template <typename MT2>
	inline auto operator=(const Matrix<MT2, SO> &rhs) -> EnableIf_t<IsComputation_v<MT2>, SymmetricMatrix &>;

	template <typename MT2> inline auto operator=(const Matrix<MT2, !SO> &rhs) -> SymmetricMatrix &;
	...
	template <typename ST> inline auto operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, SymmetricMatrix &>;
  ...
  private:
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_SCALAR_TYPE(ElementType);
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline SymmetricMatrix<MT, SO, false, false>::SymmetricMatrix() : matrix_() // The adapted sparse matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square symmetric matrix detected");
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline SymmetricMatrix<MT, SO, false, false>::SymmetricMatrix(size_t n) : matrix_(n, n) // The adapted sparse matrix
{
	...
	: matrix_() // The adapted sparse matrix
{
	...
	if (!IsSymmetric_v<MT2> && !isSymmetric(tmp)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of symmetric matrix");
	}
...
inline typename SymmetricMatrix<MT, SO, false, false>::Reference SymmetricMatrix<MT, SO, false, false>::at(size_t i,
																										   size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename SymmetricMatrix<MT, SO, false, false>::Iterator SymmetricMatrix<MT, SO, false, false>::begin(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename SymmetricMatrix<MT, SO, false, false>::Iterator SymmetricMatrix<MT, SO, false, false>::end(size_t i)
{
	...
	-> DisableIf_t<IsComputation_v<MT2>, SymmetricMatrix &>
{
	...
	if (!IsSymmetric_v<MT2> && !isSymmetric(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to symmetric matrix");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO>		// Storage order of the adapted sparse matrix
template <typename MT2> // Type of the right-hand side matrix
inline auto SymmetricMatrix<MT, SO, false, false>::operator=(const Matrix<MT2, !SO> &rhs) -> SymmetricMatrix &
{
...
inline auto SymmetricMatrix<MT, SO, false, false>::operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, SymmetricMatrix &>
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename SymmetricMatrix<MT, SO, false, false>::Iterator SymmetricMatrix<MT, SO, false, false>::erase(size_t i,
																										  Iterator pos)
{
	using mtrc::numeric::erase;
...
inline void SymmetricMatrix<MT, SO, false, false>::erase(Pred predicate)
{
	...
	erase(matrix_, [predicate = predicate](const SharedValue<ET> &value) { return predicate(*value); });
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO>		 // Storage order of the adapted sparse matrix
template <typename Pred> // Type of the unary predicate
inline void SymmetricMatrix<MT, SO, false, false>::erase(size_t i, Iterator first, Iterator last, Pred predicate)
{
	using mtrc::numeric::erase;
	...
	erase(matrix_, i, first.base(), last.base(),
		  [predicate = predicate](const SharedValue<ET> &value) { return predicate(*value); });
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename SymmetricMatrix<MT, SO, false, false>::Iterator SymmetricMatrix<MT, SO, false, false>::find(size_t i,
																											size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename SymmetricMatrix<MT, SO, false, false>::Iterator SymmetricMatrix<MT, SO, false, false>::lowerBound(size_t i,
																											   size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename SymmetricMatrix<MT, SO, false, false>::Iterator SymmetricMatrix<MT, SO, false, false>::upperBound(size_t i,
																											   size_t j)
{
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::CompressedMatrix< mtrc::numeric::StaticVector<int,1UL> > > A;
...
inline SymmetricMatrix<MT, SO, false, false> &SymmetricMatrix<MT, SO, false, false>::scale(const Other &scalar)
{
	for (size_t i = 0UL; i < rows(); ++i) {
		const auto last(matrix_.upperBound(i, i));
		for (auto element = matrix_.begin(i); element != last; ++element)
