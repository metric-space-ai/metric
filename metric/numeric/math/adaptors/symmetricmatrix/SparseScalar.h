...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_SPARSESCALAR_H
#define METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_SPARSESCALAR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/adaptors/symmetricmatrix/ScalarProxy.h>
#include <metric/numeric/math/adaptors/symmetricmatrix/SymmetricElement.h>
#include <metric/numeric/math/adaptors/symmetricmatrix/SymmetricValue.h>
...
class SymmetricMatrix<MT, SO, false, true> : public SparseMatrix<SymmetricMatrix<MT, SO, false, true>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = SymmetricMatrix<OT, !SO, false, true>; //!< Result type with opposite storage order for
																//!< expression template evaluations.
	using TransposeType =
		SymmetricMatrix<TT, !SO, false, true>;	 //!< Transpose type for expression template evaluations.
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO>														   // Storage order of the adapted sparse matrix
inline SymmetricMatrix<MT, SO, false, true>::SymmetricMatrix() : matrix_() // The adapted sparse matrix
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline SymmetricMatrix<MT, SO, false, true>::SymmetricMatrix(size_t n) : matrix_(n, n) // The adapted sparse matrix
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A{ { 1,  4,  5 },
																	  { 4,  2, -6 },
																	  { 5, -6,  3 } };
   \endcode
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename SymmetricMatrix<MT, SO, false, true>::Reference SymmetricMatrix<MT, SO, false, true>::operator()(size_t i,
																											  size_t j)
{
...
inline typename SymmetricMatrix<MT, SO, false, true>::Reference SymmetricMatrix<MT, SO, false, true>::at(size_t i,
																										 size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename SymmetricMatrix<MT, SO, false, true>::Iterator SymmetricMatrix<MT, SO, false, true>::begin(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename SymmetricMatrix<MT, SO, false, true>::Iterator SymmetricMatrix<MT, SO, false, true>::end(size_t i)
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A;
   A = { { 1,  4,  5 },
		 { 4,  2, -6 },
		 { 5, -6,  3 } };
   \endcode
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline SymmetricMatrix<MT, SO, false, true> &SymmetricMatrix<MT, SO, false, true>::operator=(const SymmetricMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO>		// Storage order of the adapted sparse matrix
template <typename MT2> // Type of the right-hand side matrix
inline auto SymmetricMatrix<MT, SO, false, true>::operator=(const Matrix<MT2, !SO> &rhs) -> SymmetricMatrix &
{
...
inline auto SymmetricMatrix<MT, SO, false, true>::operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, SymmetricMatrix &>
{
	matrix_ *= rhs;
	return *this;
}
...
// The following example demonstrates this by means of a \f$ 5 \times 5 \f$ symmetric matrix:
   ...
   \code
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::CompressedMatrix<int> > A;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename SymmetricMatrix<MT, SO, false, true>::Iterator SymmetricMatrix<MT, SO, false, true>::erase(size_t i,
																										   Iterator pos)
{
	using mtrc::numeric::erase;
...
// all elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::SymmetricMatrix< CompressedMatrix<double,mtrc::numeric::rowMajor> > A;
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO>		 // Storage order of the adapted sparse matrix
template <typename Pred> // Type of the unary predicate
inline void SymmetricMatrix<MT, SO, false, true>::erase(size_t i, Iterator first, Iterator last, Pred predicate)
{
	using mtrc::numeric::erase;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename SymmetricMatrix<MT, SO, false, true>::Iterator SymmetricMatrix<MT, SO, false, true>::find(size_t i,
																										  size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename SymmetricMatrix<MT, SO, false, true>::Iterator SymmetricMatrix<MT, SO, false, true>::lowerBound(size_t i,
																											 size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename SymmetricMatrix<MT, SO, false, true>::Iterator SymmetricMatrix<MT, SO, false, true>::upperBound(size_t i,
																											 size_t j)
{
...
inline SymmetricMatrix<MT, SO, false, true> &SymmetricMatrix<MT, SO, false, true>::scale(const Other &scalar)
{
