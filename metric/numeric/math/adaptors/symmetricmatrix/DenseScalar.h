...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_DENSESCALAR_H
#define METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_DENSESCALAR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SymmetricMatrix<MT, SO, true, true> : public DenseMatrix<SymmetricMatrix<MT, SO, true, true>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = SymmetricMatrix<OT, !SO, true, true>;	//!< Result type with opposite storage order for
	...
	template <typename Other> inline SymmetricMatrix(size_t n, const Other *array);

	template <typename Other, size_t N> inline SymmetricMatrix(const Other (&array)[N][N]);
	...
	template <typename Other, size_t N> inline SymmetricMatrix &operator=(const Other (&array)[N][N]);
  ...
  private:
	...
	//**Friend declarations*************************************************************************
	template <InversionFlag IF, typename MT2, bool SO2> friend void invert(SymmetricMatrix<MT2, SO2, true, true> &m);
	//**********************************************************************************************
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO>														  // Storage order of the adapted dense matrix
inline SymmetricMatrix<MT, SO, true, true>::SymmetricMatrix() : matrix_() // The adapted dense matrix
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A{ { 1,  4,  5 },
																	  { 4,  2, -6 },
																	  { 5, -6,  3 } };
   \endcode
...
// with a dynamic array:
   ...
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::DynamicMatrix<int,rowMajor> > v( 4UL, array );
   delete[] array;
   \endcode
...
// with a static array:
   ...
   const int init[3][3] = { { 1,  4,  5 },
							{ 4,  2, -6 },
							{ 5, -6,  3 } };
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A( init );
   \endcode
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename SymmetricMatrix<MT, SO, true, true>::Reference SymmetricMatrix<MT, SO, true, true>::operator()(size_t i,
																											   size_t j)
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
inline
	typename SymmetricMatrix<MT, SO, true, true>::Reference SymmetricMatrix<MT, SO, true, true>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
inline
	typename SymmetricMatrix<MT, SO, true, true>::ConstReference SymmetricMatrix<MT, SO, true, true>::at(size_t i,
																										 size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename SymmetricMatrix<MT, SO, true, true>::Iterator SymmetricMatrix<MT, SO, true, true>::begin(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename SymmetricMatrix<MT, SO, true, true>::Iterator SymmetricMatrix<MT, SO, true, true>::end(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
inline
	typename SymmetricMatrix<MT, SO, true, true>::ConstIterator SymmetricMatrix<MT, SO, true, true>::end(size_t i) const
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::StaticMatrix<int,3UL,3UL,rowMajor> > A;
   A = { { 1,  4,  5 },
		 { 4,  2, -6 },
		 { 5, -6,  3 } };
   \endcode
...
template <typename Other // Data type of the initialization array
		  ,
		  size_t N> // Number of rows and columns of the initialization array
inline SymmetricMatrix<MT, SO, true, true> &SymmetricMatrix<MT, SO, true, true>::operator=(const Other (&array)[N][N])
{
	MT tmp(array);
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline SymmetricMatrix<MT, SO, true, true> &SymmetricMatrix<MT, SO, true, true>::operator=(const SymmetricMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO>		// Storage order of the adapted dense matrix
template <typename MT2> // Type of the right-hand side matrix
inline auto SymmetricMatrix<MT, SO, true, true>::operator=(const Matrix<MT2, !SO> &rhs) -> SymmetricMatrix &
{
...
inline auto SymmetricMatrix<MT, SO, true, true>::operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, SymmetricMatrix &>
{
	matrix_ *= rhs;
	return *this;
}
...
// The following example demonstrates this by means of a \f$ 5 \times 5 \f$ symmetric matrix:
   ...
   \code
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::DynamicMatrix<int> > A;
...
inline SymmetricMatrix<MT, SO, true, true> &SymmetricMatrix<MT, SO, true, true>::scale(const Other &scalar)
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
METRIC_NUMERIC_ALWAYS_INLINE typename SymmetricMatrix<MT, SO, true, true>::SIMDType
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline void SymmetricMatrix<MT, SO, true, true>::store(size_t i, size_t j, const SIMDType &value) noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline void SymmetricMatrix<MT, SO, true, true>::storea(size_t i, size_t j, const SIMDType &value) noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline void SymmetricMatrix<MT, SO, true, true>::storeu(size_t i, size_t j, const SIMDType &value) noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline void SymmetricMatrix<MT, SO, true, true>::stream(size_t i, size_t j, const SIMDType &value) noexcept
{
