...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_BASETEMPLATE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_BASETEMPLATE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// The following examples give an impression of several possible symmetric matrices:
   ...
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::HybridMatrix<float,4UL,4UL,columnMajor> B;

   // Definition of a resizable row-major dense symmetric matrix based on DynamicMatrix
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::DynamicMatrix<double,rowMajor> > C;

   // Definition of a fixed-size row-major dense symmetric matrix based on CustomMatrix
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::CustomMatrix<double,unaligned,unpadded,rowMajor> > D;

   // Definition of a compressed row-major single precision symmetric matrix
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::CompressedMatrix<float,rowMajor> > E;
   \endcode
...
// mtrc::numeric::rowMajor), the symmetric matrix will also be a row-major matrix. Otherwise, if the
// adapted matrix is column-major (i.e. is specified as mtrc::numeric::columnMajor), the symmetric matrix
// will also be a column-major matrix.
//
//
// \n \section symmetricmatrix_vs_hermitianmatrix Symmetric Matrices vs. Hermitian Matrices
//
// The mtrc::numeric::SymmetricMatrix adaptor and the mtrc::numeric::HermitianMatrix adaptor share several traits.
...
// thrown:
   ...
   CustomSymmetric B( new double[9UL], 3UL, mtrc::numeric::ArrayDelete() );  // Throws an exception
   \endcode
...
// \n \subsection symmetricmatrix_initialization The Elements of a Dense Symmetric Matrix are Always Default
Initialized!
//
...
// instead of a general matrix can be a considerable performance advantage. The \b Metric numeric library
