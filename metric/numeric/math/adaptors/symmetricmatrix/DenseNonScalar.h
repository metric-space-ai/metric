...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_DENSENONSCALAR_H
#define METRIC_NUMERIC_MATH_ADAPTORS_SYMMETRICMATRIX_DENSENONSCALAR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SymmetricMatrix<MT, SO, true, false> : public DenseMatrix<SymmetricMatrix<MT, SO, true, false>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = SymmetricMatrix<OT, !SO, true, false>; //!< Result type with opposite storage order for
																//!< expression template evaluations.
	using TransposeType =
		SymmetricMatrix<TT, !SO, true, false>;	 //!< Transpose type for expression template evaluations.
	...
	class MatrixIterator {
	  public:
		...
		using Reference = If_t<IsConst_v<MatrixType>, ConstReference_t<MatrixType>, Reference_t<MatrixType>>;
		...
		*/
		template <typename MatrixType2> inline bool operator==(const MatrixIterator<MatrixType2> &rhs) const noexcept
		{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO>														   // Storage order of the adapted dense matrix
inline SymmetricMatrix<MT, SO, true, false>::SymmetricMatrix() : matrix_() // The adapted dense matrix
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline SymmetricMatrix<MT, SO, true, false>::SymmetricMatrix(size_t n) : matrix_(n, n) // The adapted dense matrix
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
inline
	typename SymmetricMatrix<MT, SO, true, false>::Reference SymmetricMatrix<MT, SO, true, false>::operator()(size_t i,
																											  size_t j)
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename SymmetricMatrix<MT, SO, true, false>::Reference SymmetricMatrix<MT, SO, true, false>::at(size_t i,
																										 size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename SymmetricMatrix<MT, SO, true, false>::Iterator SymmetricMatrix<MT, SO, true, false>::begin(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename SymmetricMatrix<MT, SO, true, false>::Iterator SymmetricMatrix<MT, SO, true, false>::end(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline SymmetricMatrix<MT, SO, true, false> &SymmetricMatrix<MT, SO, true, false>::operator=(const SymmetricMatrix &rhs)
{
	using mtrc::numeric::resize;
	...
	-> EnableIf_t<IsComputation_v<MT2>, SymmetricMatrix &>
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(!tmp.canAlias(this), "Aliasing detected");
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO>		// Storage order of the adapted dense matrix
template <typename MT2> // Type of the right-hand side matrix
inline auto SymmetricMatrix<MT, SO, true, false>::operator=(const Matrix<MT2, !SO> &rhs) -> SymmetricMatrix &
{
...
inline auto SymmetricMatrix<MT, SO, true, false>::operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, SymmetricMatrix &>
{
	if (SO) {
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
constexpr size_t SymmetricMatrix<MT, SO, true, false>::rows() const noexcept
{
	return matrix_.rows();
}
/*! \endcond */
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
constexpr size_t SymmetricMatrix<MT, SO, true, false>::columns() const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
constexpr size_t SymmetricMatrix<MT, SO, true, false>::spacing() const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
constexpr size_t SymmetricMatrix<MT, SO, true, false>::capacity() const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
constexpr size_t SymmetricMatrix<MT, SO, true, false>::capacity(size_t i) const noexcept
{
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::DynamicMatrix< mtrc::numeric::StaticVector<int,1UL> > > A;
...
inline SymmetricMatrix<MT, SO, true, false> &SymmetricMatrix<MT, SO, true, false>::scale(const Other &scalar)
{
	if (SO) {
		for (size_t j = 0UL; j < columns(); ++j)
			for (size_t i = 0UL; i <= j; ++i)
...
inline bool SymmetricMatrix<MT, SO, true, false>::isIntact() const noexcept
{
	...
	return isIntact(matrix_) && (IsCustom_v<MT> || (SO ? isUpper(matrix_) : isLower(matrix_)));
}
/*! \endcond */
//*************************************************************************************************
