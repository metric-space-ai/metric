...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_DIAGONALMATRIX_SPARSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_DIAGONALMATRIX_SPARSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DiagonalMatrix<MT, SO, false> : public SparseMatrix<DiagonalMatrix<MT, SO, false>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = DiagonalMatrix<OT, !SO, false>;  //!< Result type with opposite storage order for expression
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO>												   // Storage order of the adapted sparse matrix
inline DiagonalMatrix<MT, SO, false>::DiagonalMatrix() : matrix_() // The adapted sparse matrix
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline DiagonalMatrix<MT, SO, false>::DiagonalMatrix(size_t n) : matrix_(n, n) // The adapted sparse matrix
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::DiagonalMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A{ { 1, 0, 0 },
																	 { 0, 2 },
																	 { 0, 0, 3 } };
   \endcode
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Reference DiagonalMatrix<MT, SO, false>::operator()(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::ConstReference DiagonalMatrix<MT, SO, false>::operator()(size_t i,
																										size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Reference DiagonalMatrix<MT, SO, false>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename DiagonalMatrix<MT, SO, false>::ConstReference DiagonalMatrix<MT, SO, false>::at(size_t i, size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::begin(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::ConstIterator DiagonalMatrix<MT, SO, false>::begin(size_t i) const
{
	return matrix_.begin(i);
}
/*! \endcond */
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::ConstIterator DiagonalMatrix<MT, SO, false>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::end(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::ConstIterator DiagonalMatrix<MT, SO, false>::end(size_t i) const
{
	return matrix_.end(i);
}
/*! \endcond */
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::ConstIterator DiagonalMatrix<MT, SO, false>::cend(size_t i) const
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::DiagonalMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A;
   A = { { 1, 0, 0 },
		 { 0, 2 },
		 { 0, 0, 3 } };
   \endcode
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline DiagonalMatrix<MT, SO, false> &DiagonalMatrix<MT, SO, false>::operator=(const DiagonalMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline DiagonalMatrix<MT, SO, false> &DiagonalMatrix<MT, SO, false>::operator=(DiagonalMatrix &&rhs) noexcept
{
...
template <typename MT2 // Type of the right-hand side matrix
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline auto DiagonalMatrix<MT, SO, false>::operator%=(const Matrix<MT2, SO2> &rhs) -> DiagonalMatrix &
{
	if (!IsSquare_v<MT2> && !isSquare(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to diagonal matrix");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::set(size_t i, size_t j,
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::insert(size_t i, size_t j,
																							  const ElementType &value)
{
	if (i < j) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to upper matrix element");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::erase(size_t i, Iterator pos)
{
	using mtrc::numeric::erase;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::erase(size_t i, Iterator first,
																							 Iterator last)
{
	using mtrc::numeric::erase;
...
// elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::DiagonalMatrix< CompressedMatrix<double,mtrc::numeric::rowMajor> > A;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::find(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename DiagonalMatrix<MT, SO, false>::ConstIterator DiagonalMatrix<MT, SO, false>::find(size_t i, size_t j) const
{
	return matrix_.find(i, j);
}
/*! \endcond */
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::lowerBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::ConstIterator DiagonalMatrix<MT, SO, false>::lowerBound(size_t i,
																									   size_t j) const
{
	return matrix_.lowerBound(i, j);
}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::Iterator DiagonalMatrix<MT, SO, false>::upperBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename DiagonalMatrix<MT, SO, false>::ConstIterator DiagonalMatrix<MT, SO, false>::upperBound(size_t i,
																									   size_t j) const
{
	return matrix_.upperBound(i, j);
}
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::DiagonalMatrix< mtrc::numeric::CompressedMatrix<int> > A;
   // ... Resizing and initialization
...
inline DiagonalMatrix<MT, SO, false> &DiagonalMatrix<MT, SO, false>::scale(const Other &scalar)
{
