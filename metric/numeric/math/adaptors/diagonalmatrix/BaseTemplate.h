...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_DIAGONALMATRIX_BASETEMPLATE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_DIAGONALMATRIX_BASETEMPLATE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// The following examples give an impression of several possible diagonal matrices:
   ...
   mtrc::numeric::DiagonalMatrix< mtrc::numeric::HybridMatrix<float,4UL,4UL,columnMajor> B;

   // Definition of a resizable row-major dense diagonal matrix based on DynamicMatrix
   mtrc::numeric::DiagonalMatrix< mtrc::numeric::DynamicMatrix<double,rowMajor> > C;

   // Definition of a fixed-size row-major dense diagonal matrix based on CustomMatrix
   mtrc::numeric::DiagonalMatrix< mtrc::numeric::CustomMatrix<double,unaligned,unpadded,rowMajor> > D;

   // Definition of a compressed row-major single precision diagonal matrix
   mtrc::numeric::DiagonalMatrix< mtrc::numeric::CompressedMatrix<float,rowMajor> > E;
   \endcode
...
// as mtrc::numeric::rowMajor), the diagonal matrix will also be a row-major matrix. Otherwise, if the
// adapted matrix is column-major (i.e. is specified as mtrc::numeric::columnMajor), the diagonal matrix
...
// thrown:
   ...
   CustomDiagonal B( new double[9UL], 3UL, mtrc::numeric::ArrayDelete() );  // Throws an exception
   \endcode
...
// \n \subsection diagonalmatrix_initialization The Lower and Upper Elements of a Dense Diagonal Matrix are Always
Default Initialized!
//
...
// matrices and sparse matrices for large diagonal matrices:
   ...
   using SmallDiagonalMatrix = mtrc::numeric::DiagonalMatrix< mtrc::numeric::StaticMatrix<float,3UL,3UL> >;

   // Recommendation 2: use sparse matrices for large diagonal matrices
   using LargeDiagonalMatrix = mtrc::numeric::DiagonalMatrix< mtrc::numeric::CompressedMatrix<float> >;
   \endcode
...
//
// The \b Metric numeric library tries to exploit the properties of diagonal matrices whenever and wherever
...
//
// When multiplying two matrices, at least one of which is diagonal, \b Metric numeric can exploit the fact
...
// In this example, \b Metric numeric also exploits the structure of the matrix and performs similarly to
