...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_LOWERMATRIX_DENSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_LOWERMATRIX_DENSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/adaptors/lowermatrix/LowerProxy.h>
...
class LowerMatrix<MT, SO, true> : public DenseMatrix<LowerMatrix<MT, SO, true>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType =
		LowerMatrix<OT, !SO, true>; //!< Result type with opposite storage order for expression template evaluations.
	...
	template <typename Other> inline LowerMatrix(size_t n, const Other *array);

	template <typename Other, size_t N> inline LowerMatrix(const Other (&array)[N][N]);
	...
	template <typename Other, size_t N> inline LowerMatrix &operator=(const Other (&array)[N][N]);

	inline LowerMatrix &operator=(const LowerMatrix &rhs);
	inline LowerMatrix &operator=(LowerMatrix &&rhs) noexcept;

	template <typename MT2, bool SO2>
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> DisableIf_t<IsComputation_v<MT2>, LowerMatrix &>;

	template <typename MT2, bool SO2>
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> EnableIf_t<IsComputation_v<MT2>, LowerMatrix &>;
	...
	template <typename MT2, bool SO2> inline auto operator%=(const Matrix<MT2, SO2> &rhs) -> LowerMatrix &;

	template <typename ST> inline auto operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, LowerMatrix &>;
  ...
  private:
	...
	//**Friend declarations*************************************************************************
	template <typename MT2, bool SO2, bool DF2> friend MT2 &derestrict(LowerMatrix<MT2, SO2, DF2> &m);
	//**********************************************************************************************
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO>											// Storage order of the adapted dense matrix
inline LowerMatrix<MT, SO, true>::LowerMatrix() : matrix_() // The adapted dense matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square lower matrix detected");
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::LowerMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A{ { 1, 0, 0 },
																  { 2, 3 },
																  { 4, 5 } };
   \endcode
...
{
	if (!isLower(matrix_)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of lower matrix");
	}
...
// with a dynamic array:
   ...
   mtrc::numeric::LowerMatrix< mtrc::numeric::DynamicMatrix<int,rowMajor> > v( 4UL, array );
   delete[] array;
   \endcode
...
// with a static array:
   ...
   const int init[3][3] = { { 1, 0, 0 },
							{ 2, 3 },
							{ 4, 5 } };
   mtrc::numeric::LowerMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A( init );
   \endcode
...
inline LowerMatrix<MT, SO, true>::LowerMatrix(const Other (&array)[N][N]) : matrix_(array) // The adapted dense matrix
{
	if (!isLower(matrix_)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of lower matrix");
	}
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline LowerMatrix<MT, SO, true>::LowerMatrix(const LowerMatrix &m) : matrix_(m.matrix_) // The adapted dense matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square lower matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(), "Broken invariant detected");
}
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::Reference LowerMatrix<MT, SO, true>::operator()(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
inline
	typename LowerMatrix<MT, SO, true>::ConstReference LowerMatrix<MT, SO, true>::operator()(size_t i, size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::Reference LowerMatrix<MT, SO, true>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::ConstReference LowerMatrix<MT, SO, true>::at(size_t i, size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::ConstPointer LowerMatrix<MT, SO, true>::data() const noexcept
{
	return matrix_.data();
}
/*! \endcond */
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::ConstPointer LowerMatrix<MT, SO, true>::data(size_t i) const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::Iterator LowerMatrix<MT, SO, true>::begin(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::ConstIterator LowerMatrix<MT, SO, true>::begin(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::ConstIterator LowerMatrix<MT, SO, true>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::Iterator LowerMatrix<MT, SO, true>::end(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::ConstIterator LowerMatrix<MT, SO, true>::end(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename LowerMatrix<MT, SO, true>::ConstIterator LowerMatrix<MT, SO, true>::cend(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline LowerMatrix<MT, SO, true> &LowerMatrix<MT, SO, true>::operator=(const ElementType &rhs)
{
	if (SO) {
		for (size_t j = 0UL; j < columns(); ++j)
			for (size_t i = j; i < rows(); ++i)
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::LowerMatrix< mtrc::numeric::StaticMatrix<int,3UL,3UL,rowMajor> > A;
   A = { { 1, 0, 0 },
		 { 2, 3 },
		 { 4, 5 } };
   \endcode
...
LowerMatrix<MT, SO, true>::operator=(initializer_list<initializer_list<ElementType>> list)
{
	...
	if (!isLower(tmp)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to lower matrix");
	}
...
template <typename Other // Data type of the initialization array
		  ,
		  size_t N> // Number of rows and columns of the initialization array
inline LowerMatrix<MT, SO, true> &LowerMatrix<MT, SO, true>::operator=(const Other (&array)[N][N])
{
	MT tmp(array);
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline LowerMatrix<MT, SO, true> &LowerMatrix<MT, SO, true>::operator=(const LowerMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline LowerMatrix<MT, SO, true> &LowerMatrix<MT, SO, true>::operator=(LowerMatrix &&rhs) noexcept
{
...
template <typename MT2 // Type of the right-hand side matrix
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline auto LowerMatrix<MT, SO, true>::operator%=(const Matrix<MT2, SO2> &rhs) -> LowerMatrix &
{
	if (!IsSquare_v<MT2> && !isSquare(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to lower matrix");
	}
...
inline auto LowerMatrix<MT, SO, true>::operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, LowerMatrix &>
{
	matrix_ *= rhs;
	return *this;
}
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::LowerMatrix< mtrc::numeric::DynamicMatrix<int> > A;
...
METRIC_NUMERIC_ALWAYS_INLINE
	typename LowerMatrix<MT, SO, true>::SIMDType LowerMatrix<MT, SO, true>::load(size_t i, size_t j) const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
METRIC_NUMERIC_ALWAYS_INLINE
	typename LowerMatrix<MT, SO, true>::SIMDType LowerMatrix<MT, SO, true>::loada(size_t i, size_t j) const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
METRIC_NUMERIC_ALWAYS_INLINE
	typename LowerMatrix<MT, SO, true>::SIMDType LowerMatrix<MT, SO, true>::loadu(size_t i, size_t j) const noexcept
{
