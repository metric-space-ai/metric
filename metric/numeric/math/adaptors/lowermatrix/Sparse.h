...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_LOWERMATRIX_SPARSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_LOWERMATRIX_SPARSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class LowerMatrix<MT, SO, false> : public SparseMatrix<LowerMatrix<MT, SO, false>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType =
		LowerMatrix<OT, !SO, false>; //!< Result type with opposite storage order for expression template evaluations.
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO>											 // Storage order of the adapted sparse matrix
inline LowerMatrix<MT, SO, false>::LowerMatrix() : matrix_() // The adapted sparse matrix
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO>														 // Storage order of the adapted sparse matrix
inline LowerMatrix<MT, SO, false>::LowerMatrix(size_t n) : matrix_(n, n) // The adapted sparse matrix
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::LowerMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A{ { 1, 0, 0 },
																  { 2, 3 },
																  { 4, 5 } };
   \endcode
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline LowerMatrix<MT, SO, false>::LowerMatrix(const LowerMatrix &m) : matrix_(m.matrix_) // The adapted sparse matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square lower matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(), "Broken invariant detected");
}
...
inline LowerMatrix<MT, SO, false>::LowerMatrix(const Matrix<MT2, SO2> &m) : matrix_(*m) // The adapted sparse matrix
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Reference LowerMatrix<MT, SO, false>::operator()(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename LowerMatrix<MT, SO, false>::ConstReference LowerMatrix<MT, SO, false>::operator()(size_t i, size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Reference LowerMatrix<MT, SO, false>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::ConstReference LowerMatrix<MT, SO, false>::at(size_t i, size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::begin(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::ConstIterator LowerMatrix<MT, SO, false>::begin(size_t i) const
{
	return matrix_.begin(i);
}
/*! \endcond */
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::ConstIterator LowerMatrix<MT, SO, false>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::end(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::ConstIterator LowerMatrix<MT, SO, false>::end(size_t i) const
{
	return matrix_.end(i);
}
/*! \endcond */
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::ConstIterator LowerMatrix<MT, SO, false>::cend(size_t i) const
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::LowerMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A;
   A = { { 1, 0, 0 },
		 { 2, 3 },
		 { 4, 5 } };
   \endcode
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline LowerMatrix<MT, SO, false> &LowerMatrix<MT, SO, false>::operator=(const LowerMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline LowerMatrix<MT, SO, false> &LowerMatrix<MT, SO, false>::operator=(LowerMatrix &&rhs) noexcept
{
...
template <typename MT2 // Type of the right-hand side matrix
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline auto LowerMatrix<MT, SO, false>::operator%=(const Matrix<MT2, SO2> &rhs) -> LowerMatrix &
{
	if (!IsSquare_v<MT2> && !isSquare(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to lower matrix");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::set(size_t i, size_t j,
																					 const ElementType &value)
{
	if (i < j) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to upper matrix element");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::insert(size_t i, size_t j,
																						const ElementType &value)
{
	if (i < j) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to upper matrix element");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::erase(size_t i, Iterator pos)
{
	using mtrc::numeric::erase;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::erase(size_t i, Iterator first,
																					   Iterator last)
{
	using mtrc::numeric::erase;
...
// elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::LowerMatrix< CompressedMatrix<double,mtrc::numeric::rowMajor> > A;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::find(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::ConstIterator LowerMatrix<MT, SO, false>::find(size_t i, size_t j) const
{
	return matrix_.find(i, j);
}
/*! \endcond */
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::lowerBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename LowerMatrix<MT, SO, false>::ConstIterator LowerMatrix<MT, SO, false>::lowerBound(size_t i, size_t j) const
{
	return matrix_.lowerBound(i, j);
}
/*! \endcond */
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename LowerMatrix<MT, SO, false>::Iterator LowerMatrix<MT, SO, false>::upperBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename LowerMatrix<MT, SO, false>::ConstIterator LowerMatrix<MT, SO, false>::upperBound(size_t i, size_t j) const
{
	return matrix_.upperBound(i, j);
}
/*! \endcond */
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::LowerMatrix< mtrc::numeric::CompressedMatrix<int> > A;
   // ... Resizing and initialization
...
inline LowerMatrix<MT, SO, false> &LowerMatrix<MT, SO, false>::scale(const Other &scalar)
{
