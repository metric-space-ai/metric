...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_LOWERMATRIX_BASETEMPLATE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_LOWERMATRIX_BASETEMPLATE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// The following examples give an impression of several possible lower matrices:
   ...
   mtrc::numeric::LowerMatrix< mtrc::numeric::HybridMatrix<float,4UL,4UL,columnMajor> B;

   // Definition of a resizable row-major dense lower matrix based on DynamicMatrix
   mtrc::numeric::LowerMatrix< mtrc::numeric::DynamicMatrix<double,rowMajor> > C;

   // Definition of a fixed-size row-major dense lower matrix based on CustomMatrix
   mtrc::numeric::LowerMatrix< mtrc::numeric::CustomMatrix<double,unaligned,unpadded,rowMajor> > D;

   // Definition of a compressed row-major single precision lower matrix
   mtrc::numeric::LowerMatrix< mtrc::numeric::CompressedMatrix<float,rowMajor> > E;
   \endcode
...
// as mtrc::numeric::rowMajor), the lower matrix will also be a row-major matrix. Otherwise, if the
// adapted matrix is column-major (i.e. is specified as mtrc::numeric::columnMajor), the lower matrix
...
// of elements does not represent a lower matrix, a \a std::invalid_argument exception is thrown:
   ...
   CustomLower B( new double[9UL], 3UL, mtrc::numeric::ArrayDelete() );  // Throws an exception
   \endcode
...
//
// The \b Metric numeric library tries to exploit the properties of lower matrices whenever and wherever
...
//
// When multiplying two matrices, at least one of which is lower triangular, \b Metric numeric can exploit
