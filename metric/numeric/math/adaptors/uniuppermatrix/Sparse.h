...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_UNIUPPERMATRIX_SPARSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_UNIUPPERMATRIX_SPARSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/adaptors/Forward.h>
#include <metric/numeric/math/adaptors/uniuppermatrix/BaseTemplate.h>
#include <metric/numeric/math/adaptors/uniuppermatrix/UniUpperElement.h>
#include <metric/numeric/math/adaptors/uniuppermatrix/UniUpperProxy.h>
#include <metric/numeric/math/adaptors/uniuppermatrix/UniUpperValue.h>
...
class UniUpperMatrix<MT, SO, false> : public SparseMatrix<UniUpperMatrix<MT, SO, false>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = UniUpperMatrix<OT, !SO, false>;  //!< Result type with opposite storage order for expression
	...
	class Iterator {
	  public:
		...
		*/
		inline ReferenceType operator*() const { return ReferenceType(pos_, pos_->index() == index_); }
		//*******************************************************************************************
		...
		*/
		inline PointerType operator->() const { return PointerType(pos_, pos_->index() == index_); }
		//*******************************************************************************************
	...
	template <typename MT2, bool SO2> inline UniUpperMatrix(const Matrix<MT2, SO2> &m);
	//@}
	//**********************************************************************************************
	...
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> DisableIf_t<IsComputation_v<MT2>, UniUpperMatrix &>;

	template <typename MT2, bool SO2>
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> EnableIf_t<IsComputation_v<MT2>, UniUpperMatrix &>;
	...
	template <typename MT2, bool SO2> inline auto operator%=(const Matrix<MT2, SO2> &rhs) -> UniUpperMatrix &;
	//@}
	//**********************************************************************************************
  ...
  private:
	...
	//**Friend declarations*************************************************************************
	template <typename MT2, bool SO2, bool DF2> friend MT2 &derestrict(UniUpperMatrix<MT2, SO2, DF2> &m);
	//**********************************************************************************************
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_UNIFORM_TYPE(MT);
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO>												   // Storage order of the adapted sparse matrix
inline UniUpperMatrix<MT, SO, false>::UniUpperMatrix() : matrix_() // The adapted sparse matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square uniupper matrix detected");
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline UniUpperMatrix<MT, SO, false>::UniUpperMatrix(size_t n) : matrix_(n, n, n) // The adapted sparse matrix
{
	...
	: matrix_(n, n, nonzeros) // The adapted sparse matrix
{
	...
	for (size_t i = 0UL; i < n; ++i) {
		if (nonzeros[i] == 0UL) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid capacity specification");
		}
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::UniUpperMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A{ { 1, 2, 3 },
																	 { 0, 1 },
																	 { 0, 0, 1 } };
   \endcode
	...
	: matrix_(list) // The adapted sparse matrix
{
	if (!isUniUpper(matrix_)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of uniupper matrix");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Reference UniUpperMatrix<MT, SO, false>::operator()(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::ConstReference UniUpperMatrix<MT, SO, false>::operator()(size_t i,
																										size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Reference UniUpperMatrix<MT, SO, false>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename UniUpperMatrix<MT, SO, false>::ConstReference UniUpperMatrix<MT, SO, false>::at(size_t i, size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::begin(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::ConstIterator UniUpperMatrix<MT, SO, false>::begin(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::ConstIterator UniUpperMatrix<MT, SO, false>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::end(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::ConstIterator UniUpperMatrix<MT, SO, false>::end(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::ConstIterator UniUpperMatrix<MT, SO, false>::cend(size_t i) const
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::UniUpperMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A;
   A = { { 1, 2, 3 },
		 { 0, 1 },
		 { 0, 0, 1 } };
   \endcode
...
UniUpperMatrix<MT, SO, false>::operator=(initializer_list<initializer_list<ElementType>> list)
{
	...
	if (!isUniUpper(tmp)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to uniupper matrix");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline UniUpperMatrix<MT, SO, false> &UniUpperMatrix<MT, SO, false>::operator=(const UniUpperMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline UniUpperMatrix<MT, SO, false> &UniUpperMatrix<MT, SO, false>::operator=(UniUpperMatrix &&rhs) noexcept
{
...
inline auto UniUpperMatrix<MT, SO, false>::operator+=(const Matrix<MT2, SO2> &rhs)
	-> DisableIf_t<IsComputation_v<MT2>, UniUpperMatrix &>
{
	if (IsLower_v<MT2> || IsUniTriangular_v<MT2> || (!IsStrictlyUpper_v<MT2> && !isStrictlyUpper(*rhs))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to uniupper matrix");
	}
...
inline auto UniUpperMatrix<MT, SO, false>::operator+=(const Matrix<MT2, SO2> &rhs)
	-> EnableIf_t<IsComputation_v<MT2>, UniUpperMatrix &>
{
	if (IsLower_v<MT2> || IsUniTriangular_v<MT2> || (IsSquare_v<MT2> && !isSquare(*rhs))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to uniupper matrix");
	}
...
template <typename MT2 // Type of the right-hand side matrix
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline auto UniUpperMatrix<MT, SO, false>::operator%=(const Matrix<MT2, SO2> &rhs) -> UniUpperMatrix &
{
...
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_STATIC_TYPE(MT);
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::set(size_t i, size_t j,
																						   const ElementType &value)
{
	if (i >= j) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to diagonal or lower matrix element");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::insert(size_t i, size_t j,
																							  const ElementType &value)
{
	if (i >= j) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to diagonal or lower matrix element");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::erase(size_t i, Iterator pos)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::erase(size_t i, Iterator first,
																							 Iterator last)
{
	if (first == last)
		return last;

	if ((!SO && first.base() == matrix_.begin(i)) || (SO && last.base() == matrix_.end(i))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to diagonal matrix element");
	}
...
// elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::UniUpperMatrix< CompressedMatrix<double,mtrc::numeric::rowMajor> > A;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::find(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename UniUpperMatrix<MT, SO, false>::ConstIterator UniUpperMatrix<MT, SO, false>::find(size_t i, size_t j) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::lowerBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::ConstIterator UniUpperMatrix<MT, SO, false>::lowerBound(size_t i,
																									   size_t j) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::Iterator UniUpperMatrix<MT, SO, false>::upperBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniUpperMatrix<MT, SO, false>::ConstIterator UniUpperMatrix<MT, SO, false>::upperBound(size_t i,
																									   size_t j) const
{
