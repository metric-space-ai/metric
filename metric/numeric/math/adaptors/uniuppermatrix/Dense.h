...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_UNIUPPERMATRIX_DENSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_UNIUPPERMATRIX_DENSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class UniUpperMatrix<MT, SO, true> : public DenseMatrix<UniUpperMatrix<MT, SO, true>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType =
		UniUpperMatrix<OT, !SO, true>; //!< Result type with opposite storage order for expression template evaluations.
	...
	template <typename Other> inline UniUpperMatrix(size_t n, const Other *array);

	template <typename Other, size_t N> inline UniUpperMatrix(const Other (&array)[N][N]);
	...
	template <typename Other, size_t N> inline UniUpperMatrix &operator=(const Other (&array)[N][N]);
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO>												  // Storage order of the adapted dense matrix
inline UniUpperMatrix<MT, SO, true>::UniUpperMatrix() : matrix_() // The adapted dense matrix
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::UniUpperMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A{ { 1, 2, 3 },
																	 { 0, 1 },
																	 { 0, 0, 1 } };
   \endcode
...
// with a dynamic array:
   ...
   mtrc::numeric::UniUpperMatrix< mtrc::numeric::DynamicMatrix<int,rowMajor> > v( 4UL, array );
   delete[] array;
   \endcode
...
// with a static array:
   ...
   const int init[3][3] = { { 1, 2, 3 },
							{ 0, 1 },
							{ 0, 0, 1 } };
   mtrc::numeric::UniUpperMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A( init );
   \endcode
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::Reference UniUpperMatrix<MT, SO, true>::operator()(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::ConstReference UniUpperMatrix<MT, SO, true>::operator()(size_t i,
																									  size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::Reference UniUpperMatrix<MT, SO, true>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::ConstReference UniUpperMatrix<MT, SO, true>::at(size_t i, size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::ConstPointer UniUpperMatrix<MT, SO, true>::data() const noexcept
{
	return matrix_.data();
}
/*! \endcond */
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::ConstPointer UniUpperMatrix<MT, SO, true>::data(size_t i) const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::Iterator UniUpperMatrix<MT, SO, true>::begin(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::ConstIterator UniUpperMatrix<MT, SO, true>::begin(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::ConstIterator UniUpperMatrix<MT, SO, true>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::Iterator UniUpperMatrix<MT, SO, true>::end(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::ConstIterator UniUpperMatrix<MT, SO, true>::end(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UniUpperMatrix<MT, SO, true>::ConstIterator UniUpperMatrix<MT, SO, true>::cend(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline UniUpperMatrix<MT, SO, true> &UniUpperMatrix<MT, SO, true>::operator=(const ElementType &rhs)
{
	if (SO) {
...
template <typename Other // Data type of the initialization array
		  ,
		  size_t N> // Number of rows and columns of the initialization array
inline UniUpperMatrix<MT, SO, true> &UniUpperMatrix<MT, SO, true>::operator=(const Other (&array)[N][N])
{
	MT tmp(array);
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline UniUpperMatrix<MT, SO, true> &UniUpperMatrix<MT, SO, true>::operator=(const UniUpperMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline UniUpperMatrix<MT, SO, true> &UniUpperMatrix<MT, SO, true>::operator=(UniUpperMatrix &&rhs) noexcept
{
...
template <typename MT2 // Type of the right-hand side matrix
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline auto UniUpperMatrix<MT, SO, true>::operator%=(const Matrix<MT2, SO2> &rhs) -> UniUpperMatrix &
{
...
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniUpperMatrix<MT, SO, true>::SIMDType UniUpperMatrix<MT, SO, true>::load(size_t i,
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniUpperMatrix<MT, SO, true>::SIMDType UniUpperMatrix<MT, SO, true>::loada(size_t i,
																						size_t j) const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniUpperMatrix<MT, SO, true>::SIMDType UniUpperMatrix<MT, SO, true>::loadu(size_t i,
																						size_t j) const noexcept
{
