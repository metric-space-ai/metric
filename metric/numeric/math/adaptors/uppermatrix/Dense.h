...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_UPPERMATRIX_DENSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_UPPERMATRIX_DENSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/adaptors/uppermatrix/UpperProxy.h>
...
class UpperMatrix<MT, SO, true> : public DenseMatrix<UpperMatrix<MT, SO, true>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType =
		UpperMatrix<OT, !SO, true>; //!< Result type with opposite storage order for expression template evaluations.
	...
	template <typename Other> inline UpperMatrix(size_t n, const Other *array);

	template <typename Other, size_t N> inline UpperMatrix(const Other (&array)[N][N]);
	...
	template <typename Other, size_t N> inline UpperMatrix &operator=(const Other (&array)[N][N]);

	inline UpperMatrix &operator=(const UpperMatrix &rhs);
	inline UpperMatrix &operator=(UpperMatrix &&rhs) noexcept;

	template <typename MT2, bool SO2>
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> DisableIf_t<IsComputation_v<MT2>, UpperMatrix &>;

	template <typename MT2, bool SO2>
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> EnableIf_t<IsComputation_v<MT2>, UpperMatrix &>;
	...
	template <typename MT2, bool SO2> inline auto operator%=(const Matrix<MT2, SO2> &rhs) -> UpperMatrix &;

	template <typename ST> inline auto operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, UpperMatrix &>;
  ...
  private:
	...
	//**Friend declarations*************************************************************************
	template <typename MT2, bool SO2, bool DF2> friend MT2 &derestrict(UpperMatrix<MT2, SO2, DF2> &m);
	//**********************************************************************************************
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO>											// Storage order of the adapted dense matrix
inline UpperMatrix<MT, SO, true>::UpperMatrix() : matrix_() // The adapted dense matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square upper matrix detected");
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::UpperMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A{ { 1, 2, 3 },
																  { 0, 4 },
																  { 0, 0, 6 } };
   \endcode
...
{
	if (!isUpper(matrix_)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of upper matrix");
	}
...
// with a dynamic array:
   ...
   mtrc::numeric::UpperMatrix< mtrc::numeric::DynamicMatrix<int,rowMajor> > v( 4UL, array );
   delete[] array;
   \endcode
...
// with a static array:
   ...
   const int init[3][3] = { { 1, 2, 3 },
							{ 0, 4 },
							{ 0, 0, 6 } };
   mtrc::numeric::UpperMatrix< mtrc::numeric::StaticMatrix<int,3,3,rowMajor> > A( init );
   \endcode
...
inline UpperMatrix<MT, SO, true>::UpperMatrix(const Other (&array)[N][N]) : matrix_(array) // The adapted dense matrix
{
	if (!isUpper(matrix_)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of upper matrix");
	}
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline UpperMatrix<MT, SO, true>::UpperMatrix(const UpperMatrix &m) : matrix_(m.matrix_) // The adapted dense matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square upper matrix detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(), "Broken invariant detected");
}
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::Reference UpperMatrix<MT, SO, true>::operator()(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
inline
	typename UpperMatrix<MT, SO, true>::ConstReference UpperMatrix<MT, SO, true>::operator()(size_t i, size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::Reference UpperMatrix<MT, SO, true>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::ConstReference UpperMatrix<MT, SO, true>::at(size_t i, size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::ConstPointer UpperMatrix<MT, SO, true>::data() const noexcept
{
	return matrix_.data();
}
/*! \endcond */
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::ConstPointer UpperMatrix<MT, SO, true>::data(size_t i) const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::Iterator UpperMatrix<MT, SO, true>::begin(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::ConstIterator UpperMatrix<MT, SO, true>::begin(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::ConstIterator UpperMatrix<MT, SO, true>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::Iterator UpperMatrix<MT, SO, true>::end(size_t i)
{
	if (SO)
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::ConstIterator UpperMatrix<MT, SO, true>::end(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline typename UpperMatrix<MT, SO, true>::ConstIterator UpperMatrix<MT, SO, true>::cend(size_t i) const
{
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline UpperMatrix<MT, SO, true> &UpperMatrix<MT, SO, true>::operator=(const ElementType &rhs)
{
	if (SO) {
		for (size_t j = 0UL; j < columns(); ++j)
			for (size_t i = 0UL; i <= j; ++i)
...
UpperMatrix<MT, SO, true>::operator=(initializer_list<initializer_list<ElementType>> list)
{
	...
	if (!isUpper(tmp)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to upper matrix");
	}
...
template <typename Other // Data type of the initialization array
		  ,
		  size_t N> // Number of rows and columns of the initialization array
inline UpperMatrix<MT, SO, true> &UpperMatrix<MT, SO, true>::operator=(const Other (&array)[N][N])
{
	MT tmp(array);
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline UpperMatrix<MT, SO, true> &UpperMatrix<MT, SO, true>::operator=(const UpperMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted dense matrix
		  ,
		  bool SO> // Storage order of the adapted dense matrix
inline UpperMatrix<MT, SO, true> &UpperMatrix<MT, SO, true>::operator=(UpperMatrix &&rhs) noexcept
{
...
template <typename MT2 // Type of the right-hand side matrix
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline auto UpperMatrix<MT, SO, true>::operator%=(const Matrix<MT2, SO2> &rhs) -> UpperMatrix &
{
	if (!IsSquare_v<MT2> && !isSquare(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to upper matrix");
	}
...
inline auto UpperMatrix<MT, SO, true>::operator*=(ST rhs) -> EnableIf_t<IsScalar_v<ST>, UpperMatrix &>
{
	matrix_ *= rhs;
	return *this;
}
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::UpperMatrix< mtrc::numeric::DynamicMatrix<int> > A;
...
METRIC_NUMERIC_ALWAYS_INLINE
	typename UpperMatrix<MT, SO, true>::SIMDType UpperMatrix<MT, SO, true>::load(size_t i, size_t j) const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
METRIC_NUMERIC_ALWAYS_INLINE
	typename UpperMatrix<MT, SO, true>::SIMDType UpperMatrix<MT, SO, true>::loada(size_t i, size_t j) const noexcept
{
...
template <typename MT // Type of the adapted dense matrix
		  ...
		  bool SO> // Storage order of the adapted dense matrix
METRIC_NUMERIC_ALWAYS_INLINE
	typename UpperMatrix<MT, SO, true>::SIMDType UpperMatrix<MT, SO, true>::loadu(size_t i, size_t j) const noexcept
{
