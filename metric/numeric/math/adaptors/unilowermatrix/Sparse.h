...
#ifndef METRIC_NUMERIC_MATH_ADAPTORS_UNILOWERMATRIX_SPARSE_H
#define METRIC_NUMERIC_MATH_ADAPTORS_UNILOWERMATRIX_SPARSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/adaptors/unilowermatrix/UniLowerElement.h>
#include <metric/numeric/math/adaptors/unilowermatrix/UniLowerProxy.h>
...
class UniLowerMatrix<MT, SO, false> : public SparseMatrix<UniLowerMatrix<MT, SO, false>, SO> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using OppositeType = UniLowerMatrix<OT, !SO, false>;  //!< Result type with opposite storage order for expression
	...
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> DisableIf_t<IsComputation_v<MT2>, UniLowerMatrix &>;

	template <typename MT2, bool SO2>
	inline auto operator=(const Matrix<MT2, SO2> &rhs) -> EnableIf_t<IsComputation_v<MT2>, UniLowerMatrix &>;
	...
	template <typename MT2, bool SO2> inline auto operator%=(const Matrix<MT2, SO2> &rhs) -> UniLowerMatrix &;
	//@}
	//**********************************************************************************************
  ...
  private:
	...
	//**Friend declarations*************************************************************************
	template <typename MT2, bool SO2, bool DF2> friend MT2 &derestrict(UniLowerMatrix<MT2, SO2, DF2> &m);
	//**********************************************************************************************
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO>												   // Storage order of the adapted sparse matrix
inline UniLowerMatrix<MT, SO, false>::UniLowerMatrix() : matrix_() // The adapted sparse matrix
{
	METRIC_NUMERIC_INTERNAL_ASSERT(isSquare(matrix_), "Non-square unilower matrix detected");
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline UniLowerMatrix<MT, SO, false>::UniLowerMatrix(size_t n) : matrix_(n, n, n) // The adapted sparse matrix
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::UniLowerMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A{ { 1, 0, 0 },
																	 { 2, 1 },
																	 { 4, 5, 1 } };
   \endcode
	...
	: matrix_(list) // The adapted sparse matrix
{
	if (!isUniLower(matrix_)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of unilower matrix");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Reference UniLowerMatrix<MT, SO, false>::operator()(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::ConstReference UniLowerMatrix<MT, SO, false>::operator()(size_t i,
																										size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Reference UniLowerMatrix<MT, SO, false>::at(size_t i, size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename UniLowerMatrix<MT, SO, false>::ConstReference UniLowerMatrix<MT, SO, false>::at(size_t i, size_t j) const
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::begin(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::ConstIterator UniLowerMatrix<MT, SO, false>::begin(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::ConstIterator UniLowerMatrix<MT, SO, false>::cbegin(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::end(size_t i)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::ConstIterator UniLowerMatrix<MT, SO, false>::end(size_t i) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::ConstIterator UniLowerMatrix<MT, SO, false>::cend(size_t i) const
{
...
// matrix by means of an initializer list:
   ...
   mtrc::numeric::UniLowerMatrix< mtrc::numeric::CompressedMatrix<int,rowMajor> > A;
   A = { { 1, 0, 0 },
		 { 2, 1 },
		 { 4, 5, 1 } };
   \endcode
...
UniLowerMatrix<MT, SO, false>::operator=(initializer_list<initializer_list<ElementType>> list)
{
	...
	if (!isUniLower(tmp)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to unilower matrix");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline UniLowerMatrix<MT, SO, false> &UniLowerMatrix<MT, SO, false>::operator=(const UniLowerMatrix &rhs)
{
	matrix_ = rhs.matrix_;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline UniLowerMatrix<MT, SO, false> &UniLowerMatrix<MT, SO, false>::operator=(UniLowerMatrix &&rhs) noexcept
{
...
inline auto UniLowerMatrix<MT, SO, false>::operator+=(const Matrix<MT2, SO2> &rhs)
	-> DisableIf_t<IsComputation_v<MT2>, UniLowerMatrix &>
{
	if (IsUpper_v<MT2> || IsUniTriangular_v<MT2> || (!IsStrictlyLower_v<MT2> && !isStrictlyLower(*rhs))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to unilower matrix");
	}
...
inline auto UniLowerMatrix<MT, SO, false>::operator+=(const Matrix<MT2, SO2> &rhs)
	-> EnableIf_t<IsComputation_v<MT2>, UniLowerMatrix &>
{
	if (IsUpper_v<MT2> || IsUniTriangular_v<MT2> || (!IsSquare_v<MT2> && !isSquare(*rhs))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to unilower matrix");
	}
...
template <typename MT2 // Type of the right-hand side matrix
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline auto UniLowerMatrix<MT, SO, false>::operator%=(const Matrix<MT2, SO2> &rhs) -> UniLowerMatrix &
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::set(size_t i, size_t j,
																						   const ElementType &value)
{
	if (i <= j) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to diagonal or upper matrix element");
	}
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::insert(size_t i, size_t j,
																							  const ElementType &value)
{
	if (i <= j) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to diagonal or upper matrix element");
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::erase(size_t i, Iterator pos)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::erase(size_t i, Iterator first,
																							 Iterator last)
{
	if (first == last)
		return last;

	if ((!SO && last.base() == matrix_.end(i)) || (SO && first.base() == matrix_.begin(i))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to diagonal matrix element");
	}
...
// elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::UniLowerMatrix< CompressedMatrix<double,mtrc::numeric::rowMajor> > A;
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::find(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ...
		  bool SO> // Storage order of the adapted sparse matrix
inline
	typename UniLowerMatrix<MT, SO, false>::ConstIterator UniLowerMatrix<MT, SO, false>::find(size_t i, size_t j) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::lowerBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::ConstIterator UniLowerMatrix<MT, SO, false>::lowerBound(size_t i,
																									   size_t j) const
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::Iterator UniLowerMatrix<MT, SO, false>::upperBound(size_t i, size_t j)
{
...
template <typename MT // Type of the adapted sparse matrix
		  ,
		  bool SO> // Storage order of the adapted sparse matrix
inline typename UniLowerMatrix<MT, SO, false>::ConstIterator UniLowerMatrix<MT, SO, false>::upperBound(size_t i,
																									   size_t j) const
{
