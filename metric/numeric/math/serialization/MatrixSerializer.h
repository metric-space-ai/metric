...
#ifndef METRIC_NUMERIC_MATH_SERIALIZATION_MATRIXSERIALIZER_H
#define METRIC_NUMERIC_MATH_SERIALIZATION_MATRIXSERIALIZER_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// the (de-)serialization process of matrices:
   ...
   {
	  mtrc::numeric::StaticMatrix<double,3UL,5UL,rowMajor> D;
	  mtrc::numeric::CompressedMatrix<int,columnMajor> S;

	  // ... Resizing and initialization

	  // Creating an archive that writes into a the file "matrices.metric/numeric"
	  mtrc::numeric::Archive<std::ofstream> archive( "matrices.metric/numeric" );
   ...
   {
	  mtrc::numeric::DynamicMatrix<double,rowMajor> D1;
	  mtrc::numeric::DynamicMatrix<int,rowMajor> D2;

	  // Creating an archive that reads from the file "matrices.metric/numeric"
	  mtrc::numeric::Archive<std::ofstream> archive( "matrices.metric/numeric" );
...
// Note that it is even possible to (de-)serialize matrices with vector or matrix elements:
   ...
   // Serialization
   {
	  mtrc::numeric::CompressedMatrix< mtrc::numeric::DynamicMatrix< mtrc::numeric::complex<double> > > mat;

	  // ... Resizing and initialization

	  // Creating an archive that writes into a the file "matrix.metric/numeric"
	  mtrc::numeric::Archive<std::ofstream> archive( "matrix.metric/numeric" );
   ...
   // Deserialization
   {
	  ...
	  // Creating an archive that reads from the file "matrix.metric/numeric"
	  mtrc::numeric::Archive<std::ofstream> archive( "matrix.metric/numeric" );
...
class MatrixSerializer {
  private:
	...
	// mapping is used:
	   ...
	   \endcode
	*/
	template <bool IsDenseMatrix, bool IsRowMajorMatrix> struct MatrixValueMappingHelper;
	/*! \endcond */
	//**********************************************************************************************
	...
	*/
	template <typename T> struct MatrixValueMapping {
		enum { value = MatrixValueMappingHelper<IsDenseMatrix_v<T>, IsRowMajorMatrix_v<T>>::value };
		METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATRIX_TYPE(T);
	};
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	//@{
	template <typename Archive, typename MT, bool SO> void serialize(Archive &archive, const Matrix<MT, SO> &mat);
	//@}
	//**********************************************************************************************
	...
	//@{
	template <typename Archive, typename MT, bool SO> void deserialize(Archive &archive, Matrix<MT, SO> &mat);
	//@}
	//**********************************************************************************************
  ...
  private:
	...
	/*!\name Serialization functions */
	//@{
	template <typename Archive, typename MT> void serializeHeader(Archive &archive, const MT &mat);
	...
	/*!\name Deserialization functions */
	//@{
	template <typename Archive, typename MT> void deserializeHeader(Archive &archive, const MT &mat);

	template <typename MT, bool SO> DisableIf_t<IsResizable_v<MT>> prepareMatrix(DenseMatrix<MT, SO> &mat);

	template <typename MT, bool SO> DisableIf_t<IsResizable_v<MT>> prepareMatrix(SparseMatrix<MT, SO> &mat);

	template <typename MT> EnableIf_t<IsResizable_v<MT>> prepareMatrix(MT &mat);

	template <typename Archive, typename MT> void deserializeMatrix(Archive &archive, MT &mat);
	...
	template <typename Archive, typename MT, bool SO>
	DisableIf_t<IsNumeric_v<ElementType_t<MT>>> deserializeDenseColumnMatrix(Archive &archive,
																			 SparseMatrix<MT, SO> &mat);

	template <typename Archive, typename MT, bool SO>
	EnableIf_t<IsNumeric_v<ElementType_t<MT>>> deserializeDenseColumnMatrix(Archive &archive,
																			SparseMatrix<MT, SO> &mat);
...
void MatrixSerializer::serializeMatrix(Archive &archive, const DenseMatrix<MT, SO> &mat)
{
	...
	if (!archive) {
		METRIC_NUMERIC_THROW_RUNTIME_ERROR("Dense matrix could not be serialized");
	}
}
//*************************************************************************************************
...
void MatrixSerializer::serializeMatrix(Archive &archive, const SparseMatrix<MT, SO> &mat)
{
	...
	if (!archive) {
		METRIC_NUMERIC_THROW_RUNTIME_ERROR("Sparse matrix could not be serialized");
	}
}
//*************************************************************************************************
...
void MatrixSerializer::deserializeHeader(Archive &archive, const MT &mat)
{
	...
	if (!(archive >> version_ >> type_ >> elementType_ >> elementSize_ >> rows_ >> columns_ >> number_)) {
		...
		METRIC_NUMERIC_THROW_RUNTIME_ERROR("Invalid matrix type detected");
		...
		METRIC_NUMERIC_THROW_RUNTIME_ERROR("Invalid matrix size detected");
...
template <typename Archive // Type of the archive
		  ,
		  typename MT> // Type of the matrix
EnableIf_t<MT::simdEnabled> MatrixSerializer::deserializeDenseRowMatrix(Archive &archive,
																		DenseMatrix<MT, rowMajor> &mat)
{
	...
	if (!archive) {
		METRIC_NUMERIC_THROW_RUNTIME_ERROR("Dense matrix could not be deserialized");
	}
}
//*************************************************************************************************
...
template <typename Archive // Type of the archive
		  ...
		  ,
		  bool SO> // Storage order
EnableIf_t<IsNumeric_v<ElementType_t<MT>>> MatrixSerializer::deserializeDenseRowMatrix(Archive &archive,
																					   SparseMatrix<MT, SO> &mat)
{
	...
	if (!archive) {
		METRIC_NUMERIC_THROW_RUNTIME_ERROR("Sparse matrix could not be deserialized");
	}
}
//*************************************************************************************************
...
template <typename Archive // Type of the archive
		  ...
		  ,
		  bool SO> // Storage order
DisableIf_t<IsNumeric_v<ElementType_t<MT>>> MatrixSerializer::deserializeDenseRowMatrix(Archive &archive,
																						SparseMatrix<MT, SO> &mat)
{
	using ET = ElementType_t<MT>;
...
template <typename Archive // Type of the archive
		  ,
		  typename MT> // Type of the matrix
EnableIf_t<MT::simdEnabled> MatrixSerializer::deserializeDenseColumnMatrix(Archive &archive,
...
template <typename Archive // Type of the archive
		  ...
		  ,
		  bool SO> // Storage order
EnableIf_t<IsNumeric_v<ElementType_t<MT>>> MatrixSerializer::deserializeDenseColumnMatrix(Archive &archive,
																						  SparseMatrix<MT, SO> &mat)
{
...
template <typename Archive // Type of the archive
		  ...
		  ,
		  bool SO> // Storage order
DisableIf_t<IsNumeric_v<ElementType_t<MT>>> MatrixSerializer::deserializeDenseColumnMatrix(Archive &archive,
																						   SparseMatrix<MT, SO> &mat)
{
	using ET = ElementType_t<MT>;
 ...
 */
template <> struct MatrixSerializer::MatrixValueMappingHelper<true, true> {
	enum { value = 1 };
};
/*! \endcond */
//*************************************************************************************************
 ...
 */
template <> struct MatrixSerializer::MatrixValueMappingHelper<true, false> {
	enum { value = 5 };
};
/*! \endcond */
//*************************************************************************************************
 ...
 */
template <> struct MatrixSerializer::MatrixValueMappingHelper<false, true> {
	enum { value = 3 };
};
/*! \endcond */
//*************************************************************************************************
 ...
 */
template <> struct MatrixSerializer::MatrixValueMappingHelper<false, false> {
	enum { value = 7 };
};
/*! \endcond */
//*************************************************************************************************
