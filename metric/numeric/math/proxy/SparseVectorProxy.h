...
#ifndef METRIC_NUMERIC_MATH_PROXY_SPARSEVECTORPROXY_H
#define METRIC_NUMERIC_MATH_PROXY_SPARSEVECTORPROXY_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SparseVectorProxy : public SparseVector<PT, IsRowVector_v<VT>> {
  public:
	//**Type definitions****************************************************************************
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
inline typename SparseVectorProxy<PT, VT>::Reference SparseVectorProxy<PT, VT>::operator[](size_t index) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
inline typename SparseVectorProxy<PT, VT>::Reference SparseVectorProxy<PT, VT>::at(size_t index) const
{
	if ((**this).isRestricted()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to restricted element");
	}
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
inline typename SparseVectorProxy<PT, VT>::Iterator SparseVectorProxy<PT, VT>::set(size_t index,
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
inline typename SparseVectorProxy<PT, VT>::Iterator SparseVectorProxy<PT, VT>::insert(size_t index,
																					  const ElementType &value) const
{
	if ((**this).isRestricted()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to restricted element");
...
template <typename PT // Type of the proxy
		  ...
		  typename VT> // Type of the sparse vector
inline
	typename SparseVectorProxy<PT, VT>::Iterator SparseVectorProxy<PT, VT>::erase(Iterator first, Iterator last) const
{
	if ((**this).isRestricted()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to restricted element");
	}
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
inline typename SparseVectorProxy<PT, VT>::Iterator SparseVectorProxy<PT, VT>::find(size_t index) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
inline typename SparseVectorProxy<PT, VT>::Iterator SparseVectorProxy<PT, VT>::lowerBound(size_t index) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
inline typename SparseVectorProxy<PT, VT>::Iterator SparseVectorProxy<PT, VT>::upperBound(size_t index) const
{
...
typename SparseVectorProxy<PT, VT>::Iterator begin(const SparseVectorProxy<PT, VT> &proxy);

template <typename PT, typename VT>
typename SparseVectorProxy<PT, VT>::ConstIterator cbegin(const SparseVectorProxy<PT, VT> &proxy);

template <typename PT, typename VT>
typename SparseVectorProxy<PT, VT>::Iterator end(const SparseVectorProxy<PT, VT> &proxy);

template <typename PT, typename VT>
typename SparseVectorProxy<PT, VT>::ConstIterator cend(const SparseVectorProxy<PT, VT> &proxy);

template <typename PT, typename VT> size_t size(const SparseVectorProxy<PT, VT> &proxy);

template <typename PT, typename VT> size_t capacity(const SparseVectorProxy<PT, VT> &proxy);

template <typename PT, typename VT> size_t nonZeros(const SparseVectorProxy<PT, VT> &proxy);

template <typename PT, typename VT> void resize(const SparseVectorProxy<PT, VT> &proxy, size_t n, bool preserve = true);

template <typename PT, typename VT>
typename SparseVectorProxy<PT, VT>::Iterator find(const SparseVectorProxy<PT, VT> &proxy, size_t index);

template <typename PT, typename VT>
typename SparseVectorProxy<PT, VT>::Iterator lowerBound(const SparseVectorProxy<PT, VT> &proxy, size_t index);

template <typename PT, typename VT>
typename SparseVectorProxy<PT, VT>::Iterator upperBound(const SparseVectorProxy<PT, VT> &proxy, size_t index);
//@}
//*************************************************************************************************
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
METRIC_NUMERIC_ALWAYS_INLINE typename SparseVectorProxy<PT, VT>::Iterator begin(const SparseVectorProxy<PT, VT> &proxy)
{
	return proxy.begin();
}
//*************************************************************************************************
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
METRIC_NUMERIC_ALWAYS_INLINE typename SparseVectorProxy<PT, VT>::Iterator end(const SparseVectorProxy<PT, VT> &proxy)
{
	return proxy.end();
}
//*************************************************************************************************
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
METRIC_NUMERIC_ALWAYS_INLINE size_t size(const SparseVectorProxy<PT, VT> &proxy)
{
	return proxy.size();
}
//*************************************************************************************************
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
METRIC_NUMERIC_ALWAYS_INLINE size_t capacity(const SparseVectorProxy<PT, VT> &proxy)
{
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
METRIC_NUMERIC_ALWAYS_INLINE size_t nonZeros(const SparseVectorProxy<PT, VT> &proxy)
{
...
template <typename PT // Type of the proxy
		  ,
		  typename VT> // Type of the sparse vector
METRIC_NUMERIC_ALWAYS_INLINE void resize(const SparseVectorProxy<PT, VT> &proxy, size_t n, bool preserve)
{
