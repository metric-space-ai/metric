...
#ifndef METRIC_NUMERIC_MATH_PROXY_SPARSEMATRIXPROXY_H
#define METRIC_NUMERIC_MATH_PROXY_SPARSEMATRIXPROXY_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class SparseMatrixProxy : public SparseMatrix<PT, IsColumnMajorMatrix_v<MT>> {
  public:
	//**Type definitions****************************************************************************
...
// case METRIC_NUMERIC_USER_ASSERT() is active. In contrast, the at() function is guaranteed to perform
// a check of the given access indices.
*/
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Reference SparseMatrixProxy<PT, MT>::operator()(size_t i, size_t j) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Reference SparseMatrixProxy<PT, MT>::at(size_t i, size_t j) const
{
	if ((**this).isRestricted()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to restricted element");
	}
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::begin(size_t i) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::ConstIterator SparseMatrixProxy<PT, MT>::cbegin(size_t i) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::end(size_t i) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::ConstIterator SparseMatrixProxy<PT, MT>::cend(size_t i) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::set(size_t i, size_t j,
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::insert(size_t i, size_t j,
																					  const ElementType &value) const
{
	if ((**this).isRestricted()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to restricted element");
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::erase(size_t i, Iterator pos) const
{
	if ((**this).isRestricted()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to restricted element");
	}
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::erase(size_t i, Iterator first,
																					 Iterator last) const
{
	if ((**this).isRestricted()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid access to restricted element");
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::find(size_t i, size_t j) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::lowerBound(size_t i, size_t j) const
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
inline typename SparseMatrixProxy<PT, MT>::Iterator SparseMatrixProxy<PT, MT>::upperBound(size_t i, size_t j) const
{
...
typename SparseMatrixProxy<PT, MT>::Iterator begin(const SparseMatrixProxy<PT, MT> &proxy, size_t i);

template <typename PT, typename MT>
typename SparseMatrixProxy<PT, MT>::ConstIterator cbegin(const SparseMatrixProxy<PT, MT> &proxy, size_t i);

template <typename PT, typename MT>
typename SparseMatrixProxy<PT, MT>::Iterator end(const SparseMatrixProxy<PT, MT> &proxy, size_t i);

template <typename PT, typename MT>
typename SparseMatrixProxy<PT, MT>::ConstIterator cend(const SparseMatrixProxy<PT, MT> &proxy, size_t i);

template <typename PT, typename MT> size_t rows(const SparseMatrixProxy<PT, MT> &proxy);

template <typename PT, typename MT> size_t columns(const SparseMatrixProxy<PT, MT> &proxy);

template <typename PT, typename MT> size_t capacity(const SparseMatrixProxy<PT, MT> &proxy);

template <typename PT, typename MT> size_t capacity(const SparseMatrixProxy<PT, MT> &proxy, size_t i);

template <typename PT, typename MT> size_t nonZeros(const SparseMatrixProxy<PT, MT> &proxy);

template <typename PT, typename MT> size_t nonZeros(const SparseMatrixProxy<PT, MT> &proxy, size_t i);

template <typename PT, typename MT>
void resize(const SparseMatrixProxy<PT, MT> &proxy, size_t m, size_t n, bool preserve = true);

template <typename PT, typename MT>
typename SparseMatrixProxy<PT, MT>::Iterator find(const SparseMatrixProxy<PT, MT> &proxy, size_t i, size_t j);

template <typename PT, typename MT>
typename SparseMatrixProxy<PT, MT>::Iterator lowerBound(const SparseMatrixProxy<PT, MT> &proxy, size_t i, size_t j);

template <typename PT, typename MT>
typename SparseMatrixProxy<PT, MT>::Iterator upperBound(const SparseMatrixProxy<PT, MT> &proxy, size_t i, size_t j);
//@}
//*************************************************************************************************
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE size_t rows(const SparseMatrixProxy<PT, MT> &proxy)
{
	return proxy.rows();
}
//*************************************************************************************************
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE size_t columns(const SparseMatrixProxy<PT, MT> &proxy)
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE size_t capacity(const SparseMatrixProxy<PT, MT> &proxy)
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE size_t capacity(const SparseMatrixProxy<PT, MT> &proxy, size_t i)
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE size_t nonZeros(const SparseMatrixProxy<PT, MT> &proxy)
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE size_t nonZeros(const SparseMatrixProxy<PT, MT> &proxy, size_t i)
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE DisableIf_t<IsSquare_v<MT>> resize_backend(const SparseMatrixProxy<PT, MT> &proxy,
																		size_t m, size_t n, bool preserve)
{
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE EnableIf_t<IsSquare_v<MT>> resize_backend(const SparseMatrixProxy<PT, MT> &proxy, size_t m,
																	   size_t n, bool preserve)
{
	if (m != n) {
...
// the matrix is a compile time square matrix (as for instance the mtrc::numeric::SymmetricMatrix adaptor,
...
template <typename PT // Type of the proxy
		  ,
		  typename MT> // Type of the sparse matrix
METRIC_NUMERIC_ALWAYS_INLINE void resize(const SparseMatrixProxy<PT, MT> &proxy, size_t m, size_t n, bool preserve)
{
