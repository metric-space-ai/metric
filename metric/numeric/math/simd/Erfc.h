...
#ifndef METRIC_NUMERIC_MATH_SIMD_ERFC_H
#define METRIC_NUMERIC_MATH_SIMD_ERFC_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
METRIC_NUMERIC_ALWAYS_INLINE const SIMDfloat erfc(const SIMDf32<T> &a) noexcept
...
{
	return SIMDfloat{xsimd::erfc(xsimd::batch<float>{(*a).eval().value}).data};
}
#else
	= delete;
#endif
...
*/
template <typename T> // Type of the operand
METRIC_NUMERIC_ALWAYS_INLINE const SIMDdouble erfc(const SIMDf64<T> &a) noexcept
#if METRIC_NUMERIC_SVML_MODE
#if METRIC_NUMERIC_AVX512F_MODE || METRIC_NUMERIC_MIC_MODE
{
...
#elif METRIC_NUMERIC_XSIMD_MODE
{
	return SIMDdouble{xsimd::erfc(xsimd::batch<double>{(*a).eval().value}).data};
}
#else
	= delete;
#endif
