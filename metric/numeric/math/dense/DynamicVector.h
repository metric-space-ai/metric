...
#ifndef METRIC_NUMERIC_MATH_DENSE_DYNAMICVECTOR_H
#define METRIC_NUMERIC_MATH_DENSE_DYNAMICVECTOR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class DynamicVector : public DenseVector<DynamicVector<Type, TF, Alloc, Tag>, TF> {
  public:
	//**Type definitions****************************************************************************
	...
	template <typename Other> inline DynamicVector(size_t n, const Other *array, const Alloc &alloc = Alloc{});

	template <typename Other, size_t Dim> inline DynamicVector(const Other (&array)[Dim], const Alloc &alloc = Alloc{});
	...
	template <typename Other, size_t Dim> inline DynamicVector &operator=(const Other (&array)[Dim]) &;

	template <typename Other, size_t Dim> inline DynamicVector &operator=(const std::array<Other, Dim> &array) &;
  ...
  private:
	...
	static constexpr bool VectorizedAssign_v =
		(useOptimizedKernels && simdEnabled && VT::simdEnabled && IsSIMDCombinable_v<Type, ElementType_t<VT>>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  private:
	...
	Type *METRIC_NUMERIC_RESTRICT v_; //!< The dynamically allocated vector elements.
	...
	METRIC_NUMERIC_NO_UNIQUE_ADDRESS Alloc alloc_; //!< The allocator of the vector.
	//@}
	//**********************************************************************************************
...
template <typename Type> DynamicVector(size_t, Type *) -> DynamicVector<RemoveCV_t<Type>>;

template <typename Type, size_t N> DynamicVector(Type (&)[N]) -> DynamicVector<RemoveCV_t<Type>>;

template <typename Type, size_t N> DynamicVector(std::array<Type, N>) -> DynamicVector<Type>;
...
// within a constructor call:
   ...
   \code
   mtrc::numeric::DynamicVector<double> v1{ 4.2, 6.3, -1.2 };
   \endcode
...
// dynamic array:
   ...
   mtrc::numeric::DynamicVector<double> v( array, 4UL );
   delete[] array;
   \endcode
...
// static array:
   ...
   mtrc::numeric::DynamicVector<int> v( init );
   \endcode
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicVector<Type, TF, Alloc, Tag>::DynamicVector(const DynamicVector &v) : DynamicVector(v.size_)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(capacity_ <= v.capacity_, "Invalid capacity estimation");
...
inline DynamicVector<Type, TF, Alloc, Tag>::DynamicVector(const Vector<VT, TF> &v) : DynamicVector((*v).size())
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename DynamicVector<Type, TF, Alloc, Tag>::Reference DynamicVector<Type, TF, Alloc, Tag>::at(size_t index)
{
	if (index >= size_) {
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename DynamicVector<Type, TF, Alloc, Tag>::Pointer DynamicVector<Type, TF, Alloc, Tag>::data() noexcept
{
	return v_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename DynamicVector<Type, TF, Alloc, Tag>::Iterator DynamicVector<Type, TF, Alloc, Tag>::begin() noexcept
{
	return Iterator(v_);
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename DynamicVector<Type, TF, Alloc, Tag>::Iterator DynamicVector<Type, TF, Alloc, Tag>::end() noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicVector<Type, TF, Alloc, Tag> &DynamicVector<Type, TF, Alloc, Tag>::operator=(const Type &rhs) &
{
	for (size_t i = 0UL; i < size_; ++i)
		v_[i] = rhs;
	return *this;
...
// by means of an initializer list:
   ...
   \code
   mtrc::numeric::DynamicVector<double> v;
   v = { 4.2, 6.3, -1.2 };
   \endcode
...
// This assignment operator offers the option to directly set all elements of the vector:
   ...
   \code
   const int init[4] = { 1, 2, 3 };
   mtrc::numeric::DynamicVector<int> v;
   v = init;
   \endcode
...
template <typename Other // Data type of the static array
		  ,
		  size_t Dim> // Dimension of the static array
inline DynamicVector<Type, TF, Alloc, Tag> &DynamicVector<Type, TF, Alloc, Tag>::operator=(const Other (&array)[Dim]) &
{
	resize(Dim, false);
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicVector<Type, TF, Alloc, Tag> &DynamicVector<Type, TF, Alloc, Tag>::operator=(const DynamicVector &rhs) &
{
	if (&rhs == this)
		return *this;
...
inline DynamicVector<Type, TF, Alloc, Tag> &DynamicVector<Type, TF, Alloc, Tag>::operator=(const Vector<VT, TF> &rhs) &
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<VT>);
...
inline void DynamicVector<Type, TF, Alloc, Tag>::resize(size_t n, bool preserve)
{
	...
	if (n > capacity_) {
		...
		if (preserve) {
			mtrc::numeric::uninitialized_transfer(v_, v_ + size_, tmp.v_);
			mtrc::numeric::uninitialized_default_construct(tmp.v_ + size_, tmp.v_ + tmp.capacity_);
		} else {
...
inline void DynamicVector<Type, TF, Alloc, Tag>::reserve(size_t n)
{
	...
	if (n > capacity_) {
		...
		mtrc::numeric::uninitialized_transfer(v_, v_ + size_, tmp.v_);
		mtrc::numeric::uninitialized_value_construct(tmp.v_ + size_, tmp.v_ + tmp.capacity_);
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::DynamicVector<int> a;
   // ... Initialization
...
inline DynamicVector<Type, TF, Alloc, Tag> &DynamicVector<Type, TF, Alloc, Tag>::scale(const Other &scalar)
{
	for (size_t i = 0UL; i < size_; ++i)
		v_[i] *= scalar;
	return *this;
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE typename DynamicVector<Type, TF, Alloc, Tag>::SIMDType
...
DynamicVector<Type, TF, Alloc, Tag>::loada(size_t index) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(index + SIMDSIZE <= capacity_, "Invalid vector access index");
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicVector<Type, TF, Alloc, Tag>::store(size_t index,
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicVector<Type, TF, Alloc, Tag>::storea(size_t index,
																			  const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicVector<Type, TF, Alloc, Tag>::storeu(size_t index,
																			  const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicVector<Type, TF, Alloc, Tag>::stream(size_t index,
																			  const SIMDType &value) noexcept
{
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(size_ == (*rhs).size(), "Invalid vector sizes");
...
struct HasConstDataAccess<DynamicVector<T, TF, Alloc, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool TF, typename Alloc, typename Tag>
struct HasMutableDataAccess<DynamicVector<T, TF, Alloc, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool TF, typename Alloc, typename Tag>
struct IsAligned<DynamicVector<T, TF, Alloc, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool TF, typename Alloc, typename Tag>
struct IsContiguous<DynamicVector<T, TF, Alloc, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool TF, typename Alloc, typename Tag>
struct IsPadded<DynamicVector<T, TF, Alloc, Tag>> : public BoolConstant<usePadding> {};
/*! \endcond */
//*************************************************************************************************
...
struct AddTraitEval2<T1, T2,
					 EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && (IsDenseVector_v<T1> || IsDenseVector_v<T2>) &&
	...
	using Type = DynamicVector<ET, TransposeFlag_v<T1>, DynamicAllocator_t<ET, GetAllocator_t<T1>, GetAllocator_t<T2>>,
...
struct MultTraitEval2<T1, T2,
					  EnableIf_t<IsDenseVector_v<T1> && IsScalar_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
								 (MaxSize_v<T1, 0UL> == DefaultMaxSize_v)>> {
	using ET = MultTrait_t<ElementType_t<T1>, T2>;

	using Type = DynamicVector<ET, TransposeFlag_v<T1>, DynamicAllocator_t<ET, GetAllocator_t<T1>>,
							   MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct MultTraitEval2<T1, T2,
					  EnableIf_t<IsScalar_v<T1> && IsDenseVector_v<T2> && (Size_v<T2, 0UL> == DefaultSize_v) &&
								 (MaxSize_v<T2, 0UL> == DefaultMaxSize_v)>> {
	using ET = MultTrait_t<T1, ElementType_t<T2>>;

	using Type = DynamicVector<ET, TransposeFlag_v<T2>, DynamicAllocator_t<ET, GetAllocator_t<T2>>,
...
struct MultTraitEval2<
	...
	EnableIf_t<((IsRowVector_v<T1> && IsRowVector_v<T2>) || (IsColumnVector_v<T1> && IsColumnVector_v<T2>)) &&
			   ...
			   (Size_v<T2, 0UL> == DefaultSize_v) && (MaxSize_v<T1, 0UL> == DefaultMaxSize_v) &&
			   (MaxSize_v<T2, 0UL> == DefaultMaxSize_v)>> {
...
struct MultTraitEval2<
	T1, T2,
	EnableIf_t<IsMatrix_v<T1> && IsColumnVector_v<T2> && (IsDenseMatrix_v<T1> || IsDenseVector_v<T2>) &&
	...
	using Type =
		DynamicVector<ET, false, DynamicAllocator_t<ET, GetAllocator_t<T1>, GetAllocator_t<T2>>, AddTrait_t<M2, M2>>;
};

template <typename T1, typename T2>
struct MultTraitEval2<
	T1, T2,
	EnableIf_t<IsRowVector_v<T1> && IsMatrix_v<T2> && (IsDenseVector_v<T1> || IsDenseMatrix_v<T2>) &&
	...
	using Type =
		DynamicVector<ET, true, DynamicAllocator_t<ET, GetAllocator_t<T1>, GetAllocator_t<T2>>, AddTrait_t<M2, M2>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct KronTraitEval2<
	...
	using Type = DynamicVector<ET, TransposeFlag_v<T2>, DynamicAllocator_t<ET, GetAllocator_t<T1>, GetAllocator_t<T2>>,
							   MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct UnaryMapTraitEval2<
	T, OP, EnableIf_t<IsDenseVector_v<T> && Size_v<T, 0UL> == DefaultSize_v && MaxSize_v<T, 0UL> == DefaultMaxSize_v>> {
	using ET = EvaluateTrait_t<decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()))>;

	using Type =
		DynamicVector<ET, TransposeFlag_v<T>, DynamicAllocator_t<ET, GetAllocator_t<T>>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct PartialReduceTraitEval2<
	...
	using ET = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>(), std::declval<ElementType_t<T>>()));

	using Type =
		DynamicVector<ET, (RF == columnwise), DynamicAllocator_t<ET, GetAllocator_t<T>>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct RepeatTraitEval2<T, R0, inf, inf,
	...
	using Type = DynamicVector<ElementType_t<T>, TransposeFlag_v<T>,
...
struct SolveTraitEval2<T1, T2,
					   EnableIf_t<IsDenseMatrix_v<T1> && IsDenseVector_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
								  ...
								  (MaxSize_v<T2, 0UL> == DefaultMaxSize_v)>> {
	using Type =
		DynamicVector<ElementType_t<T2>, TransposeFlag_v<T2>,
					  DynamicAllocator_t<ElementType_t<T2>, GetAllocator_t<T1>, GetAllocator_t<T2>>, TagType_t<T2>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubvectorTraitEval2<
	...
	using Type = DynamicVector<ET, TransposeFlag_v<VT>, DynamicAllocator_t<ET, GetAllocator_t<VT>>, TagType_t<VT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename VT> struct ElementsTraitEval2<VT, 0UL, EnableIf_t<IsDenseVector_v<VT>>> {
	using ET = RemoveConst_t<ElementType_t<VT>>;
...
struct RowTraitEval2<
	...
	using Type = DynamicVector<ET, true, DynamicAllocator_t<ET, GetAllocator_t<MT>>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct ColumnTraitEval2<
	...
	using Type = DynamicVector<ET, false, DynamicAllocator_t<ET, GetAllocator_t<MT>>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BandTraitEval2<
	...
	using Type = DynamicVector<ET, defaultTransposeFlag, DynamicAllocator_t<ET, GetAllocator_t<MT>>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
