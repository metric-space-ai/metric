...
#ifndef METRIC_NUMERIC_MATH_DENSE_DENSEVECTOR_H
#define METRIC_NUMERIC_MATH_DENSE_DENSEVECTOR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
auto operator==(const DenseVector<T1, TF> &vec, T2 scalar) -> EnableIf_t<IsScalar_v<T2>, bool>;
...
template <typename VT, bool TF> VT &operator<<=(DenseVector<VT, TF> &vec, int count);
...
inline auto operator==(const DenseVector<T1, TF> &vec, T2 scalar) -> EnableIf_t<IsScalar_v<T2>, bool>
{
...
template <typename T1 // Type of the left-hand side scalar
		  ...
		  ,
		  bool TF> // Transpose flag
inline auto operator==(T1 scalar, const DenseVector<T2, TF> &vec) -> EnableIf_t<IsScalar_v<T1>, bool>
{
	return (vec == scalar);
}
//*************************************************************************************************
...
inline auto operator+=(DenseVector<VT, TF> &vec, ST scalar) -> EnableIf_t<IsScalar_v<ST>, VT &>
{
	if (IsRestricted_v<VT>) {
		if (!tryAdd(*vec, 0UL, (*vec).size(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid addition to restricted vector");
		}
	}
...
inline auto operator-=(DenseVector<VT, TF> &vec, ST scalar) -> EnableIf_t<IsScalar_v<ST>, VT &>
{
	if (IsRestricted_v<VT>) {
		if (!trySub(*vec, 0UL, (*vec).size(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid subtraction from restricted vector");
		}
	}
...
inline VT &operator<<=(DenseVector<VT, TF> &vec, int count)
{
	if (IsRestricted_v<VT>) {
		if (!tryShift(*vec, 0UL, (*vec).size(), count)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid left-shift of restricted vector");
		}
	}
...
inline VT &operator>>=(DenseVector<VT, TF> &vec, int count)
{
	if (IsRestricted_v<VT>) {
		if (!tryShift(*vec, 0UL, (*vec).size(), count)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid right-shift of restricted vector");
		}
	}
...
inline auto operator&=(DenseVector<VT, TF> &vec, ST scalar) -> EnableIf_t<IsScalar_v<ST>, VT &>
{
	if (IsRestricted_v<VT>) {
		if (!tryBitand(*vec, 0UL, (*vec).size(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid bitwise AND of restricted vector");
		}
	}
...
inline auto operator|=(DenseVector<VT, TF> &vec, ST scalar) -> EnableIf_t<IsScalar_v<ST>, VT &>
{
	if (IsRestricted_v<VT>) {
		if (!tryBitor(*vec, 0UL, (*vec).size(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid bitwise OR of restricted vector");
		}
	}
...
inline auto operator^=(DenseVector<VT, TF> &vec, ST scalar) -> EnableIf_t<IsScalar_v<ST>, VT &>
{
	if (IsRestricted_v<VT>) {
		if (!tryBitxor(*vec, 0UL, (*vec).size(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid bitwise XOR of restricted vector");
		}
	}
...
//@{
template <typename VT, bool TF> bool isnan(const DenseVector<VT, TF> &dv);

template <typename VT, bool TF> bool isinf(const DenseVector<VT, TF> &dv);

template <typename VT, bool TF> bool isfinite(const DenseVector<VT, TF> &dv);

template <typename VT, bool TF> bool isDivisor(const DenseVector<VT, TF> &dv);

template <RelaxationFlag RF, typename VT, bool TF> bool isUniform(const DenseVector<VT, TF> &dv);

template <RelaxationFlag RF, typename VT, bool TF> bool isZero(const DenseVector<VT, TF> &dv);
//@}
//*************************************************************************************************
