...
#ifndef METRIC_NUMERIC_MATH_DENSE_STATICVECTOR_H
#define METRIC_NUMERIC_MATH_DENSE_STATICVECTOR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/traits/BandTrait.h>
#include <metric/numeric/math/traits/ColumnTrait.h>
#include <metric/numeric/math/traits/CrossTrait.h>
#include <metric/numeric/math/traits/DivTrait.h>
#include <metric/numeric/math/traits/ElementsTrait.h>
...
#include <metric/numeric/math/traits/RowTrait.h>
#include <metric/numeric/math/traits/SolveTrait.h>
#include <metric/numeric/math/traits/SubTrait.h>
#include <metric/numeric/math/traits/SubvectorTrait.h>
...
#include <metric/numeric/system/Standard.h>
...
//  - TF  : specifies whether the vector is a row vector (\c mtrc::numeric::rowVector) or a column
//          vector (\c mtrc::numeric::columnVector). The default value is \c mtrc::numeric::defaultTransposeFlag.
//  - AF  : specifies whether the first element of the vector is properly aligned with respect to
//          the available instruction set (SSE, AVX, ...). Possible values are \c mtrc::numeric::aligned
//          and \c mtrc::numeric::unaligned. The default value is \c mtrc::numeric::defaultAlignmentFlag.
//  - PF  : specifies whether the vector should be padded to maximize the efficiency of vectorized
//          operations. Possible values are \c mtrc::numeric::padded and \c mtrc::numeric::unpadded. The default
//          value is \c mtrc::numeric::defaultPaddingFlag.
//  - Tag : optional type parameter to tag the vector. The default type is \c mtrc::numeric::Group0.
...
class StaticVector : public DenseVector<StaticVector<Type, N, TF, AF, PF, Tag>, TF> {
  public:
	//**Type definitions****************************************************************************
	...
	template <typename Other> inline StaticVector(size_t n, const Other *array);

	template <typename Other, size_t Dim> constexpr StaticVector(const Other (&array)[Dim]);

	template <typename Other, size_t Dim> constexpr StaticVector(const std::array<Other, Dim> &array);
	...
	template <typename Other, size_t Dim> constexpr StaticVector &operator=(const Other (&array)[Dim]) &;

	template <typename Other, size_t Dim> constexpr StaticVector &operator=(const std::array<Other, Dim> &array) &;
  ...
  private:
	...
	static constexpr bool VectorizedAssign_v = (useOptimizedKernels && NN >= SIMDSIZE && simdEnabled &&
												VT::simdEnabled && IsSIMDCombinable_v<Type, ElementType_t<VT>>);
	/*! \endcond */
	//**********************************************************************************************
	...
	//! Helper variable template for the explicit application of the SFINAE principle.
	template <typename VT>
	static constexpr bool VectorizedAddAssign_v = (VectorizedAssign_v<VT> && HasSIMDAdd_v<Type, ElementType_t<VT>>);
	/*! \endcond */
	//**********************************************************************************************
	...
	//! Helper variable template for the explicit application of the SFINAE principle.
	template <typename VT>
	static constexpr bool VectorizedSubAssign_v = (VectorizedAssign_v<VT> && HasSIMDSub_v<Type, ElementType_t<VT>>);
	/*! \endcond */
	//**********************************************************************************************
	...
	//! Helper variable template for the explicit application of the SFINAE principle.
	template <typename VT>
	static constexpr bool VectorizedMultAssign_v = (VectorizedAssign_v<VT> && HasSIMDMult_v<Type, ElementType_t<VT>>);
	/*! \endcond */
	//**********************************************************************************************
	...
	//! Helper variable template for the explicit application of the SFINAE principle.
	template <typename VT>
	static constexpr bool VectorizedDivAssign_v = (VectorizedAssign_v<VT> && HasSIMDDiv_v<Type, ElementType_t<VT>>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	METRIC_NUMERIC_ALWAYS_INLINE SIMDType load(size_t index) const noexcept;
	METRIC_NUMERIC_ALWAYS_INLINE SIMDType loada(size_t index) const noexcept;
	METRIC_NUMERIC_ALWAYS_INLINE SIMDType loadu(size_t index) const noexcept;

	METRIC_NUMERIC_ALWAYS_INLINE void store(size_t index, const SIMDType &value) noexcept;
	METRIC_NUMERIC_ALWAYS_INLINE void storea(size_t index, const SIMDType &value) noexcept;
	METRIC_NUMERIC_ALWAYS_INLINE void storeu(size_t index, const SIMDType &value) noexcept;
	METRIC_NUMERIC_ALWAYS_INLINE void stream(size_t index, const SIMDType &value) noexcept;

	template <typename VT> inline auto assign(const DenseVector<VT, TF> &rhs) -> DisableIf_t<VectorizedAssign_v<VT>>;

	template <typename VT> inline auto assign(const DenseVector<VT, TF> &rhs) -> EnableIf_t<VectorizedAssign_v<VT>>;
...
template <typename Type, typename... Ts> StaticVector(Type, Ts...) -> StaticVector<Type, 1 + sizeof...(Ts)>;

template <typename Type, size_t N> StaticVector(Type (&)[N]) -> StaticVector<RemoveCV_t<Type>, N>;

template <typename Type, size_t N> StaticVector(std::array<Type, N>) -> StaticVector<Type, N>;
...
// means of an initializer list:
   ...
   \code
   mtrc::numeric::StaticVector<double,3UL> v1{ 4.2, 6.3, -1.2 };
   \endcode
...
{
	if (list.size() > N) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of static vector");
	}
...
// dynamic array:
   ...
   mtrc::numeric::StaticVector<double,2> v( 2UL, array );
   delete[] array;
   \endcode
...
// with a static array:
   ...
   mtrc::numeric::StaticVector<double,3> v( init );
   \endcode
...
{
	METRIC_NUMERIC_STATIC_ASSERT(Dim == N);
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag>	 // Type tag
template <typename Other // Data type of the other vector
		  ,
		  AlignmentFlag AF2 // Alignment flag of the other vector
		  ,
		  PaddingFlag PF2> // Padding flag of the other vector
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
template <typename VT>	// Type of the other vector
...
// v_ is intentionally left uninitialized
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<VT>);
...
{
	METRIC_NUMERIC_USER_ASSERT(index < N, "Invalid vector access index");
	return v_[index];
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
inline
	typename StaticVector<Type, N, TF, AF, PF, Tag>::Reference StaticVector<Type, N, TF, AF, PF, Tag>::at(size_t index)
{
	if (index >= N) {
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
constexpr
	typename StaticVector<Type, N, TF, AF, PF, Tag>::Pointer StaticVector<Type, N, TF, AF, PF, Tag>::data() noexcept
{
	return v_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
constexpr
	typename StaticVector<Type, N, TF, AF, PF, Tag>::Iterator StaticVector<Type, N, TF, AF, PF, Tag>::begin() noexcept
{
	return Iterator(v_);
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
constexpr
	typename StaticVector<Type, N, TF, AF, PF, Tag>::Iterator StaticVector<Type, N, TF, AF, PF, Tag>::end() noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr StaticVector<Type, N, TF, AF, PF, Tag> &StaticVector<Type, N, TF, AF, PF, Tag>::operator=(const Type &rhs) &
{
	for (size_t i = 0UL; i < N; ++i)
		v_[i] = rhs;
	return *this;
...
// by means of an initializer list:
   ...
   \code
   mtrc::numeric::StaticVector<double,3UL> v;
   v = { 4.2, 6.3, -1.2 };
   \endcode
...
StaticVector<Type, N, TF, AF, PF, Tag>::operator=(initializer_list<Type> list) &
{
	...
	if (list.size() > N) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to static vector");
	}
...
// This assignment operator offers the option to directly set all elements of the vector:
   ...
   \code
   const double init[3] = { 1.0, 2.0 };
   mtrc::numeric::StaticVector<double,3> v;
   v = init;
   \endcode
...
StaticVector<Type, N, TF, AF, PF, Tag>::operator%=(const Vector<VT, TF> &rhs) &
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTOR_WITH_TRANSPOSE_FLAG(ResultType_t<VT>, TF);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(ResultType_t<VT>);

	using CrossType = CrossTrait_t<This, ResultType_t<VT>>;

	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_VECTOR_TYPE(CrossType);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTOR_WITH_TRANSPOSE_FLAG(CrossType, TF);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(CrossType);

	if (N != 3UL || (*rhs).size() != 3UL) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid vector size for cross product");
	}
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::StaticVector<int,3> a;
   // ... Initialization
...
inline StaticVector<Type, N, TF, AF, PF, Tag> &StaticVector<Type, N, TF, AF, PF, Tag>::scale(const Other &scalar)
{
...
inline void *StaticVector<Type, N, TF, AF, PF, Tag>::operator new(std::size_t size)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(size == sizeof(StaticVector), "Invalid number of bytes detected");
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(size >= sizeof(StaticVector), "Invalid number of bytes detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(size % sizeof(StaticVector) == 0UL, "Invalid number of bytes detected");
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE typename StaticVector<Type, N, TF, AF, PF, Tag>::SIMDType
...
StaticVector<Type, N, TF, AF, PF, Tag>::loada(size_t index) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(index < N, "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + SIMDSIZE <= NN, "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index % SIMDSIZE == 0UL, "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(checkAlignment(&v_[index]), "Invalid alignment detected");
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void StaticVector<Type, N, TF, AF, PF, Tag>::store(size_t index,
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void StaticVector<Type, N, TF, AF, PF, Tag>::storea(size_t index,
																				 const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void StaticVector<Type, N, TF, AF, PF, Tag>::storeu(size_t index,
																				 const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void StaticVector<Type, N, TF, AF, PF, Tag>::stream(size_t index,
																				 const SIMDType &value) noexcept
{
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT((*rhs).size() == N, "Invalid vector sizes");
...
// element is not 0. Following example demonstrates the use of the \a isDefault function:
   ...
   \code
   mtrc::numeric::StaticVector<double,3> a;
   // ... Initialization
   if( isDefault( a ) ) { ... }
   \endcode
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline const StaticVector<Type, 2UL, TF, AF, PF, Tag> perp(const StaticVector<Type, 2UL, TF, AF, PF, Tag> &v)
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline const StaticVector<Type, 3UL, TF, AF, PF, Tag> perp(const StaticVector<Type, 3UL, TF, AF, PF, Tag> &v)
{
...
constexpr Type &get(StaticVector<Type, N, TF, AF, PF, Tag> &v) noexcept
{
	METRIC_NUMERIC_STATIC_ASSERT_MSG(I < N, "Invalid vector access index");
	return v[I];
}
//*************************************************************************************************
...
struct Size<StaticVector<T, N, TF, AF, PF, Tag>, 0UL> : public Ptrdiff_t<static_cast<ptrdiff_t>(N)> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct MaxSize<StaticVector<T, N, TF, AF, PF, Tag>, 0UL> : public Ptrdiff_t<static_cast<ptrdiff_t>(N)> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct HasConstDataAccess<StaticVector<T, N, TF, AF, PF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct HasMutableDataAccess<StaticVector<T, N, TF, AF, PF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct IsAligned<StaticVector<T, N, TF, AF, PF, Tag>> : public BoolConstant<AF == aligned> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct IsContiguous<StaticVector<T, N, TF, AF, PF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct IsPadded<StaticVector<T, N, TF, AF, PF, Tag>> : public BoolConstant<PF == padded> {};
/*! \endcond */
//*************************************************************************************************
...
struct AddTraitEval2<T1, T2,
	...
	using Type = StaticVector<AddTrait_t<ElementType_t<T1>, ElementType_t<T2>>, max(Size_v<T1, 0UL>, Size_v<T2, 0UL>),
							  TransposeFlag_v<T1>, defaultAlignmentFlag, defaultPaddingFlag,
...
struct SubTraitEval2<T1, T2,
					 EnableIf_t<IsVector_v<T1> && IsVector_v<T2> &&
								(Size_v<T1, 0UL> != DefaultSize_v || Size_v<T2, 0UL> != DefaultSize_v)>> {
	using Type = StaticVector<SubTrait_t<ElementType_t<T1>, ElementType_t<T2>>, max(Size_v<T1, 0UL>, Size_v<T2, 0UL>),
							  TransposeFlag_v<T1>, defaultAlignmentFlag, defaultPaddingFlag,
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct MultTraitEval2<T1, T2, EnableIf_t<IsVector_v<T1> && IsScalar_v<T2> && (Size_v<T1, 0UL> != DefaultSize_v)>> {
	using Type = StaticVector<MultTrait_t<ElementType_t<T1>, T2>, Size_v<T1, 0UL>, TransposeFlag_v<T1>,
							  defaultAlignmentFlag, defaultPaddingFlag, MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct MultTraitEval2<T1, T2, EnableIf_t<IsScalar_v<T1> && IsVector_v<T2> && (Size_v<T2, 0UL> != DefaultSize_v)>> {
	using Type = StaticVector<MultTrait_t<T1, ElementType_t<T2>>, Size_v<T2, 0UL>, TransposeFlag_v<T2>,
...
struct MultTraitEval2<
	T1, T2,
	EnableIf_t<((IsRowVector_v<T1> && IsRowVector_v<T2>) || (IsColumnVector_v<T1> && IsColumnVector_v<T2>)) &&
			   IsDenseVector_v<T1> && IsDenseVector_v<T2> &&
			   (Size_v<T1, 0UL> != DefaultSize_v || Size_v<T2, 0UL> != DefaultSize_v)>> {
	using Type = StaticVector<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, max(Size_v<T1, 0UL>, Size_v<T2, 0UL>),
							  TransposeFlag_v<T1>, defaultAlignmentFlag, defaultPaddingFlag,
...
struct MultTraitEval2<
	T1, T2,
	EnableIf_t<IsMatrix_v<T1> && IsColumnVector_v<T2> &&
	...
	using Type = StaticVector<AddTrait_t<MultType, MultType>,
							  (Size_v<T1, 0UL> != DefaultSize_v ? Size_v<T1, 0UL> : Size_v<T2, 0UL>), false,
							  defaultAlignmentFlag, defaultPaddingFlag, AddTrait_t<MultTag, MultTag>>;
};

template <typename T1, typename T2>
struct MultTraitEval2<
	T1, T2,
	EnableIf_t<IsRowVector_v<T1> && IsMatrix_v<T2> &&
			   (Size_v<T2, 1UL> != DefaultSize_v || (IsSquare_v<T2> && Size_v<T1, 0UL> != DefaultSize_v))>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = StaticVector<AddTrait_t<MultType, MultType>,
							  (Size_v<T2, 1UL> != DefaultSize_v ? Size_v<T2, 1UL> : Size_v<T1, 0UL>), true,
							  defaultAlignmentFlag, defaultPaddingFlag, AddTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct KronTraitEval2<T1, T2,
					  EnableIf_t<IsDenseVector_v<T1> && IsDenseVector_v<T2> && (Size_v<T1, 0UL> != DefaultSize_v) &&
								 (Size_v<T2, 0UL> != DefaultSize_v)>> {
	using Type = StaticVector<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, Size_v<T1, 0UL> * Size_v<T2, 0UL>,
							  TransposeFlag_v<T2>, defaultAlignmentFlag, defaultPaddingFlag,
							  MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct DivTraitEval2<T1, T2, EnableIf_t<IsVector_v<T1> && IsScalar_v<T2> && (Size_v<T1, 0UL> != DefaultSize_v)>> {
	using Type = StaticVector<DivTrait_t<ElementType_t<T1>, T2>, Size_v<T1, 0UL>, TransposeFlag_v<T1>,
							  defaultAlignmentFlag, defaultPaddingFlag, DivTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct DivTraitEval2<T1, T2,
					 EnableIf_t<IsDenseVector_v<T1> && IsDenseVector_v<T2> &&
								(Size_v<T1, 0UL> != DefaultSize_v || Size_v<T2, 0UL> != DefaultSize_v)>> {
	using Type = StaticVector<DivTrait_t<ElementType_t<T1>, ElementType_t<T2>>, max(Size_v<T1, 0UL>, Size_v<T2, 0UL>),
							  TransposeFlag_v<T1>, defaultAlignmentFlag, defaultPaddingFlag,
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2> struct CrossTraitEval2<T1, T2, EnableIf_t<IsVector_v<T1> && IsVector_v<T2>>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = StaticVector<SubTrait_t<MultType, MultType>, 3UL, TransposeFlag_v<T1>, defaultAlignmentFlag,
							  defaultPaddingFlag, SubTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct UnaryMapTraitEval2<T, OP, EnableIf_t<IsVector_v<T> && Size_v<T, 0UL> != DefaultSize_v>> {
	using ElementType = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()));

	using Type = StaticVector<EvaluateTrait_t<ElementType>, Size_v<T, 0UL>, TransposeFlag_v<T>, defaultAlignmentFlag,
							  defaultPaddingFlag, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BinaryMapTraitEval2<
	...
	using Type = StaticVector<EvaluateTrait_t<ElementType>, max(Size_v<T1, 0UL>, Size_v<T2, 0UL>), TransposeFlag_v<T1>,
...
struct PartialReduceTraitEval2<
	T, OP, RF, EnableIf_t<IsMatrix_v<T> && Size_v<T, 0UL> != DefaultSize_v && Size_v<T, 1UL> != DefaultSize_v>> {
	using ET = ElementType_t<T>;

	static constexpr bool TF = (RF == columnwise);

	using Type =
		StaticVector<decltype(std::declval<OP>()(std::declval<ET>(), std::declval<ET>())), Size_v<T, TF ? 1UL : 0UL>,
					 TF, defaultAlignmentFlag, defaultPaddingFlag, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct RepeatTraitEval2<T, R0, inf, inf,
						EnableIf_t<IsDenseVector_v<T> && (R0 != inf) && (Size_v<T, 0UL> != DefaultSize_v)>> {
	using Type = StaticVector<ElementType_t<T>, R0 * Size_v<T, 0UL>, TransposeFlag_v<T>, defaultAlignmentFlag,
							  defaultPaddingFlag, TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SolveTraitEval2<T1, T2,
					   EnableIf_t<IsDenseMatrix_v<T1> && IsDenseVector_v<T2> &&
								  ((Size_v<T1, 0UL> != DefaultSize_v) || (Size_v<T1, 1UL> != DefaultSize_v) ||
								   (Size_v<T2, 0UL> != DefaultSize_v))>> {
	using Type = StaticVector<ElementType_t<T2>, max(Size_v<T1, 0UL>, Size_v<T1, 1UL>, Size_v<T2, 0UL>),
							  TransposeFlag_v<T2>, defaultAlignmentFlag, defaultPaddingFlag, TagType_t<T2>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubvectorTraitEval2<VT, I, N, EnableIf_t<I != inf && N != inf && IsDenseVector_v<VT>>> {
	using Type = StaticVector<RemoveConst_t<ElementType_t<VT>>, N, TransposeFlag_v<VT>, defaultAlignmentFlag,
							  defaultPaddingFlag, TagType_t<VT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename VT, size_t N> struct ElementsTraitEval2<VT, N, EnableIf_t<N != 0UL && IsDenseVector_v<VT>>> {
	using Type = StaticVector<RemoveConst_t<ElementType_t<VT>>, N, TransposeFlag_v<VT>, defaultAlignmentFlag,
							  defaultPaddingFlag, TagType_t<VT>>;
};
/*! \endcond */
...
struct RowTraitEval2<MT, I, EnableIf_t<IsDenseMatrix_v<MT> && Size_v<MT, 1UL> != DefaultSize_v>> {
	using Type = StaticVector<RemoveConst_t<ElementType_t<MT>>, Size_v<MT, 1UL>, true, defaultAlignmentFlag,
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t I>
struct ColumnTraitEval2<MT, I, EnableIf_t<IsDenseMatrix_v<MT> && Size_v<MT, 0UL> != DefaultSize_v>> {
	using Type = StaticVector<RemoveConst_t<ElementType_t<MT>>, Size_v<MT, 0UL>, false, defaultAlignmentFlag,
							  defaultPaddingFlag, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BandTraitEval2<
	MT, I, EnableIf_t<IsDenseMatrix_v<MT> && Size_v<MT, 0UL> != DefaultSize_v && Size_v<MT, 1UL> != DefaultSize_v>> {
	...
	using Type = StaticVector<RemoveConst_t<ElementType_t<MT>>, Min, defaultTransposeFlag, defaultAlignmentFlag,
							  defaultPaddingFlag, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
template <typename Type, size_t N, bool TF, mtrc::numeric::AlignmentFlag AF, mtrc::numeric::PaddingFlag PF,
		  typename Tag>
class tuple_size<mtrc::numeric::StaticVector<Type, N, TF, AF, PF, Tag>> : public integral_constant<size_t, N> {};

template <size_t I, typename Type, size_t N, bool TF, mtrc::numeric::AlignmentFlag AF,
		  mtrc::numeric::PaddingFlag PF, typename Tag>
class tuple_element<I, mtrc::numeric::StaticVector<Type, N, TF, AF, PF, Tag>> {
  public:
	using type = Type;
};
