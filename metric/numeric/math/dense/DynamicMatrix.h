...
#ifndef METRIC_NUMERIC_MATH_DENSE_DYNAMICMATRIX_H
#define METRIC_NUMERIC_MATH_DENSE_DYNAMICMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/system/Blocking.h>
#include <metric/numeric/system/CacheSize.h>
#include <metric/numeric/system/Inline.h>
#include <metric/numeric/system/NoUniqueAddress.h>
#include <metric/numeric/system/Optimizations.h>
#include <metric/numeric/system/Restrict.h>
#include <metric/numeric/system/Thresholds.h>
#include <metric/numeric/util/Algorithms.h>
...
//           of allocator is \c mtrc::numeric::AlignedAllocator.
...
class DynamicMatrix : public DenseMatrix<DynamicMatrix<Type, SO, Alloc, Tag>, SO> {
  private:
	//**********************************************************************************************
  ...
  private:
	...
	static constexpr bool VectorizedAssign_v =
		(useOptimizedKernels && simdEnabled && MT::simdEnabled && IsSIMDCombinable_v<Type, ElementType_t<MT>>);
	/*! \endcond */
	//**********************************************************************************************
  ...
  public:
	...
	template <typename MT> inline auto assign(const DenseMatrix<MT, SO> &rhs) -> DisableIf_t<VectorizedAssign_v<MT>>;

	template <typename MT> inline auto assign(const DenseMatrix<MT, SO> &rhs) -> EnableIf_t<VectorizedAssign_v<MT>>;
  ...
  private:
	...
	Type *METRIC_NUMERIC_RESTRICT v_; //!< The dynamically allocated matrix elements.
	...
	METRIC_NUMERIC_NO_UNIQUE_ADDRESS Alloc alloc_; //!< The allocator of the matrix.
	//@}
	//**********************************************************************************************
...
template <typename Type> DynamicMatrix(size_t, size_t, Type *) -> DynamicMatrix<RemoveCV_t<Type>>;

template <typename Type, size_t M, size_t N> DynamicMatrix(Type (&)[M][N]) -> DynamicMatrix<RemoveCV_t<Type>>;

template <typename Type, size_t M, size_t N> DynamicMatrix(std::array<std::array<Type, N>, M>) -> DynamicMatrix<Type>;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, SO, Alloc, Tag>::DynamicMatrix(size_t m, size_t n, size_t nn, const Alloc &alloc,
														  Uninitialized)
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, SO, Alloc, Tag>::DynamicMatrix(size_t m, size_t n, size_t nn, size_t capa,
	...
	: DynamicMatrix(m, n, addPadding(n), alloc, Uninitialized{})
{
	...
	mtrc::numeric::uninitialized_default_construct_n(v_, capacity_);
...
// means of an initializer list:
   ...
   mtrc::numeric::DynamicMatrix<int,rowMajor> A{ { 1, 2, 3 },
										 { 4, 5 },
										 { 7, 8, 9 } };
   \endcode
...
// a dynamic array:
   ...
   mtrc::numeric::DynamicMatrix<int,rowMajor> v( 4UL, 5UL, array );
   delete[] array;
   \endcode
...
// a static array:
   ...
   const int init[3][3] = { { 1, 2, 3 },
							{ 4, 5 },
							{ 7, 8, 9 } };
   mtrc::numeric::DynamicMatrix<int,rowMajor> A( init );
   \endcode
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, SO, Alloc, Tag>::DynamicMatrix(const DynamicMatrix &m) : DynamicMatrix(m.m_, m.n_)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(capacity_ <= m.capacity_, "Invalid capacity estimation");
...
{
	mtrc::numeric::destroy_n(v_, capacity_);
	alloc_.deallocate(v_, capacity_);
}
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename DynamicMatrix<Type, SO, Alloc, Tag>::Reference DynamicMatrix<Type, SO, Alloc, Tag>::at(size_t i, size_t j)
{
	if (i >= m_) {
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename DynamicMatrix<Type, SO, Alloc, Tag>::ConstReference DynamicMatrix<Type, SO, Alloc, Tag>::at(size_t i,
																										 size_t j) const
{
	if (i >= m_) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename DynamicMatrix<Type, SO, Alloc, Tag>::Pointer DynamicMatrix<Type, SO, Alloc, Tag>::data() noexcept
{
	return v_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename DynamicMatrix<Type, SO, Alloc, Tag>::Pointer DynamicMatrix<Type, SO, Alloc, Tag>::data(size_t i) noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < m_, "Invalid dense matrix row access index");
	return v_ + i * nn_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename DynamicMatrix<Type, SO, Alloc, Tag>::Iterator DynamicMatrix<Type, SO, Alloc, Tag>::begin(size_t i) noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < m_, "Invalid dense matrix row access index");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename DynamicMatrix<Type, SO, Alloc, Tag>::Iterator DynamicMatrix<Type, SO, Alloc, Tag>::end(size_t i) noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < m_, "Invalid dense matrix row access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, SO, Alloc, Tag> &DynamicMatrix<Type, SO, Alloc, Tag>::operator=(const Type &rhs) &
{
	for (size_t i = 0UL; i < m_; ++i)
		for (size_t j = 0UL; j < n_; ++j)
			v_[i * nn_ + j] = rhs;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, SO, Alloc, Tag> &DynamicMatrix<Type, SO, Alloc, Tag>::operator=(const DynamicMatrix &rhs) &
{
	if (&rhs == this)
		return *this;
...
inline DynamicMatrix<Type, SO, Alloc, Tag> &DynamicMatrix<Type, SO, Alloc, Tag>::operator=(const Matrix<MT, SO2> &rhs) &
{
...
void DynamicMatrix<Type, SO, Alloc, Tag>::resize(size_t m, size_t n, bool preserve)
{
	...
	if (preserve) {
		...
		for (size_t i = 0UL; i < min_m; ++i) {
			mtrc::numeric::uninitialized_transfer(v_ + i * nn_, v_ + i * nn_ + min_n, tmp.v_ + i * nn);
			mtrc::numeric::uninitialized_default_construct(tmp.v_ + i * nn + min_n, tmp.v_ + i * nn + nn);
		}
		mtrc::numeric::uninitialized_default_construct(tmp.v_ + min_m * nn, tmp.v_ + m * nn);
		...
		mtrc::numeric::uninitialized_default_construct(tmp.v_, tmp.v_ + tmp.capacity_);
...
inline void DynamicMatrix<Type, SO, Alloc, Tag>::reserve(size_t elements)
{
	...
	if (elements > capacity_) {
		...
		mtrc::numeric::uninitialized_transfer(v_, v_ + capacity_, tmp.v_);
		mtrc::numeric::uninitialized_value_construct(tmp.v_ + capacity_, tmp.v_ + elements);
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::DynamicMatrix<int> A;
...
inline DynamicMatrix<Type, SO, Alloc, Tag> &DynamicMatrix<Type, SO, Alloc, Tag>::scale(const Other &scalar)
{
	for (size_t i = 0UL; i < m_; ++i)
		for (size_t j = 0UL; j < n_; ++j)
			v_[i * nn_ + j] *= scalar;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE typename DynamicMatrix<Type, SO, Alloc, Tag>::SIMDType
...
DynamicMatrix<Type, SO, Alloc, Tag>::loada(size_t i, size_t j) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(j + SIMDSIZE <= nn_, "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(!usePadding || j % SIMDSIZE == 0UL, "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(checkAlignment(v_ + i * nn_ + j), "Invalid alignment detected");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicMatrix<Type, SO, Alloc, Tag>::store(size_t i, size_t j,
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicMatrix<Type, SO, Alloc, Tag>::storea(size_t i, size_t j,
																			  const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicMatrix<Type, SO, Alloc, Tag>::storeu(size_t i, size_t j,
																			  const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicMatrix<Type, SO, Alloc, Tag>::stream(size_t i, size_t j,
																			  const SIMDType &value) noexcept
{
	...
	-> EnableIf_t<VectorizedAssign_v<MT>>
{
	...
	if (usePadding && useStreaming && (m_ * n_ > (cacheSize / (sizeof(Type) * 3UL))) && !(*rhs).isAliased(this)) {
		for (size_t i = 0UL; i < m_; ++i) {
			size_t j(0UL);
	...
	-> EnableIf_t<VectorizedAddAssign_v<MT>>
{
	...
	for (size_t i = 0UL; i < m_; ++i) {
		...
		for (; (j + SIMDSIZE * 3UL) < jpos; j += SIMDSIZE * 4UL) {
			left.store(left.load() + right.load());
			left += SIMDSIZE;
			right += SIMDSIZE;
			left.store(left.load() + right.load());
			left += SIMDSIZE;
...
inline void DynamicMatrix<Type, SO, Alloc, Tag>::addAssign(const DenseMatrix<MT, !SO> &rhs)
{
	...
	for (size_t ii = 0UL; ii < m_; ii += block) {
		const size_t iend(min(m_, ii + block));
		for (size_t jj = 0UL; jj < n_; jj += block) {
			if (IsLower_v<MT> && ii < jj)
				break;
			if (IsUpper_v<MT> && ii > jj)
				continue;

			for (size_t i = ii; i < iend; ++i) {
				const size_t jbegin((IsUpper_v<MT>) ? (max((IsStrictlyUpper_v<MT> ? i + 1UL : i), jj)) : (jj));
				const size_t jend((IsLower_v<MT>) ? (min((IsStrictlyLower_v<MT> ? i : i + 1UL), n_, jj + block))
												  : (min(n_, jj + block)));
				METRIC_NUMERIC_INTERNAL_ASSERT(jbegin <= jend, "Invalid loop indices detected");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
class DynamicMatrix<Type, true, Alloc, Tag> : public DenseMatrix<DynamicMatrix<Type, true, Alloc, Tag>, true> {
  private:
	//**********************************************************************************************
	//! Compilation switch for the choice of alignment.
	static constexpr AlignmentFlag align = (usePadding ? aligned : unaligned);
  ...
  public:
	...
	template <typename MT> inline auto assign(const DenseMatrix<MT, true> &rhs) -> DisableIf_t<VectorizedAssign_v<MT>>;

	template <typename MT> inline auto assign(const DenseMatrix<MT, true> &rhs) -> EnableIf_t<VectorizedAssign_v<MT>>;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, true, Alloc, Tag>::DynamicMatrix(size_t m, size_t mm, size_t n, const Alloc &alloc,
															Uninitialized)
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, true, Alloc, Tag>::DynamicMatrix(size_t m, size_t mm, size_t n, size_t capa,
															const Alloc &alloc, Uninitialized)
	: m_(m) // The current number of rows of the matrix
	  ,
...
// means of an initializer list:
   ...
   mtrc::numeric::DynamicMatrix<int,columnMajor> A{ { 1, 2, 3 },
											{ 4, 5 },
											{ 7, 8, 9 } };
   \endcode
	...
	: DynamicMatrix(list.size(), determineColumns(list), alloc)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(i == m_, "Invalid number of elements detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(), "Invariant violation detected");
}
/*! \endcond */
//*************************************************************************************************
...
// a dynamic array:
   ...
   int* array = new int[20];
   // ... Initialization of the dynamic array
   mtrc::numeric::DynamicMatrix<int,columnMajor> v( array, 5UL, 4UL );
   delete[] array;
   \endcode
...
// a static array:
   ...
   const int init[3][3] = { { 1, 2, 3 },
							{ 4, 5 },
							{ 7, 8, 9 } };
   mtrc::numeric::DynamicMatrix<int,columnMajor> A( init );
   \endcode
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, true, Alloc, Tag>::DynamicMatrix(const DynamicMatrix &m) : DynamicMatrix(m.m_, m.n_)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(capacity_ <= m.capacity_, "Invalid capacity estimation");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename DynamicMatrix<Type, true, Alloc, Tag>::Reference DynamicMatrix<Type, true, Alloc, Tag>::at(size_t i,
																										   size_t j)
{
	if (i >= m_) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename DynamicMatrix<Type, true, Alloc, Tag>::Pointer DynamicMatrix<Type, true, Alloc, Tag>::data() noexcept
{
	return v_;
}
/*! \endcond */
...
{
	METRIC_NUMERIC_USER_ASSERT(j < n_, "Invalid dense matrix column access index");
	return v_ + j * mm_;
}
/*! \endcond */
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, true, Alloc, Tag> &DynamicMatrix<Type, true, Alloc, Tag>::operator=(const Type &rhs) &
{
	for (size_t j = 0UL; j < n_; ++j)
		for (size_t i = 0UL; i < m_; ++i)
			v_[i + j * mm_] = rhs;
...
// by means of an initializer list:
   ...
   mtrc::numeric::DynamicMatrix<int,columnMajor> A;
   A = { { 1, 2, 3 },
		 { 4, 5 },
		 { 7, 8, 9 } };
   \endcode
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline DynamicMatrix<Type, true, Alloc, Tag> &DynamicMatrix<Type, true, Alloc, Tag>::operator=(DynamicMatrix &&rhs) &
{
	mtrc::numeric::destroy_n(v_, capacity_);
	alloc_.deallocate(v_, capacity_);
...
void DynamicMatrix<Type, true, Alloc, Tag>::resize(size_t m, size_t n, bool preserve)
{
	...
	if (preserve) {
		...
		for (size_t j = 0UL; j < min_n; ++j) {
			mtrc::numeric::uninitialized_transfer(v_ + j * mm_, v_ + j * mm_ + min_m, tmp.v_ + j * mm);
			mtrc::numeric::uninitialized_default_construct(tmp.v_ + j * mm + min_m, tmp.v_ + j * mm + mm);
		}
		mtrc::numeric::uninitialized_default_construct(tmp.v_ + min_n * mm, tmp.v_ + mm * n);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE typename DynamicMatrix<Type, true, Alloc, Tag>::SIMDType
...
DynamicMatrix<Type, true, Alloc, Tag>::loada(size_t i, size_t j) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(i < m_, "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(i + SIMDSIZE <= mm_, "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(!usePadding || i % SIMDSIZE == 0UL, "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(j < n_, "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(checkAlignment(v_ + i + j * mm_), "Invalid alignment detected");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicMatrix<Type, true, Alloc, Tag>::store(size_t i, size_t j,
																			   const SIMDType &value) noexcept
{
	if (usePadding)
		storea(i, j, value);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicMatrix<Type, true, Alloc, Tag>::storea(size_t i, size_t j,
																				const SIMDType &value) noexcept
{
	using mtrc::numeric::storea;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicMatrix<Type, true, Alloc, Tag>::storeu(size_t i, size_t j,
																				const SIMDType &value) noexcept
{
	using mtrc::numeric::storeu;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void DynamicMatrix<Type, true, Alloc, Tag>::stream(size_t i, size_t j,
																				const SIMDType &value) noexcept
{
	using mtrc::numeric::stream;
...
inline void DynamicMatrix<Type, true, Alloc, Tag>::addAssign(const DenseMatrix<MT, false> &rhs)
{
	...
	for (size_t jj = 0UL; jj < n_; jj += block) {
		...
		for (size_t ii = 0UL; ii < m_; ii += block) {
			...
			for (size_t j = jj; j < jend; ++j) {
				const size_t ibegin((IsLower_v<MT>) ? (max((IsStrictlyLower_v<MT> ? j + 1UL : j), ii)) : (ii));
				const size_t iend((IsUpper_v<MT>) ? (min((IsStrictlyUpper_v<MT> ? j : j + 1UL), m_, ii + block))
												  : (min(m_, ii + block)));
				METRIC_NUMERIC_INTERNAL_ASSERT(ibegin <= iend, "Invalid loop indices detected");
...
struct HasConstDataAccess<DynamicMatrix<T, SO, Alloc, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Alloc, typename Tag>
struct HasMutableDataAccess<DynamicMatrix<T, SO, Alloc, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Alloc, typename Tag>
struct IsAligned<DynamicMatrix<T, SO, Alloc, Tag>> : public BoolConstant<usePadding> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Alloc, typename Tag>
struct IsContiguous<DynamicMatrix<T, SO, Alloc, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Alloc, typename Tag>
struct IsPadded<DynamicMatrix<T, SO, Alloc, Tag>> : public BoolConstant<usePadding> {};
/*! \endcond */
//*************************************************************************************************
...
struct AddTraitEval2<T1, T2,
					 EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && (IsDenseMatrix_v<T1> || IsDenseMatrix_v<T2>) &&
	...
	using Type = DynamicMatrix<ET, SO, DynamicAllocator_t<ET, GetAllocator_t<T1>, GetAllocator_t<T2>>,
...
struct SchurTraitEval2<
	...
	EnableIf_t<IsDenseMatrix_v<T1> && IsDenseMatrix_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
			   ...
			   (Size_v<T2, 1UL> == DefaultSize_v) && (MaxSize_v<T1, 0UL> == DefaultMaxSize_v) &&
			   (MaxSize_v<T2, 0UL> == DefaultMaxSize_v) && (MaxSize_v<T1, 1UL> == DefaultMaxSize_v) &&
...
struct MultTraitEval2<T1, T2,
					  EnableIf_t<IsDenseMatrix_v<T1> && IsScalar_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
	...
	using Type = DynamicMatrix<ET, StorageOrder_v<T1>, DynamicAllocator_t<ET, GetAllocator_t<T1>>,
							   MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct MultTraitEval2<T1, T2,
					  EnableIf_t<IsScalar_v<T1> && IsDenseMatrix_v<T2> && (Size_v<T2, 0UL> == DefaultSize_v) &&
								 (Size_v<T2, 1UL> == DefaultSize_v) && (MaxSize_v<T2, 0UL> == DefaultMaxSize_v) &&
								 (MaxSize_v<T2, 1UL> == DefaultMaxSize_v)>> {
	using ET = MultTrait_t<T1, ElementType_t<T2>>;

	using Type = DynamicMatrix<ET, StorageOrder_v<T2>, DynamicAllocator_t<ET, GetAllocator_t<T2>>,
...
struct MultTraitEval2<
	T1, T2,
	EnableIf_t<IsDenseVector_v<T1> && IsDenseVector_v<T2> && IsColumnVector_v<T1> && IsRowVector_v<T2> &&
			   ((Size_v<T1, 0UL> == DefaultSize_v) || (Size_v<T2, 0UL> == DefaultSize_v)) &&
			   ((MaxSize_v<T1, 0UL> == DefaultMaxSize_v) || (MaxSize_v<T2, 0UL> == DefaultMaxSize_v))>> {
	using ET = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;

	using Type = DynamicMatrix<ET, false, DynamicAllocator_t<ET, GetAllocator_t<T1>, GetAllocator_t<T2>>,
							   MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
...
struct MultTraitEval2<
	T1, T2,
	EnableIf_t<
		IsMatrix_v<T1> && IsMatrix_v<T2> && (IsDenseMatrix_v<T1> || IsDenseMatrix_v<T2>) &&
		...
		((MaxSize_v<T1, 0UL> == DefaultMaxSize_v && (!IsSquare_v<T1> || MaxSize_v<T2, 0UL> == DefaultMaxSize_v)) ||
		 (MaxSize_v<T2, 1UL> == DefaultMaxSize_v && (!IsSquare_v<T2> || MaxSize_v<T1, 1UL> == DefaultMaxSize_v)))>> {
	...
	using Type = DynamicMatrix<ET, (IsSparseMatrix_v<T1> ? StorageOrder_v<T2> : StorageOrder_v<T1>),
							   DynamicAllocator_t<ET, GetAllocator_t<T1>, GetAllocator_t<T2>>, AddTrait_t<M2, M2>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct KronTraitEval2<
	...
	using Type = DynamicMatrix<ET, StorageOrder_v<T2>, DynamicAllocator_t<ET, GetAllocator_t<T1>, GetAllocator_t<T2>>,
							   MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct UnaryMapTraitEval2<
	T, OP,
	EnableIf_t<IsDenseMatrix_v<T> && (Size_v<T, 0UL> == DefaultSize_v || Size_v<T, 1UL> == DefaultSize_v) &&
			   (MaxSize_v<T, 0UL> == DefaultMaxSize_v || MaxSize_v<T, 1UL> == DefaultMaxSize_v)>> {
	using ET = EvaluateTrait_t<decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()))>;

	using Type =
		DynamicMatrix<ET, StorageOrder_v<T>, DynamicAllocator_t<ET, GetAllocator_t<T>>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BinaryMapTraitEval2<
	...
	EnableIf_t<IsColumnVector_v<T1> && IsRowVector_v<T2> &&
			   ((Size_v<T1, 0UL> == DefaultSize_v) || (Size_v<T2, 0UL> == DefaultSize_v)) &&
			   ((MaxSize_v<T1, 0UL> == DefaultMaxSize_v) || (MaxSize_v<T2, 0UL> == DefaultMaxSize_v))>> {
	using ET = EvaluateTrait_t<decltype(std::declval<OP>()(std::declval<ElementType_t<T1>>(),
...
struct ExpandTraitEval2<T, E,
	...
	using Type = DynamicMatrix<ElementType_t<T>, (IsColumnVector_v<T> ? columnMajor : rowMajor),
							   DynamicAllocator_t<ElementType_t<T>, GetAllocator_t<T>>, TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct RepeatTraitEval2<T, R0, R1, inf,
						EnableIf_t<IsDenseMatrix_v<T> &&
								   ((R0 == inf && R1 == inf) ||
									((Size_v<T, 0UL> == DefaultSize_v) && (MaxSize_v<T, 0UL> == DefaultMaxSize_v)) ||
									((Size_v<T, 1UL> == DefaultSize_v) && (MaxSize_v<T, 1UL> == DefaultMaxSize_v)))>> {
	using Type = DynamicMatrix<ElementType_t<T>, StorageOrder_v<T>,
							   DynamicAllocator_t<ElementType_t<T>, GetAllocator_t<T>>, TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SolveTraitEval2<
	...
	EnableIf_t<IsDenseMatrix_v<T1> && IsDenseMatrix_v<T2> &&
			   (((Size_v<T1, 0UL> == DefaultSize_v) && (Size_v<T2, 0UL> == DefaultSize_v) &&
				 (Size_v<T1, 1UL> == DefaultSize_v) && (MaxSize_v<T1, 0UL> == DefaultMaxSize_v) &&
				 (MaxSize_v<T2, 0UL> == DefaultMaxSize_v) && (MaxSize_v<T1, 1UL> == DefaultMaxSize_v)) ||
				((Size_v<T2, 1UL> == DefaultSize_v) && (MaxSize_v<T2, 1UL> == DefaultMaxSize_v)))>> {
	using Type = DynamicMatrix<ElementType_t<T2>, StorageOrder_v<T2>,
							   DynamicAllocator_t<ElementType_t<T2>, GetAllocator_t<T2>>, TagType_t<T2>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubmatrixTraitEval2<
	MT, inf, inf, inf, inf,
	EnableIf_t<IsDenseMatrix_v<MT> && (Size_v<MT, 0UL> == DefaultSize_v || Size_v<MT, 1UL> == DefaultSize_v) &&
			   (MaxSize_v<MT, 0UL> == DefaultMaxSize_v || MaxSize_v<MT, 1UL> == DefaultMaxSize_v)>> {
	using ET = RemoveConst_t<ElementType_t<MT>>;

	using Type = DynamicMatrix<ET, StorageOrder_v<MT>, DynamicAllocator_t<ET, GetAllocator_t<MT>>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct RowsTraitEval2<MT, M,
					  EnableIf_t<IsDenseMatrix_v<MT> && (M == 0UL || Size_v<MT, 1UL> == DefaultSize_v) &&
								 (M == 0UL || MaxSize_v<MT, 1UL> == DefaultMaxSize_v)>> {
	using ET = RemoveConst_t<ElementType_t<MT>>;

	using Type = DynamicMatrix<ET, false, DynamicAllocator_t<ET, GetAllocator_t<MT>>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct ColumnsTraitEval2<MT, N,
						 EnableIf_t<IsDenseMatrix_v<MT> && (N == 0UL || Size_v<MT, 0UL> == DefaultSize_v) &&
									(N == 0UL || MaxSize_v<MT, 0UL> == DefaultMaxSize_v)>> {
	using ET = RemoveConst_t<ElementType_t<MT>>;

	using Type = DynamicMatrix<ET, true, DynamicAllocator_t<ET, GetAllocator_t<MT>>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
