...
#ifndef METRIC_NUMERIC_MATH_DENSE_INVERSION_H
#define METRIC_NUMERIC_MATH_DENSE_INVERSION_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/lapack/getri.h>
#include <metric/numeric/math/lapack/hetrf.h>
#include <metric/numeric/math/lapack/hetri.h>
#include <metric/numeric/math/lapack/potrf.h>
#include <metric/numeric/math/lapack/potri.h>
#include <metric/numeric/math/lapack/sytrf.h>
#include <metric/numeric/math/lapack/sytri.h>
#include <metric/numeric/math/lapack/trtri.h>
...
*/
template <typename MT> constexpr InversionFlag getInversionFlag() noexcept
{
...
inline void invertGeneral2x2(DenseMatrix<MT, SO> &dm)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).rows() == 2UL, "Invalid number of rows detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).columns() == 2UL, "Invalid number of columns detected");
	...
	if (!isDivisor(det)) {
		METRIC_NUMERIC_THROW_DIVISION_BY_ZERO("Inversion of singular matrix failed");
	}
...
inline void invert2x2(DenseMatrix<MT, SO> &dm)
{
	...
	case byLU:
		...
		break;
	case byLDLT:
		invertSymmetric2x2(*dm);
		break;
	case byLDLH:
		invertHermitian2x2(*dm);
		break;
	case byLLH:
		invertHermitian2x2(*dm);
		break;
	...
	case asLower:
		...
		break;
	case asUniLower:
		invertUniLower2x2(*dm);
		break;
	case asUpper:
		invertUpper2x2(*dm);
		break;
	case asUniUpper:
		invertUniUpper2x2(*dm);
		break;
	case asDiagonal:
		invertDiagonal2x2(*dm);
		break;
	default:
		METRIC_NUMERIC_INTERNAL_ASSERT(false, "Unhandled case detected");
	}

	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(*dm), "Broken invariant detected");
}
/*! \endcond */
//*************************************************************************************************
...
inline void invertGeneral3x3(DenseMatrix<MT, SO> &dm)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).rows() == 3UL, "Invalid number of rows detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).columns() == 3UL, "Invalid number of columns detected");
...
inline void invert3x3(DenseMatrix<MT, SO> &dm)
{
	...
	case byLU:
		...
		break;
	case byLDLT:
		invertSymmetric3x3(*dm);
		break;
	case byLDLH:
		invertHermitian3x3(*dm);
		break;
	case byLLH:
		invertHermitian3x3(*dm);
		break;
	...
	case asLower:
		...
		break;
	case asUniLower:
		invertUniLower3x3(*dm);
		break;
	case asUpper:
		invertUpper3x3(*dm);
		break;
	case asUniUpper:
		invertUniUpper3x3(*dm);
		break;
	case asDiagonal:
		invertDiagonal3x3(*dm);
		break;
	default:
		METRIC_NUMERIC_INTERNAL_ASSERT(false, "Unhandled case detected");
	}
...
inline void invertGeneral4x4(DenseMatrix<MT, SO> &dm)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).rows() == 4UL, "Invalid number of rows detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).columns() == 4UL, "Invalid number of columns detected");
...
inline void invert4x4(DenseMatrix<MT, SO> &dm)
{
	...
	case byLU:
		...
		break;
	case byLDLT:
		invertSymmetric4x4(*dm);
		break;
	case byLDLH:
		invertHermitian4x4(*dm);
		break;
	case byLLH:
		invertHermitian4x4(*dm);
		break;
	...
	case asLower:
		...
		break;
	case asUniLower:
		invertUniLower4x4(*dm);
		break;
	case asUpper:
		invertUpper4x4(*dm);
		break;
	case asUniUpper:
		invertUniUpper4x4(*dm);
		break;
	case asDiagonal:
		invertDiagonal4x4(*dm);
		break;
	default:
		METRIC_NUMERIC_INTERNAL_ASSERT(false, "Unhandled case detected");
	}
...
inline void invertGeneral5x5(DenseMatrix<MT, SO> &dm)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).rows() == 5UL, "Invalid number of rows detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).columns() == 5UL, "Invalid number of columns detected");
...
inline void invert5x5(DenseMatrix<MT, SO> &dm)
{
	...
	case byLU:
		...
		break;
	case byLDLT:
		invertSymmetric5x5(*dm);
		break;
	case byLDLH:
		invertHermitian5x5(*dm);
		break;
	case byLLH:
		invertHermitian5x5(*dm);
		break;
	...
	case asLower:
		...
		break;
	case asUniLower:
		invertUniLower5x5(*dm);
		break;
	case asUpper:
		invertUpper5x5(*dm);
		break;
	case asUniUpper:
		invertUniUpper5x5(*dm);
		break;
	case asDiagonal:
		invertDiagonal5x5(*dm);
		break;
	default:
		METRIC_NUMERIC_INTERNAL_ASSERT(false, "Unhandled case detected");
	}
...
inline void invertGeneral6x6(DenseMatrix<MT, SO> &dm)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).rows() == 6UL, "Invalid number of rows detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).columns() == 6UL, "Invalid number of columns detected");
...
inline void invertLower6x6(DenseMatrix<MT, SO> &dm)
{
	...
	B(5, 0) =
		-A(1, 0) * tmp13 +
		A(1, 1) * (A(2, 0) * tmp7 - A(2, 2) * (A(3, 0) * tmp3 - A(3, 3) * (A(4, 0) * A(5, 4) - A(4, 4) * A(5, 0))));
	B(1, 1) = A(0, 0) * tmp9;
	B(2, 1) = -A(0, 0) * tmp10;
	B(3, 1) = A(0, 0) * tmp11;
	B(4, 1) = -A(0, 0) * tmp12;
...
inline void invertUpper6x6(DenseMatrix<MT, SO> &dm)
{
	...
	B(4, 4) = -A(5, 5) * tmp16;
	B(0, 5) =
		-A(4, 4) * (A(3, 3) * (A(2, 5) * tmp1 - A(2, 2) * (A(0, 1) * A(1, 5) - A(0, 5) * A(1, 1))) - A(3, 5) * tmp5) +
		A(4, 5) * tmp12;
...
inline void invert6x6(DenseMatrix<MT, SO> &dm)
{
	...
	case byLU:
		...
		break;
	case byLDLT:
		invertSymmetric6x6(*dm);
		break;
	case byLDLH:
		invertHermitian6x6(*dm);
		break;
	case byLLH:
		invertHermitian6x6(*dm);
		break;
	...
	case asLower:
		...
		break;
	case asUniLower:
		invertUniLower6x6(*dm);
		break;
	case asUpper:
		invertUpper6x6(*dm);
		break;
	case asUniUpper:
		invertUniUpper6x6(*dm);
		break;
	case asDiagonal:
		invertDiagonal6x6(*dm);
		break;
	default:
		METRIC_NUMERIC_INTERNAL_ASSERT(false, "Unhandled case detected");
	}
...
inline void invertByLDLT(DenseMatrix<MT, SO> &dm)
{
	...
	METRIC_NUMERIC_USER_ASSERT(isSymmetric(*dm), "Invalid non-symmetric matrix detected");
...
template <typename MT // Type of the dense matrix
		  ,
		  bool SO> // Storage order of the dense matrix
inline auto invertByLDLH(DenseMatrix<MT, SO> &dm) -> EnableIf_t<IsBuiltin_v<ElementType_t<MT>>>
{
	invertByLDLT(*dm);
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  bool SO> // Storage order of the dense matrix
inline auto invertByLDLH(DenseMatrix<MT, SO> &dm) -> EnableIf_t<IsComplex_v<ElementType_t<MT>>>
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_ADAPTOR_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_BLAS_COMPATIBLE_TYPE(ElementType_t<MT>);

	METRIC_NUMERIC_USER_ASSERT(isHermitian(*dm), "Invalid non-Hermitian matrix detected");
...
inline void invertByLLH(DenseMatrix<MT, SO> &dm)
{
	...
	METRIC_NUMERIC_USER_ASSERT(isHermitian(*dm), "Invalid non-symmetric matrix detected");
...
inline void invertNxN(DenseMatrix<MT, SO> &dm)
{
	...
	case asLower:
		...
		break;
	case asUniLower:
		invertUniLowerNxN(*dm);
		break;
	case asUpper:
		invertUpperNxN(*dm);
		break;
	case asUniUpper:
		invertUniUpperNxN(*dm);
		break;
	case asDiagonal:
		invertDiagonalNxN(*dm);
		break;
	default:
		METRIC_NUMERIC_INTERNAL_ASSERT(false, "Unhandled case detected");
	}
...
//@{
template <typename MT, bool SO> inline void invert(DenseMatrix<MT, SO> &dm);

template <InversionFlag IF, typename MT, bool SO> inline void invert(DenseMatrix<MT, SO> &dm);
//@}
//*************************************************************************************************
