...
#ifndef METRIC_NUMERIC_MATH_DENSE_CUSTOMVECTOR_H
#define METRIC_NUMERIC_MATH_DENSE_CUSTOMVECTOR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// elements of arbitrary type and a fixed size as a native \b Metric numeric dense vector data structure.
...
//          respect to the available instruction set (SSE, AVX, ...) or not (\c mtrc::numeric::aligned
//          or \c mtrc::numeric::unaligned).
//  - PF  : specified whether the represented, external arrays are properly padded with
//          respect to the available instruction set (SSE, AVX, ...) or not (\c mtrc::numeric::padded
//          or \c mtrc::numeric::unpadded).
...
//
// In comparison with the remaining \b Metric numeric dense vector types CustomVector has several special
...
// it provides everything that is required to use the array just like a native \b Metric numeric dense
...
// The following examples give an impression of several possible types of custom vectors:
   ...
   \code
   using mtrc::numeric::CustomVector;
   using mtrc::numeric::Deallocate;
   using mtrc::numeric::allocate;
   using mtrc::numeric::aligned;
   using mtrc::numeric::unaligned;
   using mtrc::numeric::padded;
   using mtrc::numeric::unpadded;
...
// As with all dense vectors it is possible to copy construct a custom vector:
   ...
   CustomType a( &vec[0], 5UL );     // Represent the std::vector as Metric numeric dense vector
...
// which array is referenced by the custom vector, but modifies the values of the array:
   ...
   CustomType c( &vec2[0], 5UL );    // Represent the std::vector as Metric numeric dense vector
...
// exception is thrown.
//
// Please also note that \b Metric numeric will zero initialize the padding elements in order to achieve
// maximum performance!
//
//
...
class CustomVector : public DenseVector<CustomVector<Type, AF, PF, TF, Tag, RT>, TF> {
  public:
	//**Type definitions****************************************************************************
	...
	template <typename Other, size_t Dim> inline CustomVector &operator=(const Other (&array)[Dim]);

	template <typename Other, size_t Dim> inline CustomVector &operator=(const std::array<Other, Dim> &array);
...
{
	if (ptr == nullptr) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid array of elements");
	}

	if (AF && !checkAlignment(ptr)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid alignment detected");
	}
}
//*************************************************************************************************
...
inline CustomVector<Type, AF, PF, TF, Tag, RT>::CustomVector(Type *ptr, size_t n, size_t nn)
	: size_(0UL) // The size/dimension of the vector
	  ...
	  v_(nullptr) // The custom array of elements
{
	METRIC_NUMERIC_STATIC_ASSERT(PF == padded);
	  ...
	  v_(v.v_) // The custom array of elements
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(v.data() == nullptr, "Invalid data reference detected");
}
//*************************************************************************************************
...
inline CustomVector<Type, AF, PF, TF, Tag, RT>::~CustomVector()
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_VECTOR_TYPE(RT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTOR_WITH_TRANSPOSE_FLAG(RT, TF);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<RT>);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_CONST(RT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_VOLATILE(RT);
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename RT> // Result type
inline
	typename CustomVector<Type, AF, PF, TF, Tag, RT>::Pointer CustomVector<Type, AF, PF, TF, Tag, RT>::data() noexcept
{
	return v_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename RT> // Result type
inline
	typename CustomVector<Type, AF, PF, TF, Tag, RT>::Iterator CustomVector<Type, AF, PF, TF, Tag, RT>::begin() noexcept
{
	return Iterator(v_);
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename RT> // Result type
inline
	typename CustomVector<Type, AF, PF, TF, Tag, RT>::Iterator CustomVector<Type, AF, PF, TF, Tag, RT>::end() noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
inline CustomVector<Type, AF, PF, TF, Tag, RT> &CustomVector<Type, AF, PF, TF, Tag, RT>::operator=(const Type &rhs)
{
...
{
	if (list.size() > size_) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to custom vector");
	}
...
{
	if (size_ != Dim) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid array size");
	}
...
CustomVector<Type, AF, PF, TF, Tag, RT>::operator=(CustomVector &&rhs) noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(rhs.data() == nullptr, "Invalid data reference detected");
...
CustomVector<Type, AF, PF, TF, Tag, RT>::operator*=(const Vector<VT, TF> &rhs)
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTOR_WITH_TRANSPOSE_FLAG(MultType, TF);
...
CustomVector<Type, AF, PF, TF, Tag, RT>::operator/=(const DenseVector<VT, TF> &rhs)
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_VECTOR_TYPE(DivType);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTOR_WITH_TRANSPOSE_FLAG(DivType, TF);
...
inline CustomVector<Type, AF, PF, TF, Tag, RT> &CustomVector<Type, AF, PF, TF, Tag, RT>::scale(const Other &scalar)
{
	for (size_t i = 0UL; i < size_; ++i)
		v_[i] *= scalar;
	return *this;
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE typename CustomVector<Type, AF, PF, TF, Tag, RT>::SIMDType
...
CustomVector<Type, AF, PF, TF, Tag, RT>::loada(size_t index) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(index + SIMDSIZE <= size_, "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(!AF || index % SIMDSIZE == 0UL, "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(checkAlignment(v_ + index), "Invalid vector access index");
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomVector<Type, AF, PF, TF, Tag, RT>::store(size_t index,
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomVector<Type, AF, PF, TF, Tag, RT>::storea(size_t index,
																				  const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomVector<Type, AF, PF, TF, Tag, RT>::storeu(size_t index,
																				  const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomVector<Type, AF, PF, TF, Tag, RT>::stream(size_t index,
																				  const SIMDType &value) noexcept
{
	...
	-> EnableIf_t<VectorizedAssign_v<VT>>
{
	...
	if (AF && useStreaming && size_ > (cacheSize / (sizeof(Type) * 3UL)) && !(*rhs).isAliased(this)) {
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(i4way <= size_, "Invalid end calculation");
	  ...
	  v_(ptr) // The custom array of elements
{
	...
	if (IsVectorizable_v<Type> && capacity_ < nextMultiple<size_t>(size_, SIMDSIZE)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Insufficient capacity for padded vector");
	}
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE typename CustomVector<Type, AF, padded, TF, Tag, RT>::SIMDType
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomVector<Type, AF, padded, TF, Tag, RT>::store(size_t index,
																					 const SIMDType &value) noexcept
{
	if (AF)
		storea(index, value);
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomVector<Type, AF, padded, TF, Tag, RT>::storea(size_t index,
																					  const SIMDType &value) noexcept
{
	using mtrc::numeric::storea;
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomVector<Type, AF, padded, TF, Tag, RT>::storeu(size_t index,
																					  const SIMDType &value) noexcept
{
	using mtrc::numeric::storeu;
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomVector<Type, AF, padded, TF, Tag, RT>::stream(size_t index,
																					  const SIMDType &value) noexcept
{
	using mtrc::numeric::stream;
...
// element is not 0. Following example demonstrates the use of the \a isDefault function:
   ...
   mtrc::numeric::CustomVector<int,aligned,padded> a( ... );
...
struct HasConstDataAccess<CustomVector<T, AF, PF, TF, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, AlignmentFlag AF, PaddingFlag PF, bool TF, typename Tag, typename RT>
struct HasMutableDataAccess<CustomVector<T, AF, PF, TF, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, AlignmentFlag AF, PaddingFlag PF, bool TF, typename Tag, typename RT>
struct IsCustom<CustomVector<T, AF, PF, TF, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct IsAligned<CustomVector<T, aligned, PF, TF, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, PaddingFlag PF, bool TF, typename Tag, typename RT>
struct IsContiguous<CustomVector<T, aligned, PF, TF, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct IsPadded<CustomVector<T, AF, padded, TF, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
