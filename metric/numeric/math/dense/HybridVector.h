...
#ifndef METRIC_NUMERIC_MATH_DENSE_HYBRIDVECTOR_H
#define METRIC_NUMERIC_MATH_DENSE_HYBRIDVECTOR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// the mtrc::numeric::StaticVector and the mtrc::numeric::DynamicVector class templates: Similar to the static
...
class HybridVector : public DenseVector<HybridVector<Type, N, TF, AF, PF, Tag>, TF> {
  public:
	//**Type definitions****************************************************************************
	...
	template <typename Other> inline HybridVector(size_t n, const Other *array);

	template <typename Other, size_t Dim> constexpr HybridVector(const Other (&array)[Dim]);

	template <typename Other, size_t Dim> constexpr HybridVector(const std::array<Other, Dim> &array);
	...
	template <typename Other, size_t Dim> constexpr HybridVector &operator=(const Other (&array)[Dim]) &;

	template <typename Other, size_t Dim> constexpr HybridVector &operator=(const std::array<Other, Dim> &array) &;
...
template <typename Type, typename... Ts> HybridVector(Type, Ts...) -> HybridVector<Type, 1 + sizeof...(Ts)>;

template <typename Type, size_t N> HybridVector(Type (&)[N]) -> HybridVector<RemoveCV_t<Type>, N>;

template <typename Type, size_t N> HybridVector(std::array<Type, N>) -> HybridVector<Type, N>;
...
{
	if (n > N) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid size for hybrid vector");
	}
...
// means of an initializer list:
   ...
   \code
   mtrc::numeric::HybridVector<double,6UL> v1{ 4.2, 6.3, -1.2 };
   \endcode
...
{
	if (size_ > N) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of hybrid vector");
	}
...
// with a dynamic array:
   ...
   mtrc::numeric::HybridVector<double,6> v( 6UL, array );
   delete[] array;
   \endcode
...
// with a static array:
   ...
   mtrc::numeric::HybridVector<double,4> v( init );
   \endcode
						 ...
						 // v_ is intentionally left uninitialized
{
	...
	for (size_t i = (IsSparseVector_v<VT> ? 0UL : size_); i < (IsNumeric_v<Type> ? NN : size_); ++i) {
		clear(v_[i]);
	}
...
{
	METRIC_NUMERIC_USER_ASSERT(index < size_, "Invalid vector access index");
	return v_[index];
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
inline
	typename HybridVector<Type, N, TF, AF, PF, Tag>::Reference HybridVector<Type, N, TF, AF, PF, Tag>::at(size_t index)
{
	if (index >= size_) {
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
constexpr
	typename HybridVector<Type, N, TF, AF, PF, Tag>::Pointer HybridVector<Type, N, TF, AF, PF, Tag>::data() noexcept
{
	return v_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
constexpr
	typename HybridVector<Type, N, TF, AF, PF, Tag>::Iterator HybridVector<Type, N, TF, AF, PF, Tag>::begin() noexcept
{
	return Iterator(v_);
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
constexpr
	typename HybridVector<Type, N, TF, AF, PF, Tag>::Iterator HybridVector<Type, N, TF, AF, PF, Tag>::end() noexcept
{
	METRIC_NUMERIC_INTERNAL_ASSERT(size_ <= N, "Invalid size detected");
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr HybridVector<Type, N, TF, AF, PF, Tag> &HybridVector<Type, N, TF, AF, PF, Tag>::operator=(const Type &rhs) &
{
	METRIC_NUMERIC_INTERNAL_ASSERT(size_ <= N, "Invalid size detected");
...
// by means of an initializer list:
   ...
   \code
   mtrc::numeric::HybridVector<double,6UL> v;
   v = { 4.2, 6.3, -1.2 };
   \endcode
...
{
	if (list.size() > N) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to hybrid vector");
	}
...
// This assignment operator offers the option to directly set all elements of the vector:
   ...
   \code
   const double init[2] = { 1.0, 2.0 };
   mtrc::numeric::HybridVector<double,4> v;
   v = init;
   \endcode
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::HybridVector<int,3> a;
...
inline HybridVector<Type, N, TF, AF, PF, Tag> &HybridVector<Type, N, TF, AF, PF, Tag>::scale(const Other &scalar)
{
...
inline void *HybridVector<Type, N, TF, AF, PF, Tag>::operator new(std::size_t size)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(size == sizeof(HybridVector), "Invalid number of bytes detected");
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(size >= sizeof(HybridVector), "Invalid number of bytes detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(size % sizeof(HybridVector) == 0UL, "Invalid number of bytes detected");
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE typename HybridVector<Type, N, TF, AF, PF, Tag>::SIMDType
...
HybridVector<Type, N, TF, AF, PF, Tag>::loada(size_t index) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(index < size_, "Invalid vector access index");
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void HybridVector<Type, N, TF, AF, PF, Tag>::store(size_t index,
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void HybridVector<Type, N, TF, AF, PF, Tag>::storea(size_t index,
																				 const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void HybridVector<Type, N, TF, AF, PF, Tag>::storeu(size_t index,
																				 const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE void HybridVector<Type, N, TF, AF, PF, Tag>::stream(size_t index,
																				 const SIMDType &value) noexcept
{
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT((*rhs).size() == size_, "Invalid vector sizes");
	...
	-> EnableIf_t<VectorizedAssign_v<VT>>
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(ipos <= size_, "Invalid end calculation");
...
// return \a false. The following example demonstrates the use of the \a isDefault() function:
   ...
   \code
   mtrc::numeric::HybridVector<double,3> a;
   // ... Resizing and initialization
   if( isDefault( a ) ) { ... }
   \endcode
...
struct MaxSize<HybridVector<T, N, TF, AF, PF, Tag>, 0UL> : public Ptrdiff_t<static_cast<ptrdiff_t>(N)> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct HasConstDataAccess<HybridVector<T, N, TF, AF, PF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct HasMutableDataAccess<HybridVector<T, N, TF, AF, PF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct IsAligned<HybridVector<T, N, TF, AF, PF, Tag>> : public BoolConstant<AF == aligned> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct IsContiguous<HybridVector<T, N, TF, AF, PF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t N, bool TF, AlignmentFlag AF, PaddingFlag PF, typename Tag>
struct IsPadded<HybridVector<T, N, TF, AF, PF, Tag>> : public BoolConstant<PF == padded> {};
/*! \endcond */
//*************************************************************************************************
...
struct AddTraitEval2<T1, T2,
					 EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
	...
	using Type = HybridVector<AddTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
							  min(size_t(MaxSize_v<T1, 0UL>), size_t(MaxSize_v<T2, 0UL>)), TransposeFlag_v<T1>,
							  defaultAlignmentFlag, defaultPaddingFlag, AddTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubTraitEval2<T1, T2,
	...
	using Type = HybridVector<SubTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
							  min(size_t(MaxSize_v<T1, 0UL>), size_t(MaxSize_v<T2, 0UL>)), TransposeFlag_v<T1>,
							  defaultAlignmentFlag, defaultPaddingFlag, SubTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct MultTraitEval2<T1, T2,
					  EnableIf_t<IsVector_v<T1> && IsScalar_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
								 (MaxSize_v<T1, 0UL> != DefaultMaxSize_v)>> {
	using Type = HybridVector<MultTrait_t<ElementType_t<T1>, T2>, MaxSize_v<T1, 0UL>, TransposeFlag_v<T1>,
							  defaultAlignmentFlag, defaultPaddingFlag, MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct MultTraitEval2<T1, T2,
					  EnableIf_t<IsScalar_v<T1> && IsVector_v<T2> && (Size_v<T2, 0UL> == DefaultSize_v) &&
								 (MaxSize_v<T2, 0UL> != DefaultMaxSize_v)>> {
	using Type = HybridVector<MultTrait_t<T1, ElementType_t<T2>>, MaxSize_v<T2, 0UL>, TransposeFlag_v<T2>,
...
struct MultTraitEval2<
	...
	EnableIf_t<((IsRowVector_v<T1> && IsRowVector_v<T2>) || (IsColumnVector_v<T1> && IsColumnVector_v<T2>)) &&
			   IsDenseVector_v<T1> && IsDenseVector_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
			   (Size_v<T2, 0UL> == DefaultSize_v) &&
			   (MaxSize_v<T1, 0UL> != DefaultMaxSize_v || MaxSize_v<T2, 0UL> != DefaultMaxSize_v)>> {
...
struct MultTraitEval2<
	...
	using Type = HybridVector<AddTrait_t<MultType, MultType>,
							  (MaxSize_v<T1, 0UL> != DefaultMaxSize_v ? MaxSize_v<T1, 0UL> : MaxSize_v<T2, 0UL>), false,
...
struct MultTraitEval2<
	...
	using Type = HybridVector<AddTrait_t<MultType, MultType>,
							  (MaxSize_v<T2, 1UL> != DefaultMaxSize_v ? MaxSize_v<T2, 1UL> : MaxSize_v<T1, 0UL>), true,
							  defaultAlignmentFlag, defaultPaddingFlag, AddTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct KronTraitEval2<
	...
	EnableIf_t<IsDenseVector_v<T1> && IsDenseVector_v<T2> &&
			   ...
			   (MaxSize_v<T1, 0UL> != DefaultMaxSize_v) && (MaxSize_v<T2, 0UL> != DefaultMaxSize_v)>> {
	using Type = HybridVector<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
							  MaxSize_v<T1, 0UL> * MaxSize_v<T2, 0UL>, TransposeFlag_v<T2>, defaultAlignmentFlag,
...
struct DivTraitEval2<T1, T2,
					 EnableIf_t<IsVector_v<T1> && IsScalar_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
								(MaxSize_v<T1, 0UL> != DefaultMaxSize_v)>> {
	using Type = HybridVector<DivTrait_t<ElementType_t<T1>, T2>, MaxSize_v<T1, 0UL>, TransposeFlag_v<T1>,
...
struct DivTraitEval2<T1, T2,
	...
	using Type = HybridVector<DivTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
							  min(size_t(MaxSize_v<T1, 0UL>), size_t(MaxSize_v<T2, 0UL>)), TransposeFlag_v<T1>,
							  defaultAlignmentFlag, defaultPaddingFlag, DivTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct UnaryMapTraitEval2<
	T, OP, EnableIf_t<IsVector_v<T> && Size_v<T, 0UL> == DefaultSize_v && MaxSize_v<T, 0UL> != DefaultMaxSize_v>> {
	using ElementType = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()));

	using Type = HybridVector<EvaluateTrait_t<ElementType>, MaxSize_v<T, 0UL>, TransposeFlag_v<T>, defaultAlignmentFlag,
...
struct BinaryMapTraitEval2<
	...
	using Type = HybridVector<EvaluateTrait_t<ElementType>, min(size_t(MaxSize_v<T1, 0UL>), size_t(MaxSize_v<T2, 0UL>)),
...
struct PartialReduceTraitEval2<
	...
	using Type =
		HybridVector<decltype(std::declval<OP>()(std::declval<ET>(), std::declval<ET>())), MaxSize_v<T, TF ? 1UL : 0UL>,
...
struct RepeatTraitEval2<T, R0, inf, inf,
	...
	using Type = HybridVector<ElementType_t<T>, R0 * MaxSize_v<T, 0UL>, TransposeFlag_v<T>, defaultAlignmentFlag,
...
struct SolveTraitEval2<
	T1, T2,
	EnableIf_t<IsDenseMatrix_v<T1> && IsDenseVector_v<T2> && (Size_v<T1, 0UL> == DefaultSize_v) &&
			   (Size_v<T1, 1UL> == DefaultSize_v) && (Size_v<T2, 0UL> == DefaultSize_v) &&
	...
	using Type = HybridVector<ElementType_t<T2>, N, TransposeFlag_v<T2>, defaultAlignmentFlag, defaultPaddingFlag,
							  TagType_t<T2>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubvectorTraitEval2<
	VT, inf, inf,
	EnableIf_t<IsDenseVector_v<VT> && (Size_v<VT, 0UL> != DefaultSize_v || MaxSize_v<VT, 0UL> != DefaultMaxSize_v)>> {
	using Type = HybridVector<RemoveConst_t<ElementType_t<VT>>, max(Size_v<VT, 0UL>, MaxSize_v<VT, 0UL>),
							  TransposeFlag_v<VT>, defaultAlignmentFlag, defaultPaddingFlag, TagType_t<VT>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct RowTraitEval2<
	MT, I,
	EnableIf_t<IsDenseMatrix_v<MT> && Size_v<MT, 1UL> == DefaultSize_v && MaxSize_v<MT, 1UL> != DefaultMaxSize_v>> {
	using Type = HybridVector<RemoveConst_t<ElementType_t<MT>>, MaxSize_v<MT, 1UL>, true, defaultAlignmentFlag,
...
struct ColumnTraitEval2<
	MT, I,
	EnableIf_t<IsDenseMatrix_v<MT> && Size_v<MT, 0UL> == DefaultSize_v && MaxSize_v<MT, 0UL> != DefaultMaxSize_v>> {
	using Type = HybridVector<RemoveConst_t<ElementType_t<MT>>, MaxSize_v<MT, 0UL>, false, defaultAlignmentFlag,
							  defaultPaddingFlag, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BandTraitEval2<
	...
	using Type = HybridVector<RemoveConst_t<ElementType_t<MT>>, Min, defaultTransposeFlag, defaultAlignmentFlag,
							  defaultPaddingFlag, TagType_t<MT>>;
};

template <typename MT>
struct BandTraitEval2<
	MT, inf, EnableIf_t<IsDenseMatrix_v<MT> && Size_v<MT, 0UL> != DefaultSize_v && Size_v<MT, 1UL> != DefaultSize_v>> {
	using Type = HybridVector<RemoveConst_t<ElementType_t<MT>>, min(Size_v<MT, 0UL>, Size_v<MT, 1UL>),
							  defaultTransposeFlag, defaultAlignmentFlag, defaultPaddingFlag, TagType_t<MT>>;
};
...
struct BandTraitEval2<
	...
	EnableIf_t<IsDenseMatrix_v<MT> && (Size_v<MT, 0UL> == DefaultSize_v || Size_v<MT, 1UL> == DefaultSize_v) &&
			   MaxSize_v<MT, 0UL> != DefaultMaxSize_v && MaxSize_v<MT, 1UL> != DefaultMaxSize_v>> {
	using Type = HybridVector<RemoveConst_t<ElementType_t<MT>>, min(MaxSize_v<MT, 0UL>, MaxSize_v<MT, 1UL>),
							  defaultTransposeFlag, defaultAlignmentFlag, defaultPaddingFlag, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
