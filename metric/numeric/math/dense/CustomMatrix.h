...
#ifndef METRIC_NUMERIC_MATH_DENSE_CUSTOMMATRIX_H
#define METRIC_NUMERIC_MATH_DENSE_CUSTOMMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// elements of arbitrary type and a fixed size as a native \b Metric numeric dense matrix data structure.
...
//
// In comparison with the remaining \b Metric numeric dense matrix types CustomMatrix has several special
...
// As with all dense matrices it is possible to copy construct a custom matrix:
   ...
   CustomType A( &vec[0], 2UL, 3UL );  // Represent the std::vector as Metric numeric dense matrix
...
// which array is referenced by the custom matrices, but modifies the values of the array:
   ...
   CustomType C( &vec2[0], 2UL, 3UL );  // Represent the std::vector as Metric numeric dense matrix
...
// in case of padded matrices, \b Metric numeric will zero initialize the padding element and use them
// in all computations in order to achieve maximum performance. In case of an unpadded matrix
// \b Metric numeric will ignore the elements with the downside that it is not possible to load a complete
...
class CustomMatrix : public DenseMatrix<CustomMatrix<Type, AF, PF, SO, Tag, RT>, SO> {
  public:
	//**Type definitions****************************************************************************
...
{
	METRIC_NUMERIC_STATIC_ASSERT(PF == unpadded);
	  ...
	  v_(ptr) // The custom array of elements
{
	...
	if (PF && IsVectorizable_v<Type> && (nn_ < nextMultiple<size_t>(n_, SIMDSIZE))) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Insufficient capacity for padded matrix");
	}
	  ...
	  v_(m.v_) // The custom array of elements
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(m.data() == nullptr, "Invalid data reference detected");
}
//*************************************************************************************************
...
inline CustomMatrix<Type, AF, PF, SO, Tag, RT>::~CustomMatrix()
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(RT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_MAJOR_MATRIX_TYPE(RT);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
inline typename CustomMatrix<Type, AF, PF, SO, Tag, RT>::Reference CustomMatrix<Type, AF, PF, SO, Tag, RT>::at(size_t i,
																											   size_t j)
{
	if (i >= m_) {
...
template <typename Type // Data type of the matrix
		  ...
		  typename RT> // Result type
inline
	typename CustomMatrix<Type, AF, PF, SO, Tag, RT>::Pointer CustomMatrix<Type, AF, PF, SO, Tag, RT>::data() noexcept
{
	return v_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
inline CustomMatrix<Type, AF, PF, SO, Tag, RT> &CustomMatrix<Type, AF, PF, SO, Tag, RT>::operator=(const Type &rhs)
{
	for (size_t i = 0UL; i < m_; ++i)
...
// by means of an initializer list:
   ...
   const int array[9] = { 0, 0, 0,
						  0, 0, 0,
						  0, 0, 0 };
   mtrc::numeric::CustomMatrix<int,unaligned,unpadded,rowMajor> A( array, 3UL, 3UL );
   A = { { 1, 2, 3 },
		 { 4, 5 },
		 { 7, 8, 9 } };
   \endcode
...
CustomMatrix<Type, AF, PF, SO, Tag, RT>::operator=(initializer_list<initializer_list<Type>> list)
{
	if (list.size() != m_ || determineColumns(list) > n_) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to custom matrix");
	}

	size_t i(0UL);

	for (const auto &rowList : list) {
		std::fill(std::copy(rowList.begin(), rowList.end(), v_ + i * nn_), v_ + i * nn_ + (PF ? nn_ : n_), Type());
		++i;
	}
...
inline CustomMatrix<Type, AF, PF, SO, Tag, RT> &CustomMatrix<Type, AF, PF, SO, Tag, RT>::scale(const Other &scalar)
{
	for (size_t i = 0UL; i < m_; ++i)
		for (size_t j = 0UL; j < n_; ++j)
			v_[i * nn_ + j] *= scalar;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE typename CustomMatrix<Type, AF, PF, SO, Tag, RT>::SIMDType
...
CustomMatrix<Type, AF, PF, SO, Tag, RT>::loada(size_t i, size_t j) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(j + SIMDSIZE <= (PF ? nn_ : n_), "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(!PF || j % SIMDSIZE == 0UL, "Invalid column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomMatrix<Type, AF, PF, SO, Tag, RT>::store(size_t i, size_t j,
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomMatrix<Type, AF, PF, SO, Tag, RT>::storea(size_t i, size_t j,
																				  const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomMatrix<Type, AF, PF, SO, Tag, RT>::storeu(size_t i, size_t j,
																				  const SIMDType &value) noexcept
{
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomMatrix<Type, AF, PF, SO, Tag, RT>::stream(size_t i, size_t j,
																				  const SIMDType &value) noexcept
{
	...
	-> EnableIf_t<VectorizedAssign_v<MT>>
{
	...
	if (AF && PF && useStreaming && (m_ * n_ > (cacheSize / (sizeof(Type) * 3UL))) && !(*rhs).isAliased(this)) {
		for (size_t i = 0UL; i < m_; ++i) {
			size_t j(0UL);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
class CustomMatrix<Type, AF, PF, true, Tag, RT> : public DenseMatrix<CustomMatrix<Type, AF, PF, true, Tag, RT>, true> {
  public:
	//**Type definitions****************************************************************************
...
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(RT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(RT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_CONST(RT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_VOLATILE(RT);
}
/*! \endcond */
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
inline CustomMatrix<Type, AF, PF, true, Tag, RT> &CustomMatrix<Type, AF, PF, true, Tag, RT>::operator=(const Type &rhs)
{
	for (size_t j = 0UL; j < n_; ++j)
		for (size_t i = 0UL; i < m_; ++i)
			v_[i + j * mm_] = rhs;
...
// This assignment operator offers the option to directly set all elements of the matrix:
   ...
   const int init[3][3] = { { 1, 2, 3 },
							{ 4, 5 },
							{ 7, 8, 9 } };
   mtrc::numeric::CustomMatrix<int,unaligned,unpadded,columnMajor> A( array, 3UL, 3UL );
   A = init;
   \endcode
...
template <typename Type // Data type of the matrix
		  ...
		  typename RT>	  // Result type
template <typename Other> // Data type of the scalar value
inline CustomMatrix<Type, AF, PF, true, Tag, RT> &CustomMatrix<Type, AF, PF, true, Tag, RT>::scale(const Other &scalar)
{
	for (size_t j = 0UL; j < n_; ++j)
		for (size_t i = 0UL; i < m_; ++i)
			v_[i + j * mm_] *= scalar;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE typename CustomMatrix<Type, AF, PF, true, Tag, RT>::SIMDType
...
CustomMatrix<Type, AF, PF, true, Tag, RT>::loada(size_t i, size_t j) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(i < m_, "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(i + SIMDSIZE <= (PF ? mm_ : m_), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(!PF || i % SIMDSIZE == 0UL, "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(j < n_, "Invalid column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomMatrix<Type, AF, PF, true, Tag, RT>::store(size_t i, size_t j,
																				   const SIMDType &value) noexcept
{
	if (AF && PF)
		storea(i, j, value);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomMatrix<Type, AF, PF, true, Tag, RT>::storea(size_t i, size_t j,
																					const SIMDType &value) noexcept
{
	using mtrc::numeric::storea;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomMatrix<Type, AF, PF, true, Tag, RT>::storeu(size_t i, size_t j,
																					const SIMDType &value) noexcept
{
	using mtrc::numeric::storeu;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename RT> // Result type
METRIC_NUMERIC_ALWAYS_INLINE void CustomMatrix<Type, AF, PF, true, Tag, RT>::stream(size_t i, size_t j,
																					const SIMDType &value) noexcept
{
	using mtrc::numeric::stream;
...
// \a isDefault() function:
   ...
   mtrc::numeric::CustomMatrix<int,aligned,padded> A( ... );
...
struct HasConstDataAccess<CustomMatrix<T, AF, PF, SO, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, AlignmentFlag AF, PaddingFlag PF, bool SO, typename Tag, typename RT>
struct HasMutableDataAccess<CustomMatrix<T, AF, PF, SO, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, AlignmentFlag AF, PaddingFlag PF, bool SO, typename Tag, typename RT>
struct IsCustom<CustomMatrix<T, AF, PF, SO, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct IsAligned<CustomMatrix<T, aligned, PF, SO, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, AlignmentFlag AF, PaddingFlag PF, bool SO, typename Tag, typename RT>
struct IsContiguous<CustomMatrix<T, AF, PF, SO, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct IsPadded<CustomMatrix<T, AF, padded, SO, Tag, RT>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
