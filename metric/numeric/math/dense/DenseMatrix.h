...
#ifndef METRIC_NUMERIC_MATH_DENSE_DENSEMATRIX_H
#define METRIC_NUMERIC_MATH_DENSE_DENSEMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/lapack/clapack/potrf.h>
...
#include <metric/numeric/util/Limits.h>
...
auto operator==(const DenseMatrix<T1, false> &mat, T2 scalar) -> EnableIf_t<IsScalar_v<T2>, bool>;

template <typename T1, typename T2>
auto operator==(const DenseMatrix<T1, true> &mat, T2 scalar) -> EnableIf_t<IsScalar_v<T2>, bool>;

template <typename T1, typename T2, bool SO>
auto operator==(T1 scalar, const DenseMatrix<T2, SO> &mat) -> EnableIf_t<IsScalar_v<T2>, bool>;

template <typename T1, typename T2, bool SO>
auto operator!=(const DenseMatrix<T1, SO> &mat, T2 scalar) -> EnableIf_t<IsScalar_v<T2>, bool>;
...
template <typename MT, bool SO> MT &operator<<=(DenseMatrix<MT, SO> &mat, int count);
...
inline auto operator==(const DenseMatrix<T1, false> &mat, T2 scalar) -> EnableIf_t<IsScalar_v<T2>, bool>
{
...
template <typename T1 // Type of the left-hand side dense matrix
		  ,
		  typename T2> // Type of the right-hand side scalar
inline auto operator==(const DenseMatrix<T1, true> &mat, T2 scalar) -> EnableIf_t<IsScalar_v<T2>, bool>
{
	using CT1 = CompositeType_t<T1>;
...
inline auto operator==(T1 scalar, const DenseMatrix<T2, SO> &mat) -> EnableIf_t<IsScalar_v<T1>, bool>
{
	return (mat == scalar);
}
//*************************************************************************************************
...
template <typename T1 // Type of the left-hand side dense matrix
		  ...
		  ,
		  bool SO> // Storage order
inline auto operator!=(const DenseMatrix<T1, SO> &mat, T2 scalar) -> EnableIf_t<IsScalar_v<T2>, bool>
{
	return !(mat == scalar);
}
//*************************************************************************************************
...
inline auto operator+=(DenseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
	...
	if (IsRestricted_v<MT>) {
		if (!tryAdd(*mat, 0UL, 0UL, (*mat).rows(), (*mat).columns(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid addition to restricted matrix");
		}
	}
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(*mat), "Invariant violation detected");
...
inline auto operator-=(DenseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
	...
	if (IsRestricted_v<MT>) {
		if (!trySub(*mat, 0UL, 0UL, (*mat).rows(), (*mat).columns(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid subtraction from restricted matrix");
		}
	}
...
inline auto operator*=(DenseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
	...
	if (IsRestricted_v<MT>) {
		if (!tryMult(*mat, 0UL, 0UL, (*mat).rows(), (*mat).columns(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid scaling of restricted matrix");
		}
	}
...
inline auto operator/=(DenseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
	...
	METRIC_NUMERIC_USER_ASSERT(isDivisor(scalar), "Division by zero detected");
...
inline MT &operator<<=(DenseMatrix<MT, SO> &mat, int count)
{
	...
	if (IsRestricted_v<MT>) {
		if (!tryShift(*mat, 0UL, 0UL, (*mat).rows(), (*mat).columns(), count)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid left-shift of restricted matrix");
		}
	}
...
inline MT1 &operator<<=(DenseMatrix<MT1, SO1> &lhs, const DenseMatrix<MT2, SO2> &rhs)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(*lhs), "Invariant violation detected");
...
inline MT &operator>>=(DenseMatrix<MT, SO> &mat, int count)
{
	...
	if (IsRestricted_v<MT>) {
		if (!tryShift(*mat, 0UL, 0UL, (*mat).rows(), (*mat).columns(), count)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid right-shift of restricted matrix");
		}
	}
...
inline auto operator&=(DenseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
	...
	if (IsRestricted_v<MT>) {
		if (!tryBitand(*mat, 0UL, 0UL, (*mat).rows(), (*mat).columns(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid bitwise AND of restricted matrix");
		}
	}
...
inline auto operator|=(DenseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
	...
	if (IsRestricted_v<MT>) {
		if (!tryBitor(*mat, 0UL, 0UL, (*mat).rows(), (*mat).columns(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid bitwise OR of restricted matrix");
		}
	}
...
inline auto operator^=(DenseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
	...
	if (IsRestricted_v<MT>) {
		if (!tryBitxor(*mat, 0UL, 0UL, (*mat).rows(), (*mat).columns(), scalar)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid bitwise XOR of restricted matrix");
		}
	}
...
//@{
template <typename MT, bool SO> bool isnan(const DenseMatrix<MT, SO> &dm);

template <typename MT, bool SO> bool isinf(const DenseMatrix<MT, SO> &dm);

template <typename MT, bool SO> bool isfinite(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isSymmetric(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isHermitian(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isUniform(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isZero(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isLower(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isUniLower(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isStrictlyLower(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isUpper(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isUniUpper(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isStrictlyUpper(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isDiagonal(const DenseMatrix<MT, SO> &dm);

template <RelaxationFlag RF, typename MT, bool SO> bool isIdentity(const DenseMatrix<MT, SO> &dm);

template <typename MT, bool SO> bool isPositiveDefinite(const DenseMatrix<MT, SO> &dm);
//@}
//*************************************************************************************************
...
// \a false.
   ...
   \code
   mtrc::numeric::DynamicMatrix<double> A( 3UL, 4UL );
   // ... Initialization
   if( isnan( A ) ) { ... }
   \endcode
*/
...
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_TRIANGULAR_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(MT);

	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).rows() != 0UL, "Invalid number of rows detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*dm).columns() != 0UL, "Invalid number of columns detected");
...
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_TRIANGULAR_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(MT);
...
{
	if (IsUniform_v<MT> || (*dm).rows() == 0UL || (*dm).columns() == 0UL ||
...
bool isZero(const DenseMatrix<MT, SO> &dm)
{
	...
	if (SO == rowMajor) {
		for (size_t i = 0UL; i < M; ++i) {
			...
			const size_t jend(
				IsLower_v<MT> || IsSymmetric_v<MT> || IsHermitian_v<MT> ? (IsStrictlyLower_v<MT> ? i : i + 1UL) : N);
		...
		for (size_t j = 0UL; j < N; ++j) {
			...
			const size_t iend(
				IsUpper_v<MT> || IsSymmetric_v<MT> || IsHermitian_v<MT> ? (IsStrictlyUpper_v<MT> ? j : j + 1UL) : M);
...
// Example:
   ...
   \code
   mtrc::numeric::DynamicMatrix<double> A( 5UL, 8UL );
   // ... Initialization
   rank( A );
   \endcode
