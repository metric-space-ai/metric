...
#ifndef METRIC_NUMERIC_MATH_DENSE_UNIFORMVECTOR_H
#define METRIC_NUMERIC_MATH_DENSE_UNIFORMVECTOR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/dense/UniformIterator.h>
...
// UniformVector:
   ...
   \code
   using mtrc::numeric::UniformVector;
...
class UniformVector : public Expression<DenseVector<UniformVector<Type, TF, Tag>, TF>> {
  public:
	//**Type definitions****************************************************************************
	...
	template <typename VT> inline UniformVector(const Vector<VT, TF> &v);
	...
	template <typename ST> inline auto operator*=(ST rhs) & -> EnableIf_t<IsScalar_v<ST>, UniformVector &>;
	  ...
	  value_() // The value of all elements of the uniform vector
{
	...
	if (!IsUniform_v<VT> && !isUniform(*v)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of uniform vector");
	}
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename UniformVector<Type, TF, Tag>::ConstReference UniformVector<Type, TF, Tag>::at(size_t index) const
{
	if (index >= size_) {
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename UniformVector<Type, TF, Tag>::ConstPointer UniformVector<Type, TF, Tag>::data() const noexcept
{
	return &value_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename UniformVector<Type, TF, Tag>::ConstIterator UniformVector<Type, TF, Tag>::begin() const noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename UniformVector<Type, TF, Tag>::ConstIterator UniformVector<Type, TF, Tag>::cbegin() const noexcept
{
	return ConstIterator(&value_, 0UL);
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename UniformVector<Type, TF, Tag>::ConstIterator UniformVector<Type, TF, Tag>::end() const noexcept
{
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename UniformVector<Type, TF, Tag>::ConstIterator UniformVector<Type, TF, Tag>::cend() const noexcept
{
	return ConstIterator(&value_, size_);
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr UniformVector<Type, TF, Tag> &UniformVector<Type, TF, Tag>::operator=(const Type &rhs) &
{
	value_ = rhs;
...
inline UniformVector<Type, TF, Tag> &UniformVector<Type, TF, Tag>::operator=(const Vector<VT, TF> &rhs) &
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<VT>);

	if (!IsUniform_v<VT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment of uniform vector");
	}
...
inline UniformVector<Type, TF, Tag> &UniformVector<Type, TF, Tag>::operator+=(const Vector<VT, TF> &rhs) &
{
	...
	if (!IsUniform_v<VT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid addition assignment to uniform vector");
	}
...
inline UniformVector<Type, TF, Tag> &UniformVector<Type, TF, Tag>::operator-=(const Vector<VT, TF> &rhs) &
{
	...
	if (!IsUniform_v<VT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid subtraction assignment to uniform vector");
	}
...
inline UniformVector<Type, TF, Tag> &UniformVector<Type, TF, Tag>::operator*=(const Vector<VT, TF> &rhs) &
{
	...
	if (!IsUniform_v<VT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid multiplication assignment to uniform vector");
	}
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
template <typename VT>	// Type of the right-hand side vector
inline UniformVector<Type, TF, Tag> &UniformVector<Type, TF, Tag>::operator/=(const DenseVector<VT, TF> &rhs) &
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<VT>);
	...
	if (!IsUniform_v<VT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid division assignment to uniform vector");
	}
...
inline auto UniformVector<Type, TF, Tag>::operator*=(ST scalar) & -> EnableIf_t<IsScalar_v<ST>, UniformVector &>
{
	if (size() > 0UL) {
		value_ *= scalar;
	}
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::UniformVector<int> a;
   // ... Initialization
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniformVector<Type, TF, Tag>::SIMDType UniformVector<Type, TF, Tag>::load(size_t index) const noexcept
{
	return loada(index);
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniformVector<Type, TF, Tag>::SIMDType UniformVector<Type, TF, Tag>::loada(size_t index) const noexcept
{
	MAYBE_UNUSED(index);
...
template <typename Type // Data type of the vector
		  ...
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniformVector<Type, TF, Tag>::SIMDType UniformVector<Type, TF, Tag>::loadu(size_t index) const noexcept
{
	MAYBE_UNUSED(index);
...
template <typename Type, bool TF, typename Tag> constexpr bool isIntact(const UniformVector<Type, TF, Tag> &v) noexcept;
...
// vector is a column vector, but it is possible to specify the transpose flag explicitly:
   ...
   \endcode
*/
template <bool TF = defaultTransposeFlag, typename T> constexpr decltype(auto) uniform(size_t n, T &&init)
{
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename Type, bool TF, typename Tag> struct IsUniform<UniformVector<Type, TF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename Type, bool TF, typename Tag> struct IsAligned<UniformVector<Type, TF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct AddTraitEval1<T1, T2,
					 EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && (IsUniform_v<T1> && IsUniform_v<T2>) &&
								!(IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = UniformVector<AddTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T1>,
...
struct SubTraitEval1<T1, T2,
					 EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && (IsUniform_v<T1> && IsUniform_v<T2>) &&
								!(IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = UniformVector<SubTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T1>,
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsScalar_v<T2> && IsUniform_v<T1> && !IsZero_v<T1>>> {
	using Type = UniformVector<MultTrait_t<ElementType_t<T1>, T2>, TransposeFlag_v<T1>, MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsScalar_v<T1> && IsVector_v<T2> && IsUniform_v<T2> && !IsZero_v<T2>>> {
	using Type = UniformVector<MultTrait_t<T1, ElementType_t<T2>>, TransposeFlag_v<T2>, MultTrait_t<T1, TagType_t<T2>>>;
};
...
struct MultTraitEval1<
	...
	EnableIf_t<((IsRowVector_v<T1> && IsRowVector_v<T2>) || (IsColumnVector_v<T1> && IsColumnVector_v<T2>)) &&
			   (IsUniform_v<T1> && IsUniform_v<T2>) && !(IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = UniformVector<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T1>,
							   MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<
	T1, T2, EnableIf_t<IsMatrix_v<T1> && IsColumnVector_v<T2> && IsUniform_v<T1> && !(IsZero_v<T1> || IsZero_v<T2>)>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = UniformVector<AddTrait_t<MultType, MultType>, false, AddTrait_t<MultTag, MultTag>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<
	T1, T2, EnableIf_t<IsRowVector_v<T1> && IsMatrix_v<T2> && IsUniform_v<T2> && !(IsZero_v<T1> || IsZero_v<T2>)>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = UniformVector<AddTrait_t<MultType, MultType>, true, AddTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct KronTraitEval1<T1, T2,
					  EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && (IsUniform_v<T1> && IsUniform_v<T2>) &&
								 !(IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = UniformVector<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T2>,
							   MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct DivTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsScalar_v<T2> && IsUniform_v<T1> && !IsZero_v<T1>>> {
	using Type = UniformVector<DivTrait_t<ElementType_t<T1>, T2>, TransposeFlag_v<T1>, DivTrait_t<TagType_t<T1>, T2>>;
};
...
struct DivTraitEval1<
	...
	EnableIf_t<((IsRowVector_v<T1> && IsRowVector_v<T2>) || (IsColumnVector_v<T1> && IsColumnVector_v<T2>)) &&
			   (IsUniform_v<T1> && IsUniform_v<T2>) && !(IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = UniformVector<DivTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T1>,
...
struct UnaryMapTraitEval1<T, OP, EnableIf_t<IsVector_v<T> && YieldsUniform_v<OP, T> && !YieldsZero_v<OP, T>>> {
	using ElementType = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()));

	using Type = UniformVector<EvaluateTrait_t<ElementType>, TransposeFlag_v<T>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BinaryMapTraitEval1<
	T1, T2, OP,
	EnableIf_t<((IsRowVector_v<T1> && IsRowVector_v<T2>) || (IsColumnVector_v<T1> && IsColumnVector_v<T2>)) &&
			   YieldsUniform_v<OP, T1, T2> && !YieldsZero_v<OP, T1, T2>>> {
	using ElementType =
		decltype(std::declval<OP>()(std::declval<ElementType_t<T1>>(), std::declval<ElementType_t<T2>>()));

	using Type =
		UniformVector<EvaluateTrait_t<ElementType>, TransposeFlag_v<T1>, MapTrait_t<TagType_t<T1>, TagType_t<T2>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct PartialReduceTraitEval1<T, OP, RF, EnableIf_t<IsMatrix_v<T> && (IsUniform_v<T> || IsIdentity_v<T>)>> {
	using ET = ElementType_t<T>;

	using Type = UniformVector<decltype(std::declval<OP>()(std::declval<ET>(), std::declval<ET>())), (RF == columnwise),
...
struct RepeatTraitEval1<T, R0, inf, inf, EnableIf_t<IsVector_v<T> && IsUniform_v<T> && !IsZero_v<T>>> {
	using Type = UniformVector<ElementType_t<T>, TransposeFlag_v<T>, TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubvectorTraitEval1<VT, I, N, EnableIf_t<IsUniform_v<VT> && !IsZero_v<VT>>> {
	using Type = UniformVector<RemoveConst_t<ElementType_t<VT>>, TransposeFlag_v<VT>, TagType_t<VT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename VT, size_t N> struct ElementsTraitEval1<VT, N, EnableIf_t<IsUniform_v<VT> && !IsZero_v<VT>>> {
	using Type = UniformVector<RemoveConst_t<ElementType_t<VT>>, TransposeFlag_v<VT>, TagType_t<VT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t I> struct RowTraitEval1<MT, I, EnableIf_t<IsUniform_v<MT> && !IsZero_v<MT>>> {
	using Type = UniformVector<RemoveConst_t<ElementType_t<MT>>, true, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t I> struct ColumnTraitEval1<MT, I, EnableIf_t<IsUniform_v<MT> && !IsZero_v<MT>>> {
	using Type = UniformVector<RemoveConst_t<ElementType_t<MT>>, false, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, ptrdiff_t I> struct BandTraitEval1<MT, I, EnableIf_t<IsUniform_v<MT> && !IsZero_v<MT>>> {
	using Type = UniformVector<RemoveConst_t<ElementType_t<MT>>, defaultTransposeFlag, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
