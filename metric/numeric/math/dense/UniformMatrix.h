...
#ifndef METRIC_NUMERIC_MATH_DENSE_UNIFORMMATRIX_H
#define METRIC_NUMERIC_MATH_DENSE_UNIFORMMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// an impression of the use of UniformMatrix:
   ...
   \code
   using mtrc::numeric::UniformMatrix;
...
class UniformMatrix : public Expression<DenseMatrix<UniformMatrix<Type, SO, Tag>, SO>> {
  public:
	//**Type definitions****************************************************************************
	...
	template <typename MT, bool SO2> inline UniformMatrix(const Matrix<MT, SO2> &m);
	...
	template <typename ST> inline auto operator*=(ST rhs) & -> EnableIf_t<IsScalar_v<ST>, UniformMatrix &>;
	  ...
	  value_() // The value of all elements of the uniform vector
{
	if (!IsUniform_v<MT> && !isUniform(*m)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of uniform matrix");
	}
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename UniformMatrix<Type, SO, Tag>::ConstReference UniformMatrix<Type, SO, Tag>::at(size_t i, size_t j) const
{
	if (i >= m_) {
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename UniformMatrix<Type, SO, Tag>::ConstPointer UniformMatrix<Type, SO, Tag>::data() const noexcept
{
	return &value_;
}
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
constexpr
	typename UniformMatrix<Type, SO, Tag>::ConstPointer UniformMatrix<Type, SO, Tag>::data(size_t i) const noexcept
{
	MAYBE_UNUSED(i);

	METRIC_NUMERIC_USER_ASSERT(SO || i < m_, "Invalid dense matrix row access index");
	METRIC_NUMERIC_USER_ASSERT(!SO || i < n_, "Invalid dense matrix row access index");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
constexpr
	typename UniformMatrix<Type, SO, Tag>::ConstIterator UniformMatrix<Type, SO, Tag>::begin(size_t i) const noexcept
{
	MAYBE_UNUSED(i);
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
constexpr
	typename UniformMatrix<Type, SO, Tag>::ConstIterator UniformMatrix<Type, SO, Tag>::cbegin(size_t i) const noexcept
{
	MAYBE_UNUSED(i);
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
constexpr
	typename UniformMatrix<Type, SO, Tag>::ConstIterator UniformMatrix<Type, SO, Tag>::end(size_t i) const noexcept
{
	MAYBE_UNUSED(i);
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
constexpr
	typename UniformMatrix<Type, SO, Tag>::ConstIterator UniformMatrix<Type, SO, Tag>::cend(size_t i) const noexcept
{
	MAYBE_UNUSED(i);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
constexpr UniformMatrix<Type, SO, Tag> &UniformMatrix<Type, SO, Tag>::operator=(const Type &rhs) &
{
	value_ = rhs;
...
inline UniformMatrix<Type, SO, Tag> &UniformMatrix<Type, SO, Tag>::operator=(const Matrix<MT, SO2> &rhs) &
{
	...
	if (!IsUniform_v<MT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment of uniform matrix");
	}
...
inline UniformMatrix<Type, SO, Tag> &UniformMatrix<Type, SO, Tag>::operator+=(const Matrix<MT, SO2> &rhs) &
{
	...
	if (!IsUniform_v<MT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid addition assignment to uniform matrix");
	}
...
inline UniformMatrix<Type, SO, Tag> &UniformMatrix<Type, SO, Tag>::operator-=(const Matrix<MT, SO2> &rhs) &
{
	...
	if (!IsUniform_v<MT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid subtraction assignment to uniform matrix");
	}
...
inline UniformMatrix<Type, SO, Tag> &UniformMatrix<Type, SO, Tag>::operator%=(const Matrix<MT, SO2> &rhs) &
{
	...
	if (!IsUniform_v<MT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid Schur product assignment to uniform matrix");
	}
...
inline UniformMatrix<Type, SO, Tag> &UniformMatrix<Type, SO, Tag>::operator*=(const Matrix<MT, SO2> &rhs) &
{
	...
	if (!IsUniform_v<MT> && !isUniform(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid multiplication assignment to uniform matrix");
	}
...
inline auto UniformMatrix<Type, SO, Tag>::operator*=(ST scalar) & -> EnableIf_t<IsScalar_v<ST>, UniformMatrix &>
{
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::UniformMatrix<int> A;
...
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniformMatrix<Type, SO, Tag>::SIMDType UniformMatrix<Type, SO, Tag>::load(size_t i,
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniformMatrix<Type, SO, Tag>::SIMDType UniformMatrix<Type, SO, Tag>::loada(size_t i,
																						size_t j) const noexcept
{
	MAYBE_UNUSED(i, j);
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(SO || j + SIMDSIZE <= n_, "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(!SO || i + SIMDSIZE <= m_, "Invalid row access index");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
METRIC_NUMERIC_ALWAYS_INLINE
	typename UniformMatrix<Type, SO, Tag>::SIMDType UniformMatrix<Type, SO, Tag>::loadu(size_t i,
																						size_t j) const noexcept
{
	MAYBE_UNUSED(i, j);
...
template <typename Type, bool SO, typename Tag> constexpr bool isIntact(const UniformMatrix<Type, SO, Tag> &m) noexcept;
...
// matrix is a row-major matrix, but it is possible to specify the storage order explicitly:
   ...
   \endcode
*/
template <bool SO = defaultStorageOrder, typename T> constexpr decltype(auto) uniform(size_t m, size_t n, T &&init)
{
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename Type, bool SO, typename Tag> struct IsUniform<UniformMatrix<Type, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename Type, bool SO, typename Tag> struct IsAligned<UniformMatrix<Type, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct AddTraitEval1<T1, T2,
					 EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && (IsUniform_v<T1> && IsUniform_v<T2>) &&
	...
	using Type =
		UniformMatrix<AddTrait_t<ElementType_t<T1>, ElementType_t<T2>>, SO, AddTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubTraitEval1<T1, T2,
	...
	using Type =
		UniformMatrix<SubTrait_t<ElementType_t<T1>, ElementType_t<T2>>, SO, SubTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SchurTraitEval1<T1, T2,
	...
	using Type =
		UniformMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, SO, MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsUniform_v<T1> && !IsZero_v<T1> && IsScalar_v<T2>>> {
	using Type = UniformMatrix<MultTrait_t<ElementType_t<T1>, T2>, StorageOrder_v<T1>, MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsScalar_v<T1> && IsMatrix_v<T2> && IsUniform_v<T2> && !IsZero_v<T2>>> {
	using Type = UniformMatrix<MultTrait_t<T1, ElementType_t<T2>>, StorageOrder_v<T2>, MultTrait_t<T1, TagType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2,
					  EnableIf_t<IsColumnVector_v<T1> && IsRowVector_v<T2> && (IsUniform_v<T1> && IsUniform_v<T2>) &&
								 !(IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = UniformMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, false,
...
struct MultTraitEval1<T1, T2,
	...
	using Type = UniformMatrix<AddTrait_t<MultType, MultType>, StorageOrder_v<T1>, AddTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct KronTraitEval1<T1, T2,
					  EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && (IsUniform_v<T1> && IsUniform_v<T2>) &&
								 !(IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = UniformMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, StorageOrder_v<T2>,
							   MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct DivTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsScalar_v<T2> && IsUniform_v<T1> && !IsZero_v<T1>>> {
	using Type = UniformMatrix<DivTrait_t<ElementType_t<T1>, T2>, StorageOrder_v<T1>, DivTrait_t<TagType_t<T1>, T2>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct UnaryMapTraitEval1<T, OP, EnableIf_t<IsMatrix_v<T> && YieldsUniform_v<OP, T> && !YieldsZero_v<OP, T>>> {
	using ElementType = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()));

	using Type = UniformMatrix<EvaluateTrait_t<ElementType>, StorageOrder_v<T>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BinaryMapTraitEval1<
	T1, T2, OP,
	EnableIf_t<IsColumnVector_v<T1> && IsRowVector_v<T2> && YieldsUniform_v<OP, T1, T2> && !YieldsZero_v<OP, T1, T2>>> {
	using ElementType =
		decltype(std::declval<OP>()(std::declval<ElementType_t<T1>>(), std::declval<ElementType_t<T2>>()));

	using Type = UniformMatrix<EvaluateTrait_t<ElementType>, false, MapTrait_t<TagType_t<T1>, TagType_t<T2>, OP>>;
};

template <typename T1, typename T2, typename OP>
struct BinaryMapTraitEval1<
	T1, T2, OP,
	EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && YieldsUniform_v<OP, T1, T2> && !YieldsZero_v<OP, T1, T2>>> {
	using ElementType =
		decltype(std::declval<OP>()(std::declval<ElementType_t<T1>>(), std::declval<ElementType_t<T2>>()));
	...
	using Type = UniformMatrix<EvaluateTrait_t<ElementType>, SO, MapTrait_t<TagType_t<T1>, TagType_t<T2>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct ExpandTraitEval1<T, E, EnableIf_t<IsVector_v<T> && IsUniform_v<T> && !IsZero_v<T>>> {
	using Type = UniformMatrix<ElementType_t<T>, (IsColumnVector_v<T> ? columnMajor : rowMajor), TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct RepeatTraitEval1<T, R0, R1, inf, EnableIf_t<IsMatrix_v<T> && IsUniform_v<T> && !IsZero_v<T>>> {
	using Type = UniformMatrix<ElementType_t<T>, StorageOrder_v<T>, TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubmatrixTraitEval1<MT, I, J, M, N, EnableIf_t<IsUniform_v<MT> && !IsZero_v<MT>>> {
	using Type = UniformMatrix<RemoveConst_t<ElementType_t<MT>>, StorageOrder_v<MT>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t M> struct RowsTraitEval1<MT, M, EnableIf_t<IsUniform_v<MT> && !IsZero_v<MT>>> {
	using Type = UniformMatrix<RemoveConst_t<ElementType_t<MT>>, false, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t N> struct ColumnsTraitEval1<MT, N, EnableIf_t<IsUniform_v<MT> && !IsZero_v<MT>>> {
	using Type = UniformMatrix<RemoveConst_t<ElementType_t<MT>>, true, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
