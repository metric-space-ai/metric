...
#ifndef METRIC_NUMERIC_MATH_SPARSE_ZEROMATRIX_H
#define METRIC_NUMERIC_MATH_SPARSE_ZEROMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class ZeroMatrix : public Expression<SparseMatrix<ZeroMatrix<Type, SO, Tag>, SO>> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	template <typename MT, bool SO2> inline ZeroMatrix(const Matrix<MT, SO2> &m);
	...
	//@{
	template <typename MT, bool SO2> inline ZeroMatrix &operator=(const Matrix<MT, SO2> &rhs) &;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
template <typename MT	// Type of the other zero matrix
		  ,
		  bool SO2> // Storage order of the other zero matrix
	  ...
	  n_((*m).columns()) // The current number of columns of the zero matrix
{
	...
	if (!IsZero_v<MT> && !isZero(*m)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of zero matrix");
	}
}
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
constexpr
	typename ZeroMatrix<Type, SO, Tag>::ConstReference ZeroMatrix<Type, SO, Tag>::operator()(size_t i,
																							 size_t j) const noexcept
{
	MAYBE_UNUSED(i, j);

	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid zero matrix row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid zero matrix column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename ZeroMatrix<Type, SO, Tag>::ConstReference ZeroMatrix<Type, SO, Tag>::at(size_t i, size_t j) const
{
	if (i >= m_) {
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename ZeroMatrix<Type, SO, Tag>::ConstIterator ZeroMatrix<Type, SO, Tag>::begin(size_t i) const noexcept
{
	MAYBE_UNUSED(i);

	METRIC_NUMERIC_USER_ASSERT(i < (SO ? n_ : m_), "Invalid zero matrix row/column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename ZeroMatrix<Type, SO, Tag>::ConstIterator ZeroMatrix<Type, SO, Tag>::cbegin(size_t i) const noexcept
{
	MAYBE_UNUSED(i);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename ZeroMatrix<Type, SO, Tag>::ConstIterator ZeroMatrix<Type, SO, Tag>::end(size_t i) const noexcept
{
	MAYBE_UNUSED(i);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename ZeroMatrix<Type, SO, Tag>::ConstIterator ZeroMatrix<Type, SO, Tag>::cend(size_t i) const noexcept
{
	MAYBE_UNUSED(i);
...
inline ZeroMatrix<Type, SO, Tag> &ZeroMatrix<Type, SO, Tag>::operator=(const Matrix<MT, SO2> &rhs) &
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<MT>);

	if (!IsZero_v<MT> && !isZero(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment of zero matrix");
	}
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename ZeroMatrix<Type, SO, Tag>::ConstIterator ZeroMatrix<Type, SO, Tag>::find(size_t i, size_t j) const
{
	MAYBE_UNUSED(i, j);

	METRIC_NUMERIC_USER_ASSERT(SO || i < rows(), "Invalid zero matrix row access index");
	METRIC_NUMERIC_USER_ASSERT(!SO || j < columns(), "Invalid zero matrix column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename ZeroMatrix<Type, SO, Tag>::ConstIterator ZeroMatrix<Type, SO, Tag>::lowerBound(size_t i, size_t j) const
{
	MAYBE_UNUSED(i, j);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename ZeroMatrix<Type, SO, Tag>::ConstIterator ZeroMatrix<Type, SO, Tag>::upperBound(size_t i, size_t j) const
{
	MAYBE_UNUSED(i, j);
...
template <typename Type, bool SO, typename Tag> constexpr bool isIntact(const ZeroMatrix<Type, SO, Tag> &m) noexcept;
...
// all elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::ZeroMatrix<double,mtrc::numeric::rowMajor> Z;
   // ... Resizing and initialization
...
// explicitly:
   ...
   \endcode
*/
template <typename T, bool SO = defaultStorageOrder> constexpr decltype(auto) zero(size_t m, size_t n) noexcept
{
...
template <typename MT // Type of the matrix
		  ,
		  bool SO> // Storage order
inline ZeroMatrix<ElementType_t<MT>, SO> declzero(const Matrix<MT, SO> &m)
{
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename Type, bool SO, typename Tag> struct IsUniform<ZeroMatrix<Type, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename Type, bool SO, typename Tag> struct IsZero<ZeroMatrix<Type, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct AddTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && !IsZero_v<T1> && IsZero_v<T2>>> {
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct SubTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && !IsZero_v<T1> && IsZero_v<T2>>> {
	using Type = Rebind_t<ResultType_t<T1>, SubTrait_t<ElementType_t<T1>, ElementType_t<T2>>>;
};

template <typename T1, typename T2>
struct SubTraitEval1<
	T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && IsZero_v<T1> && !IsZero_v<T2> && !IsIdentity_v<T2>>> {
...
struct SchurTraitEval1<
	...
	EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> &&
			   (IsZero_v<T1> || IsZero_v<T2> || (IsStrictlyLower_v<T1> && IsUpper_v<T2>) ||
	...
	static constexpr bool SO =
		(IsSparseMatrix_v<T1> ^ IsSparseMatrix_v<T2> ? (IsSparseMatrix_v<T1> ? SO1 : SO2) : SO1 && SO2);

	using Type =
		ZeroMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, SO, MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsZero_v<T1> && IsScalar_v<T2>>> {
	using Type = ZeroMatrix<MultTrait_t<ElementType_t<T1>, T2>, StorageOrder_v<T1>, MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsScalar_v<T1> && IsMatrix_v<T2> && IsZero_v<T2>>> {
	using Type = ZeroMatrix<MultTrait_t<T1, ElementType_t<T2>>, StorageOrder_v<T2>, MultTrait_t<T1, TagType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsColumnVector_v<T1> && IsRowVector_v<T2> && (IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = ZeroMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
							(IsSparseVector_v<T1> && IsDenseVector_v<T2>), MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && (IsZero_v<T1> || IsZero_v<T2>)>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = ZeroMatrix<AddTrait_t<MultType, MultType>, (IsZero_v<T1> ? StorageOrder_v<T1> : StorageOrder_v<T2>),
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct KronTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && (IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type =
		ZeroMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
				   (IsZero_v<T2> ? StorageOrder_v<T2> : StorageOrder_v<T1>), MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct DivTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsScalar_v<T2> && IsZero_v<T1>>> {
	using Type = ZeroMatrix<DivTrait_t<ElementType_t<T1>, T2>, StorageOrder_v<T1>, DivTrait_t<TagType_t<T1>, T2>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, typename OP> struct UnaryMapTraitEval1<T, OP, EnableIf_t<IsMatrix_v<T> && YieldsZero_v<OP, T>>> {
	using ElementType = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()));

	using Type = ZeroMatrix<EvaluateTrait_t<ElementType>, StorageOrder_v<T>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BinaryMapTraitEval1<T1, T2, OP, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && YieldsZero_v<OP, T1, T2>>> {
	using ElementType =
		decltype(std::declval<OP>()(std::declval<ElementType_t<T1>>(), std::declval<ElementType_t<T2>>()));

	using Type = ZeroMatrix<EvaluateTrait_t<ElementType>, (StorageOrder_v<T1> && StorageOrder_v<T2>),
...
struct ExpandTraitEval1<T, E, EnableIf_t<IsVector_v<T> && IsZero_v<T>>> {
	using Type = ZeroMatrix<ElementType_t<T>, (IsColumnVector_v<T> ? columnMajor : rowMajor), TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct RepeatTraitEval1<T, R0, R1, inf, EnableIf_t<IsMatrix_v<T> && IsZero_v<T>>> {
	using Type = ZeroMatrix<ElementType_t<T>, StorageOrder_v<T>, TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclStrLowTrait<ZeroMatrix<T, SO, Tag>> {
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclStrUppTrait<ZeroMatrix<T, SO, Tag>> {
	using Type = ZeroMatrix<T, SO, Tag>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubmatrixTraitEval1<MT, I, J, M, N, EnableIf_t<IsZero_v<MT>>> {
	using Type = ZeroMatrix<RemoveConst_t<ElementType_t<MT>>, StorageOrder_v<MT>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t M> struct RowsTraitEval1<MT, M, EnableIf_t<IsZero_v<MT>>> {
	using Type = ZeroMatrix<RemoveConst_t<ElementType_t<MT>>, false, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t N> struct ColumnsTraitEval1<MT, N, EnableIf_t<IsZero_v<MT>>> {
	using Type = ZeroMatrix<RemoveConst_t<ElementType_t<MT>>, true, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
