...
#ifndef METRIC_NUMERIC_MATH_SPARSE_COMPRESSEDMATRIX_H
#define METRIC_NUMERIC_MATH_SPARSE_COMPRESSEDMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/sparse/MatrixAccessProxy.h>
#include <metric/numeric/math/sparse/ValueIndexPair.h>
...
#include <metric/numeric/util/algorithms/Transfer.h>
...
#include <metric/numeric/util/constraints/SameSize.h>
...
//  - SO  : specifies the storage order (mtrc::numeric::rowMajor, mtrc::numeric::columnMajor) of the matrix.
//          The default value is mtrc::numeric::defaultStorageOrder.
//  - Tag : optional type parameter to tag the matrix. The default type is \a mtrc::numeric::Group0.
...
class CompressedMatrix : public SparseMatrix<CompressedMatrix<Type, SO, Tag>, SO> {
  private:
	//**Type definitions****************************************************************************
	...
	*/
	struct Element : public ElementBase {
		...
		inline auto operator=(const Other &rhs) -> EnableIf_t<IsSparseElement_v<Other>, Element &>
		{
		...
		inline auto operator=(Other &&rhs)
			-> EnableIf_t<IsSparseElement_v<RemoveReference_t<Other>> && IsRValueReference_v<Other &&>, Element &>
		{
		...
		template <typename Other>
		inline auto operator=(const Other &v) -> EnableIf_t<!IsSparseElement_v<Other>, Element &>
		{
			this->value_ = v;
			return *this;
		}
  ...
  public:
	...
	template <typename Pred> inline void erase(Pred predicate);

	template <typename Pred> inline void erase(size_t i, Iterator first, Iterator last, Pred predicate);
	//@}
	//**********************************************************************************************
  ...
  private:
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_HAVE_SAME_SIZE(ElementBase, Element);
	/*! \endcond */
	//**********************************************************************************************
};
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline CompressedMatrix<Type, SO, Tag>::CompressedMatrix(size_t m, size_t n) : CompressedMatrix(m, n, Uninitialized())
{
...
CompressedMatrix<Type, SO, Tag>::CompressedMatrix(size_t m, size_t n, const std::vector<size_t> &nonzeros)
	: CompressedMatrix(m, n, Uninitialized())
{
	METRIC_NUMERIC_USER_ASSERT(nonzeros.size() == m, "Size of capacity vector and number of rows don't match");
...
// means of an initializer list:
   ...
   mtrc::numeric::CompressedMatrix<int,rowMajor> A{ { 1, 2, 3 },
											{ 4, 5 },
											{ 7, 8, 9 } };
   \endcode
...
inline CompressedMatrix<Type, SO, Tag>::CompressedMatrix(initializer_list<initializer_list<Type>> list)
	: CompressedMatrix(list.size(), determineColumns(list), mtrc::numeric::nonZeros(list))
{
	size_t i(0UL);
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
template <typename MT	// Type of the other dense matrix
		  ,
		  bool SO2> // Storage order of the other dense matrix
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
template <typename MT	// Type of the other compressed matrix
		  ,
		  bool SO2> // Storage order of the other compressed matrix
...
// case METRIC_NUMERIC_USER_ASSERT() is active. In contrast, the at() function is guaranteed to perform a
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename CompressedMatrix<Type, SO, Tag>::Reference CompressedMatrix<Type, SO, Tag>::operator()(size_t i,
																									size_t j) noexcept
{
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::Reference CompressedMatrix<Type, SO, Tag>::at(size_t i, size_t j)
{
	if (i >= m_) {
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::ConstReference CompressedMatrix<Type, SO, Tag>::at(size_t i,
																									size_t j) const
{
	if (i >= m_) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::begin(size_t i) noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < m_, "Invalid compressed matrix row access index");
	return begin_[i];
}
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::end(size_t i) noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < m_, "Invalid compressed matrix row access index");
	return end_[i];
}
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline CompressedMatrix<Type, SO, Tag> &CompressedMatrix<Type, SO, Tag>::operator=(const CompressedMatrix &rhs) &
{
	using std::swap;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline CompressedMatrix<Type, SO, Tag> &CompressedMatrix<Type, SO, Tag>::operator=(CompressedMatrix &&rhs) & noexcept
{
	if (begin_ != nullptr) {
		deallocate(begin_[0UL]);
		delete[] begin_;
...
inline CompressedMatrix<Type, SO, Tag> &CompressedMatrix<Type, SO, Tag>::operator=(const DenseMatrix<MT, SO2> &rhs) &
{
	using mtrc::numeric::assign;
...
inline CompressedMatrix<Type, SO, Tag> &CompressedMatrix<Type, SO, Tag>::operator=(const SparseMatrix<MT, SO2> &rhs) &
{
	using mtrc::numeric::assign;

	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<MT>);

	if ((*rhs).canAlias(this) || (*rhs).rows() > capacity_ || (*rhs).nonZeros() > capacity()) {
		CompressedMatrix tmp(*rhs);
		swap(tmp);
	} else {
		resize((*rhs).rows(), (*rhs).columns(), false);
...
inline CompressedMatrix<Type, SO, Tag> &CompressedMatrix<Type, SO, Tag>::operator+=(const Matrix<MT, SO2> &rhs) &
{
...
void CompressedMatrix<Type, SO, Tag>::resize(size_t m, size_t n, bool preserve)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(end_ >= begin_, "Invalid internal storage detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(begin_ == nullptr || size_t(end_ - begin_) == capacity_ + 1UL,
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(end_ >= begin_, "Invalid internal storage detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(size_t(end_ - begin_) == capacity_ + 1UL, "Invalid storage setting detected");
}
//*************************************************************************************************
...
void CompressedMatrix<Type, SO, Tag>::reserve(size_t i, size_t nonzeros)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(end_ >= begin_, "Invalid internal storage detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(static_cast<size_t>(end_ - begin_) == capacity_ + 1UL,
								   "Invalid storage setting detected");
	...
	if (end_[m_] - begin_[m_] < additional) {
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(newCapacity > capacity(), "Invalid capacity value");
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(newBegin[m_] == newEnd[m_], "Invalid pointer calculations");
...
inline size_t CompressedMatrix<Type, SO, Tag>::extendCapacity() const noexcept
{
	...
	nonzeros = mtrc::numeric::max(nonzeros, 7UL);

	METRIC_NUMERIC_INTERNAL_ASSERT(nonzeros > capacity(), "Invalid capacity value");
...
void CompressedMatrix<Type, SO, Tag>::reserveElements(size_t nonzeros)
{
	...
	for (size_t k = 0UL; k < m_; ++k) {
		METRIC_NUMERIC_INTERNAL_ASSERT(begin_[k] <= end_[k], "Invalid row pointers");
		newEnd[k] = castDown(transfer(begin_[k], end_[k], castUp(newBegin[k])));
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::set(size_t i, size_t j,
																							   const Type &value)
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::insert(size_t i, size_t j,
																								  const Type &value)
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
	...
	if (pos != end_[i] && pos->index_ == j) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid compressed matrix access index");
	}
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::insert(Iterator pos, size_t i,
...
// efficient way to add new elements to a (newly created) compressed matrix:
   ...
   //       ( 3 0 0 )
   //
   mtrc::numeric::CompressedMatrix<double,rowMajor> A( 4, 3 );
...
{
	METRIC_NUMERIC_USER_ASSERT(i < m_, "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < n_, "Invalid column access index");
	METRIC_NUMERIC_USER_ASSERT(end_[i] < end_[m_], "Not enough reserved capacity left");
	METRIC_NUMERIC_USER_ASSERT(begin_[i] == end_[i] || j > (end_[i] - 1UL)->index_, "Index is not strictly increasing");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::erase(size_t i, Iterator pos)
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(pos >= begin_[i] && pos <= end_[i], "Invalid compressed matrix iterator");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::erase(size_t i, Iterator first,
																							  Iterator last)
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(first <= last, "Invalid iterator range");
	METRIC_NUMERIC_USER_ASSERT(first >= begin_[i] && first <= end_[i], "Invalid compressed matrix iterator");
	METRIC_NUMERIC_USER_ASSERT(last >= begin_[i] && last <= end_[i], "Invalid compressed matrix iterator");
...
// all elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::CompressedMatrix<double,mtrc::numeric::rowMajor> A;
...
inline void CompressedMatrix<Type, SO, Tag>::erase(Pred predicate)
{
	for (size_t i = 0UL; i < m_; ++i) {
		end_[i] = castDown(
			std::remove_if(castUp(begin_[i]), castUp(end_[i]),
						   [predicate = predicate](const ElementBase &element) { return predicate(element.value()); }));
	}
}
//*************************************************************************************************
...
inline void CompressedMatrix<Type, SO, Tag>::erase(size_t i, Iterator first, Iterator last, Pred predicate)
{
	...
	const auto pos = std::remove_if(castUp(first), castUp(last), [predicate = predicate](const ElementBase &element) {
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::find(size_t i, size_t j)
{
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, SO, Tag>::ConstIterator CompressedMatrix<Type, SO, Tag>::find(size_t i,
																									 size_t j) const
{
	const ConstIterator pos(lowerBound(i, j));
	if (pos != end_[i] && pos->index_ == j)
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::lowerBound(size_t i, size_t j)
{
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename CompressedMatrix<Type, SO, Tag>::ConstIterator CompressedMatrix<Type, SO, Tag>::lowerBound(size_t i,
																										size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	return std::lower_bound(begin_[i], end_[i], j,
							[](const Element &element, size_t index) { return element.index() < index; });
}
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename CompressedMatrix<Type, SO, Tag>::Iterator CompressedMatrix<Type, SO, Tag>::upperBound(size_t i, size_t j)
{
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename CompressedMatrix<Type, SO, Tag>::ConstIterator CompressedMatrix<Type, SO, Tag>::upperBound(size_t i,
																										size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	return std::upper_bound(begin_[i], end_[i], j,
							[](size_t index, const Element &element) { return index < element.index(); });
}
//*************************************************************************************************
...
// multiplication assignment operator:
   ...
   \code
   mtrc::numeric::CompressedMatrix<int> A;
   // ... Resizing and initialization
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(m_ == (*rhs).rows(), "Invalid number of rows");
	METRIC_NUMERIC_INTERNAL_ASSERT(n_ == (*rhs).columns(), "Invalid number of columns");
...
inline void CompressedMatrix<Type, SO, Tag>::assign(const SparseMatrix<MT, SO> &rhs)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(m_ == (*rhs).rows(), "Invalid number of rows");
	METRIC_NUMERIC_INTERNAL_ASSERT(n_ == (*rhs).columns(), "Invalid number of columns");
	METRIC_NUMERIC_INTERNAL_ASSERT(nonZeros() == 0UL, "Invalid non-zero elements detected");
	METRIC_NUMERIC_INTERNAL_ASSERT(capacity() >= (*rhs).nonZeros(), "Invalid capacity detected");
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
class CompressedMatrix<Type, true, Tag> : public SparseMatrix<CompressedMatrix<Type, true, Tag>, true> {
  private:
	//**Type definitions****************************************************************************
	using ElementBase = ValueIndexPair<Type>; //!< Base class for the compressed matrix element.
	using IteratorBase = ElementBase *;		  //!< Iterator over non-constant base elements.
  ...
  public:
	...
	template <typename Pred> inline void erase(size_t j, Iterator first, Iterator last, Pred predicate);
	//@}
	//**********************************************************************************************
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline CompressedMatrix<Type, true, Tag>::CompressedMatrix(size_t m, size_t n) : CompressedMatrix(m, n, Uninitialized())
{
...
CompressedMatrix<Type, true, Tag>::CompressedMatrix(size_t m, size_t n, const std::vector<size_t> &nonzeros)
	: CompressedMatrix(m, n, Uninitialized())
{
	METRIC_NUMERIC_USER_ASSERT(nonzeros.size() == n, "Size of capacity vector and number of columns don't match");
...
// means of an initializer list:
   ...
   mtrc::numeric::CompressedMatrix<int,columnMajor> A{ { 1, 2, 3 },
											   { 4, 5 },
											   { 7, 8, 9 } };
   \endcode
...
template <typename MT	// Type of the other dense matrix
		  ,
		  bool SO> // Storage order of the other dense matrix
...
template <typename MT	// Type of the other compressed matrix
		  ,
		  bool SO> // Storage order of the other compressed matrix
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::Reference CompressedMatrix<Type, true, Tag>::at(size_t i, size_t j)
{
	if (i >= m_) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::ConstReference CompressedMatrix<Type, true, Tag>::at(size_t i,
																										size_t j) const
{
	if (i >= m_) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::Iterator CompressedMatrix<Type, true, Tag>::begin(size_t j) noexcept
{
	METRIC_NUMERIC_USER_ASSERT(j < n_, "Invalid compressed matrix column access index");
	return begin_[j];
}
/*! \endcond */
//*************************************************************************************************
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::Iterator CompressedMatrix<Type, true, Tag>::end(size_t j) noexcept
{
	METRIC_NUMERIC_USER_ASSERT(j < n_, "Invalid compressed matrix column access index");
	return end_[j];
}
...
// by means of an initializer list:
   ...
   mtrc::numeric::CompressedMatrix<int,columnMajor> A;
   A = { { 1, 2, 3 },
		 { 4, 5 },
		 { 7, 8, 9 } };
   \endcode
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline CompressedMatrix<Type, true, Tag> &CompressedMatrix<Type, true, Tag>::operator=(const CompressedMatrix &rhs) &
{
	using std::swap;
...
inline CompressedMatrix<Type, true, Tag> &CompressedMatrix<Type, true, Tag>::operator=(const DenseMatrix<MT, SO> &rhs) &
{
	using mtrc::numeric::assign;
...
CompressedMatrix<Type, true, Tag>::operator=(const SparseMatrix<MT, SO> &rhs) &
{
	...
	if ((*rhs).canAlias(this) || (*rhs).columns() > capacity_ || (*rhs).nonZeros() > capacity()) {
		CompressedMatrix tmp(*rhs);
		swap(tmp);
	} else {
		resize((*rhs).rows(), (*rhs).columns(), false);
...
inline CompressedMatrix<Type, true, Tag> &CompressedMatrix<Type, true, Tag>::operator+=(const Matrix<MT, SO> &rhs) &
{
	using mtrc::numeric::addAssign;
...
void CompressedMatrix<Type, true, Tag>::reserve(size_t j, size_t nonzeros)
{
	...
	if (end_[n_] - begin_[n_] < additional) {
		...
		METRIC_NUMERIC_INTERNAL_ASSERT(newBegin[n_] == newEnd[n_], "Invalid pointer calculations");
...
void CompressedMatrix<Type, true, Tag>::reserveElements(size_t nonzeros)
{
	...
	for (size_t k = 0UL; k < n_; ++k) {
		METRIC_NUMERIC_INTERNAL_ASSERT(begin_[k] <= end_[k], "Invalid column pointers");
		newEnd[k] = castDown(transfer(begin_[k], end_[k], castUp(newBegin[k])));
		newBegin[k + 1UL] = newBegin[k] + (begin_[k + 1UL] - begin_[k]);
	}
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::Iterator CompressedMatrix<Type, true, Tag>::set(size_t i, size_t j,
																								   const Type &value)
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Type tag
inline
	typename CompressedMatrix<Type, true, Tag>::Iterator CompressedMatrix<Type, true, Tag>::insert(size_t i, size_t j,
																								   const Type &value)
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
// efficient way to add new elements to a (new created) compressed matrix:
   ...
   //       ( 0 0 0 0 )
   //
   mtrc::numeric::CompressedMatrix<double,columnMajor> A( 3, 4 );
...
inline void CompressedMatrix<Type, true, Tag>::append(size_t i, size_t j, const Type &value, bool check)
{
	METRIC_NUMERIC_USER_ASSERT(i < m_, "Invalid row access index");
	METRIC_NUMERIC_USER_ASSERT(j < n_, "Invalid column access index");
	METRIC_NUMERIC_USER_ASSERT(end_[j] < end_[n_], "Not enough reserved capacity left");
	METRIC_NUMERIC_USER_ASSERT(begin_[j] == end_[j] || i > (end_[j] - 1UL)->index_, "Index is not strictly increasing");
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::Iterator CompressedMatrix<Type, true, Tag>::erase(size_t j,
																									 Iterator pos)
{
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
	METRIC_NUMERIC_USER_ASSERT(pos >= begin_[j] && pos <= end_[j], "Invalid compressed matrix iterator");
...
CompressedMatrix<Type, true, Tag>::erase(size_t j, Iterator first, Iterator last)
{
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
	METRIC_NUMERIC_USER_ASSERT(first <= last, "Invalid iterator range");
	METRIC_NUMERIC_USER_ASSERT(first >= begin_[j] && first <= end_[j], "Invalid compressed matrix iterator");
	METRIC_NUMERIC_USER_ASSERT(last >= begin_[j] && last <= end_[j], "Invalid compressed matrix iterator");
...
// all elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::CompressedMatrix<double,mtrc::numeric::columnMajor> A;
   // ... Resizing and initialization
...
inline void CompressedMatrix<Type, true, Tag>::erase(Pred predicate)
{
	for (size_t j = 0UL; j < n_; ++j) {
		end_[j] = castDown(
			std::remove_if(castUp(begin_[j]), castUp(end_[j]),
						   [predicate = predicate](const ElementBase &element) { return predicate(element.value()); }));
	}
}
/*! \endcond */
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::Iterator CompressedMatrix<Type, true, Tag>::find(size_t i, size_t j)
{
	return const_cast<Iterator>(const_cast<const This &>(*this).find(i, j));
}
/*! \endcond */
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::ConstIterator CompressedMatrix<Type, true, Tag>::find(size_t i,
																										 size_t j) const
{
	const ConstIterator pos(lowerBound(i, j));
	if (pos != end_[j] && pos->index_ == i)
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::Iterator CompressedMatrix<Type, true, Tag>::lowerBound(size_t i,
																										  size_t j)
{
	return const_cast<Iterator>(const_cast<const This &>(*this).lowerBound(i, j));
}
...
template <typename Type // Data type of the matrix
		  ,
		  typename Tag> // Type tag
inline typename CompressedMatrix<Type, true, Tag>::Iterator CompressedMatrix<Type, true, Tag>::upperBound(size_t i,
																										  size_t j)
{
	return const_cast<Iterator>(const_cast<const This &>(*this).upperBound(i, j));
}
...
template <typename Type, bool SO, typename Tag> bool isIntact(const CompressedMatrix<Type, SO, Tag> &m);
...
struct AddTraitEval2<T1, T2, EnableIf_t<IsSparseMatrix_v<T1> && IsSparseMatrix_v<T2>>> {
	using Type = CompressedMatrix<AddTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
								  (StorageOrder_v<T1> && StorageOrder_v<T2>), AddTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct SubTraitEval2<T1, T2, EnableIf_t<IsSparseMatrix_v<T1> && IsSparseMatrix_v<T2>>> {
	using Type = CompressedMatrix<SubTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
								  (StorageOrder_v<T1> && StorageOrder_v<T2>), SubTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SchurTraitEval2<T1, T2,
					   EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && (IsSparseMatrix_v<T1> || IsSparseMatrix_v<T2>)>> {
	static constexpr bool SO = (IsSparseMatrix_v<T1> && IsSparseMatrix_v<T2>
									? (StorageOrder_v<T1> && StorageOrder_v<T2>)
									: (IsSparseMatrix_v<T1> ? StorageOrder_v<T1> : StorageOrder_v<T2>));

	using Type = CompressedMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, SO,
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2> struct MultTraitEval2<T1, T2, EnableIf_t<IsSparseMatrix_v<T1> && IsScalar_v<T2>>> {
	using Type =
		CompressedMatrix<MultTrait_t<ElementType_t<T1>, T2>, StorageOrder_v<T1>, MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2> struct MultTraitEval2<T1, T2, EnableIf_t<IsScalar_v<T1> && IsSparseMatrix_v<T2>>> {
	using Type =
		CompressedMatrix<MultTrait_t<T1, ElementType_t<T2>>, StorageOrder_v<T2>, MultTrait_t<T1, TagType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval2<
	T1, T2, EnableIf_t<(IsSparseVector_v<T1> || IsSparseVector_v<T2>) && IsColumnVector_v<T1> && IsRowVector_v<T2>>> {
	using Type =
		CompressedMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>,
						 (IsSparseVector_v<T2> ? rowMajor : columnMajor), MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval2<
	T1, T2, EnableIf_t<IsSparseMatrix_v<T1> && IsSparseMatrix_v<T2> && !(IsIdentity_v<T1> && IsIdentity_v<T2>)>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = CompressedMatrix<AddTrait_t<MultType, MultType>, StorageOrder_v<T1>, AddTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2> struct DivTraitEval2<T1, T2, EnableIf_t<IsSparseMatrix_v<T1> && IsScalar_v<T2>>> {
	using Type = CompressedMatrix<DivTrait_t<ElementType_t<T1>, T2>, StorageOrder_v<T1>, DivTrait_t<TagType_t<T1>, T2>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, typename OP> struct UnaryMapTraitEval2<T, OP, EnableIf_t<IsSparseMatrix_v<T>>> {
	using ElementType = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()));

	using Type = CompressedMatrix<EvaluateTrait_t<ElementType>, StorageOrder_v<T>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct ExpandTraitEval2<T, E, EnableIf_t<IsSparseVector_v<T>>> {
	using Type = CompressedMatrix<ElementType_t<T>, (IsColumnVector_v<T> ? columnMajor : rowMajor), TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t R0, size_t R1> struct RepeatTraitEval2<T, R0, R1, inf, EnableIf_t<IsSparseMatrix_v<T>>> {
	using Type = CompressedMatrix<ElementType_t<T>, StorageOrder_v<T>, TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct SubmatrixTraitEval2<MT, I, J, M, N, EnableIf_t<IsSparseMatrix_v<MT>>> {
	using Type = CompressedMatrix<RemoveConst_t<ElementType_t<MT>>, StorageOrder_v<MT>, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t M> struct RowsTraitEval2<MT, M, EnableIf_t<IsSparseMatrix_v<MT>>> {
	using Type = CompressedMatrix<RemoveConst_t<ElementType_t<MT>>, false, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t N> struct ColumnsTraitEval2<MT, N, EnableIf_t<IsSparseMatrix_v<MT>>> {
	using Type = CompressedMatrix<RemoveConst_t<ElementType_t<MT>>, true, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
