...
#ifndef METRIC_NUMERIC_MATH_SPARSE_SPARSEMATRIX_H
#define METRIC_NUMERIC_MATH_SPARSE_SPARSEMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
inline auto operator*=(SparseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
...
inline auto operator/=(SparseMatrix<MT, SO> &mat, ST scalar) -> EnableIf_t<IsScalar_v<ST>, MT &>
{
	...
	using ScalarType = If_t<IsFloatingPoint_v<UnderlyingBuiltin_t<MT>> || IsFloatingPoint_v<UnderlyingBuiltin_t<ST>>,
							If_t<IsComplex_v<UnderlyingScalar_t<MT>> && IsBuiltin_v<ST>,
								 DivTrait_t<UnderlyingBuiltin_t<MT>, ST>, DivTrait_t<UnderlyingScalar_t<MT>, ST>>,
							ST>;
...
//@{
template <typename MT, bool SO> bool isnan(const SparseMatrix<MT, SO> &sm);

template <typename MT, bool SO> bool isinf(const SparseMatrix<MT, SO> &sm);

template <typename MT, bool SO> bool isfinite(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isSymmetric(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isHermitian(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isUniform(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isZero(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isLower(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isUniLower(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isStrictlyLower(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isUpper(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isUniUpper(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isStrictlyUpper(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isDiagonal(const SparseMatrix<MT, SO> &sm);

template <RelaxationFlag RF, typename MT, bool SO> bool isIdentity(const SparseMatrix<MT, SO> &sm);
//@}
//*************************************************************************************************
...
// \a false.
   ...
   \code
   mtrc::numeric::CompressedMatrix<double> A( 3UL, 4UL );
   // ... Initialization
   if( isnan( A ) ) { ... }
   \endcode
*/
...
// following code example demonstrates the use of the function:
   ...
   \code
   mtrc::numeric::CompressedMatrix<int,mtrc::numeric::rowMajor> A, B;
   // ... Initialization
   if( isSymmetric( A ) ) { ... }
   \endcode
...
bool isUniform_backend(const SparseMatrix<MT, false> &sm, TrueType)
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT((*sm).rows() != 0UL, "Invalid number of rows detected");
	METRIC_NUMERIC_INTERNAL_ASSERT((*sm).columns() != 0UL, "Invalid number of columns detected");
...
bool isUniform_backend(const SparseMatrix<MT, false> &sm, FalseType)
{
	...
	if ((*sm).nonZeros() != maxElements) {
		...
		METRIC_NUMERIC_INTERNAL_ASSERT((*sm).find(0UL, 0UL) != (*sm).end(0UL), "Missing element detected");
...
{
	if (IsUniform_v<MT> || (*sm).rows() == 0UL || (*sm).columns() == 0UL ||
...
auto erase(SparseMatrix<MT, SO> &sm, Args &&...args) -> decltype((*sm).erase(std::forward<Args>(args)...))
{
