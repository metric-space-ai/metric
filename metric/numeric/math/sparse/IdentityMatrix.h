...
#ifndef METRIC_NUMERIC_MATH_SPARSE_IDENTITYMATRIX_H
#define METRIC_NUMERIC_MATH_SPARSE_IDENTITYMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
// of the use of IdentityMatrix:
   ...
   \code
   using mtrc::numeric::IdentityMatrix;
...
class IdentityMatrix : public Expression<SparseMatrix<IdentityMatrix<Type, SO, Tag>, SO>> {
  public:
	//**Type definitions****************************************************************************
	...
	class ConstIterator {
	  public:
		 ...
		 */
		constexpr ConstIterator() noexcept : index_() // Index to the current identity matrix element
		{
		}
		//*******************************************************************************************
		...
		*/
		constexpr ConstIterator(size_t index) noexcept : index_(index) // Index to the current identity matrix element
		{
		}
		//*******************************************************************************************
		...
		*/
		constexpr const Element operator*() const noexcept { return Element(Type(1), index_); }
		//*******************************************************************************************
		...
		*/
		constexpr const ConstIterator *operator->() const noexcept { return this; }
		//*******************************************************************************************
		...
		// \return The current value of the sparse element.
		*/
		constexpr Type value() const noexcept { return Type(1); }
		//*******************************************************************************************
		...
		*/
		constexpr bool operator==(const ConstIterator &rhs) const noexcept { return index_ == rhs.index_; }
		//*******************************************************************************************
		...
		*/
		constexpr DifferenceType operator-(const ConstIterator &rhs) const noexcept { return index_ - rhs.index_; }
		//*******************************************************************************************
	...
	template <typename MT, bool SO2> inline IdentityMatrix(const Matrix<MT, SO2> &m);
	...
	//@{
	template <typename MT, bool SO2> inline IdentityMatrix &operator=(const Matrix<MT, SO2> &rhs) &;
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Tag type
template <typename MT	// Type of the other identity matrix
		  ,
		  bool SO2> // Storage order of the other identity matrix
	...
	: n_((*m).rows()) // The current number of rows and columns of the identity matrix
{
	...
	if (!IsIdentity_v<MT> && !isIdentity(*m)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of identity matrix");
	}
}
//*************************************************************************************************
...
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid identity matrix row access index");
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid identity matrix column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Tag type
inline
	typename IdentityMatrix<Type, SO, Tag>::ConstReference IdentityMatrix<Type, SO, Tag>::at(size_t i, size_t j) const
{
	if (i >= n_) {
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Tag type
constexpr
	typename IdentityMatrix<Type, SO, Tag>::ConstIterator IdentityMatrix<Type, SO, Tag>::begin(size_t i) const noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < n_, "Invalid identity matrix row/column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Tag type
constexpr
	typename IdentityMatrix<Type, SO, Tag>::ConstIterator IdentityMatrix<Type, SO, Tag>::cbegin(size_t i) const noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < n_, "Invalid identity matrix row/column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Tag type
constexpr
	typename IdentityMatrix<Type, SO, Tag>::ConstIterator IdentityMatrix<Type, SO, Tag>::end(size_t i) const noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < n_, "Invalid identity matrix row/column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Tag type
constexpr
	typename IdentityMatrix<Type, SO, Tag>::ConstIterator IdentityMatrix<Type, SO, Tag>::cend(size_t i) const noexcept
{
	METRIC_NUMERIC_USER_ASSERT(i < n_, "Invalid identity matrix row/column access index");
...
inline IdentityMatrix<Type, SO, Tag> &IdentityMatrix<Type, SO, Tag>::operator=(const Matrix<MT, SO2> &rhs) &
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<MT>);

	if (!IsIdentity_v<MT> && !isIdentity(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment of identity matrix");
	}
...
template <typename Type // Data type of the matrix
		  ...
		  typename Tag> // Tag type
inline
	typename IdentityMatrix<Type, SO, Tag>::ConstIterator IdentityMatrix<Type, SO, Tag>::find(size_t i, size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(SO || i < rows(), "Invalid identity matrix row access index");
	METRIC_NUMERIC_USER_ASSERT(!SO || j < columns(), "Invalid identity matrix column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Tag type
inline typename IdentityMatrix<Type, SO, Tag>::ConstIterator IdentityMatrix<Type, SO, Tag>::lowerBound(size_t i,
																									   size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(SO || i < rows(), "Invalid identity matrix row access index");
	METRIC_NUMERIC_USER_ASSERT(!SO || j < columns(), "Invalid identity matrix column access index");
...
template <typename Type // Data type of the matrix
		  ...
		  ,
		  typename Tag> // Tag type
inline typename IdentityMatrix<Type, SO, Tag>::ConstIterator IdentityMatrix<Type, SO, Tag>::upperBound(size_t i,
																									   size_t j) const
{
	METRIC_NUMERIC_USER_ASSERT(SO || i < rows(), "Invalid identity matrix row access index");
	METRIC_NUMERIC_USER_ASSERT(!SO || j < columns(), "Invalid identity matrix column access index");
...
// \a isDefault() function:
   ...
   \code
   mtrc::numeric::IdentityMatrix<int> I;
...
template <typename MT // Type of the sparse matrix
		  ,
		  bool SO> // Storage order
inline IdentityMatrix<ElementType_t<MT>, SO> declid(const Matrix<MT, SO> &m)
{
	METRIC_NUMERIC_FUNCTION_TRACE;

	if (!isSquare(*m)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid identity matrix specification");
	}
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, typename Tag> struct IsSquare<IdentityMatrix<MT, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, typename Tag> struct IsSymmetric<IdentityMatrix<MT, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, typename Tag> struct IsHermitian<IdentityMatrix<MT, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, typename Tag> struct IsUniLower<IdentityMatrix<MT, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, typename Tag> struct IsUniUpper<IdentityMatrix<MT, SO, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct SchurTraitEval1<
	...
	EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> &&
			   ((IsIdentity_v<T1> && IsIdentity_v<T2>) || (IsIdentity_v<T1> && IsUniTriangular_v<T2>) ||
				(IsUniTriangular_v<T1> && IsIdentity_v<T2>) || (IsUniLower_v<T1> && IsUniUpper_v<T2>) ||
	...
	static constexpr bool SO =
		((IsDenseMatrix_v<T1> && IsDenseMatrix_v<T2>) || (IsSparseMatrix_v<T1> && IsSparseMatrix_v<T2>)
			 ? SO1 && SO2
			 : (IsSparseMatrix_v<T1> ? SO1 : SO2));

	using Type = IdentityMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, SO,
...
struct MultTraitEval1<
	T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && !IsIdentity_v<T1> && !IsZero_v<T1> && IsIdentity_v<T2>>> {
	using Type = Rebind_t<ResultType_t<T1>, MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<
	T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && IsIdentity_v<T1> && !IsIdentity_v<T2> && !IsZero_v<T2>>> {
	using Type = Rebind_t<ResultType_t<T2>, MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && IsIdentity_v<T1> && IsIdentity_v<T2>>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = IdentityMatrix<AddTrait_t<MultType, MultType>, StorageOrder_v<T1>, AddTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct KronTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && IsIdentity_v<T1> && IsIdentity_v<T2>>> {
	using Type = IdentityMatrix<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, StorageOrder_v<T2>,
								MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct UnaryMapTraitEval1<T, OP, EnableIf_t<IsMatrix_v<T> && YieldsIdentity_v<OP, T>>> {
	using ElementType = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()));

	using Type = IdentityMatrix<EvaluateTrait_t<ElementType>, StorageOrder_v<T>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BinaryMapTraitEval1<T1, T2, OP, EnableIf_t<IsMatrix_v<T1> && IsMatrix_v<T2> && YieldsIdentity_v<OP, T1, T2>>> {
	using ElementType =
		decltype(std::declval<OP>()(std::declval<ElementType_t<T1>>(), std::declval<ElementType_t<T2>>()));

	using Type = IdentityMatrix<EvaluateTrait_t<ElementType>, (StorageOrder_v<T1> && StorageOrder_v<T2>),
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclSymTrait<IdentityMatrix<T, SO, Tag>> {
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclHermTrait<IdentityMatrix<T, SO, Tag>> {
	using Type = IdentityMatrix<T, SO, Tag>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclLowTrait<IdentityMatrix<T, SO, Tag>> {
	using Type = IdentityMatrix<T, SO, Tag>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclUniLowTrait<IdentityMatrix<T, SO, Tag>> {
	using Type = IdentityMatrix<T, SO, Tag>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclStrLowTrait<IdentityMatrix<T, SO, Tag>> {
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclUppTrait<IdentityMatrix<T, SO, Tag>> {
	using Type = IdentityMatrix<T, SO, Tag>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclUniUppTrait<IdentityMatrix<T, SO, Tag>> {
	using Type = IdentityMatrix<T, SO, Tag>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclStrUppTrait<IdentityMatrix<T, SO, Tag>> {
	using Type = INVALID_TYPE;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, bool SO, typename Tag> struct DeclDiagTrait<IdentityMatrix<T, SO, Tag>> {
	using Type = IdentityMatrix<T, SO, Tag>;
};
/*! \endcond */
//*************************************************************************************************
