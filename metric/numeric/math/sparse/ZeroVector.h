...
#ifndef METRIC_NUMERIC_MATH_SPARSE_ZEROVECTOR_H
#define METRIC_NUMERIC_MATH_SPARSE_ZEROVECTOR_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class ZeroVector : public Expression<SparseVector<ZeroVector<Type, TF, Tag>, TF>> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	//@{
	template <typename VT> inline ZeroVector &operator=(const Vector<VT, TF> &rhs) &;
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag>													// Type tag
constexpr ZeroVector<Type, TF, Tag>::ZeroVector() noexcept : size_(0UL) // The current size/dimension of the zero vector
{
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
template <typename VT>	// Type of the other zero vector
	...
	: size_((*v).size()) // The current size/dimension of the zero vector
{
	...
	if (!IsZero_v<VT> && !isZero(*v)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid setup of zero vector");
	}
}
//*************************************************************************************************
...
ZeroVector<Type, TF, Tag>::operator[](size_t index) const noexcept
{
	...
	METRIC_NUMERIC_USER_ASSERT(index < size(), "Invalid zero vector access index");
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename ZeroVector<Type, TF, Tag>::ConstReference ZeroVector<Type, TF, Tag>::at(size_t index) const
{
	if (index >= size_) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid zero vector access index");
	}
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename ZeroVector<Type, TF, Tag>::ConstIterator ZeroVector<Type, TF, Tag>::begin() const noexcept
{
	return nullptr;
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename ZeroVector<Type, TF, Tag>::ConstIterator ZeroVector<Type, TF, Tag>::cbegin() const noexcept
{
	return nullptr;
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename ZeroVector<Type, TF, Tag>::ConstIterator ZeroVector<Type, TF, Tag>::end() const noexcept
{
	return nullptr;
}
//*************************************************************************************************
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
constexpr typename ZeroVector<Type, TF, Tag>::ConstIterator ZeroVector<Type, TF, Tag>::cend() const noexcept
{
	return nullptr;
}
//*************************************************************************************************
...
inline ZeroVector<Type, TF, Tag> &ZeroVector<Type, TF, Tag>::operator=(const Vector<VT, TF> &rhs) &
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SAME_TAG(Tag, TagType_t<VT>);

	if (!IsZero_v<VT> && !isZero(*rhs)) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment of zero vector");
	}
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename ZeroVector<Type, TF, Tag>::ConstIterator ZeroVector<Type, TF, Tag>::find(size_t index) const
{
	MAYBE_UNUSED(index);
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename ZeroVector<Type, TF, Tag>::ConstIterator ZeroVector<Type, TF, Tag>::lowerBound(size_t index) const
{
	MAYBE_UNUSED(index);
...
template <typename Type // Data type of the vector
		  ...
		  ,
		  typename Tag> // Type tag
inline typename ZeroVector<Type, TF, Tag>::ConstIterator ZeroVector<Type, TF, Tag>::upperBound(size_t index) const
{
	MAYBE_UNUSED(index);
...
template <typename Type, bool TF, typename Tag> constexpr bool isIntact(const ZeroVector<Type, TF, Tag> &v) noexcept;
...
// return \a false. The following example demonstrates the use of the \a isDefault() function:
   ...
   \code
   mtrc::numeric::ZeroVector<double> z;
...
// all elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::ZeroVector<double,mtrc::numeric::columnVector> z;
   // ... Resizing and initialization
...
// explicitly:
   ...
   \endcode
*/
template <typename T, bool TF = defaultTransposeFlag> constexpr decltype(auto) zero(size_t n) noexcept
{
...
// vector. The following example demonstrates the use of the \a declzero function:
   ...
   \code
   mtrc::numeric::ZeroVector<double> a, b;
   // ... Resizing and initialization
   b = declzero( a );
   \endcode
*/
template <typename VT // Type of the vector
		  ,
		  bool TF> // Transpose flag
inline ZeroVector<ElementType_t<VT>, TF> declzero(const Vector<VT, TF> &v)
{
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename Type, bool TF, typename Tag> struct IsUniform<ZeroVector<Type, TF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename Type, bool TF, typename Tag> struct IsZero<ZeroVector<Type, TF, Tag>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
struct AddTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && !IsZero_v<T1> && IsZero_v<T2>>> {
...
template <typename T1, typename T2>
struct AddTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && IsZero_v<T1> && IsZero_v<T2>>> {
	using Type = ZeroVector<AddTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T1>,
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct SubTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && !IsZero_v<T1> && IsZero_v<T2>>> {
...
template <typename T1, typename T2>
struct SubTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && IsZero_v<T1> && IsZero_v<T2>>> {
	using Type = ZeroVector<SubTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T1>,
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsScalar_v<T2> && IsZero_v<T1>>> {
	using Type = ZeroVector<MultTrait_t<ElementType_t<T1>, T2>, TransposeFlag_v<T1>, MultTrait_t<TagType_t<T1>, T2>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsScalar_v<T1> && IsVector_v<T2> && IsZero_v<T2>>> {
	using Type = ZeroVector<MultTrait_t<T1, ElementType_t<T2>>, TransposeFlag_v<T2>, MultTrait_t<T1, TagType_t<T2>>>;
};
...
struct MultTraitEval1<
	...
	using Type = ZeroVector<MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T1>,
							MultTrait_t<TagType_t<T1>, TagType_t<T2>>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsMatrix_v<T1> && IsColumnVector_v<T2> && (IsZero_v<T1> || IsZero_v<T2>)>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = ZeroVector<AddTrait_t<MultType, MultType>, false, AddTrait_t<MultTag, MultTag>>;
};

template <typename T1, typename T2>
struct MultTraitEval1<T1, T2, EnableIf_t<IsRowVector_v<T1> && IsMatrix_v<T2> && (IsZero_v<T1> || IsZero_v<T2>)>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = ZeroVector<AddTrait_t<MultType, MultType>, true, AddTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct KronTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && (IsZero_v<T1> || IsZero_v<T2>)>> {
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct DivTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsScalar_v<T2> && IsZero_v<T1>>> {
	using Type = ZeroVector<DivTrait_t<ElementType_t<T1>, T2>, TransposeFlag_v<T1>, DivTrait_t<TagType_t<T1>, T2>>;
};
...
struct DivTraitEval1<
	...
	EnableIf_t<((IsRowVector_v<T1> && IsRowVector_v<T2>) || (IsColumnVector_v<T1> && IsColumnVector_v<T2>)) &&
			   (IsZero_v<T1> || IsZero_v<T2>)>> {
	using Type = ZeroVector<DivTrait_t<ElementType_t<T1>, ElementType_t<T2>>, TransposeFlag_v<T1>,
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T1, typename T2>
struct CrossTraitEval1<T1, T2, EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && (IsZero_v<T1> || IsZero_v<T2>)>> {
	using MultType = MultTrait_t<ElementType_t<T1>, ElementType_t<T2>>;
	using MultTag = MultTrait_t<TagType_t<T1>, TagType_t<T2>>;

	using Type = ZeroVector<SubTrait_t<MultType, MultType>, TransposeFlag_v<T1>, SubTrait_t<MultTag, MultTag>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, typename OP> struct UnaryMapTraitEval1<T, OP, EnableIf_t<IsVector_v<T> && YieldsZero_v<OP, T>>> {
	using ElementType = decltype(std::declval<OP>()(std::declval<ElementType_t<T>>()));

	using Type = ZeroVector<EvaluateTrait_t<ElementType>, TransposeFlag_v<T>, MapTrait_t<TagType_t<T>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
struct BinaryMapTraitEval1<T1, T2, OP, EnableIf_t<IsVector_v<T1> && IsVector_v<T2> && YieldsZero_v<OP, T1, T2>>> {
	using ElementType =
		decltype(std::declval<OP>()(std::declval<ElementType_t<T1>>(), std::declval<ElementType_t<T2>>()));

	using Type =
		ZeroVector<EvaluateTrait_t<ElementType>, TransposeFlag_v<T1>, MapTrait_t<TagType_t<T1>, TagType_t<T2>, OP>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename T, size_t R0> struct RepeatTraitEval1<T, R0, inf, inf, EnableIf_t<IsVector_v<T> && IsZero_v<T>>> {
	using Type = ZeroVector<ElementType_t<T>, TransposeFlag_v<T>, TagType_t<T>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename VT, size_t I, size_t N> struct SubvectorTraitEval1<VT, I, N, EnableIf_t<IsZero_v<VT>>> {
	using Type = ZeroVector<RemoveConst_t<ElementType_t<VT>>, TransposeFlag_v<VT>, TagType_t<VT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename VT, size_t N> struct ElementsTraitEval1<VT, N, EnableIf_t<IsZero_v<VT>>> {
	using Type = ZeroVector<RemoveConst_t<ElementType_t<VT>>, TransposeFlag_v<VT>, TagType_t<VT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t I> struct RowTraitEval1<MT, I, EnableIf_t<IsZero_v<MT>>> {
	using Type = ZeroVector<RemoveConst_t<ElementType_t<MT>>, true, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, size_t I> struct ColumnTraitEval1<MT, I, EnableIf_t<IsZero_v<MT>>> {
	using Type = ZeroVector<RemoveConst_t<ElementType_t<MT>>, false, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
...
//*************************************************************************************************
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, ptrdiff_t I> struct BandTraitEval1<MT, I, EnableIf_t<IsZero_v<MT>>> {
	using Type = ZeroVector<RemoveConst_t<ElementType_t<MT>>, defaultTransposeFlag, TagType_t<MT>>;
};
/*! \endcond */
//*************************************************************************************************
