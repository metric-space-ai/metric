...
#ifndef METRIC_NUMERIC_MATH_VIEWS_BAND_H
#define METRIC_NUMERIC_MATH_VIEWS_BAND_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/views/band/Dense.h>
#include <metric/numeric/math/views/band/Sparse.h>
...
#include <metric/numeric/util/mpl/Min.h>
...
inline decltype(auto) band(const MatMatAddExpr<MT> &matrix, RBAs... args)
{
	...
	return band<CBAs...>((*matrix).leftOperand(), args...) + band<CBAs...>((*matrix).rightOperand(), args...);
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) band(const MatMatMapExpr<MT> &matrix, RBAs... args)
{
	...
	return map(band<CBAs...>((*matrix).leftOperand(), args...), band<CBAs...>((*matrix).rightOperand(), args...),
			   (*matrix).operation());
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) band(const VecTVecMapExpr<MT> &matrix, RBAs... args)
{
	...
	return map(transTo<defaultTransposeFlag>(subvector(leftOperand, row, size, unchecked)),
			   transTo<defaultTransposeFlag>(subvector(rightOperand, column, size, unchecked)), (*matrix).operation());
}
/*! \endcond */
//*************************************************************************************************
...
template <AlignmentFlag AF // Alignment flag
		  ...
		  ,
		  EnableIf_t<IsBand_v<RemoveReference_t<VT>> && RemoveReference_t<VT>::compileTimeArgs> * = nullptr>
...
inline bool isIntact(const Band<MT, TF, DF, MF, CBAs...> &band) noexcept
{
	...
	return ((index >= 0L || size_t(-index) < band.operand().rows()) &&
			(index <= 0L || size_t(index) < band.operand().columns()) && isIntact(band.operand()));
}
/*! \endcond */
//*************************************************************************************************
...
inline auto isSame_backend(const Band<MT1, TF, DF, MF, CBAs1...> &a, const Band<MT2, TF, DF, MF, CBAs2...> &b) noexcept
...
{
	return (isSame(a.operand().operand(), b.operand()) && (a.size() == b.size()) &&
			(a.row() + a.operand().row() == b.row()) && (a.column() + a.operand().column() == b.column()));
}
/*! \endcond */
//*************************************************************************************************
...
inline auto isSame_backend(const Band<MT1, TF, DF, MF, CBAs1...> &a, const Band<MT2, TF, DF, MF, CBAs2...> &b) noexcept
	-> EnableIf_t<!IsSubmatrix_v<MT1> && IsSubmatrix_v<MT2>, bool>
{
	return (isSame(a.operand(), b.operand().operand()) && (a.size() == b.size()) &&
			(a.row() == b.row() + b.operand().row()) && (a.column() == b.column() + b.operand().column()));
}
/*! \endcond */
//*************************************************************************************************
...
inline auto isSame_backend(const Band<MT1, TF, DF, MF, CBAs1...> &a, const Band<MT2, TF, DF, MF, CBAs2...> &b) noexcept
	-> EnableIf_t<IsSubmatrix_v<MT1> && IsSubmatrix_v<MT2>, bool>
{
	return (isSame(a.operand().operand(), b.operand().operand()) && (a.size() == b.size()) &&
...
template <typename MT1 // Type of the matrix of the left-hand side band
		  ...
		  ,
		  ptrdiff_t... CBAs2> // Compile time band arguments of the right-hand side band
inline bool isSame(const Band<MT1, TF, DF, MF, CBAs1...> &a, const Band<MT2, TF, DF, MF, CBAs2...> &b) noexcept
{
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index < band.size(), "Invalid vector access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySet(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
										 const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryAdd(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
										 const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySub(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
										 const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryMult(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
										  const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryDiv(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
										 const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
METRIC_NUMERIC_ALWAYS_INLINE bool tryShift(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
										   int count)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitand(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
											const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitor(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
										   const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitxor(const Band<MT, TF, DF, MF, CBAs...> &band, size_t index, size_t size,
											const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*band).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*band).size(), "Invalid range size");
...
inline bool tryAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryAddAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool trySubAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryMultAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryDivAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryShiftAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitandAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitorAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitxorAssign(const Band<MT, TF, DF, MF, CBAs...> &lhs, const Vector<VT, TF> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
struct Size<Band<MT, TF, DF, MF, I>, 0UL>
	: public If_t<
		  (Size_v<MT, 0UL> >= 0L && Size_v<MT, 1UL> >= 0L),
		  Min_t<Ptrdiff_t<Size_v<MT, 0UL> - (I >= 0L ? 0L : -I)>, Ptrdiff_t<Size_v<MT, 1UL> - (I >= 0L ? I : 0L)>>,
		  Ptrdiff_t<-1L>> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool TF, bool DF, bool MF, ptrdiff_t I>
struct MaxSize<Band<MT, TF, DF, MF, I>, 0UL> : public If_t<(MaxSize_v<MT, 0UL> >= 0L && MaxSize_v<MT, 1UL> >= 0L),
...
struct IsRestricted<Band<MT, TF, DF, MF, CBAs...>> : public IsRestricted<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct HasConstDataAccess<Band<MT, TF, true, MF, CBAs...>> : public HasConstDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool TF, bool MF, ptrdiff_t... CBAs>
struct HasMutableDataAccess<Band<MT, TF, true, MF, CBAs...>> : public HasMutableDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool TF, bool DF, bool MF, ptrdiff_t... CBAs>
struct IsOpposedView<Band<MT, TF, DF, MF, CBAs...>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
