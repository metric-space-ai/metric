...
#ifndef METRIC_NUMERIC_MATH_VIEWS_COLUMNS_SPARSE_H
#define METRIC_NUMERIC_MATH_VIEWS_COLUMNS_SPARSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
class Columns<MT, true, false, SF, CCAs...> : public View<SparseMatrix<Columns<MT, true, false, SF, CCAs...>, true>>,
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
inline typename Columns<MT, true, false, SF, CCAs...>::Reference Columns<MT, true, false, SF, CCAs...>::at(size_t i,
																										   size_t j)
{
	if (i >= rows()) {
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
inline typename Columns<MT, true, false, SF, CCAs...>::Iterator Columns<MT, true, false, SF, CCAs...>::begin(size_t j)
{
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
inline typename Columns<MT, true, false, SF, CCAs...>::Iterator Columns<MT, true, false, SF, CCAs...>::end(size_t j)
{
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
{
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid row access index");
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
inline Columns<MT, true, false, SF, CCAs...> &Columns<MT, true, false, SF, CCAs...>::operator=(const Columns &rhs)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_SYMMETRIC_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_HERMITIAN_MATRIX_TYPE(MT);
...
inline size_t Columns<MT, true, false, SF, CCAs...>::extendCapacity(size_t j) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(nonzeros > capacity(j), "Invalid capacity value");
...
template <typename MT // Type of the sparse matrix
		  ...
		  typename... CCAs> // Compile time column arguments
inline
	typename Columns<MT, true, false, SF, CCAs...>::Iterator Columns<MT, true, false, SF, CCAs...>::erase(size_t j,
																										  Iterator pos)
{
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
inline typename Columns<MT, true, false, SF, CCAs...>::Iterator Columns<MT, true, false, SF, CCAs...>::find(size_t i,
																											size_t j)
{
...
template <typename MT // Type of the sparse matrix
		  ...
		  typename... CCAs> // Compile time column arguments
inline
	typename Columns<MT, true, false, SF, CCAs...>::Iterator Columns<MT, true, false, SF, CCAs...>::lowerBound(size_t i,
																											   size_t j)
{
...
template <typename MT // Type of the sparse matrix
		  ...
		  typename... CCAs> // Compile time column arguments
inline
	typename Columns<MT, true, false, SF, CCAs...>::Iterator Columns<MT, true, false, SF, CCAs...>::upperBound(size_t i,
																											   size_t j)
{
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
inline Columns<MT, true, false, SF, CCAs...> &Columns<MT, true, false, SF, CCAs...>::transpose()
{
	if (rows() != columns()) {
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
inline Columns<MT, true, false, SF, CCAs...> &Columns<MT, true, false, SF, CCAs...>::ctranspose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic matrix");
	}
...
inline Columns<MT, true, false, SF, CCAs...> &Columns<MT, true, false, SF, CCAs...>::scale(const Other &scalar)
{
...
class Columns<MT, false, false, false, CCAs...>
	: public View<SparseMatrix<Columns<MT, false, false, false, CCAs...>, true>>, private ColumnsData<CCAs...> {
  private:
	//**Type definitions****************************************************************************
	using DataType = ColumnsData<CCAs...>;				//!< The type of the ColumnsData base class.
  ...
  public:
	...
	class ColumnsElement : private SparseElement {
	  public:
		//**Constructor******************************************************************************
		...
		*/
		inline const ColumnsElement *operator->() const { return this; }
		//*******************************************************************************************
...
inline void Columns<MT, false, false, false, CCAs...>::reset(size_t j)
{
	...
	const size_t ibegin((IsLower_v<MT>) ? ((IsUniLower_v<MT> || IsStrictlyLower_v<MT>) ? (index + 1UL) : (index))
										: (0UL));
	const size_t iend((IsUpper_v<MT>) ? ((IsUniUpper_v<MT> || IsStrictlyUpper_v<MT>) ? (index) : (index + 1UL))
									  : (rows()));
...
template <typename MT // Type of the sparse matrix
		  ,
		  typename... CCAs> // Compile time column arguments
inline Columns<MT, false, false, false, CCAs...> &Columns<MT, false, false, false, CCAs...>::transpose()
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_SYMMETRIC_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_HERMITIAN_MATRIX_TYPE(MT);
...
template <typename MT // Type of the sparse matrix
		  ,
		  typename... CCAs> // Compile time column arguments
inline Columns<MT, false, false, false, CCAs...> &Columns<MT, false, false, false, CCAs...>::ctranspose()
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_SYMMETRIC_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_HERMITIAN_MATRIX_TYPE(MT);
...
template <typename MT // Type of the sparse matrix
		  ...
		  typename... CCAs> // Compile time column arguments
template <typename Other>	// Data type of the scalar value
inline Columns<MT, false, false, false, CCAs...> &Columns<MT, false, false, false, CCAs...>::scale(const Other &scalar)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_SYMMETRIC_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_HERMITIAN_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_UNITRIANGULAR_MATRIX_TYPE(MT);
	...
	for (size_t i = 0UL; i < rows(); ++i) {
		for (ConstIterator_t<MT2> element = (*rhs).begin(i); element != (*rhs).end(i); ++element) {
			...
			if (!isDefault<strict>(value))
				...
				matrix_.erase(i, idx(element->index()));
		}
	}
}
/*! \endcond */
	...
	for (size_t j = 0UL; j < columns(); ++j) {
		...
		for (ConstIterator_t<MT2> element = (*rhs).begin(j); element != (*rhs).end(j); ++element) {
			...
			if (!isDefault<strict>(value))
				...
				matrix_.erase(element->index(), index);
		}
	}
}
/*! \endcond */
...
class Columns<MT, false, false, true, CCAs...>
	: public View<SparseMatrix<Columns<MT, false, false, true, CCAs...>, true>>, private ColumnsData<CCAs...> {
  private:
	//**Type definitions****************************************************************************
	using DataType = ColumnsData<CCAs...>;				//!< The type of the ColumnsData base class.
	using Operand = If_t<IsExpression_v<MT>, MT, MT &>; //!< Composite data type of the matrix expression.
...
template <typename MT // Type of the sparse matrix
		  ...
		  typename... CCAs> // Compile time column arguments
inline
	typename Columns<MT, false, false, true, CCAs...>::Reference Columns<MT, false, false, true, CCAs...>::at(size_t i,
																											  size_t j)
{
	if (i >= rows()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
...
template <typename MT // Type of the sparse matrix
		  ...
		  typename... CCAs> // Compile time column arguments
inline
	typename Columns<MT, false, false, true, CCAs...>::Iterator Columns<MT, false, false, true, CCAs...>::end(size_t j)
{
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid column access index");
...
// efficient way to add new elements to a sparse column selection:
   ...
   \code
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::CompressedMatrix<double,mtrc::numeric::rowMajor> > A( 42 );
   auto B = columns( A, { 10, 16, 4, 3 } );
...
// how to remove all elements that are smaller than a certain threshold value:
   ...
   \code
   mtrc::numeric::SymmetricMatrix< mtrc::numeric::CompressedMatrix<double,mtrc::numeric::rowMajor> > A;
   // ... Resizing and initialization
...
template <typename MT // Type of the sparse matrix
		  ...
		  typename... CCAs> // Compile time column arguments
inline
	typename Columns<MT, false, false, true, CCAs...>::Iterator Columns<MT, false, false, true, CCAs...>::find(size_t i,
																											   size_t j)
{
