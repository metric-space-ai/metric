...
#ifndef METRIC_NUMERIC_MATH_VIEWS_SUBVECTOR_DENSE_H
#define METRIC_NUMERIC_MATH_VIEWS_SUBVECTOR_DENSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/views/subvector/SubvectorData.h>
...
class Subvector<VT, unaligned, TF, true, CSAs...>
	: public View<DenseVector<Subvector<VT, unaligned, TF, true, CSAs...>, TF>>, private SubvectorData<CSAs...> {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	class SubvectorIterator {
	  public:
		 ...
		 */
		inline METRIC_NUMERIC_DEVICE_CALLABLE SubvectorIterator()
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE SubvectorIterator(IteratorType iterator, bool isMemoryAligned)
		...
		*/
		...
		inline METRIC_NUMERIC_DEVICE_CALLABLE SubvectorIterator(const SubvectorIterator<IteratorType2> &it)
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE SubvectorIterator &operator+=(size_t inc)
		{
			iterator_ += inc;
			return *this;
		}
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE SubvectorIterator &operator-=(size_t dec)
		{
			iterator_ -= dec;
			return *this;
		}
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE SubvectorIterator &operator++()
		{
			++iterator_;
			return *this;
		}
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE const SubvectorIterator operator++(int)
		{
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE ReferenceType operator*() const { return *iterator_; }
		//*******************************************************************************************
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE IteratorType operator->() const { return iterator_; }
		//*******************************************************************************************
		...
		*/
		inline SIMDType loada() const { return iterator_.loada(); }
		//*******************************************************************************************
		...
		// might result in erroneous results and/or in compilation errors.
		*/
		inline SIMDType loadu() const { return iterator_.loadu(); }
		//*******************************************************************************************
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE bool operator==(const SubvectorIterator &rhs) const
		{
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE DifferenceType operator-(const SubvectorIterator &rhs) const
		{
			return iterator_ - rhs.iterator_;
		}
		//*******************************************************************************************
		...
		// \return The incremented iterator.
		*/
		friend inline METRIC_NUMERIC_DEVICE_CALLABLE const SubvectorIterator operator+(const SubvectorIterator &it,
																					   size_t inc)
		{
		...
		// \return The incremented iterator.
		*/
		friend inline METRIC_NUMERIC_DEVICE_CALLABLE const SubvectorIterator operator+(size_t inc,
		...
		*/
		inline METRIC_NUMERIC_DEVICE_CALLABLE IteratorType base() const { return iterator_; }
		//*******************************************************************************************
		...
		*/
		inline bool isAligned() const { return isAligned_; }
		//*******************************************************************************************
	...
	/*!\name Constructors */
	//@{
	template <typename... RSAs> explicit inline Subvector(VT &vector, RSAs... args);
  ...
  private:
	...
	static constexpr bool VectorizedAssign_v =
		(useOptimizedKernels && simdEnabled && VT2::simdEnabled && IsSIMDCombinable_v<ElementType, ElementType_t<VT2>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedAddAssign_v =
		(VectorizedAssign_v<VT2> && HasSIMDAdd_v<ElementType, ElementType_t<VT2>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedSubAssign_v =
		(VectorizedAssign_v<VT2> && HasSIMDSub_v<ElementType, ElementType_t<VT2>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedMultAssign_v =
		(VectorizedAssign_v<VT2> && HasSIMDMult_v<ElementType, ElementType_t<VT2>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedDivAssign_v =
		(VectorizedAssign_v<VT2> && HasSIMDDiv_v<ElementType, ElementType_t<VT2>>);
	//**********************************************************************************************
  ...
  public:
	...
	template <typename VT2> inline auto assign(const DenseVector<VT2, TF> &rhs) -> DisableIf_t<VectorizedAssign_v<VT2>>;

	template <typename VT2> inline auto assign(const DenseVector<VT2, TF> &rhs) -> EnableIf_t<VectorizedAssign_v<VT2>>;
  ...
  private:
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_TRANSEXPR_TYPE(VT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_SUBVECTOR_TYPE(VT);
...
// exception is thrown. The checks can be skipped by providing the optional \a mtrc::numeric::unchecked
// argument.
*/
template <typename VT // Type of the dense vector
		  ,
...
inline Subvector<VT, unaligned, TF, true, CSAs...>::Subvector(VT &vector, RSAs... args)
	: DataType(args...) // Base class initialization
	  ...
	  ,
	  isAligned_(simdEnabled && IsContiguous_v<VT> && vector.data() != nullptr && checkAlignment(data()))
{
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(offset() + size() <= vector.size(), "Invalid subvector specification");
	}
}
/*! \endcond */
//*************************************************************************************************
...
{
	METRIC_NUMERIC_USER_ASSERT(index < size(), "Invalid subvector access index");
...
{
	if (index >= size()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid subvector access index");
	}
	return (*this)[index];
}
/*! \endcond */
...
template <typename VT // Type of the dense vector
		  ...
		  size_t... CSAs> // Compile time subvector arguments
inline
	typename Subvector<VT, unaligned, TF, true, CSAs...>::Iterator Subvector<VT, unaligned, TF, true, CSAs...>::begin()
{
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
inline typename Subvector<VT, unaligned, TF, true, CSAs...>::Iterator Subvector<VT, unaligned, TF, true, CSAs...>::end()
{
...
{
	if (list.size() > size()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to subvector");
	}

	if (IsRestricted_v<VT>) {
		const InitializerVector<ElementType, TF> tmp(list, size());
		if (!tryAssign(vector_, tmp, offset())) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to restricted vector");
		}
	}
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(vector_), "Invariant violation detected");
...
Subvector<VT, unaligned, TF, true, CSAs...>::operator=(const Subvector &rhs)
{
	...
	if (size() != rhs.size()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Subvector sizes do not match");
	}
...
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTOR_WITH_TRANSPOSE_FLAG(ResultType_t<VT2>, TF);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(ResultType_t<VT2>);
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
template <typename VT2	  // Data type of the other dense subvector
		  ,
		  AlignmentFlag AF2 // Alignment flag of the other dense subvector
		  ,
		  bool TF2 // Transpose flag of the other dense subvector
		  ,
		  size_t... CSAs2> // Compile time subvector arguments of the other dense subvector
inline bool Subvector<VT, unaligned, TF, true, CSAs...>::canAlias(
	const Subvector<VT2, AF2, TF2, true, CSAs2...> *alias) const noexcept
{
	return (vector_.isAliased(&alias->vector_) && (offset() + size() > alias->offset()) &&
...
template <typename VT2	  // Data type of the other dense subvector
		  ...
		  ,
		  size_t... CSAs2> // Compile time subvector arguments of the other dense subvector
inline bool Subvector<VT, unaligned, TF, true, CSAs...>::isAliased(
	const Subvector<VT2, AF2, TF2, true, CSAs2...> *alias) const noexcept
{
	return (vector_.isAliased(&alias->vector_) && (offset() + size() > alias->offset()) &&
			(offset() < alias->offset() + alias->size()));
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE typename Subvector<VT, unaligned, TF, true, CSAs...>::SIMDType
...
Subvector<VT, unaligned, TF, true, CSAs...>::loada(size_t index) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(index < size(), "Invalid subvector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + SIMDSIZE <= size(), "Invalid subvector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index % SIMDSIZE == 0UL, "Invalid subvector access index");
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE void Subvector<VT, unaligned, TF, true, CSAs...>::store(size_t index,
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE void Subvector<VT, unaligned, TF, true, CSAs...>::storea(size_t index,
																					  const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE void Subvector<VT, unaligned, TF, true, CSAs...>::storeu(size_t index,
																					  const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE void Subvector<VT, unaligned, TF, true, CSAs...>::stream(size_t index,
																					  const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
	...
	-> DisableIf_t<VectorizedAssign_v<VT2>>
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(ipos <= size(), "Invalid end calculation");
	...
	-> EnableIf_t<VectorizedAssign_v<VT2>>
{
	...
	if (useStreaming && isAligned_ && (size() > (cacheSize / (sizeof(ElementType) * 3UL))) && !(*rhs).isAliased(this)) {
...
class Subvector<VT, aligned, TF, true, CSAs...>
	: public View<DenseVector<Subvector<VT, aligned, TF, true, CSAs...>, TF>>, private SubvectorData<CSAs...> {
  private:
	//**Type definitions****************************************************************************
	using DataType = SubvectorData<CSAs...>;			//!< The type of the SubvectorData base class.
	using Operand = If_t<IsExpression_v<VT>, VT, VT &>; //!< Composite data type of the vector expression.
	  ...
	  vector_(vector) // The vector containing the subvector
{
	if (isChecked(args...)) {
		...
		if (simdEnabled && IsContiguous_v<VT> && !checkAlignment(data())) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid subvector alignment");
		}
	} else {
		METRIC_NUMERIC_USER_ASSERT(offset() + size() <= vector.size(), "Invalid subvector specification");
		METRIC_NUMERIC_USER_ASSERT(!simdEnabled || !IsContiguous_v<VT> || checkAlignment(data()),
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
inline typename Subvector<VT, aligned, TF, true, CSAs...>::Iterator Subvector<VT, aligned, TF, true, CSAs...>::begin()
{
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
inline typename Subvector<VT, aligned, TF, true, CSAs...>::Iterator Subvector<VT, aligned, TF, true, CSAs...>::end()
{
...
template <typename VT // Type of the dense vector
		  ...
		  size_t... CSAs> // Compile time subvector arguments
template <typename Other> // Data type of the scalar value
inline Subvector<VT, aligned, TF, true, CSAs...> &Subvector<VT, aligned, TF, true, CSAs...>::scale(const Other &scalar)
{
	const size_t iend(offset() + size());
	for (size_t i = offset(); i < iend; ++i)
		vector_[i] *= scalar;
...
template <typename VT2	  // Data type of the other dense subvector
		  ...
		  ,
		  size_t... CSAs2> // Compile time subvector arguments of the other dense subvector
inline bool Subvector<VT, aligned, TF, true, CSAs...>::canAlias(
	const Subvector<VT2, AF2, TF2, true, CSAs2...> *alias) const noexcept
{
	return (vector_.isAliased(&alias->vector_) && (offset() + size() > alias->offset()) &&
			(offset() < alias->offset() + alias->size()));
...
template <typename VT2	  // Data type of the other dense subvector
		  ...
		  ,
		  size_t... CSAs2> // Compile time subvector arguments of the other dense subvector
inline bool Subvector<VT, aligned, TF, true, CSAs...>::isAliased(
	const Subvector<VT2, AF2, TF2, true, CSAs2...> *alias) const noexcept
{
	return (vector_.isAliased(&alias->vector_) && (offset() + size() > alias->offset()) &&
			(offset() < alias->offset() + alias->size()));
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE typename Subvector<VT, aligned, TF, true, CSAs...>::SIMDType
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE void Subvector<VT, aligned, TF, true, CSAs...>::store(size_t index,
																				   const SIMDType &value) noexcept
{
	storea(index, value);
}
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE void Subvector<VT, aligned, TF, true, CSAs...>::storea(size_t index,
																					const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE void Subvector<VT, aligned, TF, true, CSAs...>::storeu(size_t index,
																					const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename VT // Type of the dense vector
		  ...
		  ,
		  size_t... CSAs> // Compile time subvector arguments
METRIC_NUMERIC_ALWAYS_INLINE void Subvector<VT, aligned, TF, true, CSAs...>::stream(size_t index,
																					const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
class Subvector<DVecDVecCrossExpr<VT1, VT2, TF>, unaligned, TF, true, CSAs...>
  ...
  public:
	...
	*/
	inline CPE operand() const noexcept { return vector_; }
	//**********************************************************************************************
...
class Subvector<SVecSVecCrossExpr<VT1, VT2, TF>, unaligned, TF, true, CSAs...>
  ...
  public:
	...
	*/
	...
	// \return \a true in case an alias effect is detected, \a false otherwise.
	*/
	template <typename T> inline bool isAliased(const T *alias) const { return vector_.isAliased(&unview(*alias)); }
	//**********************************************************************************************
