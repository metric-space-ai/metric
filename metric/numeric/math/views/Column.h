...
#ifndef METRIC_NUMERIC_MATH_VIEWS_COLUMN_H
#define METRIC_NUMERIC_MATH_VIEWS_COLUMN_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/views/column/Dense.h>
#include <metric/numeric/math/views/column/Sparse.h>
...
inline decltype(auto) column(const MatMatAddExpr<MT> &matrix, RCAs... args)
{
	...
	return column<CCAs...>((*matrix).leftOperand(), args...) + column<CCAs...>((*matrix).rightOperand(), args...);
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) column(const MatMatMapExpr<MT> &matrix, RCAs... args)
{
	...
	return map(column<CCAs...>((*matrix).leftOperand(), args...), column<CCAs...>((*matrix).rightOperand(), args...),
			   (*matrix).operation());
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) column(const VecTVecMapExpr<MT> &matrix, RCAs... args)
{
	...
	return map((*matrix).leftOperand(), mtrc::numeric::bind2nd((*matrix).operation(), (*matrix).rightOperand()[I]));
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) column(const VecTVecMapExpr<MT> &matrix, size_t index, RCAs... args)
{
	...
	return map((*matrix).leftOperand(),
			   mtrc::numeric::bind2nd((*matrix).operation(), (*matrix).rightOperand()[index]));
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) column(const MatRepeatExpr<MT, CRAs...> &matrix, RCAs... args)
{
	...
	return repeat(column((*matrix).operand(), cd.column() % (*matrix).operand().columns(), unchecked),
...
{
	return (column.column() < column.operand().columns() && isIntact(column.operand()));
}
/*! \endcond */
//*************************************************************************************************
...
inline bool isSame(const Column<MT1, SO, DF, SF1, CCAs1...> &a, const Column<MT2, SO, DF, SF2, CCAs2...> &b) noexcept
{
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index < column.size(), "Invalid vector access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySet(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
										 const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryAdd(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
										 const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySub(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
										 const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryMult(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
										  const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryDiv(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
										 const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  size_t... CCAs> // Compile time column arguments
METRIC_NUMERIC_ALWAYS_INLINE bool tryShift(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
										   int count)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitand(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
											const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitor(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
										   const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitxor(const Column<MT, SO, DF, SF, CCAs...> &column, size_t index, size_t size,
											const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= (*column).size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + size <= (*column).size(), "Invalid range size");
...
inline bool tryAssign(const Column<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryAddAssign(const Column<MT, SO, SF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool trySubAssign(const Column<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryMultAssign(const Column<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryDivAssign(const Column<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryShiftAssign(const Column<MT, SO, SF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitandAssign(const Column<MT, SO, SF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitorAssign(const Column<MT, SO, SF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitxorAssign(const Column<MT, SO, SF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t index)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(index <= lhs.size(), "Invalid vector access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(index + (*rhs).size() <= lhs.size(), "Invalid vector size");
...
struct Size<Column<MT, SO, DF, SF, CRAs...>, 0UL> : public Size<MT, 0UL> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool DF, bool SF, size_t... CRAs>
struct MaxSize<Column<MT, SO, DF, SF, CRAs...>, 0UL> : public MaxSize<MT, 0UL> {};
/*! \endcond */
//*************************************************************************************************
...
struct IsRestricted<Column<MT, SO, DF, SF, CCAs...>> : public IsRestricted<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct HasConstDataAccess<Column<MT, SO, true, SF, CCAs...>> : public HasConstDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool SF, size_t... CCAs>
struct HasMutableDataAccess<Column<MT, SO, true, SF, CCAs...>> : public HasMutableDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct IsContiguous<Column<MT, true, true, SF, CCAs...>> : public IsContiguous<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct IsOpposedView<Column<MT, false, DF, false, CCAs...>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
