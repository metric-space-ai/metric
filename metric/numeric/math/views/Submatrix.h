...
#ifndef METRIC_NUMERIC_MATH_VIEWS_SUBMATRIX_H
#define METRIC_NUMERIC_MATH_VIEWS_SUBMATRIX_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/views/column/ColumnData.h>
#include <metric/numeric/math/views/row/RowData.h>
#include <metric/numeric/math/views/submatrix/BaseTemplate.h>
#include <metric/numeric/math/views/submatrix/Dense.h>
#include <metric/numeric/math/views/submatrix/Sparse.h>
...
// The following example demonstrates the creation of a dense and sparse submatrix:
   ...
   \code
   mtrc::numeric::DynamicMatrix<double,mtrc::numeric::rowMajor> D;
   mtrc::numeric::CompressedMatrix<int,mtrc::numeric::columnMajor> S;
...
// skipped by providing the optional \a mtrc::numeric::unchecked argument.
...
// matrix. The following example demonstrates the creation of a dense and sparse submatrix:
   ...
   \code
   const mtrc::numeric::DynamicMatrix<double,mtrc::numeric::rowMajor> D( ... );
   const mtrc::numeric::CompressedMatrix<int,mtrc::numeric::columnMajor> S( ... );
...
// enabled and that AVX is available, which packs 4 \c double values into a SIMD vector:
   ...
   \code
   mtrc::numeric::DynamicMatrix<double,mtrc::numeric::rowMajor> D( 13UL, 17UL );
   // ... Resizing and initialization
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  typename... RSAs> // Option submatrix arguments
inline decltype(auto) submatrix(Matrix<MT, SO> &matrix, size_t row, size_t column, size_t m, size_t n, RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  typename... RSAs> // Option submatrix arguments
inline decltype(auto) submatrix(const Matrix<MT, SO> &matrix, size_t row, size_t column, size_t m, size_t n,
								RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) submatrix(const MatMatMultExpr<MT> &matrix, RSAs... args)
{
	...
	const size_t begin(
		max((IsUpper_v<MT1>) ? ((!AF && IsStrictlyUpper_v<MT1>) ? (sd.row() + 1UL) : (sd.row())) : (0UL),
			(IsLower_v<MT2>) ? ((!AF && IsStrictlyLower_v<MT2>) ? (sd.column() + 1UL) : (sd.column())) : (0UL)));
	const size_t end(
		min((IsLower_v<MT1>)
				? ((IsStrictlyLower_v<MT1> && sd.rows() > 0UL) ? (sd.row() + sd.rows() - 1UL) : (sd.row() + sd.rows()))
				: (left.columns()),
			(IsUpper_v<MT2>) ? ((IsStrictlyUpper_v<MT2> && sd.columns() > 0UL) ? (sd.column() + sd.columns() - 1UL)
...
inline decltype(auto) submatrix(const MatMatKronExpr<MT> &matrix, RSAs... args)
{
	...
	try {
		return columns(rows(*matrix, make_shifted_index_sequence<I, M>(), args...), make_shifted_index_sequence<J, N>(),
					   args...);
	} catch (...) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid submatrix specification");
	}
}
/*! \endcond */
//*************************************************************************************************
...
template <AlignmentFlag AF // Alignment flag
		  ...
		  ,
		  typename... RSAs> // Optional submatrix arguments
inline decltype(auto) submatrix(const MatMatKronExpr<MT> &matrix, size_t row, size_t column, size_t m, size_t n,
								RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
	...
	try {
		return columns(
			rows(
				*matrix, [row](size_t i) { return i + row; }, m, args...),
...
template <AlignmentFlag AF // Alignment flag
		  ...
		  ,
		  typename... RSAs> // Runtime submatrix arguments
inline decltype(auto) submatrix(const VecTVecMultExpr<MT> &matrix, size_t row, size_t column, size_t m, size_t n,
								RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) submatrix(const MatMatMapExpr<MT> &matrix, RSAs... args)
{
	...
	return map(submatrix<AF, CSAs...>((*matrix).leftOperand(), args...),
			   submatrix<AF, CSAs...>((*matrix).rightOperand(), args...), (*matrix).operation());
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) submatrix(const VecTVecMapExpr<MT> &matrix, RSAs... args)
{
	...
	try {
		return map(subvector<AF, I, M>((*matrix).leftOperand(), args...),
				   subvector<AF, J, N>((*matrix).rightOperand(), args...), (*matrix).operation());
	} catch (...) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid submatrix specification");
	}
}
...
template <AlignmentFlag AF // Alignment flag
		  ...
		  ,
		  typename... RSAs> // Runtime submatrix arguments
inline decltype(auto) submatrix(const VecTVecMapExpr<MT> &matrix, size_t row, size_t column, size_t m, size_t n,
								RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
	...
	try {
		return map(subvector<AF>((*matrix).leftOperand(), row, m, args...),
				   subvector<AF>((*matrix).rightOperand(), column, n, args...), (*matrix).operation());
	} catch (...) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid submatrix specification");
	}
}
...
template <AlignmentFlag AF // Alignment flag
		  ...
		  ,
		  typename... RSAs> // Optional submatrix arguments
inline decltype(auto) submatrix(const MatTransExpr<MT> &matrix, size_t row, size_t column, size_t m, size_t n,
								RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) submatrix(const VecExpandExpr<MT, CEAs...> &matrix, RSAs... args)
{
	...
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(I + M <= (*matrix).rows(), "Invalid submatrix specification");
		METRIC_NUMERIC_USER_ASSERT(J + N <= (*matrix).columns(), "Invalid submatrix specification");
	}
...
template <AlignmentFlag AF // Alignment flag
		  ...
		  ,
		  typename... RSAs> // Optional submatrix arguments
inline decltype(auto) submatrix(const VecExpandExpr<MT, CEAs...> &matrix, size_t row, size_t column, size_t m, size_t n,
								RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
	...
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(row + m <= (*matrix).rows(), "Invalid submatrix specification");
		METRIC_NUMERIC_USER_ASSERT(column + n <= (*matrix).columns(), "Invalid submatrix specification");
	}
...
inline decltype(auto) submatrix(const MatRepeatExpr<MT, CRAs...> &matrix, RSAs... args)
{
	...
	return columns(
		rows((*matrix).operand(), [M2](size_t i) { return (i + I) % M2; }, M, args...),
...
template <AlignmentFlag AF // Alignment flag
		  ...
		  ,
		  typename... RSAs> // Optional submatrix arguments
inline decltype(auto) submatrix(const MatRepeatExpr<MT, CRAs...> &matrix, size_t row, size_t column, size_t m, size_t n,
								RSAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
	...
	return columns(
		rows((*matrix).operand(), [row, M](size_t i) { return (i + row) % M; }, m, args...),
...
template <AlignmentFlag AF // Required alignment flag
		  ...
		  typename... RSAs // Optional submatrix arguments
		  ,
		  EnableIf_t<IsSubmatrix_v<RemoveReference_t<MT>> && RemoveReference_t<MT>::compileTimeArgs> * = nullptr>
inline decltype(auto) submatrix(MT &&sm, RSAs... args)
{
	...
	METRIC_NUMERIC_STATIC_ASSERT_MSG(I + M <= M2, "Invalid submatrix specification");
	METRIC_NUMERIC_STATIC_ASSERT_MSG(J + N <= N2, "Invalid submatrix specification");
...
inline decltype(auto) submatrix(MT &&sm, RSAs... args)
{
	...
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(I + M <= sm.rows(), "Invalid submatrix specification");
		METRIC_NUMERIC_USER_ASSERT(J + N <= sm.columns(), "Invalid submatrix specification");
	}
...
inline decltype(auto) submatrix(MT &&sm, size_t row, size_t column, size_t m, size_t n, RSAs... args)
{
	...
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(row + m <= sm.rows(), "Invalid submatrix specification");
		METRIC_NUMERIC_USER_ASSERT(column + n <= sm.columns(), "Invalid submatrix specification");
	}
...
inline decltype(auto) subvector(const MatVecMultExpr<VT> &vector, RSAs... args)
{
	...
	const size_t column((IsUpper_v<MT>) ? ((!AF && IsStrictlyUpper_v<MT>) ? (sd.offset() + 1UL) : (sd.offset()))
										: (0UL));
	const size_t n((IsLower_v<MT>) ? ((IsUpper_v<MT>) ? (sd.size())
													  : ((IsStrictlyLower_v<MT> && sd.size() > 0UL)
															 ? (sd.offset() + sd.size() - 1UL)
															 : (sd.offset() + sd.size())))
								   : ((IsUpper_v<MT>) ? (left.columns() - column) : (left.columns())));

	constexpr auto check(getCheck(args...));

	try {
		return submatrix<AF>(left, sd.offset(), column, sd.size(), n, check) * subvector<AF>(right, column, n, check);
	} catch (...) {
...
inline decltype(auto) subvector(const TVecMatMultExpr<VT> &vector, RSAs... args)
{
	...
	const size_t row((IsLower_v<MT>) ? ((!AF && IsStrictlyLower_v<MT>) ? (sd.offset() + 1UL) : (sd.offset())) : (0UL));
	const size_t m((IsUpper_v<MT>) ? ((IsLower_v<MT>) ? (sd.size())
													  : ((IsStrictlyUpper_v<MT> && sd.size() > 0UL)
															 ? (sd.offset() + sd.size() - 1UL)
															 : (sd.offset() + sd.size())))
								   : ((IsLower_v<MT>) ? (right.rows() - row) : (right.rows())));

	constexpr auto check(getCheck(args...));

	try {
		return subvector<AF>(left, row, m, check) * submatrix<AF>(right, row, sd.offset(), m, sd.size(), check);
	} catch (...) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid subvector specification");
	}
}
...
inline decltype(auto) row(MT &&sm, RRAs... args)
{
	...
	METRIC_NUMERIC_STATIC_ASSERT_MSG(I < M, "Invalid row access index");
...
inline decltype(auto) row(MT &&sm, size_t index, RRAs... args)
{
	...
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(index < M, "Invalid row access index");
	}
...
inline decltype(auto) row(MT &&sm, RRAs... args)
{
	...
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(rd.row() < sm.rows(), "Invalid row access index");
	}
...
inline decltype(auto) rows(MT &&sm, RRAs... args)
{
	...
	return submatrix(rows(sm.operand(), {I + sm.row(), Is + sm.row()...}, unchecked), 0UL, sm.column(),
					 sizeof...(Is) + 1UL, sm.columns(), unchecked);
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) rows(MT &&sm, T *indices, size_t n, RRAs... args)
{
	...
	std::for_each(newIndices.begin(), newIndices.end(), [row = sm.row()](size_t &index) { index += row; });

	return submatrix(rows(sm.operand(), newIndices.data(), n, unchecked), 0UL, sm.column(), n, sm.columns(), unchecked);
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) rows(MT &&sm, P p, size_t n, RRAs... args)
{
	...
	return submatrix(rows(
						 sm.operand(), [p, offset = sm.row()](size_t i) { return p(i) + offset; }, n, unchecked),
...
inline decltype(auto) column(MT &&sm, RCAs... args)
{
	...
	METRIC_NUMERIC_STATIC_ASSERT_MSG(I < N, "Invalid column access index");
...
inline decltype(auto) column(MT &&sm, size_t index, RCAs... args)
{
	...
	if (isChecked(args...)) {
		if ((index >= N)) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid column access index");
		}
	} else {
		METRIC_NUMERIC_USER_ASSERT(index < N, "Invalid column access index");
	}
...
inline decltype(auto) column(MT &&sm, RCAs... args)
{
	...
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(cd.column() < sm.columns(), "Invalid column access index");
	}
...
inline decltype(auto) columns(MT &&sm, RCAs... args)
{
	...
	return submatrix(columns(sm.operand(), {I + sm.column(), Is + sm.column()...}, unchecked), sm.row(), 0UL, sm.rows(),
					 sizeof...(Is) + 1UL, unchecked);
}
/*! \endcond */
//*************************************************************************************************
	...
	if (isChecked(args...)) {
		for (size_t j = 0UL; j < n; ++j) {
			if (sm.columns() <= size_t(indices[j])) {
				METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid column specification");
			}
		}
	}

	SmallArray<size_t, 128UL> newIndices(indices, indices + n);
	std::for_each(newIndices.begin(), newIndices.end(), [column = sm.column()](size_t &index) { index += column; });

	return submatrix(columns(sm.operand(), newIndices.data(), n, unchecked), sm.row(), 0UL, sm.rows(), n, unchecked);
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) columns(MT &&sm, P p, size_t n, RCAs... args)
{
	...
	return submatrix(columns(
						 sm.operand(), [p, offset = sm.column()](size_t i) { return p(i) + offset; }, n, unchecked),
...
{
	return (sm.row() + sm.rows() <= sm.operand().rows() && sm.column() + sm.columns() <= sm.operand().columns() &&
			isIntact(sm.operand()));
}
/*! \endcond */
//*************************************************************************************************
...
// following code example demonstrates the use of the function:
   ...
   \code
   mtrc::numeric::DynamicMatrix<int,mtrc::numeric::rowMajor> A( 32UL, 16UL );
   // ... Initialization
...
{
	return (isSame(a.operand(), *b) && (a.rows() == (*b).rows()) && (a.columns() == (*b).columns()));
}
/*! \endcond */
//*************************************************************************************************
...
{
	return (isSame(*a, b.operand()) && ((*a).rows() == b.rows()) && ((*a).columns() == b.columns()));
}
/*! \endcond */
//*************************************************************************************************
...
{
	return (isSame(a.operand(), b.operand()) && (a.row() == b.row()) && (a.column() == b.column()) &&
...
template <InversionFlag IF // Inversion algorithm
		  ...
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline auto invert(Submatrix<MT, AF, SO, true, CSAs...> &sm) -> DisableIf_t<HasMutableDataAccess_v<MT>>
{
	using RT = ResultType_t<Submatrix<MT, AF, SO, true, CSAs...>>;

	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(RT);
	METRIC_NUMERIC_CONSTRAINT_MUST_HAVE_MUTABLE_DATA_ACCESS(RT);
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(i < sm.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(j < sm.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySet(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
										 size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(row + m <= (*sm).rows(), "Invalid number of rows");
	METRIC_NUMERIC_INTERNAL_ASSERT(column + n <= (*sm).columns(), "Invalid number of columns");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryAdd(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
										 size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySub(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
										 size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryMult(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
										  size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryDiv(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
										 size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE bool tryShift(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
										   size_t m, size_t n, int count)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(row + m <= (*sm).rows(), "Invalid number of rows");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitand(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
											size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitor(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
										   size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitxor(const Submatrix<MT, AF, SO, DF, CSAs...> &sm, size_t row, size_t column,
											size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*sm).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*sm).columns(), "Invalid column access index");
...
inline bool tryAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
					  size_t column)
{
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
					  size_t row, size_t column)
{
	...
	return tryAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
					 lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
inline bool tryAssign(const Submatrix<MT1, AF, SO1, DF, CSAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
					  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryAddAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryAddAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						 size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	return tryAddAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
						lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryAddAssign(const Submatrix<MT1, AF, SO1, DF, CSAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool trySubAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool trySubAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						 size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	return trySubAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
						lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool trySubAssign(const Submatrix<MT1, AF, SO1, DF, CSAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryMultAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
						  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryMultAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						  size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	return tryMultAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
						 lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool trySchurAssign(const Submatrix<MT1, AF, SO1, DF, CSAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryDivAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryDivAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						 size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	return tryDivAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
						lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryShiftAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryShiftAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						   size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	return tryShiftAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
						  lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryShiftAssign(const Submatrix<MT1, AF, SO1, DF, CSAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitandAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitandAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
							size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	return tryBitandAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
						   lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitandAssign(const Submatrix<MT1, AF, SO1, DF, CSAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitorAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitorAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						   size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	return tryBitorAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
						  lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitorAssign(const Submatrix<MT1, AF, SO1, DF, CSAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitxorAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitxorAssign(const Submatrix<MT, AF, SO, DF, CSAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
							size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	return tryBitxorAssign(lhs.operand(), *rhs, band + ptrdiff_t(lhs.column() - lhs.row()), lhs.row() + row,
						   lhs.column() + column);
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitxorAssign(const Submatrix<MT1, AF, SO1, DF, CSAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
struct Size<Submatrix<MT, AF, SO, DF, I, J, M, N>, 0UL> : public Ptrdiff_t<static_cast<ptrdiff_t>(M)> {};

template <typename MT, AlignmentFlag AF, bool SO, bool DF, size_t I, size_t J, size_t M, size_t N>
struct Size<Submatrix<MT, AF, SO, DF, I, J, M, N>, 1UL> : public Ptrdiff_t<static_cast<ptrdiff_t>(N)> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, AlignmentFlag AF, bool SO, bool DF, size_t I, size_t J, size_t M, size_t N>
struct MaxSize<Submatrix<MT, AF, SO, DF, I, J, M, N>, 0UL> : public Ptrdiff_t<static_cast<ptrdiff_t>(M)> {};

template <typename MT, AlignmentFlag AF, bool SO, bool DF, size_t I, size_t J, size_t M, size_t N>
struct MaxSize<Submatrix<MT, AF, SO, DF, I, J, M, N>, 1UL> : public Ptrdiff_t<static_cast<ptrdiff_t>(N)> {};
/*! \endcond */
//*************************************************************************************************
...
struct IsRestricted<Submatrix<MT, AF, SO, DF, CSAs...>> : public IsRestricted<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct HasConstDataAccess<Submatrix<MT, AF, SO, true, CSAs...>> : public HasConstDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, AlignmentFlag AF, bool SO, size_t... CSAs>
struct HasMutableDataAccess<Submatrix<MT, AF, SO, true, CSAs...>> : public HasMutableDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct IsAligned<Submatrix<MT, aligned, SO, true, CSAs...>> : public TrueType {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, AlignmentFlag AF, bool SO, size_t... CSAs>
struct IsContiguous<Submatrix<MT, AF, SO, true, CSAs...>> : public IsContiguous<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct IsLower<Submatrix<MT, AF, SO, DF, I, J, M, N>>
	: public BoolConstant<(IsLower_v<MT> && I == J && M == N) || (IsStrictlyLower_v<MT> && I == J + 1UL && M == N)> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, AlignmentFlag AF, bool SO, bool DF, size_t I, size_t J, size_t M, size_t N>
struct IsUniLower<Submatrix<MT, AF, SO, DF, I, J, M, N>> : public BoolConstant<(IsUniLower_v<MT> && I == J && M == N)> {
};
/*! \endcond */
//*************************************************************************************************
...
struct IsStrictlyLower<Submatrix<MT, AF, SO, DF, I, J, M, N>>
	: public BoolConstant<(IsLower_v<MT> && I < J && M == N) || (IsStrictlyLower_v<MT> && I == J && M == N)> {};
/*! \endcond */
//*************************************************************************************************
...
struct IsUpper<Submatrix<MT, AF, SO, DF, I, J, M, N>>
	: public BoolConstant<(IsUpper_v<MT> && I == J && M == N) || (IsStrictlyUpper_v<MT> && I + 1UL == J && M == N)> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, AlignmentFlag AF, bool SO, bool DF, size_t I, size_t J, size_t M, size_t N>
struct IsUniUpper<Submatrix<MT, AF, SO, DF, I, J, M, N>> : public BoolConstant<(IsUniUpper_v<MT> && I == J && M == N)> {
};
/*! \endcond */
//*************************************************************************************************
...
struct IsStrictlyUpper<Submatrix<MT, AF, SO, DF, I, J, M, N>>
	: public BoolConstant<(IsUpper_v<MT> && I > J && M == N) || (IsStrictlyUpper_v<MT> && I == J && M == N)> {};
/*! \endcond */
//*************************************************************************************************
