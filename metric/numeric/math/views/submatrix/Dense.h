...
#ifndef METRIC_NUMERIC_MATH_VIEWS_SUBMATRIX_DENSE_H
#define METRIC_NUMERIC_MATH_VIEWS_SUBMATRIX_DENSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/dense/InitializerMatrix.h>
...
#include <metric/numeric/math/views/submatrix/SubmatrixData.h>
...
class Submatrix<MT, unaligned, false, true, CSAs...>
	: public View<DenseMatrix<Submatrix<MT, unaligned, false, true, CSAs...>, false>>, private SubmatrixData<CSAs...> {
  private:
	//**Type definitions****************************************************************************
	...
	static constexpr bool EnforceEvaluation_v = (IsRestricted_v<MT1> && RequiresEvaluation_v<MT2>);
	//**********************************************************************************************
  ...
  public:
	...
	class SubmatrixIterator {
	  public:
		...
		*/
		inline const SubmatrixIterator operator++(int) { return SubmatrixIterator(iterator_++, isAligned_); }
		//*******************************************************************************************
		...
		*/
		inline ReferenceType operator*() const { return *iterator_; }
		//*******************************************************************************************
		...
		*/
		inline IteratorType operator->() const { return iterator_; }
		//*******************************************************************************************
		...
		*/
		inline SIMDType loada() const noexcept { return iterator_.loada(); }
		//*******************************************************************************************
		...
		*/
		inline SIMDType loadu() const noexcept { return iterator_.loadu(); }
		//*******************************************************************************************
		...
		*/
		...
		// might result in erroneous results and/or in compilation errors.
		*/
		inline void storea(const SIMDType &value) const { iterator_.storea(value); }
		//*******************************************************************************************
		...
		// might result in erroneous results and/or in compilation errors.
		*/
		inline void storeu(const SIMDType &value) const { iterator_.storeu(value); }
		//*******************************************************************************************
		...
		// explicitly might result in erroneous results and/or in compilation errors.
		*/
		inline void stream(const SIMDType &value) const { iterator_.stream(value); }
		//*******************************************************************************************
		...
		*/
		inline bool operator==(const SubmatrixIterator &rhs) const { return iterator_ == rhs.iterator_; }
		//*******************************************************************************************
		...
		*/
		inline DifferenceType operator-(const SubmatrixIterator &rhs) const { return iterator_ - rhs.iterator_; }
		//*******************************************************************************************
		...
		*/
		inline IteratorType base() const { return iterator_; }
		//*******************************************************************************************
		...
		*/
		inline bool isAligned() const noexcept { return isAligned_; }
		//*******************************************************************************************
	...
	/*!\name Constructors */
	//@{
	template <typename... RSAs> explicit inline Submatrix(MT &matrix, RSAs... args);
	...
	template <typename MT2, bool SO2> inline Submatrix &operator=(const Matrix<MT2, SO2> &rhs);

	template <typename MT2, bool SO2>
	inline auto operator+=(const Matrix<MT2, SO2> &rhs) -> DisableIf_t<EnforceEvaluation_v<MT, MT2>, Submatrix &>;

	template <typename MT2, bool SO2>
	inline auto operator+=(const Matrix<MT2, SO2> &rhs) -> EnableIf_t<EnforceEvaluation_v<MT, MT2>, Submatrix &>;
  ...
  private:
	...
	static constexpr bool VectorizedAssign_v =
		(useOptimizedKernels && simdEnabled && MT2::simdEnabled && IsSIMDCombinable_v<ElementType, ElementType_t<MT2>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedAddAssign_v =
		(VectorizedAssign_v<MT2> && HasSIMDAdd_v<ElementType, ElementType_t<MT2>> && !IsDiagonal_v<MT2>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedSubAssign_v =
		(VectorizedAssign_v<MT2> && HasSIMDSub_v<ElementType, ElementType_t<MT2>> && !IsDiagonal_v<MT2>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedSchurAssign_v =
		(VectorizedAssign_v<MT2> && HasSIMDMult_v<ElementType, ElementType_t<MT2>>);
	//**********************************************************************************************
  ...
  private:
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_TRANSEXPR_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_SUBMATRIX_TYPE(MT);
...
// the optional \a mtrc::numeric::unchecked argument.
*/
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs>	// Compile time submatrix arguments
...
inline Submatrix<MT, unaligned, false, true, CSAs...>::Submatrix(MT &matrix, RSAs... args)
	: DataType(args...) // Base class initialization
	  ...
	  ,
	  isAligned_(simdEnabled && IsContiguous_v<MT> && matrix.data() != nullptr && checkAlignment(data()) &&
				 (rows() < 2UL || (matrix.spacing() % SIMDSIZE) == 0UL))
{
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(row() + rows() <= matrix_.rows(), "Invalid submatrix specification");
		METRIC_NUMERIC_USER_ASSERT(column() + columns() <= matrix_.columns(), "Invalid submatrix specification");
	}
}
/*! \endcond */
//*************************************************************************************************
...
{
	METRIC_NUMERIC_USER_ASSERT(i < rows(), "Invalid dense submatrix row access index");
...
Submatrix<MT, unaligned, false, true, CSAs...>::operator=(const ElementType &rhs)
{
	...
	for (size_t i = row(); i < iend; ++i) {
		const size_t jbegin((IsUpper_v<MT>) ? ((IsUniUpper_v<MT> || IsStrictlyUpper_v<MT>) ? (max(i + 1UL, column()))
																						   : (max(i, column())))
											: (column()));
		const size_t jend((IsLower_v<MT>)
							  ? ((IsUniLower_v<MT> || IsStrictlyLower_v<MT>) ? (min(i, column() + columns()))
...
{
	if (list.size() != rows()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to submatrix");
	}

	if (IsRestricted_v<MT>) {
		const InitializerMatrix<ElementType> tmp(list, columns());
		if (!tryAssign(matrix_, tmp, row(), column())) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to restricted matrix");
		}
	}
...
Submatrix<MT, unaligned, false, true, CSAs...>::operator=(const Submatrix &rhs)
{
	...
	if (rows() != rhs.rows() || columns() != rhs.columns()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Submatrix sizes do not match");
	}
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(isIntact(matrix_), "Invariant violation detected");
...
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(ResultType_t<MT2>);
	...
	-> DisableIf_t<EnforceEvaluation_v<MT, MT2>, Submatrix &>
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(AddType);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(AddType);
	...
	-> DisableIf_t<EnforceEvaluation_v<MT, MT2>, Submatrix &>
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(SubType);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(SubType);
	...
	-> DisableIf_t<EnforceEvaluation_v<MT, MT2>, Submatrix &>
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(SchurType);
...
inline void Submatrix<MT, unaligned, false, true, CSAs...>::reset(size_t i)
{
	...
	const size_t jbegin(
		(IsUpper_v<MT>) ? ((IsUniUpper_v<MT> || IsStrictlyUpper_v<MT>) ? (max(i + 1UL, column())) : (max(i, column())))
						: (column()));
	const size_t jend((IsLower_v<MT>)
						  ? ((IsUniLower_v<MT> || IsStrictlyLower_v<MT>) ? (min(i, column() + columns()))
																		 : (min(i + 1UL, column() + columns())))
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(IsSymmetric_v<MT> || IsHermitian_v<MT>, "Invalid matrix detected");
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline Submatrix<MT, unaligned, false, true, CSAs...> &Submatrix<MT, unaligned, false, true, CSAs...>::transpose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic submatrix");
	}

	if (!tryAssign(matrix_, trans(*this), row(), column())) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose operation");
	}
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline Submatrix<MT, unaligned, false, true, CSAs...> &Submatrix<MT, unaligned, false, true, CSAs...>::ctranspose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic submatrix");
	}
...
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_UNITRIANGULAR_MATRIX_TYPE(MT);

	const size_t iend(row() + rows());

	for (size_t i = row(); i < iend; ++i) {
		const size_t jbegin((IsUpper_v<MT>) ? ((IsStrictlyUpper_v<MT>) ? (max(i + 1UL, column())) : (max(i, column())))
											: (column()));
		const size_t jend((IsLower_v<MT>) ? ((IsStrictlyLower_v<MT>) ? (min(i, column() + columns()))
																	 : (min(i + 1UL, column() + columns())))
										  : (column() + columns()));
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
template <typename MT2	  // Data type of the other dense submatrix
		  ,
		  AlignmentFlag AF2 // Alignment flag of the other dense submatrix
		  ,
		  bool SO2 // Storage order of the other dense submatrix
		  ,
		  size_t... CSAs2> // Compile time submatrix arguments of the other dense submatrix
inline bool Submatrix<MT, unaligned, false, true, CSAs...>::canAlias(
	const Submatrix<MT2, AF2, SO2, true, CSAs2...> *alias) const noexcept
{
	return (matrix_.isAliased(&alias->matrix_) && (row() + rows() > alias->row()) &&
			(row() < alias->row() + alias->rows()) && (column() + columns() > alias->column()) &&
...
template <typename MT2	  // Data type of the other dense submatrix
		  ...
		  ,
		  size_t... CSAs2> // Compile time submatrix arguments of the other dense submatrix
inline bool Submatrix<MT, unaligned, false, true, CSAs...>::isAliased(
	const Submatrix<MT2, AF2, SO2, true, CSAs2...> *alias) const noexcept
{
	return (matrix_.isAliased(&alias->matrix_) && (row() + rows() > alias->row()) &&
			(row() < alias->row() + alias->rows()) && (column() + columns() > alias->column()) &&
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE typename Submatrix<MT, unaligned, false, true, CSAs...>::SIMDType
...
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(j + SIMDSIZE <= columns(), "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(j % SIMDSIZE == 0UL, "Invalid column access index");
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, unaligned, false, true, CSAs...>::store(size_t i, size_t j,
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, unaligned, false, true, CSAs...>::storea(size_t i, size_t j,
																						 const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, unaligned, false, true, CSAs...>::storeu(size_t i, size_t j,
																						 const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, unaligned, false, true, CSAs...>::stream(size_t i, size_t j,
																						 const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(rows() == (*rhs).rows(), "Invalid number of rows");
	METRIC_NUMERIC_INTERNAL_ASSERT(columns() == (*rhs).columns(), "Invalid number of columns");

	const size_t jpos(prevMultiple(columns(), 2UL));
	METRIC_NUMERIC_INTERNAL_ASSERT(jpos <= columns(), "Invalid end calculation");
	...
	-> EnableIf_t<VectorizedAssign_v<MT2>>
{
	...
	if (useStreaming && isAligned_ && rows() * columns() > (cacheSize / (sizeof(ElementType) * 3UL)) &&
		!(*rhs).isAliased(this)) {
		for (size_t i = 0UL; i < rows(); ++i) {
			size_t j(0UL);
	...
	-> EnableIf_t<VectorizedAddAssign_v<MT2>>
{
	...
	for (size_t i = 0UL; i < rows(); ++i) {
		const size_t jbegin((IsUpper_v<MT2>) ? (prevMultiple((IsStrictlyUpper_v<MT2> ? i + 1UL : i), SIMDSIZE))
											 : (0UL));
		const size_t jend((IsLower_v<MT2>) ? (IsStrictlyLower_v<MT2> ? i : i + 1UL) : (columns()));
...
class Submatrix<MT, unaligned, true, true, CSAs...>
	: public View<DenseMatrix<Submatrix<MT, unaligned, true, true, CSAs...>, true>>, private SubmatrixData<CSAs...> {
  private:
	//**Type definitions****************************************************************************
	using DataType = SubmatrixData<CSAs...>;			//!< The type of the SubmatrixData base class.
	using Operand = If_t<IsExpression_v<MT>, MT, MT &>; //!< Composite data type of the matrix expression.
...
{
	METRIC_NUMERIC_USER_ASSERT(j < columns(), "Invalid dense submatrix column access index");
...
Submatrix<MT, unaligned, true, true, CSAs...>::operator=(const ElementType &rhs)
{
	...
	for (size_t j = column(); j < jend; ++j) {
		const size_t ibegin(
			(IsLower_v<MT>) ? ((IsUniLower_v<MT> || IsStrictlyLower_v<MT>) ? (max(j + 1UL, row())) : (max(j, row())))
							: (row()));
		const size_t iend((IsUpper_v<MT>)
							  ? ((IsUniUpper_v<MT> || IsStrictlyUpper_v<MT>) ? (min(j, row() + rows()))
																			 : (min(j + 1UL, row() + rows())))
							  : (row() + rows()));
...
inline void Submatrix<MT, unaligned, true, true, CSAs...>::reset(size_t j)
{
	...
	const size_t ibegin((IsLower_v<MT>)
							? ((IsUniLower_v<MT> || IsStrictlyLower_v<MT>) ? (max(j + 1UL, row())) : (max(j, row())))
							: (row()));
	const size_t iend((IsUpper_v<MT>) ? ((IsUniUpper_v<MT> || IsStrictlyUpper_v<MT>) ? (min(j, row() + rows()))
																					 : (min(j + 1UL, row() + rows())))
									  : (row() + rows()));
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline Submatrix<MT, unaligned, true, true, CSAs...> &Submatrix<MT, unaligned, true, true, CSAs...>::transpose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic submatrix");
	}
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline Submatrix<MT, unaligned, true, true, CSAs...> &Submatrix<MT, unaligned, true, true, CSAs...>::ctranspose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic submatrix");
	}
...
Submatrix<MT, unaligned, true, true, CSAs...>::scale(const Other &scalar)
{
	...
	for (size_t j = column(); j < jend; ++j) {
		const size_t ibegin((IsLower_v<MT>) ? ((IsStrictlyLower_v<MT>) ? (max(j + 1UL, row())) : (max(j, row())))
											: (row()));
		const size_t iend((IsUpper_v<MT>)
							  ? ((IsStrictlyUpper_v<MT>) ? (min(j, row() + rows())) : (min(j + 1UL, row() + rows())))
							  : (row() + rows()));
...
template <typename MT2	  // Data type of the other dense submatrix
		  ...
		  ,
		  size_t... CSAs2> // Compile time submatrix arguments of the other dense submatrix
inline bool Submatrix<MT, unaligned, true, true, CSAs...>::canAlias(
	const Submatrix<MT2, AF2, SO2, true, CSAs2...> *alias) const noexcept
{
	return (matrix_.isAliased(&alias->matrix_) && (row() + rows() > alias->row()) &&
			(row() < alias->row() + alias->rows()) && (column() + columns() > alias->column()) &&
...
template <typename MT2	  // Data type of the other dense submatrix
		  ...
		  ,
		  size_t... CSAs2> // Compile time submatrix arguments of the other dense submatrix
inline bool Submatrix<MT, unaligned, true, true, CSAs...>::isAliased(
	const Submatrix<MT2, AF2, SO2, true, CSAs2...> *alias) const noexcept
{
	return (matrix_.isAliased(&alias->matrix_) && (row() + rows() > alias->row()) &&
			(row() < alias->row() + alias->rows()) && (column() + columns() > alias->column()) &&
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE typename Submatrix<MT, unaligned, true, true, CSAs...>::SIMDType
...
Submatrix<MT, unaligned, true, true, CSAs...>::loada(size_t i, size_t j) const noexcept
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(i < rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(i + SIMDSIZE <= rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(i % SIMDSIZE == 0UL, "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(j < columns(), "Invalid column access index");
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, unaligned, true, true, CSAs...>::store(size_t i, size_t j,
																					   const SIMDType &value) noexcept
{
	if (isAligned_)
		storea(i, j, value);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, unaligned, true, true, CSAs...>::storea(size_t i, size_t j,
																						const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, unaligned, true, true, CSAs...>::storeu(size_t i, size_t j,
																						const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, unaligned, true, true, CSAs...>::stream(size_t i, size_t j,
																						const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
	...
	-> DisableIf_t<VectorizedAssign_v<MT2>>
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(ipos <= rows(), "Invalid end calculation");
	...
	-> EnableIf_t<VectorizedAddAssign_v<MT2>>
{
	...
	for (size_t j = 0UL; j < columns(); ++j) {
		...
		const size_t iend((IsUpper_v<MT>) ? (IsStrictlyUpper_v<MT> ? j : j + 1UL) : (rows()));
...
class Submatrix<MT, aligned, false, true, CSAs...>
	: public View<DenseMatrix<Submatrix<MT, aligned, false, true, CSAs...>, false>>, private SubmatrixData<CSAs...> {
  private:
	//**Type definitions****************************************************************************
	using DataType = SubmatrixData<CSAs...>;			//!< The type of the SubmatrixData base class.
	using Operand = If_t<IsExpression_v<MT>, MT, MT &>; //!< Composite data type of the matrix expression.
	  ...
	  matrix_(matrix) // The matrix containing the submatrix
{
	if (isChecked(args...)) {
		...
		if (simdEnabled && IsContiguous_v<MT> &&
			(!checkAlignment(data()) || (rows() > 1UL && matrix_.spacing() % SIMDSIZE != 0UL))) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid submatrix alignment");
		}
	} else {
		METRIC_NUMERIC_USER_ASSERT(row() + rows() <= matrix_.rows(), "Invalid submatrix specification");
		METRIC_NUMERIC_USER_ASSERT(column() + columns() <= matrix_.columns(), "Invalid submatrix specification");

		METRIC_NUMERIC_USER_ASSERT(!simdEnabled || !IsContiguous_v<MT> || checkAlignment(data()),
								   "Invalid submatrix alignment");
		METRIC_NUMERIC_USER_ASSERT(!simdEnabled || !IsContiguous_v<MT> || rows() <= 1UL ||
									   matrix_.spacing() % SIMDSIZE == 0UL,
								   "Invalid submatrix alignment");
	}
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline Submatrix<MT, aligned, false, true, CSAs...> &Submatrix<MT, aligned, false, true, CSAs...>::transpose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic submatrix");
	}
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline Submatrix<MT, aligned, false, true, CSAs...> &Submatrix<MT, aligned, false, true, CSAs...>::ctranspose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic submatrix");
	}
...
template <typename MT2	  // Data type of the other dense submatrix
		  ...
		  ,
		  size_t... CSAs2> // Compile time submatrix arguments of the other dense submatrix
inline bool Submatrix<MT, aligned, false, true, CSAs...>::canAlias(
	const Submatrix<MT2, AF2, SO2, true, CSAs2...> *alias) const noexcept
{
	return (matrix_.isAliased(&alias->matrix_) && (row() + rows() > alias->row()) &&
			(row() < alias->row() + alias->rows()) && (column() + columns() > alias->column()) &&
...
template <typename MT2	  // Data type of the other dense submatrix
		  ...
		  ,
		  size_t... CSAs2> // Compile time submatrix arguments of the other dense submatrix
inline bool Submatrix<MT, aligned, false, true, CSAs...>::isAliased(
	const Submatrix<MT2, AF2, SO2, true, CSAs2...> *alias) const noexcept
{
	return (matrix_.isAliased(&alias->matrix_) && (row() + rows() > alias->row()) &&
			(row() < alias->row() + alias->rows()) && (column() + columns() > alias->column()) &&
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE typename Submatrix<MT, aligned, false, true, CSAs...>::SIMDType
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, aligned, false, true, CSAs...>::store(size_t i, size_t j,
																					  const SIMDType &value) noexcept
{
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, aligned, false, true, CSAs...>::storea(size_t i, size_t j,
																					   const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, aligned, false, true, CSAs...>::storeu(size_t i, size_t j,
																					   const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, aligned, false, true, CSAs...>::stream(size_t i, size_t j,
																					   const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
	...
	-> EnableIf_t<VectorizedAssign_v<MT2>>
{
	...
	if (useStreaming && rows() * columns() > (cacheSize / (sizeof(ElementType) * 3UL)) && !(*rhs).isAliased(this)) {
		for (size_t i = 0UL; i < rows(); ++i) {
			size_t j(0UL);
			Iterator left(begin(i));
			ConstIterator_t<MT2> right((*rhs).begin(i));
...
class Submatrix<MT, aligned, true, true, CSAs...>
	: public View<DenseMatrix<Submatrix<MT, aligned, true, true, CSAs...>, true>>, private SubmatrixData<CSAs...> {
  private:
	//**Type definitions****************************************************************************
	using DataType = SubmatrixData<CSAs...>;			//!< The type of the SubmatrixData base class.
	using Operand = If_t<IsExpression_v<MT>, MT, MT &>; //!< Composite data type of the matrix expression.
	  ...
	  matrix_(matrix) // The matrix containing the submatrix
{
	if (isChecked(args...)) {
		...
		if (simdEnabled && IsContiguous_v<MT> &&
			(!checkAlignment(data()) || (columns() > 1UL && matrix_.spacing() % SIMDSIZE != 0UL))) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid submatrix alignment");
		}
	} else {
		METRIC_NUMERIC_USER_ASSERT(row() + rows() <= matrix_.rows(), "Invalid submatrix specification");
		...
		METRIC_NUMERIC_USER_ASSERT(!simdEnabled || !IsContiguous_v<MT> || checkAlignment(data()),
								   "Invalid submatrix alignment");
		METRIC_NUMERIC_USER_ASSERT(!simdEnabled || !IsContiguous_v<MT> || columns() <= 1UL ||
									   matrix_.spacing() % SIMDSIZE == 0UL,
								   "Invalid submatrix alignment");
	}
}
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline Submatrix<MT, aligned, true, true, CSAs...> &Submatrix<MT, aligned, true, true, CSAs...>::transpose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic submatrix");
	}
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
inline Submatrix<MT, aligned, true, true, CSAs...> &Submatrix<MT, aligned, true, true, CSAs...>::ctranspose()
{
	if (rows() != columns()) {
		METRIC_NUMERIC_THROW_LOGIC_ERROR("Invalid transpose of a non-quadratic submatrix");
	}
...
template <typename MT2	  // Data type of the other dense submatrix
		  ...
		  ,
		  size_t... CSAs2> // Compile time submatrix arguments of the other dense submatrix
inline bool Submatrix<MT, aligned, true, true, CSAs...>::canAlias(
	const Submatrix<MT2, AF2, SO2, true, CSAs2...> *alias) const noexcept
{
	return (matrix_.isAliased(&alias->matrix_) && (row() + rows() > alias->row()) &&
			(row() < alias->row() + alias->rows()) && (column() + columns() > alias->column()) &&
...
template <typename MT2	  // Data type of the other dense submatrix
		  ...
		  ,
		  size_t... CSAs2> // Compile time submatrix arguments of the other dense submatrix
inline bool Submatrix<MT, aligned, true, true, CSAs...>::isAliased(
	const Submatrix<MT2, AF2, SO2, true, CSAs2...> *alias) const noexcept
{
	return (matrix_.isAliased(&alias->matrix_) && (row() + rows() > alias->row()) &&
			(row() < alias->row() + alias->rows()) && (column() + columns() > alias->column()) &&
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE typename Submatrix<MT, aligned, true, true, CSAs...>::SIMDType
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, aligned, true, true, CSAs...>::store(size_t i, size_t j,
																					 const SIMDType &value) noexcept
{
	storea(i, j, value);
}
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, aligned, true, true, CSAs...>::storea(size_t i, size_t j,
																					  const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, aligned, true, true, CSAs...>::storeu(size_t i, size_t j,
																					  const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CSAs> // Compile time submatrix arguments
METRIC_NUMERIC_ALWAYS_INLINE void Submatrix<MT, aligned, true, true, CSAs...>::stream(size_t i, size_t j,
																					  const SIMDType &value) noexcept
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTORIZABLE_TYPE(ElementType);
