...
#ifndef METRIC_NUMERIC_MATH_VIEWS_BAND_SPARSE_H
#define METRIC_NUMERIC_MATH_VIEWS_BAND_SPARSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/views/band/BandData.h>
...
class Band<MT, TF, false, false, CBAs...> : public View<SparseVector<Band<MT, TF, false, false, CBAs...>, TF>>,
  ...
  public:
	...
	class BandElement : private SparseElement {
	  public:
		//**Constructor******************************************************************************
		...
		*/
		inline const BandElement *operator->() const { return this; }
		//*******************************************************************************************
	...
	class BandIterator {
	  public:
		...
		*/
		...
		{
			METRIC_NUMERIC_INTERNAL_ASSERT(matrix.find(row_, column_) == pos, "Invalid initial iterator position");
		}
		//*******************************************************************************************
		...
		*/
		inline ReferenceType operator*() const { return ReferenceType(pos_, min(row_, column_)); }
		//*******************************************************************************************
		...
		// \return Reference to the sparse vector element at the current iterator position.
		*/
		inline PointerType operator->() const { return PointerType(pos_, min(row_, column_)); }
		//*******************************************************************************************
	...
	/*!\name Constructors */
	//@{
	template <typename... RBAs> explicit inline Band(MT &matrix, RBAs... args);
...
{
	if (isChecked(args...)) {
		if ((band() > 0L && column() >= matrix.columns()) || (band() < 0L && row() >= matrix.rows())) {
			METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid band access index");
		}
	} else {
		METRIC_NUMERIC_USER_ASSERT(band() <= 0L || column() < matrix.columns(), "Invalid band access index");
		METRIC_NUMERIC_USER_ASSERT(band() >= 0L || row() < matrix.rows(), "Invalid band access index");
	}
}
/*! \endcond */
//*************************************************************************************************
...
{
	METRIC_NUMERIC_USER_ASSERT(index < size(), "Invalid band access index");
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::Reference Band<MT, TF, false, false, CBAs...>::at(size_t index)
{
	if (index >= size()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid band access index");
	}
	return (*this)[index];
}
/*! \endcond */
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::Iterator Band<MT, TF, false, false, CBAs...>::begin()
{
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::ConstIterator Band<MT, TF, false, false, CBAs...>::begin() const
{
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::ConstIterator Band<MT, TF, false, false, CBAs...>::cbegin() const
{
	return ConstIterator(matrix_, row(), column());
}
/*! \endcond */
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::Iterator Band<MT, TF, false, false, CBAs...>::end()
{
	const size_t n(size());
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::ConstIterator Band<MT, TF, false, false, CBAs...>::end() const
{
	const size_t n(size());
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::ConstIterator Band<MT, TF, false, false, CBAs...>::cend() const
{
	const size_t n(size());
	return ConstIterator(matrix_, row() + n, column() + n);
}
...
Band<MT, TF, false, false, CBAs...>::operator=(initializer_list<ElementType> list)
{
	...
	if (list.size() > size()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to band");
	}
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline Band<MT, TF, false, false, CBAs...> &Band<MT, TF, false, false, CBAs...>::operator=(const Band &rhs)
{
	using mtrc::numeric::assign;

	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SPARSE_VECTOR_TYPE(ResultType);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_VECTOR_WITH_TRANSPOSE_FLAG(ResultType, TF);
...
inline Band<MT, TF, false, false, CBAs...> &Band<MT, TF, false, false, CBAs...>::operator=(const Vector<VT, TF> &rhs)
{
	using mtrc::numeric::assign;
...
inline void Band<MT, TF, false, false, CBAs...>::reset()
{
	...
	if ((IsLower_v<MT> && column() > 0UL) || ((IsUniLower_v<MT> || IsStrictlyLower_v<MT>) && row() == 0UL) ||
		(IsUpper_v<MT> && row() > 0UL) || ((IsUniUpper_v<MT> || IsStrictlyUpper_v<MT>) && column() == 0UL))
		return;
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::Iterator Band<MT, TF, false, false, CBAs...>::erase(Iterator pos)
{
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::Iterator Band<MT, TF, false, false, CBAs...>::erase(Iterator first,
																										 Iterator last)
{
...
template <typename MT // Type of the sparse matrix
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
inline typename Band<MT, TF, false, false, CBAs...>::Iterator Band<MT, TF, false, false, CBAs...>::find(size_t index)
{
	const size_t rowIndex(row() + index);
	const size_t columnIndex(column() + index);
...
template <typename MT // Type of the sparse matrix
		  ...
		  ptrdiff_t... CBAs> // Compile time band arguments
inline
	typename Band<MT, TF, false, false, CBAs...>::Iterator Band<MT, TF, false, false, CBAs...>::lowerBound(size_t index)
{
...
template <typename MT // Type of the sparse matrix
		  ...
		  ptrdiff_t... CBAs> // Compile time band arguments
inline
	typename Band<MT, TF, false, false, CBAs...>::Iterator Band<MT, TF, false, false, CBAs...>::upperBound(size_t index)
{
...
inline Band<MT, TF, false, false, CBAs...> &Band<MT, TF, false, false, CBAs...>::scale(const Other &scalar)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_UNITRIANGULAR_MATRIX_TYPE(MT);

	if ((IsLower_v<MT> && column() > 0UL) || (IsStrictlyLower_v<MT> && row() == 0UL) ||
		(IsUpper_v<MT> && row() > 0UL) || (IsStrictlyUpper_v<MT> && column() == 0UL))
		return *this;
...
template <typename MT // Type of the sparse matrix multiplication
		  ...
		  ,
		  ptrdiff_t... CBAs> // Compile time band arguments
class Band<MT, TF, false, true, CBAs...> : public View<SparseVector<Band<MT, TF, false, true, CBAs...>, TF>>,
										   private BandData<CBAs...>,
										   private Computation {
  private:
	//**Type definitions****************************************************************************
  ...
  public:
	...
	using LT = If_t<IsSparseMatrix_v<LeftOperand> && IsColumnMajorMatrix_v<LeftOperand>, OppositeType_t<LeftOperand>,
					CompositeType_t<LeftOperand>>;

	//! Type for the assignment of the right-hand side matrix operand.
	using RT = If_t<IsSparseMatrix_v<RightOperand> && IsRowMajorMatrix_v<RightOperand>, OppositeType_t<RightOperand>,
	...
	*/
	...
	{
		if (isChecked(args...)) {
			if ((band() > 0L && column() >= mmm.columns()) || (band() < 0L && row() >= mmm.rows())) {
				METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid band access index");
			}
		} else {
			METRIC_NUMERIC_USER_ASSERT(band() <= 0L || column() < mmm.columns(), "Invalid band access index");
			METRIC_NUMERIC_USER_ASSERT(band() >= 0L || row() < mmm.rows(), "Invalid band access index");
		}
	}
	//**********************************************************************************************
	...
	*/
	inline size_t size() const noexcept { return min(matrix_.rows() - row(), matrix_.columns() - column()); }
	//**********************************************************************************************
	...
	*/
	inline size_t nonZeros() const { return 0UL; }
	//**********************************************************************************************
	...
	*/
	inline const MT &operand() const noexcept { return matrix_; }
	//**********************************************************************************************
  ...
  private:
	...
	*/
	...
	//**Compile time checks*************************************************************************
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SPARSE_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_MATMATMULTEXPR_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_POINTER_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_REFERENCE_TYPE(MT);
	//**********************************************************************************************
};
...
class Band<SMatRepeatExpr<MT, SO, CRAs...>, TF, false, false, CBAs...>
  ...
  public:
	...
	*/
	...
	{
		if (isChecked(args...)) {
			if ((band() > 0L && column() >= re.columns()) || (band() < 0L && row() >= re.rows())) {
				METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid band access index");
			}
		} else {
			METRIC_NUMERIC_USER_ASSERT(band() <= 0L || column() < re.columns(), "Invalid band access index");
			METRIC_NUMERIC_USER_ASSERT(band() >= 0L || row() < re.rows(), "Invalid band access index");
		}
	}
	//**********************************************************************************************
