...
#ifndef METRIC_NUMERIC_MATH_VIEWS_COLUMNS_H
#define METRIC_NUMERIC_MATH_VIEWS_COLUMNS_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/views/columns/Dense.h>
#include <metric/numeric/math/views/columns/Sparse.h>
...
// This function returns an expression representing a selection of columns of the given matrix.
   ...
   mtrc::numeric::DynamicMatrix<double,columnMajor> D;
   mtrc::numeric::CompressedMatrix<double,columnMajor> S;
...
// matrix.
   ...
   const mtrc::numeric::DynamicMatrix<double,columnMajor> D( ... );
   const mtrc::numeric::CompressedMatrix<double,columnMajor> S( ... );
...
// This function returns an expression representing a selection of columns of the given matrix.
   ...
   // Creating a view on the 1st and 3rd column of the dense matrix D
   mtrc::numeric::SmallArray<size_t,2UL> indices1{ 1UL, 3UL };
   auto columns1 = columns( D, indices1 );

   // Creating a view on the 4th and 2nd column of the sparse matrix S
   mtrc::numeric::SmallArray<size_t,2UL> indices2{ 4UL, 2UL };
   auto columns2 = columns( S, indices2 );
   \endcode
...
inline decltype(auto) columns(const MatMatAddExpr<MT> &matrix, RCAs... args)
{
	...
	return columns<CCAs...>((*matrix).leftOperand(), args...) + columns<CCAs...>((*matrix).rightOperand(), args...);
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) columns(const MatMatMapExpr<MT> &matrix, RCAs... args)
{
	...
	return map(columns<CCAs...>((*matrix).leftOperand(), args...), columns<CCAs...>((*matrix).rightOperand(), args...),
			   (*matrix).operation());
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) columns(const VecTVecMapExpr<MT> &matrix, RCAs... args)
{
	...
	try {
		return map((*matrix).leftOperand(), elements<CCAs...>((*matrix).rightOperand(), args...),
				   (*matrix).operation());
	} catch (...) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid column access index");
	}
...
template <size_t... CCAs // Compile time column arguments
		  ...
		  typename... RCAs // Runtime column arguments
		  ,
		  EnableIf_t<(sizeof...(CCAs) > 0UL) && IsColumnMajorMatrix_v<MT>> * = nullptr>
...
template <size_t... CCAs // Compile time column arguments
		  ...
		  typename... RCAs // Runtime column arguments
		  ,
		  EnableIf_t<(sizeof...(CCAs) + sizeof...(RCAs) > 0UL) && !IsColumnMajorMatrix_v<MT>> * = nullptr>
inline decltype(auto) columns(const VecExpandExpr<MT, CEAs...> &matrix, RCAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) columns(const MatRepeatExpr<MT, CRAs...> &matrix, T *indices, size_t n, RCAs... args)
{
	...
	return repeat(columns((*matrix).operand(), newIndices, unchecked), (*matrix).template repetitions<0UL>(), 1UL);
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) columns(const MatRepeatExpr<MT, CRAs...> &matrix, P p, size_t n, RCAs... args)
{
	...
	auto lambda = [columns = (*matrix).operand().columns(), p](size_t i) { return p(i) % columns; };

	return repeat(columns((*matrix).operand(), std::move(lambda), n, unchecked), (*matrix).template repetitions<0UL>(),
				  1UL);
}
/*! \endcond */
//*************************************************************************************************
...
template <size_t I // First required column index
		  ...
		  typename... RCAs // Optional column arguments
		  ,
		  EnableIf_t<IsColumns_v<RemoveReference_t<MT>> && RemoveReference_t<MT>::compileTimeArgs> * = nullptr>
...
{
	return (columns.rows() == columns.operand().rows() && columns.columns() <= columns.operand().columns() &&
...
inline bool isSame(const Columns<MT, SO, DF, SF, CCAs1...> &a, const Columns<MT, SO, DF, SF, CCAs2...> &b) noexcept
{
	if (!isSame(a.operand(), b.operand()) || a.rows() != b.rows() || a.columns() != b.columns())
		return false;
...
template <InversionFlag IF // Inversion algorithm
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
inline auto invert(Columns<MT, SO, true, SF, CCAs...> &c) -> DisableIf_t<HasMutableDataAccess_v<MT>>
{
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(i < c.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(j < c.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySet(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column, size_t m,
										 size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*c).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*c).columns(), "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(row + m <= (*c).rows(), "Invalid number of rows");
	METRIC_NUMERIC_INTERNAL_ASSERT(column + n <= (*c).columns(), "Invalid number of columns");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryAdd(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column, size_t m,
										 size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*c).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*c).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySub(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column, size_t m,
										 size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*c).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*c).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryMult(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column,
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryDiv(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column, size_t m,
										 size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*c).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*c).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename... CCAs> // Compile time column arguments
METRIC_NUMERIC_ALWAYS_INLINE bool tryShift(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column,
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitand(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column,
											size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*c).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*c).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitor(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column,
										   size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*c).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*c).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitxor(const Columns<MT, SO, DF, SF, CCAs...> &c, size_t row, size_t column,
											size_t m, size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*c).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*c).columns(), "Invalid column access index");
...
inline bool tryAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
					  size_t column)
{
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
					  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
inline bool tryAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryAssign(const Columns<MT1, SO1, DF, SF, CCAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
					  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t j = 0UL; j < (*rhs).columns(); ++j) {
		if (!tryAssign(lhs.operand(), mtrc::numeric::column(*rhs, j, unchecked), row, lhs.idx(column + j)))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryAddAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryAddAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryAddAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						 size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryAddAssign(const Columns<MT1, SO1, DF, SF, CCAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t j = 0UL; j < (*rhs).columns(); ++j) {
		if (!tryAddAssign(lhs.operand(), mtrc::numeric::column(*rhs, j, unchecked), row, lhs.idx(column + j)))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool trySubAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool trySubAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool trySubAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						 size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool trySubAssign(const Columns<MT1, SO1, DF, SF, CCAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t j = 0UL; j < (*rhs).columns(); ++j) {
		if (!trySubAssign(lhs.operand(), mtrc::numeric::column(*rhs, j, unchecked), row, lhs.idx(column + j)))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryMultAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryMultAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryMultAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						  size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool trySchurAssign(const Columns<MT1, SO1, DF, SF, CCAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t j = 0UL; j < (*rhs).columns(); ++j) {
		if (!tryMultAssign(lhs.operand(), mtrc::numeric::column(*rhs, j, unchecked), row, lhs.idx(column + j)))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryDivAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryDivAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryDivAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						 size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryShiftAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryShiftAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryShiftAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						   size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryShiftAssign(const Columns<MT1, SO1, DF, SF, CCAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t j = 0UL; j < (*rhs).columns(); ++j) {
		if (!tryShiftAssign(lhs.operand(), mtrc::numeric::column(*rhs, j, unchecked), row, lhs.idx(column + j)))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitandAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitandAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitandAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
							size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitandAssign(const Columns<MT1, SO1, DF, SF, CCAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t j = 0UL; j < (*rhs).columns(); ++j) {
		if (!tryBitandAssign(lhs.operand(), mtrc::numeric::column(*rhs, j, unchecked), row, lhs.idx(column + j)))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitorAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitorAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitorAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						   size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitorAssign(const Columns<MT1, SO1, DF, SF, CCAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t j = 0UL; j < (*rhs).columns(); ++j) {
		if (!tryBitorAssign(lhs.operand(), mtrc::numeric::column(*rhs, j, unchecked), row, lhs.idx(column + j)))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitxorAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitxorAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitxorAssign(const Columns<MT, SO, DF, SF, CCAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
							size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitxorAssign(const Columns<MT1, SO1, DF, SF, CCAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t j = 0UL; j < (*rhs).columns(); ++j) {
		if (!tryBitxorAssign(lhs.operand(), mtrc::numeric::column(*rhs, j, unchecked), row, lhs.idx(column + j)))
			return false;
	}
...
struct Size<Columns<MT, SO, DF, SF, CCAs...>, 0UL> : public Size<MT, 0UL> {};
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool DF, bool SF, typename... CCAs>
struct MaxSize<Columns<MT, SO, DF, SF, CCAs...>, 0UL> : public MaxSize<MT, 0UL> {};
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool DF, bool SF, typename... CCAs>
struct IsRestricted<Columns<MT, SO, DF, SF, CCAs...>> : public IsRestricted<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct HasConstDataAccess<Columns<MT, SO, true, SF, CCAs...>> : public HasConstDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool SF, typename... CCAs>
struct HasMutableDataAccess<Columns<MT, SO, true, SF, CCAs...>> : public HasMutableDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool SF, typename... CCAs>
struct IsAligned<Columns<MT, SO, true, SF, CCAs...>> : public IsAligned<MT> {};
/*! \endcond */
//*************************************************************************************************
