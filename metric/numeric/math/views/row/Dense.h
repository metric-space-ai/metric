...
#ifndef METRIC_NUMERIC_MATH_VIEWS_ROW_DENSE_H
#define METRIC_NUMERIC_MATH_VIEWS_ROW_DENSE_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/dense/InitializerVector.h>
...
class Row<MT, true, true, SF, CRAs...> : public View<DenseVector<Row<MT, true, true, SF, CRAs...>, true>>,
  ...
  public:
	...
	/*!\name Constructors */
	//@{
	template <typename... RRAs> explicit inline Row(MT &matrix, RRAs... args);
  ...
  private:
	...
	static constexpr bool VectorizedAssign_v =
		(useOptimizedKernels && simdEnabled && VT::simdEnabled && IsSIMDCombinable_v<ElementType, ElementType_t<VT>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedAddAssign_v =
		(VectorizedAssign_v<VT> && HasSIMDAdd_v<ElementType, ElementType_t<VT>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedSubAssign_v =
		(VectorizedAssign_v<VT> && HasSIMDSub_v<ElementType, ElementType_t<VT>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedMultAssign_v =
		(VectorizedAssign_v<VT> && HasSIMDMult_v<ElementType, ElementType_t<VT>>);
	//**********************************************************************************************
	...
	static constexpr bool VectorizedDivAssign_v =
		(VectorizedAssign_v<VT> && HasSIMDDiv_v<ElementType, ElementType_t<VT>>);
	//**********************************************************************************************
  ...
  public:
	...
	template <typename VT> inline auto assign(const DenseVector<VT, true> &rhs) -> DisableIf_t<VectorizedAssign_v<VT>>;

	template <typename VT> inline auto assign(const DenseVector<VT, true> &rhs) -> EnableIf_t<VectorizedAssign_v<VT>>;
	  ...
	  matrix_(matrix) // The matrix containing the row
{
	if (isChecked(args...)) {
		...
		METRIC_NUMERIC_USER_ASSERT(row() < matrix_.rows(), "Invalid row access index");
	}
}
/*! \endcond */
//*************************************************************************************************
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::Reference Row<MT, true, true, SF, CRAs...>::operator[](size_t index)
{
	METRIC_NUMERIC_USER_ASSERT(index < size(), "Invalid row access index");
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::Reference Row<MT, true, true, SF, CRAs...>::at(size_t index)
{
	if (index >= size()) {
...
template <typename MT // Type of the dense matrix
		  ...
		  size_t... CRAs> // Compile time row arguments
inline
	typename Row<MT, true, true, SF, CRAs...>::ConstReference Row<MT, true, true, SF, CRAs...>::at(size_t index) const
{
	if (index >= size()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::Pointer Row<MT, true, true, SF, CRAs...>::data() noexcept
{
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::ConstPointer Row<MT, true, true, SF, CRAs...>::data() const noexcept
{
	return matrix_.data(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::Iterator Row<MT, true, true, SF, CRAs...>::begin()
{
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::ConstIterator Row<MT, true, true, SF, CRAs...>::begin() const
{
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::ConstIterator Row<MT, true, true, SF, CRAs...>::cbegin() const
{
	return matrix_.cbegin(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::Iterator Row<MT, true, true, SF, CRAs...>::end()
{
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::ConstIterator Row<MT, true, true, SF, CRAs...>::end() const
{
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, true, true, SF, CRAs...>::ConstIterator Row<MT, true, true, SF, CRAs...>::cend() const
{
	return matrix_.cend(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline Row<MT, true, true, SF, CRAs...> &Row<MT, true, true, SF, CRAs...>::operator=(const ElementType &rhs)
{
	decltype(auto) left(derestrict(matrix_));

	const size_t jbegin((IsUpper_v<MT>) ? ((IsUniUpper_v<MT> || IsStrictlyUpper_v<MT>) ? (row() + 1UL) : (row()))
										: (0UL));
	const size_t jend((IsLower_v<MT>) ? ((IsUniLower_v<MT> || IsStrictlyLower_v<MT>) ? (row()) : (row() + 1UL))
									  : (size()));
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline Row<MT, true, true, SF, CRAs...> &Row<MT, true, true, SF, CRAs...>::operator=(initializer_list<ElementType> list)
{
	if (list.size() > size()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid assignment to row");
	}
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
inline Row<MT, true, true, SF, CRAs...> &Row<MT, true, true, SF, CRAs...>::operator=(const Row &rhs)
{
	if (&rhs == this)
		return *this;

	if (size() != rhs.size()) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Row sizes do not match");
	}
...
inline Row<MT, true, true, SF, CRAs...> &Row<MT, true, true, SF, CRAs...>::operator=(const Vector<VT, true> &rhs)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_VECTOR_TYPE(ResultType_t<VT>);
...
inline Row<MT, true, true, SF, CRAs...> &Row<MT, true, true, SF, CRAs...>::operator/=(const DenseVector<VT, true> &rhs)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_VECTOR_TYPE(ResultType_t<VT>);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(ResultType_t<VT>);
...
inline Row<MT, true, true, SF, CRAs...> &Row<MT, true, true, SF, CRAs...>::operator%=(const Vector<VT, true> &rhs)
{
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_VECTOR_TYPE(CrossType);
...
inline Row<MT, true, true, SF, CRAs...> &Row<MT, true, true, SF, CRAs...>::scale(const Other &scalar)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_UNITRIANGULAR_MATRIX_TYPE(MT);

	const size_t jbegin((IsUpper_v<MT>) ? ((IsStrictlyUpper_v<MT>) ? (row() + 1UL) : (row())) : (0UL));
	const size_t jend((IsLower_v<MT>) ? ((IsStrictlyLower_v<MT>) ? (row()) : (row() + 1UL)) : (size()));
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
template <typename MT2	  // Data type of the other dense row
		  ,
		  bool SO2 // Storage order of the other dense row
		  ,
		  bool SF2 // Symmetry flag of the other dense row
		  ,
		  size_t... CRAs2> // Compile time row arguments of the other dense row
inline bool Row<MT, true, true, SF, CRAs...>::canAlias(const Row<MT2, SO2, true, SF2, CRAs2...> *alias) const noexcept
{
...
template <typename MT2	  // Data type of the other dense row
		  ...
		  ,
		  size_t... CRAs2> // Compile time row arguments of the other dense row
inline bool Row<MT, true, true, SF, CRAs...>::isAliased(const Row<MT2, SO2, true, SF2, CRAs2...> *alias) const noexcept
{
	return matrix_.isAliased(&alias->matrix_) && (row() == alias->row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE typename Row<MT, true, true, SF, CRAs...>::SIMDType
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE void Row<MT, true, true, SF, CRAs...>::store(size_t index, const SIMDType &value) noexcept
{
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE void Row<MT, true, true, SF, CRAs...>::storea(size_t index, const SIMDType &value) noexcept
{
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE void Row<MT, true, true, SF, CRAs...>::storeu(size_t index, const SIMDType &value) noexcept
{
...
template <typename MT // Type of the dense matrix
		  ...
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE void Row<MT, true, true, SF, CRAs...>::stream(size_t index, const SIMDType &value) noexcept
{
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(size() == (*rhs).size(), "Invalid vector sizes");

	const size_t jpos(prevMultiple((*rhs).size(), 2UL));
	METRIC_NUMERIC_INTERNAL_ASSERT(jpos <= (*rhs).size(), "Invalid end calculation");
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
class Row<MT, false, true, false, CRAs...> : public View<DenseVector<Row<MT, false, true, false, CRAs...>, true>>,
											 private RowData<CRAs...> {
  private:
	//**Type definitions****************************************************************************
	using DataType = RowData<CRAs...>;					//!< The type of the RowData base class.
	...
	class RowIterator {
	  public:
		...
		*/
		...
		{
			METRIC_NUMERIC_USER_ASSERT(column_ + index < matrix_->columns(), "Invalid access index detected");
		...
		*/
		inline ReferenceType operator*() const { return *pos_; }
		//*******************************************************************************************
		...
		*/
		inline PointerType operator->() const { return pos_; }
		//*******************************************************************************************
		...
		*/
		inline DifferenceType operator-(const RowIterator &rhs) const noexcept { return column_ - rhs.column_; }
		//*******************************************************************************************
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, false, CRAs...>::Reference Row<MT, false, true, false, CRAs...>::at(size_t index)
{
	if (index >= size()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, false, CRAs...>::Pointer Row<MT, false, true, false, CRAs...>::data() noexcept
{
	return matrix_.data() + row();
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, false, CRAs...>::Iterator Row<MT, false, true, false, CRAs...>::begin()
{
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, false, CRAs...>::ConstIterator Row<MT, false, true, false, CRAs...>::begin() const
{
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, false, CRAs...>::ConstIterator Row<MT, false, true, false, CRAs...>::cbegin() const
{
	return ConstIterator(matrix_, row(), 0UL);
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, false, CRAs...>::Iterator Row<MT, false, true, false, CRAs...>::end()
{
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, false, CRAs...>::ConstIterator Row<MT, false, true, false, CRAs...>::end() const
{
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, false, CRAs...>::ConstIterator Row<MT, false, true, false, CRAs...>::cend() const
{
	return ConstIterator(matrix_, row(), size());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline Row<MT, false, true, false, CRAs...> &Row<MT, false, true, false, CRAs...>::operator=(const ElementType &rhs)
{
	decltype(auto) left(derestrict(matrix_));
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline Row<MT, false, true, false, CRAs...> &Row<MT, false, true, false, CRAs...>::operator=(const Row &rhs)
{
	if (&rhs == this)
		return *this;
...
template <typename MT // Type of the dense matrix
		  ...
		  size_t... CRAs> // Compile time row arguments
template <typename Other> // Data type of the scalar value
inline Row<MT, false, true, false, CRAs...> &Row<MT, false, true, false, CRAs...>::scale(const Other &scalar)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_UNITRIANGULAR_MATRIX_TYPE(MT);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
class Row<MT, false, true, true, CRAs...> : public View<DenseVector<Row<MT, false, true, true, CRAs...>, true>>,
											private RowData<CRAs...> {
  private:
	//**Type definitions****************************************************************************
	using DataType = RowData<CRAs...>;					//!< The type of the RowData base class.
  ...
  private:
	...
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_DENSE_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_COLUMN_MAJOR_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_SYMMETRIC_MATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_COMPUTATION_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_TRANSEXPR_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_SUBMATRIX_TYPE(MT);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_POINTER_TYPE(MT);
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, true, CRAs...>::Reference Row<MT, false, true, true, CRAs...>::at(size_t index)
{
	if (index >= size()) {
		METRIC_NUMERIC_THROW_OUT_OF_RANGE("Invalid row access index");
	}
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, true, CRAs...>::Pointer Row<MT, false, true, true, CRAs...>::data() noexcept
{
	return matrix_.data(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, true, CRAs...>::Iterator Row<MT, false, true, true, CRAs...>::begin()
{
	return matrix_.begin(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, true, CRAs...>::ConstIterator Row<MT, false, true, true, CRAs...>::begin() const
{
	return matrix_.cbegin(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, true, CRAs...>::ConstIterator Row<MT, false, true, true, CRAs...>::cbegin() const
{
	return matrix_.cbegin(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, true, CRAs...>::Iterator Row<MT, false, true, true, CRAs...>::end()
{
	return matrix_.end(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, true, CRAs...>::ConstIterator Row<MT, false, true, true, CRAs...>::end() const
{
	return matrix_.cend(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline typename Row<MT, false, true, true, CRAs...>::ConstIterator Row<MT, false, true, true, CRAs...>::cend() const
{
	return matrix_.cend(row());
}
/*! \endcond */
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline Row<MT, false, true, true, CRAs...> &Row<MT, false, true, true, CRAs...>::operator=(const ElementType &rhs)
{
	decltype(auto) left(derestrict(matrix_));

	const size_t ibegin((IsLower_v<MT>) ? ((IsUniLower_v<MT> || IsStrictlyLower_v<MT>) ? (row() + 1UL) : (row()))
										: (0UL));
	const size_t iend((IsUpper_v<MT>) ? ((IsUniUpper_v<MT> || IsStrictlyUpper_v<MT>) ? (row()) : (row() + 1UL))
									  : (size()));
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
inline Row<MT, false, true, true, CRAs...> &Row<MT, false, true, true, CRAs...>::operator=(const Row &rhs)
{
	if (&rhs == this)
		return *this;
...
template <typename MT // Type of the dense matrix
		  ...
		  size_t... CRAs> // Compile time row arguments
template <typename VT>	  // Type of the right-hand side vector
inline Row<MT, false, true, true, CRAs...> &Row<MT, false, true, true, CRAs...>::operator=(const Vector<VT, true> &rhs)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_BE_ROW_VECTOR_TYPE(ResultType_t<VT>);
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_REQUIRE_EVALUATION(ResultType_t<VT>);
...
template <typename MT // Type of the dense matrix
		  ...
		  size_t... CRAs> // Compile time row arguments
template <typename Other> // Data type of the scalar value
inline Row<MT, false, true, true, CRAs...> &Row<MT, false, true, true, CRAs...>::scale(const Other &scalar)
{
	METRIC_NUMERIC_CONSTRAINT_MUST_NOT_BE_UNITRIANGULAR_MATRIX_TYPE(MT);

	const size_t ibegin((IsLower_v<MT>) ? ((IsStrictlyLower_v<MT>) ? (row() + 1UL) : (row())) : (0UL));
	const size_t iend((IsUpper_v<MT>) ? ((IsStrictlyUpper_v<MT>) ? (row()) : (row() + 1UL)) : (size()));
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE typename Row<MT, false, true, true, CRAs...>::SIMDType
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE void Row<MT, false, true, true, CRAs...>::store(size_t index,
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE void Row<MT, false, true, true, CRAs...>::storea(size_t index,
																			  const SIMDType &value) noexcept
{
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE void Row<MT, false, true, true, CRAs...>::storeu(size_t index,
																			  const SIMDType &value) noexcept
{
...
template <typename MT // Type of the dense matrix
		  ,
		  size_t... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE void Row<MT, false, true, true, CRAs...>::stream(size_t index,
																			  const SIMDType &value) noexcept
{
	...
	-> DisableIf_t<VectorizedAssign_v<VT>>
{
	...
	METRIC_NUMERIC_INTERNAL_ASSERT(ipos <= (*rhs).size(), "Invalid end calculation");
