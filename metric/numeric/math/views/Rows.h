...
#ifndef METRIC_NUMERIC_MATH_VIEWS_ROWS_H
#define METRIC_NUMERIC_MATH_VIEWS_ROWS_H
//*************************************************************************************************
// Includes
//*************************************************************************************************
...
#include <metric/numeric/math/views/rows/Dense.h>
#include <metric/numeric/math/views/rows/Sparse.h>
...
inline decltype(auto) rows(const MatMatAddExpr<MT> &matrix, RRAs... args)
{
	...
	return rows<CRAs...>((*matrix).leftOperand(), args...) + rows<CRAs...>((*matrix).rightOperand(), args...);
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) rows(const MatMatMapExpr<MT> &matrix, RRAs... args)
{
	...
	return map(rows<CRAs...>((*matrix).leftOperand(), args...), rows<CRAs...>((*matrix).rightOperand(), args...),
			   (*matrix).operation());
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) rows(const VecTVecMapExpr<MT> &matrix, RRAs... args)
{
	...
	try {
		return map(elements<CRAs...>((*matrix).leftOperand(), args...), (*matrix).rightOperand(),
				   (*matrix).operation());
	} catch (...) {
		METRIC_NUMERIC_THROW_INVALID_ARGUMENT("Invalid row access index");
	}
...
template <size_t... CRAs // Compile time row arguments
		  ...
		  typename... RRAs // Runtime row arguments
		  ,
		  EnableIf_t<(sizeof...(CRAs) > 0UL) && IsRowMajorMatrix_v<MT>> * = nullptr>
...
template <size_t... CRAs // Compile time row arguments
		  ...
		  typename... RRAs // Runtime row arguments
		  ,
		  EnableIf_t<(sizeof...(CRAs) + sizeof...(RRAs) > 0UL) && !IsRowMajorMatrix_v<MT>> * = nullptr>
inline decltype(auto) rows(const VecExpandExpr<MT, CEAs...> &matrix, RRAs... args)
{
	METRIC_NUMERIC_FUNCTION_TRACE;
...
inline decltype(auto) rows(const MatRepeatExpr<MT, CRAs...> &matrix, RRAs... args)
{
	...
	return repeat(rows((*matrix).operand(), std::move(lambda), sizeof...(Is) + 1UL, unchecked), 1UL,
...
inline decltype(auto) rows(const MatRepeatExpr<MT, CRAs...> &matrix, T *indices, size_t n, RRAs... args)
{
	...
	return repeat(rows((*matrix).operand(), newIndices, unchecked), 1UL, (*matrix).template repetitions<1UL>());
}
/*! \endcond */
//*************************************************************************************************
...
inline decltype(auto) rows(const MatRepeatExpr<MT, CRAs...> &matrix, P p, size_t n, RRAs... args)
{
	...
	auto lambda = [rows = (*matrix).operand().rows(), p](size_t i) { return p(i) % rows; };

	return repeat(rows((*matrix).operand(), std::move(lambda), n, unchecked), 1UL,
				  (*matrix).template repetitions<1UL>());
}
/*! \endcond */
//*************************************************************************************************
...
template <size_t I // First required row index
		  ...
		  typename... RRAs // Optional row arguments
		  ,
		  EnableIf_t<IsRows_v<RemoveReference_t<MT>> && RemoveReference_t<MT>::compileTimeArgs> * = nullptr>
...
{
	return (rows.rows() <= rows.operand().rows() && rows.columns() == rows.operand().columns() &&
...
inline bool isSame(const Rows<MT, SO, DF, SF, CRAs1...> &a, const Rows<MT, SO, DF, SF, CRAs2...> &b) noexcept
{
	if (!isSame(a.operand(), b.operand()) || a.rows() != b.rows() || a.columns() != b.columns())
		return false;
...
template <InversionFlag IF // Inversion algorithm
		  ...
		  ,
		  typename... CRAs> // Compile time row arguments
inline auto invert(Rows<MT, SO, true, SF, CRAs...> &r) -> DisableIf_t<HasMutableDataAccess_v<MT>>
{
...
{
	METRIC_NUMERIC_INTERNAL_ASSERT(i < r.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(j < r.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySet(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
										 size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(row + m <= (*r).rows(), "Invalid number of rows");
	METRIC_NUMERIC_INTERNAL_ASSERT(column + n <= (*r).columns(), "Invalid number of columns");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryAdd(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
										 size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool trySub(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
										 size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryMult(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
										  size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryDiv(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
										 size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename... CRAs> // Compile time row arguments
METRIC_NUMERIC_ALWAYS_INLINE bool tryShift(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
										   size_t n, int count)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitand(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
											size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitor(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
										   size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename ET> // Type of the element
METRIC_NUMERIC_ALWAYS_INLINE bool tryBitxor(const Rows<MT, SO, DF, SF, CRAs...> &r, size_t row, size_t column, size_t m,
											size_t n, const ET &value)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= (*r).rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= (*r).columns(), "Invalid column access index");
...
inline bool tryAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row, size_t column)
{
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row, size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
inline bool tryAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band, size_t row,
					  size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryAssign(const Rows<MT1, SO1, DF, SF, CRAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
					  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t i = 0UL; i < (*rhs).rows(); ++i) {
		if (!tryAssign(lhs.operand(), mtrc::numeric::row(*rhs, i, unchecked), lhs.idx(row + i), column))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryAddAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryAddAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryAddAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryAddAssign(const Rows<MT1, SO1, DF, SF, CRAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t i = 0UL; i < (*rhs).rows(); ++i) {
		if (!tryAddAssign(lhs.operand(), mtrc::numeric::row(*rhs, i, unchecked), lhs.idx(row + i), column))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool trySubAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool trySubAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool trySubAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						 size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool trySubAssign(const Rows<MT1, SO1, DF, SF, CRAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t i = 0UL; i < (*rhs).rows(); ++i) {
		if (!trySubAssign(lhs.operand(), mtrc::numeric::row(*rhs, i, unchecked), lhs.idx(row + i), column))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryMultAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryMultAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						  size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryMultAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						  size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool trySchurAssign(const Rows<MT1, SO1, DF, SF, CRAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t i = 0UL; i < (*rhs).rows(); ++i) {
		if (!tryMultAssign(lhs.operand(), mtrc::numeric::row(*rhs, i, unchecked), lhs.idx(row + i), column))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryDivAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryDivAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						 size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryDivAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						 size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryShiftAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryShiftAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryShiftAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						   size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryShiftAssign(const Rows<MT1, SO1, DF, SF, CRAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t i = 0UL; i < (*rhs).rows(); ++i) {
		if (!tryShiftAssign(lhs.operand(), mtrc::numeric::row(*rhs, i, unchecked), lhs.idx(row + i), column))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitandAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitandAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitandAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
							size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitandAssign(const Rows<MT1, SO1, DF, SF, CRAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t i = 0UL; i < (*rhs).rows(); ++i) {
		if (!tryBitandAssign(lhs.operand(), mtrc::numeric::row(*rhs, i, unchecked), lhs.idx(row + i), column))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitorAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitorAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitorAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
						   size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitorAssign(const Rows<MT1, SO1, DF, SF, CRAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
						   size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t i = 0UL; i < (*rhs).rows(); ++i) {
		if (!tryBitorAssign(lhs.operand(), mtrc::numeric::row(*rhs, i, unchecked), lhs.idx(row + i), column))
			return false;
	}
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitxorAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, false> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  typename VT> // Type of the right-hand side vector
inline bool tryBitxorAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, true> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
...
template <typename MT // Type of the matrix
		  ...
		  ,
		  bool TF> // Transpose flag of the right-hand side vector
inline bool tryBitxorAssign(const Rows<MT, SO, DF, SF, CRAs...> &lhs, const Vector<VT, TF> &rhs, ptrdiff_t band,
							size_t row, size_t column)
{
	MAYBE_UNUSED(band);
...
template <typename MT1 // Type of the matrix
		  ...
		  ,
		  bool SO2> // Storage order of the right-hand side matrix
inline bool tryBitxorAssign(const Rows<MT1, SO1, DF, SF, CRAs...> &lhs, const Matrix<MT2, SO2> &rhs, size_t row,
							size_t column)
{
	METRIC_NUMERIC_INTERNAL_ASSERT(row <= lhs.rows(), "Invalid row access index");
	METRIC_NUMERIC_INTERNAL_ASSERT(column <= lhs.columns(), "Invalid column access index");
	...
	for (size_t i = 0UL; i < (*rhs).rows(); ++i) {
		if (!tryBitxorAssign(lhs.operand(), mtrc::numeric::row(*rhs, i, unchecked), lhs.idx(row + i), column))
			return false;
	}
...
struct Size<Rows<MT, SO, DF, SF, CRAs...>, 1UL> : public Size<MT, 1UL> {};
/*! \endcond */
//*************************************************************************************************
...
template <typename MT, bool SO, bool DF, bool SF, typename... CRAs>
struct MaxSize<Rows<MT, SO, DF, SF, CRAs...>, 1UL> : public MaxSize<MT, 1UL> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool DF, bool SF, typename... CRAs>
struct IsRestricted<Rows<MT, SO, DF, SF, CRAs...>> : public IsRestricted<MT> {};
/*! \endcond */
//*************************************************************************************************
...
struct HasConstDataAccess<Rows<MT, SO, true, SF, CRAs...>> : public HasConstDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool SF, typename... CRAs>
struct HasMutableDataAccess<Rows<MT, SO, true, SF, CRAs...>> : public HasMutableDataAccess<MT> {};
/*! \endcond */
//*************************************************************************************************
...
/*! \cond METRIC_NUMERIC_INTERNAL */
template <typename MT, bool SO, bool SF, typename... CRAs>
struct IsAligned<Rows<MT, SO, true, SF, CRAs...>> : public IsAligned<MT> {};
/*! \endcond */
//*************************************************************************************************
