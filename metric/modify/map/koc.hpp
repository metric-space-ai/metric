...
template <typename Space> class KOCModel {
  public:
	...
	explicit KOCModel(std::size_t clusters) : clusters_(clusters) {}

	auto transform(const Space &) const -> result_type { detail::throw_koc_unavailable(); }
...
class KOCMapping {
  public:
	explicit KOCMapping(std::size_t clusters) : clusters_(clusters)
	{
		if (clusters_ == 0) {
...
inline auto koc(std::size_t clusters) -> KOCMapping { return KOCMapping(clusters); }
