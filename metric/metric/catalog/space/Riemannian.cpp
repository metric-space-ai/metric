...
template <typename RecType, typename Metric>
template <typename T>
T RiemannianDistance<RecType, Metric>::matDistance(mtrc::numeric::DynamicMatrix<T> A,
												   mtrc::numeric::DynamicMatrix<T> B) const
{
	...
	return sqrt(mtrc::numeric::sum(mtrc::numeric::pow(mtrc::numeric::log(eigenValues), 2)));
}
