...
template <typename Scalar> class MultiClassEntropy : public Output<Scalar> {
  ...
  public:
	...
	void evaluate(const Matrix &prev_layer_data, const Matrix &target)
	{
		...
		// d(L) / d(in) = -y / phat
		//
		// Match the row-major (observations x classes) convention used everywhere else in this
		// engine (the previous resize(nclass, nobs) was transposed, and the gradient was left
		// commented out — a silently-zero backprop). For one-hot `target` this reduces to the same
		// -1/phat-at-the-active-class form as the integer-label overload below.
		m_din.resize(nobs, nclass);
		for (int i = 0; i < nobs; i++) {
			for (int j = 0; j < nclass; j++) {
				m_din(i, j) =
					(target(i, j) != Scalar(0)) ? (-target(i, j) / prev_layer_data(i, j)) : Scalar(0);
			}
		}
	}
	...
	Scalar loss() const
	{
		...
		Scalar res = Scalar(0);
		const int nelem = mtrc::numeric::size(m_din);
