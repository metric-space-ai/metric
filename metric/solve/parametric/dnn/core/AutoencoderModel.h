...
// Positional bottleneck inference: assumes a (near-)symmetric encoder/decoder
// stack and picks the middle layer's output as the latent code. It does NOT
// inspect layer widths, so it does not verify the chosen layer is the narrowest.
// For even layer counts the encoder and decoder are equal length and the
// bottleneck is the last encoder layer; for odd counts the encoder carries the
// extra middle layer. Build an AutoencoderModel with an explicit AutoencoderTopology
// when the architecture is asymmetric. (Inference verified for counts 2..6 in
...
inline auto autoencoder_topology_to_json(const AutoencoderTopology &topology) -> mtrc::core::Metadata
{
...
inline auto autoencoder_topology_from_json(const mtrc::core::Metadata &json) -> AutoencoderTopology
{
	AutoencoderTopology topology;
...
template <class Scalar> auto training_spec_to_json(const TrainingSpec<Scalar> &spec) -> mtrc::core::Metadata
{
...
template <class Scalar> auto loss_term_report_to_json(const LossTermReport<Scalar> &term) -> mtrc::core::Metadata
{
...
template <class Scalar> auto epoch_report_to_json(const EpochReport<Scalar> &epoch) -> mtrc::core::Metadata
{
...
template <class Scalar> auto training_report_to_json(const TrainingReport<Scalar> &report) -> mtrc::core::Metadata
{
	mtrc::core::Metadata epochs = mtrc::core::Metadata::array();
...
template <class Scalar> class AutoencoderModel {
  public:
	...
	explicit AutoencoderModel(network_type network)
		: network_(std::move(network)), topology_(infer_autoencoder_topology(network_.layers.size()))
	{
		validate_topology();
	}

	AutoencoderModel(network_type network, AutoencoderTopology topology)
		: network_(std::move(network)), topology_(std::move(topology))
	{
		validate_topology();
	}

	AutoencoderModel(const AutoencoderModel &other)
		: network_(clone_network(other.network_)), topology_(other.topology_), backend_name_(other.backend_name_),
...
template <class Scalar> class NativeDnnTrainer {
  public:
	...
	auto fit(AutoencoderModel<Scalar> &model, const EncodedDataset<Scalar> &dataset,
			 const CompositeLoss<Scalar> &objective, const TrainingSpec<Scalar> &spec) const -> TrainingReport<Scalar>
	{
		if (dataset.size() == 0) {
  ...
  private:
	class EpochAccumulator {
	  public:
		explicit EpochAccumulator(std::size_t epoch) : epoch_(epoch) {}
	...
	static auto clip_gradients(std::vector<matrix_type> &gradients_by_layer, Scalar clip_norm) -> void
	{
		...
		for (const auto &gradient : gradients_by_layer) {
			if (mtrc::numeric::size(gradient) != 0) {
				squared_norm += mtrc::numeric::sqrNorm(gradient);
			}
		}
