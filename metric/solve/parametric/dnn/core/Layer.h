...
template <typename Scalar> class Layer {
  public:
	...
	Layer(const mtrc::core::Metadata &json) {}

	virtual mtrc::core::Metadata toJson()
	{
		mtrc::core::Metadata json = {{"inputSize", inputSize}, {"outputSize", outputSize}};
		return json;
	}
	///
	...
	///                        `getInputSize` columns as in the constructor, and each
	///                        row of `prev_layer_data` is an observation.
	///
	...
	///         matrix should have `getOutputSize` columns as in the constructor,
	///         and have number of rows equal to that of `prev_layer_data` in the
	///         Layer::forward() function. Each row represents an observation.
	///
	...
	///                        `getOutputSize` columns as in the constructor, and the same
	///                        number of rows as `prev_layer_data`.
	///
