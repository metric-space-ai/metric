...
This module is a native C++ support component for METRIC mapping pipelines. It
is not the public product model of METRIC and should not be presented as a
general deep-learning framework. Use it only when an engine workflow needs a
native parametric mapping component over an already-defined finite metric space.

The source geometry must come from records plus an admitted metric `d(a, b)`.
DNN feature tensors, autoencoder bottlenecks, and serialized networks are
derived execution artifacts, not replacements for the metric-space contract.
...
net.load("path_to_metric_native_network_file");
```
