...
template <typename Scalar> class NormalInitializer : public Initializer<Scalar> {
  ...
  public:
	...
	{
		matrix = mtrc::numeric::generate<mtrc::numeric::columnMajor>(
			rows, columns, [this](size_t i, size_t j) { return normalDistribution(randomEngine); });
	}

	void init(const size_t size, RowVector &vector)
	{
		vector = mtrc::numeric::generate<mtrc::numeric::rowVector>(
			size, [this](size_t pos) { return normalDistribution(randomEngine); });
	}
};
} // namespace mtrc::solve::parametric::dnn
