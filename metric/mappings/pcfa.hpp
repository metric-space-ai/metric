...
template <typename Record, typename CodeMetric = default_pcfa_metric_t<Record>> class PCFAModel {
  public:
	...
	using legacy_model_type = metric::PCFA<record_type, void>;
	...
	PCFAModel(legacy_model_type model, std::size_t component_count, code_metric_type code_metric = code_metric_type{})
		...
		, code_metric_(std::move(code_metric))
	{
	}
	...
	auto transform(const Space &space) const -> result_type
	{
		...
		result_type result{std::move(derived_space), lineage.first, lineage.second, space.size(), true, "pcfa",
						   "pcfa", "metric_space"};
		return result;
	}
...
class PCFAMapping {
  public:
	explicit PCFAMapping(std::size_t component_count)
		: component_count_(component_count)
	{
...
inline auto pcfa(std::size_t component_count) -> PCFAMapping
{
	return PCFAMapping(component_count);
}
