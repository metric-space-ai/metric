...
class BinaryClassEntropy : public Output {
  ...
  public:
	void check_target_data(const Matrix &target)
	{
		...
		const int nelem = blaze::size(target);
	...
	void evaluate(const Matrix &prev_layer_data, const Matrix &target)
	{
		...
		m_din.array() =
			(target.array() < Scalar(0.5))
				.select((Scalar(1) - prev_layer_data.array()).cwiseInverse(), -prev_layer_data.cwiseInverse());
	}
	...
	void evaluate(const Matrix &prev_layer_data, const IntegerVector &target)
	{
		...
		if (nvar != 1) {
			throw std::invalid_argument("[class BinaryClassEntropy]: Only one response variable is allowed when class "
										"labels are used as target data");
		}
		...
		// Same as above
		m_din.resize(1, nobs);
		m_din = (target == 0).select((Scalar(1) - prev_layer_data).cwiseInverse(), -blaze::pow(prev_layer_data, -1));
	}
	...
	Scalar loss() const
	{
		...
		// L = log(abs(m_din)).sum()
		return m_din.array().abs().log().sum() / m_din.columns();
	}
};
