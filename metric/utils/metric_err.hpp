...
template <typename T> std::tuple<double, double> stats(std::vector<T> v)
{
...
template <typename Metric, typename Container> typename Metric::distance_type norm(Container obj)
{ // now support only STL containers and k-related metrics
	size_t len = obj.size();
...
template <typename Metric, typename Container> typename Metric::distance_type distance(Container obj1, Container obj2)
{
	Metric metric;
...
template <typename Metric, typename Container, template <typename, typename> class OuterContainer,
		  typename OuterAllocator>
std::vector<typename Metric::distance_type> normalized_errors(OuterContainer<Container, OuterAllocator> original,
															  OuterContainer<Container, OuterAllocator> predicted)
{
...
normalized_err_stats(OuterContainer<Container, OuterAllocator> original,
					 OuterContainer<Container, OuterAllocator> predicted)
{
	...
	return std::make_tuple(std::get<0>(norm_stats), std::get<1>(norm_stats), std::get<0>(err_stats),
						   std::get<1>(err_stats), std::get<0>(n_err_stats), std::get<1>(n_err_stats));
}
