...
void printResult(std::vector<Record> filtered_results)
{
	for (int i = 0; i < filtered_results.size(); ++i)
	{
		std::cout << "\"" << filtered_results[i].date << "\"" << " " << filtered_results[i].quantity << " " << filtered_results[i].total << " " << filtered_results[i].tip << " " << filtered_results[i].type << " "
			<< "{\"";
		for (int j = 0; j < filtered_results[i].productIDS.size() - 1; ++j)
		{
			std::cout << filtered_results[i].productIDS[j] << "\", \"";
		}
		std::cout << filtered_results[i].productIDS[filtered_results[i].productIDS.size() - 1] << "\"}" << std::endl;
	}
}
...
int main()
{
	...
	// "2011-11-14T17:07:21Z", 2, 90, 6, "visa", {"004", "005"}
	// "2011-11-14T17:22:59Z", 2, 90, 7, "tab", {"001", "002", "004", "005"}
	std::cout << "-----------  filtered by value  -------------" << std::endl;
	printResult(filtered_results1);
	...
	// "2011-11-14T16:58:03Z", 2, 90, 5, "tab", {"001"}
	// "2011-11-14T17:07:21Z", 2, 90, 6, "visa", {"004", "005"}
	std::cout << "-----------  filtered by range  -------------" << std::endl;
	printResult(filtered_results1);
	...
	// "2011-11-14T16:53:41Z", 2, 90, 3, "tab", {"001", "004", "005"}
	// "2011-11-14T17:07:21Z", 2, 90, 6, "visa", {"004", "005"}
	std::cout << "-----------  filtered by custom function  -------------" << std::endl;
	printResult(filtered_results1);
	...
	filtered_results1 = payments.all_filtered();
	// "2011-11-14T17:07:21Z", 2, 90, 6, "visa", {"004", "005"}
	std::cout << "-----------  filtered by pseudo-boolean  -------------" << std::endl;
	printResult(filtered_results1);
	...
	std::cout << "-----------  filtered with added value  -------------" << std::endl;
	printResult(filtered_results1);

	// clear no_tabs and totals filters
	no_tabs.filter();
	totals.filter();
	...
	std::cout << "-----------  filtered after some filters cleared  -------------" << std::endl;
	printResult(filtered_results1);
