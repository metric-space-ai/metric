...
#include "clustering_som_anomaly_detector.hpp"
...
template <typename T>
ClusteringSomAnomalyDetector<T>::ClusteringSomAnomalyDetector(const std::string & filename) {  // load model from JSON
...
template <typename T>
ClusteringSomAnomalyDetector<T>::ClusteringSomAnomalyDetector(    // create by training using features
            const std::vector<std::vector<T>> & ds,
            const size_t num_clusters, // = 4,
...
//        std::cout << "PCFA completed and written" << std::endl;
    ...
    for (auto & cluster_data : clustered_energies) {
        ...
        // add CDFs and subband confidence intervals per window size
        std::vector<std::vector<std::vector<T>>> cluster_cdfs;
...
template <typename T>
void
ClusteringSomAnomalyDetector<T>::save(const std::string & filename) {
...
template <typename T>
std::vector<T>
ClusteringSomAnomalyDetector<T>::encode(const std::vector<std::vector<T>> & dataset, T entropy_threshold) {
        ...
        for (auto pos_idx = wnd_size; pos_idx < dataset.size(); ++pos_idx) { // sliging window  // TODO check if last value is reached
            ...
            auto entries = cluster_entries(dataset, pos_idx, wnd_size);
...
template <typename T>
std::vector<std::vector<T>>
ClusteringSomAnomalyDetector<T>::encode_debug(const std::vector<std::vector<T>> & dataset, T entropy_threshold) {  // test version with intermediate output
    ...
    for (auto pos_idx = wnd_size; pos_idx < dataset.size(); ++pos_idx) { // sliging window  // TODO check if last value is reached
        ...
        if (entropy < entropy_threshold * std::log2((T)num_clusters)) {  // locked in best fitting cluster
            lock = 1;
        }
...
template <typename T>
std::vector<T>
ClusteringSomAnomalyDetector<T>::cluster_entries( // numbers of points that fall into each cluster (by BMUs)
...
//T
std::tuple<T, T, T>
ClusteringSomAnomalyDetector<T>::anomaly_score(
        const std::vector<std::vector<T>> & dataset,
        const size_t pos_idx, // position just after window
        const size_t wnd_size,
        const size_t cl_idx
        ...
        )
{
    ...
    for (size_t sb_idx = 0; sb_idx < num_subbands; ++sb_idx) {
        ...
        for (auto ws_idx = 0; ws_idx < window_sizes.size(); ++ws_idx) {
            ...
            // here we have avg window probability evaluated
        ...
        avg_prob += avg_subband_prob;  // adding each subband's summand to overall prob
