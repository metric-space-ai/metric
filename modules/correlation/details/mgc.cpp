/* This Source Code Form is subject to the terms of the PANDA GmbH License. 
You are not allowed to use or edit the code without license. */
...
template <typename T>
blaze::DynamicMatrix<T> MGC_direct::local_covariance(const blaze::DynamicMatrix<T> &A,
											...
											const blaze::DynamicMatrix<size_t> &RY)
{
    ...
    //std::cout << " 1 = " << double(std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count()) / 1000 << "ms)" << std::endl;
    //std::cout << " 2 = " << double(std::chrono::duration_cast<std::chrono::microseconds>(t3 - t2).count()) / 1000 << "ms)" << std::endl;
    //std::cout << " 3 = " << double(std::chrono::duration_cast<std::chrono::microseconds>(t4 - t3).count()) / 1000 << "ms)" << std::endl;
    //std::cout << " 4 = " << double(std::chrono::duration_cast<std::chrono::microseconds>(t5 - t4).count()) / 1000 << "ms)" << std::endl;
    //std::cout << " 5 = " << double(std::chrono::duration_cast<std::chrono::microseconds>(t6 - t5).count()) / 1000 << "ms)" << std::endl;
...
{
    // static_assert(p <= 0.0 || p >= 1.0, "Invalid input argument; must be larger than 0 but less than 1.");
    if (p < 0.5)
    {
...
template <typename T>
T MGC_direct::max_in_matrix_regarding_second_boolean_matrix(const blaze::DynamicMatrix<T> &m1, const blaze::DynamicMatrix<bool> &m2)
{
...
template <typename T>
void MGC_direct::normalize_generalized_correlation(blaze::DynamicMatrix<T> &corr,
...
// template <typename T>
// blaze::DynamicMatrix<T> getBlazeMatrix(const std::vector<std::vector<T>> &m)
// {
// 	blaze::DynamicMatrix<T> matrix(m.size(), m[0].size());
...
T MGC_direct::operator()(const DistanceMatrix<T> &X, const DistanceMatrix<T> &Y)
{
	...
	blaze::clear(A); A.shrinkToFit();
	blaze::clear(At); At.shrinkToFit();
	blaze::clear(B); B.shrinkToFit();
	blaze::clear(Bt); Bt.shrinkToFit();
	blaze::clear(RXt); RXt.shrinkToFit();
	blaze::clear(RX); RX.shrinkToFit();
	blaze::clear(RY); RY.shrinkToFit();
	blaze::clear(RYt); RYt.shrinkToFit();
    ...
    auto result = optimal_local_generalized_correlation(corr, R);
...
double MGC<recType1, Metric1, recType2, Metric2>::operator()(const Container1 &a, const Container2 &b) const
{
    ...
    for (size_t i = 0; i < X.rows(); ++i) {
        ...
        {
            //double distance = distance_function1(a[i], a[j]);
...
template <class recType1, class Metric1, class recType2, class Metric2>
double MGC<recType1, Metric1, recType2, Metric2>::variance(const std::vector<double> &data,
                                                           const double mean)
{
	double sum = 0;
	for (size_t i = 0; i < data.size(); ++i) {
...
template <class recType1, class Metric1, class recType2, class Metric2>
std::vector<double> MGC<recType1, Metric1, recType2, Metric2>::icdf(const std::vector<double> &prob,
																			const double mu,
																			const double sigma)
{
