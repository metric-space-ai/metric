...
namespace metric
{
...

	{
		...
		blaze::DynamicMatrix<Tv> trainDe(trainData.size(), trainData[0].size());
		trainModel(trainDe, nIter);
	}


	template <typename recType, class Metric, typename Tv>
	void Redif<recType, Metric, Tv>::trainModel(blaze::DynamicMatrix<Tv> & xTrainDe, size_t nIter)
	{
		xTrainDe = xTrain;
		size_t n = xTrainDe.rows();
	...
	template <typename recType, class Metric, typename Tv>
	template<bool flag>
	size_t Redif<recType, Metric, Tv>::partition(blaze::DynamicVector<size_t> & idx, blaze::DynamicVector<Tv, flag> & val, size_t start, size_t end)
	{
	...
	template <typename recType, class Metric, typename Tv>
	template<bool flag>
	void Redif<recType, Metric, Tv>::Quicksort(blaze::DynamicVector<size_t> & idx, blaze::DynamicVector<Tv, flag> & data, size_t start, size_t end)
	{
		if (start < end)
		{
