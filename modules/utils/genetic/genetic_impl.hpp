...
class Genetic
{
   ...
   template <typename H, typename K=double>
   using Func = std::vector< K> (*)(const H&);

   template <typename H, typename K>
   using SelMethodFunc = void (*)(genetic_details::Population<H,K>&);
   template <typename H, typename K>
   using CrsMethodFunc = void (*)(const genetic_details::Population<H,K>&, genetic_details::CHR<H,K>&, genetic_details::CHR<H,K>&);
   template <typename H, typename K>
   using MutMethodFunc = void (*)(genetic_details::CHR<H,K>&);
   template <typename H, typename K>
   using AdpMethodFunc = void (*)(genetic_details::Population<H,K>&);
   template <typename H>
   using HardConstraintFunc = bool (const H&);
   template <typename H, typename K=double>
   using SoftConstraintFunc = T (const H&);
...
public: 
   ...
   void change_evolution_method(SelMethodFunc<P,T> sel, CrsMethodFunc<P,T> crs,MutMethodFunc<P,T> mut, AdpMethodFunc<P,T> adp );
   ...
   void refresh_population(const std::vector<P>&, bool random= true);
      template <typename H>
   void addParam(const std::vector<H>&, const H&, const H&);
...
init(const std::vector<std::tuple<Tp...> >& t);
...
template <int I, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), void>::type 
printParam( std::tuple<Tp...>& paramset) const;
...
// end of recursion for initializing parameter(s) data
template <typename P, typename T> template <int I, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), void>::type 
...
// check inputs validity
template <typename P, typename T>
void Genetic<P,T>::check() const
{
   if (SP < 1.0 || SP > 2.0) {
      throw std::invalid_argument("Error: in class genetic_details::Genetic<P,T>, selective pressure (SP) cannot be outside [1.0,2.0], please choose a real value within this interval.");
   }
   if (elitpop > popsize || elitpop < 0) {
      throw std::invalid_argument("Error: in class genetic_details::Genetic<P,T>, elit population (elitpop) cannot outside [0,popsize], please choose an integral value within this interval.");
   }
   if (covrate < 0.0 || covrate > 1.0) {
      throw std::invalid_argument("Error: in class genetic_details::Genetic<P,T>, cross-over rate (covrate) cannot outside [0.0,1.0], please choose a real value within this interval.");
   }
   if (genstep <= 0) {
      throw std::invalid_argument("Error: in class genetic_details::Genetic<P,T>, generation step (genstep) cannot be <= 0, please choose an integral value > 0.");
   }
}
...
//set evolution methods
template <typename P, typename T>
void Genetic<P,T>::change_evolution_method(SelMethodFunc<P,T> sel, CrsMethodFunc<P,T> crs,MutMethodFunc<P,T> mut, AdpMethodFunc<P,T> adp )
{
   // selection method initialized to roulette wheel selection                                   
...
// return best chromosome
template <typename P, typename T>
inline const genetic_details::CHR<P,T>& Genetic<P,T>::result() const
{
   return pop(0);
}
...
template <typename P, typename T>
std::vector<P> Genetic<P,T>::top(unsigned int n) const
{
  std::vector<P> vP;
...
template <typename P, typename T>
std::vector<P> Genetic<P,T>::bottom(unsigned int n) const
{
  std::vector<P> vP;
...
// print results for each new generation
template <typename P, typename T>
void Genetic<P,T>::print(int n) const
{
...
// resize population
template <typename P, typename T>
void Genetic<P,T>::resize_population(int npop)
{
  this->popsize = npop;
  this->matsize = npop;
  // initializing population
...
// resize population
template <typename P, typename T>
void Genetic<P,T>::refresh_population(int refreshNum, bool random)
{
...
// resize population
template <typename P, typename T>
void Genetic<P,T>::refresh_population(const std::vector<P>& newParamset, bool random)
{
...
// run genetic algorithm
template <typename P, typename T>
void Genetic<P,T>::perform_evolutionary_steps(int numEvolStep)
{
