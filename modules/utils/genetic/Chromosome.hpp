...
// constructor
template <typename P, typename T>
genetic_details::Chromosome<P, T>::Chromosome(const Genetic<P, T> &ga)
{
...
// copy constructor
template <typename P, typename T>
genetic_details::Chromosome<P, T>::Chromosome(const Chromosome<P, T> &rhs)
{
   param = rhs.param;
   result = rhs.result;
   chr = rhs.chr;
...
// create new chromosome
template <typename P, typename T>
inline void Chromosome<P, T>::create()
{
   chr.clear();
...
template <int I, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), void>::type
Chromosome<P, T>::encodeChr(std::tuple<Tp...> &tp) {}
...
template <typename P, typename T>
inline void Chromosome<P, T>::initialize(P &initP)
{
   chr.clear();
   encodeChr<0>(initP);
}
...
template <int I, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), void>::type
Chromosome<P, T>::decodeChr(std::tuple<Tp...> &tp) {}
...
template <typename P, typename T>
inline void Chromosome<P, T>::evaluate()
{
   decodeChr<0>(param);
...
// reset chromosome
template <typename P, typename T>
inline void Chromosome<P, T>::reset()
{
   chr.clear();
   result.clear();
   total = 0.0;
...
// set or replace kth gene by a new one
template <typename P, typename T>
inline void Chromosome<P, T>::setGene(int k)
{
#ifndef NDEBUG
   if (k < 0 || k >= ptr->nbparam)
   {
      throw std::invalid_argument("Error: in genetic_details::Chromosome< P,T>::setGene(int), argument cannot be outside interval [0,nbparam-1], please amend.");
   }
#endif
...
// initialize or replace kth gene by a know value
template <typename P, typename T>
inline void Chromosome<P, T>::initGene(int k, T x)
{
#ifndef NDEBUG
   if (k < 0 || k >= ptr->nbparam)
   {
      throw std::invalid_argument("Error: in genetic_details::Chromosome< P,T>::initGene(int), first argument cannot be outside interval [0,nbparam-1], please amend.");
   }
#endif
...
template <typename P, typename T>
inline void Chromosome<P, T>::addBit(char bit)
{
   chr.push_back(bit);
   ...
   if (chr.size() > chrsize)
   {
      throw std::out_of_range("Error: in genetic_details::Chromosome< P,T>::setBit(char), exceeding chromosome size.");
   }
#endif
}
...
// initialize or replace an existing chromosome bit
template <typename P, typename T>
inline void Chromosome<P, T>::setBit(char bit, int pos)
{
#ifndef NDEBUG
   if (pos >= chrsize)
   {
      throw std::out_of_range("Error: in genetic_details::Chromosome< P,T>::replaceBit(char, int), second argument cannot be equal or greater than chromosome size.");
   }
#endif
...
// flip an existing chromosome bit
template <typename P, typename T>
inline void Chromosome<P, T>::flipBit(int pos)
{
#ifndef NDEBUG
   if (pos >= chrsize)
   {
      throw std::out_of_range("Error: in genetic_details::Chromosome< P,T>::flipBit(int), argument cannot be equal or greater than chromosome size.");
   }
#endif
...
template <typename P, typename T>
inline char Chromosome<P, T>::getBit(int pos) const
{
#ifndef NDEBUG
   if (pos >= chrsize)
   {
      throw std::out_of_range("Error: in genetic_details::Chromosome< P,T>::getBit(int), argument cannot be equal or greater than chromosome size.");
   }
#endif
...
inline void Chromosome<P, T>::setPortion(const Chromosome<P, T> &x, int start, int end)
{
   ...
   if (start > chrsize)
   {
      throw std::out_of_range("Error: in genetic_details::Chromosome< P,T>::setPortion(const Chromosome< P,T>&, int, int), second argument cannot be greater than chromosome size.");
   }
#endif
...
template <typename P, typename T>
inline void Chromosome<P, T>::setPortion(const Chromosome<P, T> &x, int start)
{
#ifndef NDEBUG
   if (start > chrsize)
   {
      throw std::out_of_range("Error: in genetic_details::Chromosome< P,T>::setPortion(const Chromosome< P,T>&, int), second argument cannot be greater than chromosome size.");
   }
#endif
...
// get parameter value(s) from chromosome
template <typename P, typename T>
inline const P &Chromosome<P, T>::getParam() const
{
   return param;
}
...
// get objective function result
template <typename P, typename T>
inline const std::vector<T> &Chromosome<P, T>::getResult() const
{
   return result;
}
...
// get the total sum of all objective function(s) result
template <typename P, typename T>
inline T Chromosome<P, T>::getTotal() const
{
   return total;
}
...
template <typename P, typename T>
const std::vector<T> Chromosome<P, T>::getConstraint() const
{
...
// return chromosome size in number of bits
template <typename P, typename T>
inline int Chromosome<P, T>::size() const
{
   return chrsize;
}
...
template <typename P, typename T>
inline T Chromosome<P, T>::mutrate() const
{
   return ptr->mutrate;
}
...
// return number of genes in chromosome
template <typename P, typename T>
inline int Chromosome<P, T>::nbgene() const
{
   return ptr->nbparam;
}
...
// return numero of generation this chromosome belongs to
template <typename P, typename T>
inline int Chromosome<P, T>::nogen() const
{
   return numgen;
}
...
// return lower bound(s)
template <typename P, typename T>
inline const P &Chromosome<P, T>::lowerBound() const
{
...
// return upper bound(s)
template <typename P, typename T>
inline const P &Chromosome<P, T>::upperBound() const
{
