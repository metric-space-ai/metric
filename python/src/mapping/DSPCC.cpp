...
template <typename RecType, typename Metric> void register_wrapper_DSPCC(py::module &m)
{
	using Mapping = mtrc::DSPCC<RecType, Metric>;
	using Matrix = std::vector<std::vector<RecType>>;
	using Queue = std::deque<std::vector<RecType>>;

	auto dspcc = py::class_<Mapping>(m, "DSPCC");
	dspcc.def(py::init<const std::vector<RecType> &, size_t, size_t, float, size_t>(), py::arg("training_dataset"),
			  py::arg("n_features") = 1, py::arg("n_subbands") = 4, py::arg("time_freq_balance") = 0.5,
...
void export_metric_DSPCC(py::module &m) { register_wrapper_DSPCC<std::vector<double>, void>(m); }
