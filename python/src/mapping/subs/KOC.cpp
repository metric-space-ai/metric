...
auto create_KOC(Graph graph, Metric metric, Distribution distribution, double anomaly_sigma = 1.0,
				double start_learn_rate = 0.8, double finish_learn_rate = 0.0, size_t iterations = 20,
				std::optional<double> nbh_start_size = std::nullopt, double nbh_range_decay = 2.0,
				std::optional<long long> random_seed = std::nullopt)
	-> mtrc::KOC_details::KOC<recType, Graph, Metric, Distribution>
{
	return mtrc::KOC_details::KOC<recType, Graph, Metric, Distribution>(
		graph, metric, anomaly_sigma, start_learn_rate, finish_learn_rate, iterations, distribution,
		nbh_start_size.value_or(std::sqrt(double(graph.getNodesNumber()))), nbh_range_decay,
...
{
	using Factory = mtrc::KOC_factory<Record, Graph, Metric, Distribution>;
	using Class = typename Factory::KOC;
	using value_type = typename Factory::T;

	// KOC factory
	m.def("KOC", &create_KOC<Record, Graph, Metric, Distribution>, py::arg("graph"), py::arg("metric"),
		  py::arg("distribution"), py::arg("anomaly_sigma") = 1.0, py::arg("start_learn_rate") = 0.8,
		  py::arg("finish_learn_rate") = 0.0, py::arg("iterations") = 20, py::arg("nbh_start_size") = (double *)nullptr,
		  py::arg("nbh_range_decay") = 2.0, py::arg("random_seed") = (long long *)nullptr);

	// KOC implementation
	const std::string className = "KOC_" + mtrc::getTypeName<Graph>() + "_" + mtrc::getTypeName<Metric>();
	...
	koc.def("train", &Class::train, py::arg("samples"), py::arg("num_clusters"), py::arg("min_cluster_size") = 1);
	koc.def("top_outliers", &Class::top_outliers, py::arg("samples"), py::arg("count") = 10);
...
void export_metric_KOC(py::module &m)
{
	...
	using MetricTypes =
		boost::mpl::vector<mtrc::Euclidean<Value>, mtrc::Manhattan<Value>, mtrc::Chebyshev<Value>,
						   mtrc::P_norm<Value>, std::function<Value(const Container &, const Container &)>>;

	using GraphTypes = boost::mpl::vector<mtrc::Grid4, mtrc::Grid6, mtrc::Grid8>;
...
PYBIND11_MODULE(koc, m) { export_metric_KOC(m); }