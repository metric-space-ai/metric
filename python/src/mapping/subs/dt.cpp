...
class Dimension {
  public:
	 ...
	 */
	Dimension(py::object accessor, py::object distance) : _accessor(accessor), _distance(distance) {}
	/**
...
template <typename Record> void register_wrapper_DT(py::module &m)
{
	using Class = mtrc::DT<Record>;
	...
	dt.def(py::init<double, double>(), py::arg("entropy_threshold") = 0, py::arg("gain_threshold") = 0);
	dt.def(
		"train",
		+[](Class &self, const Container &data, const std::vector<Dimension> &dims, Callback &response) {
		...
		py::arg("payments"), py::arg("dimensions"), py::arg("response"));
	dt.def(
		"predict",
		+[](Class &self, const Container &data, const std::vector<Dimension> &dims) {
...
PYBIND11_MODULE(dt, m) { export_metric_DT(m); }