...
template <typename Value, typename Metric> void register_wrapper_Redif(py::module &m)
{
	using Class = mtrc::Redif<Value, Metric>;
	using Container = std::vector<std::vector<Value>>;

	auto cls = py::class_<Class>(m, "Redif");
	cls.def(py::init<const Container &, size_t, size_t, Metric>(), py::arg("train_data"), py::arg("n_neighbors") = 10,
			py::arg("n_iter") = 15, py::arg("metric") = Metric());
...
{
	using Value = double;
	register_wrapper_Redif<Value, mtrc::Euclidean<Value>>(m);
}

PYBIND11_MODULE(redif, m) { export_metric_Redif(m); }