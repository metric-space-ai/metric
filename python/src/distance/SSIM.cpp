...
template <typename DistanceType, typename Value> void register_wrapper_SSIM(py::module &m)
{
	using Metric = mtrc::SSIM<DistanceType, Value>;
	...
	py::class_<Metric>(m, "SSIM", "Structural similarity (for images)")
		...
		.def(py::init<ValueType, ValueType>(), "Init constructor", py::arg("dynamic_range"), py::arg("masking"))
...
void export_metric_SSIM(py::module &m) { register_wrapper_SSIM<double, std::vector<double>>(m); }
