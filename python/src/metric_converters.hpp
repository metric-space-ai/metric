...
class PyObjectHelper {
  ...
  public:
	PyObjectHelper(PyObject *obj_ptr) : object(boost::python::handle<>(boost::python::borrowed(obj_ptr))) {}

	std::string name() { return boost::python::extract<std::string>(this->object.attr("__class__").attr("__name__")); }

	bool isNumpyArray() { return this->name() == "ndarray"; }

	unsigned numberOfDimensions() { return boost::python::extract<unsigned>(this->object.attr("ndim")); }
};
...
struct IterableConverter {
	template <typename Container> IterableConverter &from_python()
	{
		boost::python::converter::registry::push_back(&IterableConverter::convertible<Container>,
	...
	*/
	template <typename Container> static void *convertible(PyObject *object)
	{
...
struct NumpyArrayConverter {
	...
	template <typename ArrayType> NumpyArrayConverter &from_python()
	{
		boost::python::converter::registry::push_back(&NumpyArrayConverter::convertible,
...
struct NumpyScalarConverter {
	...
	template <typename ScalarType> NumpyScalarConverter &from_python()
	{
		boost::python::converter::registry::push_back(&NumpyScalarConverter::convertible,
	...
	static void *convertible(PyObject *obj_ptr)
	{
		...
		if (name == "float32" || name == "float64" || name == "int8" || name == "int16" || name == "int32" ||
			name == "int64" || name == "uint8" || name == "uint16" || name == "uint32" || name == "uint64") {
			return obj_ptr;
		}
