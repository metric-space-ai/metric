...
/*  TODO:
	...
	mtrc::numeric::SymmetricMatrix<mtrc::numeric::DynamicMatrix<Distance, mtrc::numeric::rowMajor>> matrix()
   const;
*/

template <typename RecType, typename Metric> void register_wrapper_Tree(py::module &m)
{
	using Tree = mtrc::Tree<RecType, Metric>;
	using Node = mtrc::Node<RecType, Metric>;
	...
	//        )
	//    ));
	tree.def(py::init<const Container &, int>(), py::arg("p"), py::arg("truncate") = -1);
	std::vector<std::vector<std::size_t>> (Tree::*clustering1)(const std::vector<double> &,
															   const std::vector<std::size_t> &,
															   const std::vector<RecType> &) = &Tree::clustering;
	...
	std::vector<std::vector<std::size_t>> (Tree::*clustering3)(const std::vector<double> &,
															   const std::vector<RecType> &) = &Tree::clustering;
...
void export_metric_Tree(py::module &m) { register_wrapper_Tree<std::vector<double>, mtrc::Euclidean<double>>(m); }
