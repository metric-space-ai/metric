...
template <typename V, typename T> void register_wrapper_dimension(py::module &m)
{
	...
	dim.def("feature", +[](Dimension &self, Callback &key) { return self.feature(key); });
	dim.def(
		"feature", +[](Dimension &self, Callback &add_func, Callback &remove_func, Callback &init_func) {
			return self.feature(add_func, remove_func, init_func);
		});
	dim.def(
		"feature", +[](Dimension &self, Callback &add_func, Callback &remove_func, Callback &init_func,
					   Callback &key_func) { return self.feature(add_func, remove_func, init_func, key_func); });
	dim.def("feature_count", +[](Dimension &self) { return self.feature_count(); });
	dim.def("feature_count", +[](Dimension &self, Callback &key_func) { return self.feature_count(key_func); });
	dim.def("feature_sum", +[](Dimension &self, Callback &val_func) { return self.feature_sum(val_func); });
	dim.def(
		"feature_sum",
		+[](Dimension &self, Callback &val_func, Callback &key_func) { return self.feature_sum(val_func, key_func); });
	dim.def(
		"feature_all", +[](Dimension &self, Callback &add_func, Callback &remove_func, Callback &init_func) {
			return self.feature_all(add_func, remove_func, init_func);
		});
	dim.def("feature_all_sum", +[](Dimension &self, Callback &val_func) { return self.feature_all_sum(val_func); });
}
