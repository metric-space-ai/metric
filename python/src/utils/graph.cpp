...
// mtrc::Graph<WeightType, isDense, isSymmetric> createGraph(bool isDense = false, bool isSymmetric = true)
//{
//    return mtrc::Graph<WeightType, isDense, isSymmetric>();
//}

void register_wrapper_graph(py::module &m)
{
	using Class = mtrc::Graph<>;
	auto cls = py::class_<Class>(m, "Graph", "Graph based on numeric-lib");
	cls.def(py::init<>());
	cls.def(py::init<size_t>(), py::arg("nodes_number"));
	cls.def(py::init<const std::vector<std::pair<size_t, size_t>> &>(), py::arg("edges_pairs"));
	cls.def_property_readonly("nodes_number", &Class::getNodesNumber);
	cls.def_property_readonly("is_valid", &Class::isValid);
	cls.def("neighbours", &Class::template getNeighbours<>, py::arg("node_index"), py::arg("max_deep"));
	cls.def("get_matrix", &Class::get_matrix);
	cls.def("build", &Class::buildEdges, py::arg("edges_pairs"));
...
void register_wrapper_grids(py::module &m)
{
	...
	py::class_<Grid4, mtrc::Graph<>>(m, "Grid4")
	...
	py::class_<Grid6, mtrc::Graph<>>(m, "Grid6", "Hexagonal grid")
		.def(py::init<size_t>(), py::arg("nodes_number"))
		.def(py::init<size_t, size_t>(), py::arg("width"), py::arg("height"))
		.def("__repr__", [](const Grid6 &a) { return "<Grid6>"; });

	py::class_<Grid8, mtrc::Graph<>>(m, "Grid8")
		.def(py::init<size_t>(), py::arg("nodes_number"))
		.def(py::init<size_t, size_t>(), py::arg("width"), py::arg("height"))
