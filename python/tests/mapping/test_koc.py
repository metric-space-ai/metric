...
@pytest.mark.xfail(reason="randomness in KOC class")
def test_ok():
    best_w_grid_size = 5
    best_h_grid_size = 4
    ...
    expected_anomaly = [False, False, False, True, True, True, True, True, True, False, False, False]
    ...
    assert list(koc.check_if_anomaly(samples=dataset, anomaly_threshold=-0.5)) == expected_anomaly
