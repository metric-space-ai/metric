...
- Python helpers: `metric.Space`, `metric.metrics`, `metric.spaces.FiniteMetricSpace`, `metric.operators`, `metric.OutlierResult`, `metric.strategies.KMedoids`, `metric.strategies.DBSCAN`, `metric.strategies.FarthestFirst`, and `metric.strategies.DistanceProfileCorrelation`
- Python beta bridges: `metric.mappings`, `metric.transforms`
- nearest-neighbor, range-neighbor, pairwise-distance, exact graph-result, graph-connectivity-diagnostics, graph-degree-diagnostics, graph-stretch-diagnostics, graph-symmetrization, graph-out-degree-pruning, exact graph-edge, intrinsic-dimension, grouping, outlier-detection, representative-selection, cross-space comparison, and Python `Space.to_matrix()` / `Space.groups()` / `Space.outliers()` / `Space.compare()` / `Space.describe()` / `Space.representatives()` helpers
...
- minimal Python `Space` facade for `to_matrix`, `neighbors`, `nearest`, `within_radius`, `groups`, `outliers`, `representatives`, `describe`, and `compare`
...
| Python core facade | `python/pkg/metric/metrics.py`, `python/pkg/metric/spaces.py`, `python/pkg/metric/strategies.py`, `python/pkg/metric/operators.py`, `python/pkg/metric/__init__.py`, `python/examples/engine/` | Core Revival | Promoted Python API and examples, including deterministic graph-result, graph-connectivity-diagnostics, graph-degree-diagnostics, graph-stretch-diagnostics, graph-symmetrization, graph-out-degree-pruning, graph-edge, grouping, outlier-detection, representative-selection, medoid, separated-representative, radius-coverage helpers, `Space.to_matrix()`, `Space.outliers()`, and `Space.compare()`, covered by wheel CI and core Python tests. |
