...
- Python helpers: `metric.Space`, `metric.metrics`, `metric.spaces.FiniteMetricSpace`, `metric.operators`, `metric.MappingResult`, `metric.OutlierResult`, `metric.ReductionResult`, `metric.strategies.KMedoids`, `metric.strategies.DBSCAN`, `metric.strategies.FarthestFirst`, and `metric.strategies.DistanceProfileCorrelation`
...
| Python core facade | `python/pkg/metric/metrics.py`, `python/pkg/metric/spaces.py`, `python/pkg/metric/strategies.py`, `python/pkg/metric/operators.py`, `python/pkg/metric/__init__.py`, `python/examples/engine/` | Core Revival | Promoted Python API and examples, including deterministic graph-result, graph-connectivity-diagnostics, graph-degree-diagnostics, graph-stretch-diagnostics, graph-symmetrization, graph-out-degree-pruning, graph-edge, grouping, outlier-detection, DBSCAN-noise filtering, representative-selection, representative-reduction, deterministic mapping, medoid, separated-representative, radius-coverage helpers, `Space.to_matrix()`, `Space.outliers()`, `Space.denoise()`, `Space.reduce()`, `Space.map()`, and `Space.compare()`, covered by wheel CI and core Python tests. |
