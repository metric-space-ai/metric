...
- minimal Python `Space` facade for `to_matrix`, `neighbors` with count/radius arguments, `nearest`, `within_radius`, `groups`, `embed`, `outliers`, `denoise`, `representatives`, `reduce`, `compress`, `map`, `describe`, and `compare`
