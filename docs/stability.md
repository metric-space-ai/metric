...
- minimal Python `Space` facade for `to_matrix`, `neighbors`, `nearest`, `within_radius`, `groups`, `outliers`, `representatives`, `reduce`, `map`, `describe`, and `compare`
...
| Python core facade | `python/pkg/metric/metrics.py`, `python/pkg/metric/spaces.py`, `python/pkg/metric/strategies.py`, `python/pkg/metric/operators.py`, `python/pkg/metric/__init__.py`, `python/examples/engine/` | Core Revival | Promoted Python API and examples, including deterministic graph-result, graph-connectivity-diagnostics, graph-degree-diagnostics, graph-stretch-diagnostics, graph-symmetrization, graph-out-degree-pruning, graph-edge, grouping, outlier-detection, representative-selection, representative-reduction, deterministic mapping, medoid, separated-representative, radius-coverage helpers, `Space.to_matrix()`, `Space.outliers()`, `Space.reduce()`, `Space.map()`, and `Space.compare()`, covered by wheel CI and core Python tests. |
