...
- minimal Python `Space` facade for `to_matrix`, `to_tree`, `to_graph`, `neighbors` with count/radius arguments, `nearest`, `within_radius`, `groups`, `embed`, `outliers`, `denoise`, `representatives`, `reduce`, `compress`, `map`, `describe`, and `compare`
- entropy and MGC regression examples
- Python core helpers under `metric.metrics`, `metric.exceptions`, `metric.spaces`, `metric.intent`, `metric.representations`, `metric.strategies`, and `metric.operators`
