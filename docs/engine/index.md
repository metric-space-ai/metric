...
The first C++ mapping adapters are `metric::mappings::make_clustered_space_mapping`, deterministic `metric::map`, `metric::map(space, metric::mappings::pcfa(components))`, PCFA-backed `metric::embed`, and the PCFA adapter in `<metric/mappings/pcfa.hpp>`. They fit or transform source spaces into derived metric spaces while preserving source `RecordId` lineage.
