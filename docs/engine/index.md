...
The first C++ intent helpers are `metric::find_neighbors`, `metric::find_groups`, `metric::find_representatives`, representative `metric::compress`, `metric::find_outliers`, `metric::describe_structure`, `metric::compare`, `metric::correlate`, deterministic `metric::map`, DBSCAN-backed `metric::denoise`, the PCFA-backed `metric::embed` helper in `<metric/intent/embed.hpp>`, and the PCFA-backed `metric::reduce` helper in `<metric/intent/reduce.hpp>`. They keep the user-facing vocabulary semantic while routing to explicit strategy objects such as `metric::strategies::cover_tree`, `metric::strategies::matrix_cache`, `metric::strategies::k_medoids`, `metric::strategies::dbscan`, `metric::strategies::farthest_first`, `metric::strategies::mgc`, and `metric::strategies::pcfa`.
