...
The first C++ intent helpers are `metric::find_neighbors`, `metric::find_groups`, `metric::compare`, `metric::correlate`, and the PCFA-backed `metric::reduce` helper in `<metric/intent/reduce.hpp>`. They keep the user-facing vocabulary semantic while routing to explicit strategy objects such as `metric::strategies::cover_tree`, `metric::strategies::matrix_cache`, `metric::strategies::k_medoids`, `metric::strategies::dbscan`, `metric::strategies::mgc`, and `metric::strategies::pcfa`.
