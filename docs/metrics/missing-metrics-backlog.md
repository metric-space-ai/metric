...
| B6 | AIRM fix | `RiemannianDistance` | Encode both operands with the **same** SPD operator (current code uses `−d` for one, `+d` for the other → symmetry & identity fail; see `Riemannian.cpp:422` vs `:432`). Guard SPD/PD of both Gram matrices. | Pennec, Fillard & Ayache 2006, *IJCV* 66(1) | Medium |
