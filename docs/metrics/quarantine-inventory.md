...
| `mtrc::RiemannianDistance<RecType,Metric>` | `metric/metric/catalog.hpp` | Unresolved/proof gap | Space/matrix distance may be metric only under exact SPD or finite-space model assumptions. Current domain and preconditions are not admitted. | Admit only after record model and proof. | Keep separate from MGC; document domain, SPD checks, induced-distance requirements, and tests. |
