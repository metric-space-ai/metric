...
| Runtime inspection and picking | checked headlessly and in browser | `MetricVisualRuntime.pickAt()` and `inspectAt()` now use one engine path for GPU picking on stock point, glyph and ground-projection record layers, relation-matrix picking, graph-edge picking and deterministic projected-record fallback. `visual/tools/check-runtime-picking-preview.mjs` verifies record selection, pair selection, runtime state and preview resolution; `visual/tools/check-visual-regression-public-examples.mjs` verifies public examples stay renderable, interactive and report `gpu-picking` hits with record IDs where a pickable record layer is present. |
...
preview state. Stock point, glyph and ground-projection record layers now expose
`renderPicking()` implementations. The browser public-regression gate requires
native preview examples with pickable record layers to return `source:
gpu-picking` and a concrete record ID. Remaining picking work is specialized
edge, field, curve and matrix GPU picking where semantic pickers are not enough.
