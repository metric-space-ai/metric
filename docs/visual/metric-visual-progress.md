...
| Runtime inspection and picking | checked headlessly and in browser | `MetricVisualRuntime.pickAt()` and `inspectAt()` now use one engine path for GPU picking on stock point, glyph, ground-projection, heat-field, relation-edge and curve-ribbon layers, relation-matrix picking, graph-edge picking and deterministic projected-record fallback. `visual/tools/check-runtime-picking-preview.mjs` verifies record selection, pair selection, runtime state and preview resolution; `visual/tools/check-visual-regression-public-examples.mjs` verifies public examples stay renderable, interactive and report `gpu-picking` hits with record IDs or edge IDs where a pickable record, field, graph-edge or ribbon-trajectory layer is present. |
...
heat-field layers, relation-edge layers and curve-ribbon layers now expose
`renderPicking()` implementations. The browser public-regression gate requires
native preview examples with pickable record, field, graph-edge or
ribbon-trajectory layers to return `source: gpu-picking` and a concrete record
or edge ID. Remaining picking work is specialized tube-curve and matrix GPU
picking where semantic pickers are not enough.
