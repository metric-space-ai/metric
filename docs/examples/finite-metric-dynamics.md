...
Randomness, disorder, forward diffusion and reverse diffusion are usually
presented as probabilistic machinery bolted onto data. This example shows the
opposite: they are all an **evolution over the structure of a finite metric
space**, and the probabilities are *derived from the geometry*, never assumed.
...
// vector-signal forward dynamics / degradation
mtrc::DynamicsSchedule forward = schedule;
forward.steps = 6; forward.diffusivity = 0.05; forward.perturbation_scale = 0.06; forward.seed = 1234;
auto degraded = mtrc::metric_diffuse(space, forward, transition);

// vector-signal reverse dynamics / reconstruction over the fixed metric structure
mtrc::DynamicsSchedule reverse = schedule;
reverse.steps = 40; reverse.diffusivity = 0.18; reverse.perturbation_scale = 0.0;
...
## Act 2 — forward dynamics / degradation

`metric_diffuse` evolves the node signal with graph heat flow plus seeded
coordinate perturbation. This is the vector-record specialization of metric
dynamics, not the universal definition of noise. The signal leaves the manifold
and the graph Dirichlet energy rises. The run is reproducible bit-for-bit for a
fixed `(seed, schedule)`, and a different seed degrades the space differently.
The terminal `MappingResult` keeps the node count and dimension and carries
one-to-one lineage back to the source records.
...
`metric_reconstruct` runs the same graph heat flow without coordinate
perturbation. As a low-pass filter on the metric graph it contracts the
fluctuations back onto the structure. The example asserts:
...
- even when the graph is rebuilt from the **degraded observations alone** (the
  structure is not handed to us, mirroring adaptive Redif measure dynamics),
...
The promoted behavior is the `mtrc::modify::dynamics` surface. For universal
finite metric spaces, use the Redif measure APIs (`redif_add_noise`,
`redif_remove_noise`, `redif_outliers`): their states are measures over the original
atoms and their path lengths are transport costs under the intrinsic metric. The
`metric_diffuse`/`metric_reconstruct` calls shown here are the vector-signal
specialization used by this fixture. The schedule constants in the example are
tuned for the fixture; they are a demonstration, not a contract.
