# PHATE-AE Pipeline Workflow

This report records the CI-safe PHATE-AE preset workflow for METRIC production
readiness. In METRIC terms, the workflow builds diffusion compression from
source distances, fits a native map, and reports lineage plus diagnostics.
...
- native training uses the C++ autoencoder path as a fitted-map component with
...
- the vector comparison can identify an existing anchor, but it is not a fitted
