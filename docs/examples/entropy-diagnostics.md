...
mtrc::Entropy<void, mtrc::Edit<int>> entropy(mtrc::Edit<int>(), 3, 2);
...
A deliberate decision (Track C11) governs the small-space regime: the **only** behavior C11
changes is the total-failure sentinel (when *no* point yields a valid local Gaussian, return
NaN; see "No-information spaces" above). Every **normalization** question is left **pinned**
to the promoted regression values under a single consistent standard — do not alter
valid-input fixtures without sign-off plus regeneration against the
